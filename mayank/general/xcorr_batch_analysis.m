...
%         lf7Seg = loflf7(windowBeg:windowEnd);
        ...
        [temp_xcorr_LF2(i,:),lagVec] = xcov(wcvSeg,lf2Seg,maxLag,'coef');
        [temp_xcorr_LF3(i,:),lagVec] = xcov(wcvSeg,lf3Seg,maxLag,'coef');
        [temp_xcorr_LF5(i,:),lagVec] = xcov(wcvSeg,lf5Seg,maxLag,'coef');
%         [temp_xcorr_LF7(i,:),lagVec] = xcov(wcvSeg,lf7Seg,maxLag,'coef');
        [temp_xcorr_LF8(i,:),lagVec] = xcov(wcvSeg,lf8Seg,maxLag,'coef');
        [temp_xcorr_LF2_LF8(i,:),lagVec] = xcov(lf2Seg,lf8Seg,maxLag,'coef');
        [temp_xcorr_LF3_LF8(i,:),lagVec] = xcov(lf3Seg,lf8Seg,maxLag,'coef');
        [temp_xcorr_LF5_LF8(i,:),lagVec] = xcov(lf5Seg,lf8Seg,maxLag,'coef');
        [temp_xcorr_LF3_LF5(i,:),lagVec] = xcov(lf3Seg,lf5Seg,maxLag,'coef');

    end
    
    xcorr_LF2(d,:) = mean(temp_xcorr_LF2);
    xcorr_LF3(d,:) = mean(temp_xcorr_LF3);
    xcorr_LF5(d,:) = mean(temp_xcorr_LF5);
%     xcorr_LF7(d,:) = mean(temp_xcorr_LF7);
    xcorr_LF8(d,:) = mean(temp_xcorr_LF8);
    xcorr_LF2_LF8(d,:) = mean(temp_xcorr_LF2_LF8);
    xcorr_LF3_LF8(d,:) = mean(temp_xcorr_LF3_LF8);
    xcorr_LF5_LF8(d,:) = mean(temp_xcorr_LF5_LF8);
    xcorr_LF3_LF5(d,:) = mean(temp_xcorr_LF3_LF5);



    d = d +1

    save E:\WC_Germany\JMM_Analysis\overall_data_jmm_xcorr_hp d xcorr* lagVec

    %prepare for next iteration
    clear all

    load E:\WC_Germany\JMM_Analysis\file_directories_jmm.mat
    load E:\WC_Germany\JMM_Analysis\overall_data_jmm_xcorr_hp.mat
