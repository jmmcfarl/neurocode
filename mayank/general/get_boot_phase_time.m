function [boot_phase_time] = get_boot_phase_time(lf8_phase,time_sls,phase_hist,lags,num_samps)

net_time = zeros(length(phase_hist),1);
...
for i = 1:num_samps
   ...
   if ~isempty(phase_inst)
      ...
      cur_time_bin = find(lags > cur_time_inst,1,'first');
       ...
       net_time(cur_phase_bin) = net_time(cur_phase_bin) + cur_time_bin;
...
boot_phase_time = net_time./net_samps;
