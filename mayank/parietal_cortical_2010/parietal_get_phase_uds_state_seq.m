function [sm_state_seq,state_seq,Fs,fhmm] = parietal_get_phase_uds_state_seq(raw_data,raw_Fs,desynch_times,f_names)
...
cos_phase = -0.17; %for LF4
% cos_phase = -0.19;
% cos_phase = -0.24;

%% initializations
[obs_lf,t_axis,Fs] = get_lf_features(raw_data,raw_Fs,Fs,[lf_lcf lf_hcf]);
obs_phase = angle(hilbert(obs_lf));
emiss = (cos(obs_phase - cos_phase) + 1)/2;
