...
%     load smoothness_lf4
    for i = 1:length(hcf_vals)
        [hmm_bbstate_seq4{i},hmm4{i},state_durations4{d,i}] = parietal_get_hmm_lf_test(lf4,raw_Fs,hcf_vals(i),desynch_times_lf4{thom_el(d)});
...
%     save smoothness_lf4 hmm_bbstate_seq4 hmm4
    clear hmm*
...
%     save kl_data_robust_dp_mp_lf8_lf4_hf
end
...
%     load used_data lf8
%     for i = 1:length(hcf_vals)
%         [hmm_bbstate_seq8{i},hmm8{i},state_durations8{d,i}] = parietal_get_hmm_lf_test(lf8,raw_Fs,hcf_vals(i),desynch_times_lf8{d});
%         up_dur_pmf8(d,i,:) = hist(state_durations8{d,i}{2},dur_hist_bincenters);
%         up_dur_pmf8(d,i,:) = up_dur_pmf8(d,i,:)/sum(up_dur_pmf8(d,i,:));
%         down_dur_pmf8(d,i,:) = hist(state_durations8{d,i}{1},dur_hist_bincenters);
%         down_dur_pmf8(d,i,:) = down_dur_pmf8(d,i,:)/sum(down_dur_pmf8(d,i,:));
%         min_up_dur8(d,i) = min(state_durations8{d,i}{2});
%         min_down_dur8(d,i) = min(state_durations8{d,i}{1});
%         mean_up_dur8(d,i) = mean(state_durations8{d,i}{2});
%         mean_down_dur8(d,i) = mean(state_durations8{d,i}{1});
%         meandiff = [];
%         for s = 1:hmm8{i}.Nsegs
%             meandiff = [meandiff; hmm8{i}.state(2).meanfun{s}-hmm8{i}.state(1).meanfun{s}];
%         end
%         %         meandiff = mean(meandiff);
%         covar1 = hmm8{i}.state(1).var;
%         covar2 = hmm8{i}.state(2).var;
%         lf8_kl_lf(d,i) = gauss_kl_div(meandiff,covar1,covar2);
%     end
%     save smoothness_lf8 hmm_bbstate_seq8 hmm8
%     clear hmm*
%    
%     for i = 1:length(smooth_vals)
%         [hmm_bbstate_seq8_hf{i},hmm8_hf{i},state_durations_hf8{d,i}] = parietal_get_hmm_hf_test(lf8,raw_Fs,smooth_vals(i),desynch_times_lf8{d});
%         up_dur_hf_pmf8(d,i,:) = hist(state_durations_hf8{d,i}{2},dur_hist_bincenters);
%         up_dur_hf_pmf8(d,i,:) = up_dur_hf_pmf8(d,i,:)/sum(up_dur_hf_pmf8(d,i,:));
%         down_dur_hf_pmf8(d,i,:) = hist(state_durations_hf8{d,i}{1},dur_hist_bincenters);
%         down_dur_hf_pmf8(d,i,:) = down_dur_hf_pmf8(d,i,:)/sum(down_dur_hf_pmf8(d,i,:));
%         min_up_dur_hf8(d,i) = min(state_durations_hf8{d,i}{2});
%         min_down_dur_hf8(d,i) = min(state_durations_hf8{d,i}{1});
%         mean_up_dur_hf8(d,i) = mean(state_durations_hf8{d,i}{2});
%         mean_down_dur_hf8(d,i) = mean(state_durations_hf8{d,i}{1});
%         meandiff = [];
%         for s = 1:hmm8_hf{i}.Nsegs
%             meandiff = [meandiff; hmm8_hf{i}.state(2).meanfun{s}-hmm8_hf{i}.state(1).meanfun{s}];
%         end
%         %         meandiff = mean(meandiff);
%         covar1 = hmm8_hf{i}.state(1).var;
%         covar2 = hmm8_hf{i}.state(2).var;
%         lf8_kl_hf(d,i) = gauss_kl_div(meandiff,covar1,covar2);
%     end
%     save smoothness_lf8_hf hmm_bbstate_seq8_hf hmm8_hf
%     clear hmm*
%     
...
%     cd G:\WC_Germany\parietal_cortical_2010
% %     save smoothness_test_results_robust_dp_mp_lf8_lf4
%     save kl_data_robust_dp_mp_lf8_lf4
% end
...
figure
errorbar(smooth_vals,mean(lf4_kl_hf),std(lf4_kl_hf)/sqrt(21)), hold on
figure
errorbar(hcf_vals,mean(lf4_kl_lf),std(lf4_kl_lf)/sqrt(21),'r'), hold on
