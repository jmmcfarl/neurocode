...
min_rec_dur = 500; %min duration of total rec in sec
used_dirs = find([data(:).ep] > min_rec_dur & [data(:).dp] > min_rec_dur); %dont use recs that are too short
data = data(used_dirs);
...
Fsd = raw_Fs/dsf;
niqf = raw_Fs/2;
lcf = 0.05;hcf = 10; %bandpass filter range for LF amp
...
%     lfp_hf = get_hf_features(lf7,raw_Fs,Fsd,[lcf_hf hcf_hf],hcf_sm);
    ...
    core_data(d).tot_uds_dur = sum(diff(new_seg_inds,[],2))/Fsd; %total duration of UDS segs
...
save fin_pdown_core_analysis_corticalMP_fin core_data 
