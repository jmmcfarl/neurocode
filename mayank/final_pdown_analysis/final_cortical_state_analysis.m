...
min_hpcrate = 1; %min hpc MUA rate for using
...
Fsd = raw_Fs/dsf;
niqf = raw_Fs/2;
lcf = 0.05;hcf = 10; %bandpass for LF amp
lcf_hf = 30; hcf_hf = 100; %bandpass for HF power
hcf_sm = 0.025; %power smoothing sigma
rate_sm = round(Fsd*0.05); %rate smoothing sigma

%if excluding a window around state transitions
% trans_buffer = round(Fsd*0.1);
trans_buffer = round(Fsd*0);

%axes for computing amplitude densities
...
%     cd(data(d).dir)
    ...
    cfun_data(d).data_id = data(d).id;
    
    load ./used_data lf7 wcv_minus_spike
    
    %process LF amp signals and HF power
    ...
    %chop data occurring after last good sample
    ...
    %bin MP spikes, smooth and zscore
    ...
    [corresp_mp_upinds,corresp_mp_downinds] = find_corresponding_state_transitions_lookback(...
        up_trans_inds_lfp,down_trans_inds_lfp,up_trans_inds_mp,down_trans_inds_mp);
           
%     ctx_up_cents = (up_trans_inds_lfp + down_trans_inds_lfp)/2; %these are the mid-pts of each ctx up state
%     mp_up_cents = (up_trans_inds + down_trans_inds)/2; %mid pts of each MP up state
%     nearest_mp_up = nan(size(ctx_up_cents)); %nearest MP up state to each ctx up state
%     for ii = 1:length(ctx_up_cents)
%        [~,bb] = min(abs(ctx_up_cents(ii)-mp_up_cents));
%        nearest_mp_up(ii) = bb;
%     end
    %% process ctx MUA data
...
% %         ctx_mua_rate = ctx_mua_rate/avg_ctx_mua_rate(d);
    ...
    %% get mean values within each state
...
%     [ctx_up_amps_mua,ctx_down_amps_mua] = get_state_amplitudes(ctx_mua_rate,up_trans_inds8,down_trans_inds8);
    [ctx_up_amps,ctx_down_amps] = get_state_amplitudes_mean(lfp_lf,up_trans_inds_lfp,down_trans_inds_lfp,trans_buffer);
    [ctx_up_amps_hf,ctx_down_amps_hf] = get_state_amplitudes_mean(lfp_hf,up_trans_inds_lfp,down_trans_inds_lfp,trans_buffer);
%     [ctx_up_amps_mua,ctx_down_amps_mua] = get_state_amplitudes_mean(ctx_mua_rate,up_trans_inds_lfp,down_trans_inds_lfp,trans_buffer);
    [mp_up_amps_spkrate,mp_down_amps_spkrate] = get_state_amplitudes_mean(mp_spike_rate,up_trans_inds_mp,down_trans_inds_mp,trans_buffer);
    
    %remove trend in MP up-cond spike rate across time
...
%get densities of ctx state amplitudes for plotting
...
%% now compute and store a variety of properties of each state for comparison with skipping probs
    ...
    %get rid of skipped/nonskipped states that could not have been skipped
    %anyways
    ...
    corresp_mp_uplag = nan(length(up_trans_inds_lfp),1);
    corresp_mp_uplag(uset) = core_data(d).mp_uplags(corresp_mp_upinds(uset))/Fsd;
    cfun_data(d).corresp_mp_uplag = corresp_mp_uplag;
    
    %MP spike rate in the corresponding MP state
    corresp_mp_spkrate = nan(length(up_trans_inds_lfp),1);
...
%     corresp_mp_spkrate = mp_up_amps_spkrate(nearest_mp_up);
    ...
    %up-trans
    uset = find(~isnan(corresp_mp_upinds));
    corresp_mp_downlag = nan(length(up_trans_inds_lfp),1);
    ...
    %store binary vector for skipped up states
    ctx_up_isskipped = nan(length(up_trans_inds_lfp),1);
    ctx_up_isskipped(skipped_lfp_ups) = 1;
    ctx_up_isskipped(non_skipped_lfp_ups) = 0;    
    cfun_data(d).ctx_up_isskipped = ctx_up_isskipped;
    
    %store binary vector for skipped down states
    ctx_down_isskipped = nan(length(down_trans_inds_lfp),1);
    ctx_down_isskipped(skipped_lfp_downs) = 1;
    ctx_down_isskipped(non_skipped_lfp_downs) = 0;    
    cfun_data(d).ctx_down_isskipped = ctx_down_isskipped;
    
    %duration of preceding cortical Down state
    prev_ctx_downdur = nan(length(up_trans_inds_lfp),1);
    ...
    TSL_mp_down = nan(size(up_trans_inds_lfp));
    for i = 1:length(TSL_mp_down)
        prev_mp_down = find(down_trans_inds_mp < up_trans_inds_lfp(i),1,'last');
        if ~isempty(prev_mp_down)
            TSL_mp_down(i) = (up_trans_inds_lfp(i) - down_trans_inds_mp(prev_mp_down))/Fsd;
        end
    end
    cfun_data(d).TSL_mp_down = TSL_mp_down;
    
    %time since the last MP up-transition (from current cortical down)
    TSL_mp_up = nan(size(up_trans_inds_lfp));
    for i = 1:length(TSL_mp_up)
        prev_mp_up = find(up_trans_inds_mp < down_trans_inds_lfp(i),1,'last');
        if ~isempty(prev_mp_up)
            TSL_mp_up(i) = (down_trans_inds_lfp(i) - up_trans_inds_mp(prev_mp_up))/Fsd;
        end
    end
...
save final_cortical_state_data_fin_nobuff_wc5_fin cfun_data state_amp_*
