...
load sleep_dirs
% load sleep_dirs_old
fig_dir = '/Users/james/Analysis/Mayank/sleep/sleep_figs/';
...
lf_cut_off_freqs = [0.05 10]; %for "low-frequency amplitude" features, these are the cutoff-frequencies for the bandpass filter
...
if sum(is_uds) > 0
mp_pdf = hist(mp_features(is_uds),xx); 
mp_kpdf = ksdensity(mp_features(is_uds),xx);
mp_pdf = mp_pdf/mean(mp_pdf)*mean(mp_kpdf);

[~,dens_peaks] = findpeaks(mp_kpdf,'npeaks',2,'sortstr','descend','minpeakdistance',round(0.25/median(diff(xx))));
dens_peaks = xx(dens_peaks);
...
for jj = 1:length(poss_amp_prc)
    ...
    if length(cur_events) >= min_n_events
        ...
        for ii = 1:length(cur_lags)
            all_down_dists(jj,ii,:) = ksdensity(cur_mat(:,ii),mp_dist_xx,'bandwidth',bndwidth);
        end
    end
...
if length(cur_events) >= min_n_events
        ...
        for ii = 1:length(cur_lags)
            all_up_dists(jj,ii,:) = ksdensity(cur_mat(:,ii),mp_dist_xx,'bandwidth',bndwidth);
        end
    end
...
for jj = 1:length(poss_amp_prc)
    ...
    if length(cur_events) >= min_n_events
        ...
        for ii = 1:length(cur_lags)
            all_down_dists_lfp(jj,ii,:) = ksdensity(cur_mat(:,ii),lfp_dist_xx,'bandwidth',bndwidth);
        end
    end
    ...
    if length(cur_events) >= min_n_events
        ...
        for ii = 1:length(cur_lags)
            all_up_dists_lfp(jj,ii,:) = ksdensity(cur_mat(:,ii),lfp_dist_xx,'bandwidth',bndwidth);
        end
    end
...
if isnan(max_utrig_Pdown)
    max_utrig_Pdown = 0.1;
end
if isnan(max_dtrig_Pup) 
    max_dtrig_Pup = 0.1;
end
f1 = figure();
