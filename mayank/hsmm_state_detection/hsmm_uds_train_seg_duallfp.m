function [hmm,gamma,hmm_window_post]=hsmm_uds_train_seg_duallfp(hmm,emiss)

%modified from hsmm_uds_train_seg, see that code for further info
...
%         gamma(:)=0;
    ...
    if strcmp(hmm.meantype,'variable')
        ...
        for i = 1:hmm.Nsegs
            ...
            temp_meanfun2{i} = nan(hmm.K,numWins);
            for l = 1:K
                for w = 1:numWins
                    ...
                    temp_meanfun2{i}(l,w) = sum(gamma{i}(l,begT:endT).*...
                        emiss(UDS_seg_inds{i}(begT:endT),2)',2)/sum(gamma{i}(l,begT:endT),2);
                    tot_prob{i}(l,w) = sum(gamma{i}(l,begT:endT),2)/(windowSize*Fs);
                end
                temp_meanfun{i}(l,tot_prob{i}(l,:) < p_thresh) = nan;
                temp_meanfun2{i}(l,tot_prob{i}(l,:) < p_thresh) = nan;
            end
            meandiff = [meandiff temp_meanfun{i}(2,tot_prob{i}(l,:) >= p_thresh)-...
                temp_meanfun{i}(1,tot_prob{i}(l,:) >= p_thresh)];
            meandiff2 = [meandiff2 temp_meanfun2{i}(2,tot_prob{i}(l,:) >= p_thresh)-...
                temp_meanfun2{i}(1,tot_prob{i}(l,:) >= p_thresh)];
        end
    ...
    for l = 1:K
        for i = 1:hmm.Nsegs
            curT = length(UDS_seg_inds{i});
            state_mean_est = [meanfun_est{i}(:,l) meanfun_est{i}(:,l)];
