function [state_seq,max_lik] = hsms_viterbi_real_fixmean(hmm,emiss)
...
% state_durs = (1:max_state_dur)/Fs;
% p_d(:,1) = gamma_pmf(state_durs,hmm.state(1).alpha,hmm.state(1).beta);
% p_d(:,2) = gamma_pmf(state_durs,hmm.state(2).alpha,hmm.state(2).beta);
p_d = log(hmm.P');
% p_d(1:min_state_dur-1,:) = [];
