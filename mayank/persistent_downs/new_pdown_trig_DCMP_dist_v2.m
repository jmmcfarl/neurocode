...
dc_interp_data_normu = dc_interp_data_relu./interp_win_amp;
    ...
    %%
    cur_dc_amps = dc_interp_data_normd(up_trans_inds8);
    if size(cur_dc_amps,2) > size(cur_dc_amps,1)
        cur_dc_amps = cur_dc_amps';
    end
    ctx_up_mp_down = up_trans_inds8(mp_state_vec(up_trans_inds8) == 0 & cur_dc_amps < 0.5);
    cur_dc_amps = dc_interp_data_normd(down_trans_inds8);
    if size(cur_dc_amps,2) > size(cur_dc_amps,1)
        cur_dc_amps = cur_dc_amps';
    end
    ctx_down_mp_up = down_trans_inds8(mp_state_vec(down_trans_inds8) == 1 & cur_dc_amps > 0.5);
    ...
    pdown_set = [];
    non_pdown_set = [];
    utrig_matN = nan(length(ctx_up_mp_down),length(lags));
    for ii = 1:length(ctx_up_mp_down)
        cur_inds = (ctx_up_mp_down(ii)-backlag):(ctx_up_mp_down(ii)+forwardlag);
        last_mp_down = find(down_trans_inds < ctx_up_mp_down(ii),1,'last');
        if ~isempty(last_mp_down)
            buffer_inds = (down_trans_inds(last_mp_down) + down_buffer);
            cur_inds(cur_inds <= buffer_inds) = nan;
        end
        next_mp_down = find(down_trans_inds > ctx_up_mp_down(ii),1,'first');
        if ~isempty(next_mp_down)
            next_mp_down = down_trans_inds(next_mp_down);
            cur_inds(cur_inds >= next_mp_down) = nan;
        end
        cur_inds(cur_inds < 1 | cur_inds > length(wcv_lf)) = nan;
        uset = ~isnan(cur_inds);
        utrig_matN(ii,uset) = dc_interp_data_normd(cur_inds(uset));
        
        if ismember(ctx_up_mp_down(ii),up_trans_inds8(dskipped_lfp_trans))
            pdown_set = [pdown_set ii];
        elseif ismember(ctx_up_mp_down(ii),up_trans_inds8(non_dskipped_lfp_trans))
            non_pdown_set = [non_pdown_set ii];
        end
    end
    ...
    for ii = 1:length(lags)
        ...
        uset = ~isnan(utrig_matN(pdown_set,ii));
        if sum(uset) > 5
            cur_amp_gram_pdownN(ii,:) = ksdensity(utrig_matN(pdown_set,ii),xi_drel_norm);
        else
            cur_amp_gram_pdownN(ii,:) = nan(1,length(xi_drel));
        end
        uset = ~isnan(utrig_matN(non_pdown_set,ii));
        if sum(uset) > 5
            cur_amp_gram_npdownN(ii,:) = ksdensity(utrig_matN(non_pdown_set,ii),xi_drel_norm);
        else
            cur_amp_gram_npdownN(ii,:) = nan(1,length(xi_drel));
        end
    end
    
    ctx_utrig_ampdistN(d,:,:) = cur_amp_gramN;
    ctx_utrig_ampdist_npdownN(d,:,:) = cur_amp_gram_npdownN;
...
%%
    ...
    pup_set = [];
    non_pup_set = [];
    dtrig_matN = nan(length(ctx_down_mp_up),length(lags));
    for ii = 1:length(ctx_down_mp_up)
        cur_inds = (ctx_down_mp_up(ii)-backlag):(ctx_down_mp_up(ii)+forwardlag);
        last_mp_up = find(up_trans_inds < ctx_down_mp_up(ii),1,'last');
        if ~isempty(last_mp_up)
            buffer_inds = (up_trans_inds(last_mp_up) + down_buffer);
            cur_inds(cur_inds <= buffer_inds) = nan;
        end
        next_mp_up = find(up_trans_inds > ctx_down_mp_up(ii),1,'first');
        if ~isempty(next_mp_up)
            next_mp_up = up_trans_inds(next_mp_up);
            cur_inds(cur_inds >= next_mp_up) = nan;
        end
        cur_inds(cur_inds < 1 | cur_inds > length(wcv_lf)) = nan;
        uset = ~isnan(cur_inds);
        dtrig_matN(ii,uset) = dc_interp_data_normu(cur_inds(uset));
        
        if ismember(ctx_down_mp_up(ii),down_trans_inds8(skipped_lfp_trans))
            pup_set = [pup_set ii];
        elseif ismember(ctx_down_mp_up(ii),down_trans_inds8(non_skipped_lfp_trans))
            non_pup_set = [non_pup_set ii];
        end
    end
    
    cur_uamp_gramN = nan(length(lags),length(xi_urel));
    cur_uamp_gram_npupN = nan(length(lags),length(xi_urel));
    cur_uamp_gram_pupN = nan(length(lags),length(xi_urel));
    for ii = 1:length(lags)
        uset = ~isnan(dtrig_matN(:,ii));
        if sum(uset) > 5
            cur_uamp_gramN(ii,:) = ksdensity(dtrig_matN(:,ii),xi_urel_norm);
        else
            cur_uamp_gramN(ii,:) = nan(1,length(xi_urel));
        end
        uset = ~isnan(dtrig_matN(pup_set,ii));
        if sum(uset) > 5
            cur_uamp_gram_pupN(ii,:) = ksdensity(dtrig_matN(pup_set,ii),xi_urel_norm);
        else
            cur_uamp_gram_pupN(ii,:) = nan(1,length(xi_urel));
        end
        uset = ~isnan(dtrig_matN(non_pup_set,ii));
        if sum(uset) > 5
            cur_uamp_gram_npupN(ii,:) = ksdensity(dtrig_matN(non_pup_set,ii),xi_urel_norm);
        else
            cur_uamp_gram_npupN(ii,:) = nan(1,length(xi_urel));
        end
    end
    
    ctx_dtrig_ampdistN(d,:,:) = cur_uamp_gramN;
    ctx_dtrig_ampdist_npupN(d,:,:) = cur_uamp_gram_npupN;
    if length(skipped_lfp_trans) > 5
        ctx_dtrig_ampdist_pupN(d,:,:) = cur_uamp_gram_pupN;
    else
...
%%
cd C:\WC_Germany\persistent_downs\
save new_down_trig_DCMPdist_winnorm lags xi* ctx_*
...
% set(gca,'ydir','normal');
%%
[~,ctx_utrig_maxlocs_npdown] = nanmax(ctx_utrig_ampdist_npdownN(pdown_used,:,:),[],3);
uu = find(~isnan(ctx_utrig_maxlocs_npdown));
ctx_utrig_maxlocs_npdown(uu) = xi_drel_norm(ctx_utrig_maxlocs_npdown(uu));

[~,ctx_utrig_maxlocs_npdown_lec] = nanmax(ctx_utrig_ampdist_npdownN(l3lec,:,:),[],3);
uu = find(~isnan(ctx_utrig_maxlocs_npdown_lec));
ctx_utrig_maxlocs_npdown_lec(uu) = xi_drel_norm(ctx_utrig_maxlocs_npdown_lec(uu));
...
figure
imagesc(lags/Fsd,xi_drel_norm,squeeze(nanmean(sqrt(ctx_utrig_ampdist_npdownN(l3mec,:,:))))');
set(gca,'ydir','normal');
...
line([0 0],yl,'color','k');
hold on
shadedErrorBar(lags/Fsd,nanmedian(ctx_utrig_maxlocs_npdown),nanstd(ctx_utrig_maxlocs_npdown)/sqrt(length(l3mec)));
xlim([-0.4 1])
set(gca,'fontsize',10,'fontname','arial');
fillPage(gcf,'papersize',[6 5]);

figure
imagesc(lags/Fsd,xi_drel_norm,squeeze(nanmean(sqrt(ctx_utrig_ampdist_npdownN(l3lec,:,:))))');
set(gca,'ydir','normal');
xlabel('Time since ctx up-trans (s)','fontsize',12);
ylabel('MP relative amplitude','fontsize',12);
yl = ylim();
line([0 0],yl,'color','k');
hold on
shadedErrorBar(lags/Fsd,nanmedian(ctx_utrig_maxlocs_npdown_lec),nanstd(ctx_utrig_maxlocs_npdown_lec)/sqrt(length(l3lec)));
xlim([-0.4 1])
set(gca,'fontsize',10,'fontname','arial');
fillPage(gcf,'papersize',[6 5]);

figure
imagesc(lags/Fsd,xi_drel_norm,squeeze(nanmean(sqrt(ctx_utrig_ampdist_pdownN(pdown_used,:,:))))');
set(gca,'ydir','normal');
xlabel('Time since ctx up-trans (s)','fontsize',12);
ylabel('MP relative amplitude','fontsize',12);
yl = ylim();
...
% set(gca,'ydir','normal');
%%
[~,ctx_dtrig_maxlocs_npup] = nanmax(ctx_dtrig_ampdist_npupN(pup_used,:,:),[],3);
uu = find(~isnan(ctx_dtrig_maxlocs_npup));
ctx_dtrig_maxlocs_npup(uu) = xi_urel_norm(ctx_dtrig_maxlocs_npup(uu));

[~,ctx_dtrig_maxlocs] = nanmax(ctx_dtrig_ampdist_pupN(pup_used,:,:),[],3);
uu = find(~isnan(ctx_dtrig_maxlocs));
ctx_dtrig_maxlocs(uu) = xi_urel_norm(ctx_dtrig_maxlocs(uu));

figure
imagesc(lags/Fsd,xi_urel_norm,squeeze(nanmean(sqrt(ctx_dtrig_ampdist_npupN(pup_used,:,:))))');
set(gca,'ydir','normal');
xlabel('Time since ctx up-trans (s)','fontsize',12);
ylabel('MP relative amplitude','fontsize',12);
yl = ylim();
line([0 0],yl,'color','k');
hold on
shadedErrorBar(lags/Fsd,nanmedian(ctx_dtrig_maxlocs_npup),nanstd(ctx_dtrig_maxlocs_npup)/sqrt(length(pup_used)));
xlim([-0.4 1])
set(gca,'fontsize',10,'fontname','arial');
fillPage(gcf,'papersize',[6 5]);
...
line([0 0],yl,'color','k');
hold on
shadedErrorBar(lags/Fsd,nanmedian(ctx_dtrig_maxlocs),nanstd(ctx_dtrig_maxlocs)/sqrt(length(pup_used)));
xlim([-0.4 1])
ylim([-1.4 0.5])
set(gca,'fontsize',10,'fontname','arial');
fillPage(gcf,'papersize',[6 5]);
...
for ii = 1:length(pdown_used)
    ...
    ii
    subplot(3,1,1);
    imagesc(lags/Fsd,xi_drel_norm,squeeze(sqrt(ctx_utrig_ampdist_npdownN(pdown_used(ii),:,:)))');
    set(gca,'ydir','normal');
    xlabel('Time since ctx up-trans (s)','fontsize',12);
    ylabel('MP relative amplitude','fontsize',12);
    hold on
...
%%
close all
for ii = 1:length(pup_used)
    %     ii = 16
    ii
    subplot(3,1,1);
    imagesc(lags/Fsd,xi_urel_norm,squeeze(sqrt(ctx_dtrig_ampdist_npupN(pup_used(ii),:,:)))');
    set(gca,'ydir','normal');
    xlabel('Time since ctx up-trans (s)','fontsize',12);
    ylabel('MP relative amplitude','fontsize',12);
    hold on
    plot(lags/Fsd,ctx_dtrig_maxlocs_npup(ii,:),'w','linewidth',2);
    yl = ylim();
    line([0 0],yl,'color','k');
    subplot(3,1,2);
    imagesc(lags/Fsd,xi_urel_norm,squeeze(sqrt(ctx_dtrig_ampdist_pupN(pup_used(ii),:,:)))');
    set(gca,'ydir','normal');
    xlabel('Time since ctx up-trans (s)','fontsize',12);
    ylabel('MP relative amplitude','fontsize',12);
    hold on
    plot(lags/Fsd,ctx_dtrig_maxlocs(ii,:),'w','linewidth',2);
    yl = ylim();
    line([0 0],yl,'color','k');
    subplot(3,1,3)
    hist(mp_downlags{pup_used(ii)}(nrt2_ups{pup_used(ii)})/Fsd,50);
    xlim(lags([1 end])/Fsd);
    pause
    clf
end
