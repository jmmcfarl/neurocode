...
%     end
    ...
    if ~isempty(ep)
        ...
        lf8_lf(ep+1:end) = []; wcv_lf(ep+1:end) = []; t_axis(ep+1:end) = []; lf8_hf(ep+1:end) = []; %hpc_hf(ep+1:end) = [];
    else
        ep = length(t_axis);
    end
...
%         corresp_lf8_upinds{d},corresp_lf8_downinds{d},lfp_state_durations{d}{2},lfp_state_durations{d}{1},thresh_lf8_downdur,thresh_lf8_updur);
   ...
   fract_t2_downs(d) = length(t2_downs{d})/(length(t2_downs{d}) + length(nrt2_downs{d}));
...
%     rmp_downdurs_lfpc{d}(mp_downskipped{d}.min_dur < thresh_lf8_updur) = [];
    ...
    for ii = 1:length(rt2_downs{d})
        skipped_ctx_ups = cat(2,skipped_ctx_ups,mp_downskipped{d}.inds{rt2_downs{d}(ii)});
    end
    non_skipped_ctx_ups = [];
    for ii = 1:length(up_trans_inds)
        non_skipped_ctx_ups = cat(2,non_skipped_ctx_ups,corresp_lf8_upinds{d}(ii));
    end
    non_skipped_ctx_ups(isnan(non_skipped_ctx_ups)) = [];
 
    pers_ctx_ups = [];
    for ii = 1:length(rt2_ups{d})
        pers_ctx_ups = cat(2,pers_ctx_ups,corresp_lf8_upinds{d}(rt2_ups{d}(ii)));
    end
    npers_ctx_ups = [];
    for ii = 1:length(nrt2_ups{d})
        npers_ctx_ups = cat(2,npers_ctx_ups,corresp_lf8_upinds{d}(nrt2_ups{d}(ii)));
    end

    [up_amps,down_amps] = get_state_amplitudes(lf8_lf,up_trans_inds8,down_trans_inds8);
    [up_amps_hf,down_amps_hf] = get_state_amplitudes(lf8_hf,up_trans_inds8,down_trans_inds8);
    [up_amps_mua,down_amps_mua] = get_state_amplitudes(ctx_mua_rate,up_trans_inds8,down_trans_inds8);
    if ~isempty(skipped_ctx_ups)
        skipped_up_amp(d) = mean(up_amps(skipped_ctx_ups));
        skipped_up_amp_hf(d) = mean(up_amps_hf(skipped_ctx_ups));
        skipped_up_amp_mua(d) = mean(up_amps_mua(skipped_ctx_ups));
        skipped_up_dur(d) = mean(lfp_state_durations{d}{2}(skipped_ctx_ups));
    else
        skipped_up_amp(d) = nan;
        skipped_up_amp_hf(d) = nan;
        skipped_up_amp_mua(d) = nan;
        skipped_up_dur(d) = nan;
    end
    nskipped_up_amp(d) = mean(up_amps(non_skipped_ctx_ups));
    nskipped_up_amp_hf(d) = mean(up_amps_hf(non_skipped_ctx_ups));
    nskipped_up_amp_mua(d) = mean(up_amps_mua(non_skipped_ctx_ups));
    nskipped_up_dur(d) = mean(lfp_state_durations{d}{2}(non_skipped_ctx_ups));
    
    if ~isempty(pers_ctx_ups)
        pers_up_amp(d) = mean(up_amps(pers_ctx_ups));
        pers_up_amp_hf(d) = mean(up_amps_hf(pers_ctx_ups));
        pers_up_amp_mua(d) = mean(up_amps_mua(pers_ctx_ups));
        pers_up_dur(d) = mean(lfp_state_durations{d}{2}(pers_ctx_ups));
    else
        pers_up_amp(d) = nan;
        pers_up_amp_hf(d) = nan;
        pers_up_amp_mua(d) = nan;
    end
    npers_up_amp(d) = mean(up_amps(npers_ctx_ups));
    npers_up_amp_hf(d) = mean(up_amps_hf(npers_ctx_ups));
    npers_up_amp_mua(d) = mean(up_amps_mua(npers_ctx_ups));
    npers_up_dur(d) = mean(lfp_state_durations{d}{2}(npers_ctx_ups));
   
    %%
    nrt2_time_since_last_mp_up{d} = nan(size(up_trans_inds));
    for i = 1:length(nrt2_time_since_last_mp_up{d})
    ...
    pup_time_since_last_mp_up{d} = nrt2_time_since_last_mp_up{d}(rt2_ups{d});
    npup_time_since_last_mp_up{d} = nrt2_time_since_last_mp_up{d}(nrt2_ups{d});
    
    rt2_time_since_last_mp_up{d} = nan(length(rt2_downs{d}),1);
    for i = 1:length(rt2_downs{d})
        ...
        if ~isempty(prev_mp_down)
        rt2_time_since_last_mp_up{d}(i) = (up_trans_inds8(cur_first_skipped) - down_trans_inds(prev_mp_down))/Fsd;
        end
    end
    rt2_time_since_last_mp_up{d}(rt2_time_since_last_mp_up{d} > 10) = 10;
    
    pdown_diff_tslmu(d) = ranksum(rt2_time_since_last_mp_up{d},nrt2_time_since_last_mp_up{d});
    pup_diff_tslmu(d) = ranksum(pup_time_since_last_mp_up{d},npup_time_since_last_mp_up{d});
    
end

cd C:\WC_Germany\persistent_downs\
% save allEC_core_analysis
save new_down_core_analysis_backlook
...
figure;
plot(median_downlag(l3mec)/Fsd,fract_rt2_ups(l3mec),'r.','markersize',8)
hold on
plot(median_downlag(l3lec)/Fsd,fract_rt2_ups(l3lec),'.','markersize',8)
box off
legend('MEC','LEC');
...
% subplot(2,1,2)
figure
plot(median_uplag(l3mec)/Fsd,fract_rt2_downs(l3mec),'r.','markersize',8)
hold on
plot(median_uplag(l3lec)/Fsd,fract_rt2_downs(l3lec),'.','markersize',8)
box off
legend('MEC','LEC');
...
%%
figure;hold on
plot(median_uplag(l3mec)/Fsd,median_downlag(l3mec)/Fsd,'r.','markersize',8);
plot(median_uplag(l3lec)/Fsd,median_downlag(l3lec)/Fsd,'.','markersize',8);
box off
legend('MEC','LEC');
xlabel('Up-transition lag (s)','fontsize',14);
...
figure;hold on
plot(100*fract_rt2_ups(l3lec),100*fract_rt2_downs(l3lec),'b.','markersize',8);
plot(100*fract_rt2_ups(l3mec),100*fract_rt2_downs(l3mec),'r.','markersize',8);
legend('LEC','MEC');
...
%% DISTRIBUTION OF LATENCIES SINCE MP DOWN-TRANS FOR PERS VS NON-PERS
close all
min_pdown_frac = 0.1;
% tt_bins = linspace(0.05,6,50);
tt_bins = logspace(log10(0.1),log10(6),25);
tt_bin_cents = 0.5*tt_bins(1:end-1) + 0.5*tt_bins(2:end);
clear nn_nrt2 nn_rt2
uset = l3mec(fract_rt2_downs(l3mec) >= min_pdown_frac);
for ii = 1:length(uset)
   nn_rt2(ii,:) = histc(rt2_time_since_last_mp_up{uset(ii)},tt_bins);
   nn_nrt2(ii,:) = histc(nrt2_time_since_last_mp_up{uset(ii)},tt_bins);
end
nn_rt2 = bsxfun(@rdivide,nn_rt2,sum(nn_rt2,2));
nn_nrt2 = bsxfun(@rdivide,nn_nrt2,sum(nn_nrt2,2));

h1=shadedErrorBar(tt_bin_cents,mean(nn_nrt2(:,1:end-1)),std(nn_nrt2(:,1:end-1))/sqrt(length(uset)));
hold on
h2=shadedErrorBar(tt_bin_cents,mean(nn_rt2(:,1:end-1)),std(nn_rt2(:,1:end-1))/sqrt(length(uset)),{'color','r'});
yl = ylim(); ylim([0 yl(2)]);
xlabel('Time since last MP down-transition (s)','fontsize',12);
...
tt_bin_cents = 0.5*tt_bins(1:end-1) + 0.5*tt_bins(2:end);
clear nn_nrt2 nn_rt2
uset = l3mec(fract_rt2_ups(l3mec) >= min_pup_frac);
for ii = 1:length(uset)
   nn_rt2(ii,:) = histc(pup_time_since_last_mp_up{uset(ii)},tt_bins);
   nn_nrt2(ii,:) = histc(npup_time_since_last_mp_up{uset(ii)},tt_bins);
end
nn_rt2 = bsxfun(@rdivide,nn_rt2,sum(nn_rt2,2));
nn_nrt2 = bsxfun(@rdivide,nn_nrt2,sum(nn_nrt2,2));
...
h2=shadedErrorBar(tt_bin_cents,mean(nn_rt2(:,1:end-1)),std(nn_rt2(:,1:end-1))/sqrt(length(uset)),{'color','r'});
yl = ylim(); ylim([0 yl(2)]);
xlabel('Time since MP down-transition (s)','fontsize',12);
ylabel('Relative frequency','fontsize',12);
legend([h1.mainLine h2.mainLine],{'Non-persistent','Persistent'});
box off
set(gca,'fontsize',10,'fontname','arial');
