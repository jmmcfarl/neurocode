...
for d = 1 :length(used_dirs)
    ...
    [ev_avg,lags,ev_std,n_events,utrig_matN] = get_event_trig_avg(dc_interp_data_normd(:),ctx_up_mp_down,backlag,forwardlag,1,mp_state_number,-1);
     [ev_avg,lags,ev_std,n_events,utrig_matN_npdown] = get_event_trig_avg(dc_interp_data_normd(:),up_trans_inds8(non_dskipped_lfp_trans),backlag,forwardlag,1,mp_state_number,-1);
...
l3lec = find(strcmp(data_type,'L3LEC'));
...
%%
figure
imagesc(lags/Fsd,xi_drel_norm,squeeze(nanmean(sqrt(ctx_utrig_ampdistN(pdown_used,:,:))))');
set(gca,'ydir','normal');
