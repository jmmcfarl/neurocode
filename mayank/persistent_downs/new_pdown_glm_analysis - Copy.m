...
used_dirs = find(data_ep > min_rec_dur & ~isnan(data_hpc_mua));
...
%     std_ctx_mua_rate(d) = nanstd(ctx_mua_rate);
    hpc_mua_rate = hpc_mua_rate/avg_hpc_mua_rate(d);
    ...
    used_uinds = up_trans_inds(up_trans_inds > round(backlag*Fsd));
    up_trans_vec(used_uinds-round(backlag*Fsd)) = 1;
    up_trans8_vec = zeros(size(t_axis))';
    used_uinds8 = up_trans_inds8(up_trans_inds8 > round(backlag*Fsd));
    up_trans8_vec(used_uinds8-round(backlag*Fsd)) = 1;
    down_trans_vec = zeros(size(t_axis))';
    used_dinds = down_trans_inds(down_trans_inds > round(backlag*Fsd));
    down_trans_vec(used_dinds-round(backlag*Fsd)) = 1;
    down_trans8_vec = zeros(size(t_axis))';
    used_dinds8 = down_trans_inds8(down_trans_inds8 > round(backlag*Fsd));
    down_trans8_vec(used_dinds8-round(backlag*Fsd)) = 1;
    ...
    hpc_filts(d,:,:) = [glm1.mods(:).filtK];

    Robs = ctx_mua_rate;
    glm1 = NMMfit_filters(glm1,Robs,stim_mat,[],[],silent,optim_p);
    ctx_filts(d,:,:) = [glm1.mods(:).filtK];
...
n_pts = length(used_dirs);
shadedErrorBar(t_ax,squeeze(mean(hpc_filts(:,:,1))),squeeze(std(hpc_filts(:,:,1)))/sqrt(n_pts),{'color','r'});
hold on
shadedErrorBar(t_ax,squeeze(mean(hpc_filts(:,:,2))),squeeze(std(hpc_filts(:,:,2)))/sqrt(n_pts),{'color','k'});

figure
shadedErrorBar(t_ax,squeeze(mean(hpc_filts(:,:,3))),squeeze(std(hpc_filts(:,:,3)))/sqrt(n_pts),{'color','r'});
hold on
shadedErrorBar(t_ax,squeeze(mean(hpc_filts(:,:,4))),squeeze(std(hpc_filts(:,:,4)))/sqrt(n_pts),{'color','k'});

figure
shadedErrorBar(t_ax,squeeze(mean(ctx_filts(:,:,1))),squeeze(std(ctx_filts(:,:,1)))/sqrt(n_pts),{'color','r'});
hold on
shadedErrorBar(t_ax,squeeze(mean(ctx_filts(:,:,2))),squeeze(std(ctx_filts(:,:,2)))/sqrt(n_pts),{'color','k'});

figure
shadedErrorBar(t_ax,squeeze(mean(ctx_filts(:,:,3))),squeeze(std(ctx_filts(:,:,3)))/sqrt(n_pts),{'color','r'});
hold on
shadedErrorBar(t_ax,squeeze(mean(ctx_filts(:,:,4))),squeeze(std(ctx_filts(:,:,4)))/sqrt(n_pts),{'color','k'});
