...
shadedErrorBar(lags,mean(lf8_dtrig_wcv_nsk(used,:)),std(lf8_dtrig_wcv_nsk(used,:))/sqrt(length(used)),{'r'});
shadedErrorBar(lags,mean(lf8_dtrig_wcv_sk(used,:)),std(lf8_dtrig_wcv_sk(used,:))/sqrt(length(used)),{'color',[0.5 0.1 0.1]});
xlim([-1.5 2.5])
ylabel('Amplitude (z)','fontsize',16,'fontname','arial')
ax1 = gca;set(ax1,'box','off')
ax2 = axes('Position',get(ax1,'Position'),...
...
% errorbar_tick(h,.001,'units');
shadedErrorBar(lags,nanmean(lf8_dtrig_mua_sk(used,:)),nanstd(lf8_dtrig_mua_sk(used,:))/sqrt(length(used)),{'color',[0.1 0.6 0.1]});
shadedErrorBar(lags,nanmean(lf8_dtrig_mua_nsk(used,:)),nanstd(lf8_dtrig_mua_nsk(used,:))/sqrt(length(used)),{'color',[0.3 0.8 0.3]});
xlim([-1.5 2.5])
ylim([-0.4 1.2])
xlabel('Time since cortical down transition (s)','fontsize',16,'fontname','arial')
