...
%         hpc_lf = get_lf_features(lf3,raw_Fs,raw_Fs,[lcf hcf]);
        hpc_hf = get_hf_features(lf3,raw_Fs,raw_Fs,[lcf_hf hcf_hf],hcf_sm);
    else
        load ./used_data lf2
%         hpc_lf = get_lf_features(lf2,raw_Fs,raw_Fs,[lcf hcf]);
...
%     [desynch_times,desynch_inds,P_lf8,f,t] = locate_desynch_times_individual(lf8);
    [desynch_times,desynch_inds,P_lf8,f,t] = locate_desynch_times_individual_v2(lf7);
...
%%
cd C:\WC_Germany\sven_thomas_combined\
save combined_spectra_fin_nd S* f
...
%%
shadedErrorBar(f,mean(norm_hpc_mua(l3mec_m,:)),std(norm_hpc_mua(l3mec_m,:))/sqrt(length(l3mec_m)),{'c'});
