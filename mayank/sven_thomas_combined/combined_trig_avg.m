...
cd C:\WC_Germany\sven_thomas_combined\
save combined_trig_mua_fin_nd *_mua *_lf8 *_wcv maxlag Fsd *dtrig* *utrig*

%% LF8 D-trig NO MUA (FINAL)
all_cells = [l3mec l3lec];
figure; set(gca,'fontname','arial','fontsize',14)
plot(lags,mean(lf8_dtrig_lf8(all_cells,:)),'color',[0.2 0.2 0.2],'linewidth',2)
hold on
plot(lags,mean(lf8_dtrig_wcv(l3mec,:)),'r','linewidth',2)
plot(lags,mean(lf8_dtrig_wcv(l3lec,:)),'b','linewidth',2)
legend('Ctx LFP','MEC MP','LEC MP')
shadedErrorBar(lags,mean(lf8_dtrig_lf8(all_cells,:)),std(lf8_dtrig_lf8(all_cells,:))/sqrt(length(all_cells)),{'color',[0.2 0.2 0.2]});
shadedErrorBar(lags,mean(lf8_dtrig_wcv(l3mec,:)),std(lf8_dtrig_wcv(l3mec,:))/sqrt(length(l3mec)),{'r'});
shadedErrorBar(lags,mean(lf8_dtrig_wcv(l3lec,:)),std(lf8_dtrig_wcv(l3lec,:))/sqrt(length(l3lec)),{'b'});
xlim([-1 1.5])
...
shadedErrorBar(lags,mean(lf8_dtrig_wcv(l3mec,:)),std(lf8_dtrig_wcv(l3mec,:))/sqrt(length(l3mec)),{'r'});
shadedErrorBar(lags,mean(lf8_dtrig_wcv(l3lec,:)),std(lf8_dtrig_wcv(l3lec,:))/sqrt(length(l3lec)),{'b'});
% h = errorbar(lags,mean(lf8_dtrig_hpch(all_h,:)),std(lf8_dtrig_hpch(all_h,:))/sqrt(length(all_h)),'g','linewidth',2);
% errorbar_tick(h,.001,'units');
% h = errorbar(lags,mean(lf8_dtrig_hpch(l3lec_h,:)),std(lf8_dtrig_hpch(l3lec_h,:))/sqrt(length(l3lec_h)),'c','linewidth',2);
% errorbar_tick(h,.001,'units');
xlim([-1 1.5])
...
hold on
shadedErrorBar(lags,mean(lf8_dtrig_mua(l3mec_m,:)),std(lf8_dtrig_mua(l3mec_m,:))/sqrt(length(l3mec_m)),{'color',[0.3 0.8 0.3],'parent',ax2});
ylim([-0.5 0.75])
xlim([-1 1.5])
...
errorbar_tick(h,.001,'units');
h = errorbar(lags,nanmean(lf8_dtrig_wcv_nsk(l3lec,:)),nanstd(lf8_dtrig_wcv_nsk(l3lec,:))/sqrt(length(l3lec)),'b','linewidth',2);
errorbar_tick(h,.001,'units');
h = errorbar(lags,nanmean(lf8_dtrig_wcv_sk(l3lec,:)),nanstd(lf8_dtrig_wcv_sk(l3lec,:))/sqrt(length(l3lec)),'color',[0.1 0.1 0.5],'linewidth',2);
errorbar_tick(h,.001,'units');
xlim([-1.5 1.5])
ylabel('Amplitude (z)','fontsize',14)
xlabel('Time since cortical down transition (s)','fontsize',14)
...
hold on
plot(lags,mean(lf8_dtrig_lf8(all_cells,:)),'color',[0.2 0.2 0.2],'linewidth',2)
plot(lags,mean(lf8_dtrig_wcv_nsk(l3mec,:)),'r','linewidth',2)
plot(lags,mean(lf8_dtrig_wcv_sk(l3mec,:)),'color',[0.5 0.1 0.1],'linewidth',2)
...
% h = errorbar(lags,mean(lf8_dtrig_wcv_nsk(l3mec,:)),std(lf8_dtrig_wcv_nsk(l3mec,:))/sqrt(length(l3mec)),'r','linewidth',2);
% errorbar_tick(h,.001,'units');
% h = errorbar(lags,mean(lf8_dtrig_wcv_sk(l3mec,:)),std(lf8_dtrig_wcv_sk(l3mec,:))/sqrt(length(l3mec)),'color',[0.5 0.1 0.1],'linewidth',1);
% errorbar_tick(h,.001,'units');
shadedErrorBar(lags,mean(lf8_dtrig_lf8_nsk(all_cells,:)),std(lf8_dtrig_lf8_nsk(all_cells,:))./sqrt(sum(~isnan(lf8_dtrig_lf8_nsk(all_cells,:)))),{'color',[0.2 0.2 0.2]});
hold on
shadedErrorBar(lags,nanmean(lf8_dtrig_lf8_sk(all_cells,:)),nanstd(lf8_dtrig_lf8_sk(all_cells,:))./sqrt(sum(~isnan(lf8_dtrig_lf8_sk(all_cells,:)))),{'color',[0.4 0.4 0.4]});
shadedErrorBar(lags,mean(lf8_dtrig_wcv_nsk(l3mec,:)),std(lf8_dtrig_wcv_nsk(l3mec,:))/sqrt(length(l3mec)),{'r'});
shadedErrorBar(lags,mean(lf8_dtrig_wcv_sk(l3mec,:)),std(lf8_dtrig_wcv_sk(l3mec,:))/sqrt(length(l3mec)),{'color',[0.5 0.1 0.1]});
xlim([-1.5 2.5])
ylabel('Amplitude (z)','fontsize',16,'fontname','arial')
ax1 = gca;set(ax1,'box','off')
ax2 = axes('Position',get(ax1,'Position'),...
...
% errorbar_tick(h,.001,'units');
shadedErrorBar(lags,nanmean(lf8_dtrig_mua_sk(l3mec_m,:)),nanstd(lf8_dtrig_mua_sk(l3mec_m,:))/sqrt(length(l3mec_m)),{'color',[0.1 0.6 0.1]});
shadedErrorBar(lags,nanmean(lf8_dtrig_mua_nsk(l3mec_m,:)),nanstd(lf8_dtrig_mua_nsk(l3mec_m,:))/sqrt(length(l3mec_m)),{'color',[0.3 0.8 0.3]});
xlim([-1.5 2.5])
ylim([-0.4 1.2])
xlabel('Time since cortical down transition (s)','fontsize',16,'fontname','arial')
