...
%     load ./pa_hsmm_state_seq7_combined_fin.mat
    ...
    mp_dutycyc{d}(isnan(mp_dutycyc{d})) = nanmean(mp_dutycyc{d});
    lf8_dutycyc{d}(isnan(lf8_dutycyc{d})) = nanmean(lf8_dutycyc{d});
    mp_medupdur{d}(isnan(mp_medupdur{d})) = nanmean(mp_medupdur{d});
    lf8_medupdur{d}(isnan(lf8_medupdur{d})) = nanmean(lf8_medupdur{d});
...
set(gca,'fontsize',14,'fontname','arial')
hold on
shadedErrorBar(lags/spec_Fs,nanmean(Cw8_updur(l3lec,:)),std(Cw8_updur(l3lec,:))/sqrt(length(l3lec)),{'b'});
shadedErrorBar(lags/spec_Fs,nanmean(Cw8_updur(l3mec,:)),std(Cw8_updur(l3mec,:))/sqrt(length(l3mec)),{'r'});
