function [desynch_times,desynch_ids,P,f,t] = locate_desynch_times_individual_v2(sig,is_relative)
...
df = f(2)-f(1);
f_so = find(f > 0.25 & f < 1.5); %define the slow-oscillation band as 0.2-1.5Hz
...
net_hf = (trapz(log10(P(:,f_hf)),2)*df); %z-score of the integral HF power
