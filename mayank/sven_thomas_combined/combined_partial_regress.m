...
for d = 1:length(combined_dir)
    if ~isnan(hpc_mua(d))
        ...
        if ~isnan(hpc_mua(d))
            ...
            %         C3w_dpart_good(d,:) = B_avg_good(1:NLags);
            %         C3w_upart_good(d,:) = B_avg_good(NLags+1:2*NLags);
            %         C38_upart_good(d,:) = B_avg_good(2*NLags+1:3*NLags);
            %         C38_dpart_good(d,:) = B_avg_good(3*NLags+1:4*NLags);
            %
            %         C3w_dpart_pyr(d,:) = B_avg_pyr(1:NLags);
            %         C3w_upart_pyr(d,:) = B_avg_pyr(NLags+1:2*NLags);
            %         C38_upart_pyr(d,:) = B_avg_pyr(2*NLags+1:3*NLags);
            %         C38_dpart_pyr(d,:) = B_avg_pyr(3*NLags+1:4*NLags);
...
cd C:\WC_Germany\sven_thomas_combined\
save combined_partial_regress_mua_fin_nd C3* lags maxLag
% load ./mua_rate_data
...
% % errorbar_tick(h,.001,'units');
% shadedErrorBar(lags/Fsd,mean(C3w_upart(l3mec_m,:)),std(C3w_upart(l3mec_m,:))/sqrt(length(l3mec_m)),{'r'});
% hold on
% shadedErrorBar(lags/Fsd,mean(C38_upart(l3mec_m,:)),std(C38_upart(l3mec_m,:))/sqrt(length(l3mec_m)),{'k'});
% xlim([-1.5 1.5])
...
% % errorbar_tick(h,.001,'units');
% shadedErrorBar(lags/Fsd,mean(C3w_dpart(l3mec_m,:)),std(C3w_dpart(l3mec_m,:))/sqrt(length(l3mec_m)),{'r'});
% shadedErrorBar(lags/Fsd,mean(C38_dpart(l3mec_m,:)),std(C38_dpart(l3mec_m,:))/sqrt(length(l3mec_m)),{'k'});
% xlim([-1.5 1.5])
% xlabel('Time lag (s)','fontsize',14)
% ylabel('Regression Coefficient','fontsize',14)
% legend('MP','Ctx LFP')
