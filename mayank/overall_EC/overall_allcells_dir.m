...
sess_data(cur_tot+21).heka_dir = '';
sess_data(cur_tot+22).heka_dir = '';
sess_data(cur_tot+23).heka_dir = '';
...
sess_data(cur_tot+18).depth = nan;
...
sess_data(cur_tot+19).depth = nan;
...
sess_data(cur_tot+20).depth = nan;
...
sess_data(cur_tot+21).name = '2007_05-23_B';
sess_data(cur_tot+21).depth = nan;
...
sess_data(cur_tot+21).heka_type = 'nan';

sess_data(cur_tot+22).name = '2007_05-28_B';
sess_data(cur_tot+22).depth = nan;
...
sess_data(cur_tot+23).name = '2007_10-10_B';
sess_data(cur_tot+23).depth = nan;
...
sess_data(cur_tot+23).heka_type = 'nan';
...
sess_data(cur_tot+c).heka_type = 'sweep';
sess_data(cur_tot+c).layer = '56';
sess_data(cur_tot+c).cell_type = 'non-pyramidal';
sess_data(cur_tot+c).region = 'MEC';
c = c+1;
...
sess_data(cur_tot+c).gains = [50; 1e3; 2e3; 2e3; 2e3; 2e3; 2e3; 3e3];
sess_data(cur_tot+c).heka_type = 'cont';
sess_data(cur_tot+c).layer = '';
sess_data(cur_tot+c).cell_type = '';
...
sess_data(cur_tot+c).heka_type = 'nan';
sess_data(cur_tot+c).layer = '2';
sess_data(cur_tot+c).cell_type = 'excitatory';
sess_data(cur_tot+c).region = 'LEC';
c = c+1;
...
%%
cd F:\WC_Germany
load wc_database wc_db

%% ADD Hippocampal cells
cur_tot = cur_tot + c -1;
c=1;
...
hipp = unique([ca1 ca3 dg]);
for i = 1:length(hipp)
    sess_data(cur_tot+c).directory = wc_db(hipp(i)).directory;
    sess_data(cur_tot+c).name = wc_db(hipp(i)).date;
    sess_data(cur_tot+c).depth = nan;
    sess_data(cur_tot+c).ant_post = nan;
    sess_data(cur_tot+c).lateral = nan;
    sess_data(cur_tot+c).dors_vent = nan;
    sess_data(cur_tot+c).dist_prh = nan;
    sess_data(cur_tot+c).heka_type = 'nan';
    sess_data(cur_tot+c).layer = 'na';
    sess_data(cur_tot+c).gains = wc_db(hipp(i)).gains;
    sess_data(cur_tot+c).cell_type = wc_db(hipp(i)).celltype;
    sess_data(cur_tot+c).region = wc_db(hipp(i)).region;
    c = c+1;
end

sess_data_ov = sess_data;
%% ADD cortical cells
cd F:\WC_Germany\parietal_cortical_2010\
load parietal_cortical_2010 sess_data

cur_tot = cur_tot + c-1;
c = 1;
for i = 1:length(sess_data)
    sess_data_ov(cur_tot+c).directory = sess_data(i).directory;
    sess_data_ov(cur_tot+c).name = sess_data(i).name;
    sess_data_ov(cur_tot+c).depth = sess_data(i).depth;
    sess_data_ov(cur_tot+c).ant_post = sess_data(i).ant_post;
    sess_data_ov(cur_tot+c).lateral = sess_data(i).lateral;
    sess_data_ov(cur_tot+c).dors_vent = nan;
    sess_data_ov(cur_tot+c).dist_prh = nan
    sess_data_ov(cur_tot+c).heka_type = 'nan';
    sess_data_ov(cur_tot+c).layer = sess_data(i).layer;
    sess_data_ov(cur_tot+c).gains = sess_data(i).gains;
    sess_data_ov(cur_tot+c).cell_type = sess_data(i).cell_type;
    sess_data_ov(cur_tot+c).region = sess_data(i).region;
    sess_data_ov(cur_tot+c).thom_elec = 0;
    c = c+1;
end

%%
sess_data = sess_data_ov;
for i = 1:length(sess_data)
    if isempty(sess_data(i).thom_elec)
        sess_data(i).thom_elec = 0;
    end
end
cd F:\WC_Germany\overall_EC
save overall_allcells_dir sess_data
