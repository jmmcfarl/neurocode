...
for d = 1:length(sess_data)
    ...
    for ns = 1:hmm.Nsegs
        ...
        for i = 1:length(cur_up_trans)
            ...
            if n_lfp_ups > near_lfp_up+1 %if the nearest up transition is not the final LFP up-transition
                corresp_lfp_downdur{d}(cnt) = (cur_up_trans8(near_lfp_up+1)-cur_down_trans8(near_lfp_up))/Fsd;
