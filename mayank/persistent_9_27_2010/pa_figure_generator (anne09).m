...
n_mec = length(mec_cells);
n_lec = length(lec_cells);
load F:\WC_Germany\persistent_9_27_2010\pa_corresponding_lfp_state_data_rtest

%good examples are 1,2,5,8,14,15,19
hist_range = linspace(0,7,1000);
binsize = hist_range(2)-hist_range(1);
for i = 1:n_mec+n_lec
%     useable_ups = find(corresp_lfp_downdur{i} > 0.5);
    useable_ups = find(corresp_lfp_dmindowndur{i} > 0.5);
    cell_hist(i,:) = hist(mp_updur_lfpc_delay{i}(useable_ups),hist_range);
%     cell_hist(i,:) = hist(mp_updur_lfpc{i}(useable_ups),hist_range);
    cell_hist(i,:) = cell_hist(i,:)/sum(cell_hist(i,:))/binsize;
...
y = hist(mp_updur_lfpc_delay{examp_ind},x);
sy = sum(y);
...
[mec_peaks,mec_peak_locs] = findpeaks(m_corr_mec(zero_lag+min_loF:end));
...
[peak_lec,peak_loc_lec] = max(m_corr_lec(zero_lag+min_lec_loF:end));
...
[peak_lfp,peak_loc_lfp] = max(m_corr_lfp(zero_lag+min_lfp_loF:end));
...
load F:\WC_Germany\persistent_revised\pers_sim_cortlfp_dir
pre_cells = 1:16;
fro_cells = 17:24;
...
% 
% uds_freqs = find(f > 0 & f < 1);
% [peak_Cw8,peak_Cw8_loc] = nanmax(aCw8_cor(:,uds_freqs),[],2);
% [peak_Cw3,peak_Cw3_loc] = nanmax(aCw3_cor(:,uds_freqs),[],2);
...
%
load F:\WC_Germany\persistent_9_27_2010\pa_trigmat_cell14
d = 14;
trans_num_1 = 24;
begt_1 = 100.8;
endt_1 = 103.5;
...
figure
plot(t,wcv_d,t,lf8_d+3,'r')
% hold on
% plot(t,mp_state_vec,'c',t,lf8_state_vec-1.1,'k')
% xlim([xl1 xl2])