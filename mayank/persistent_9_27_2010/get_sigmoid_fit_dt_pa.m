...
function [rlamp,rlshift,rltau,rsquared,t_50,t_90,t_10,fit_data] = ...
    get_sigmoid_fit_dt_pa(up_trans,down_trans,data,time,Fs)

addpath('F:\Code\WC_anal\general\')

data = data(:);

n_downs = length(down_trans);

rlamp = nan(n_downs,1);
rltau = nan(n_downs,1);
rlshift = nan(n_downs,1);
rsquared = nan(n_downs,1);
t_10 = nan(n_downs,1);
t_90 = nan(n_downs,1);
t_50 = nan(n_downs,1);
...
for i=1:length(down_trans)
    ...
    cur_trans = down_trans(i);
    
    %if this is not the first down transition, set the beggining edge of the
    %fitting window to be the previous up transition plus a buffer to
    %bypass the transition region
    prev_up = up_trans(i)+state_buffer;
    if i < length(up_trans)
        next_up = up_trans(i+1);
    else
        next_up = length(data);
    end
    
    %if the current down state and previous up states last for the minimum duration (necessary to fit a
    %sigmoid)
    if (next_up - cur_trans) > state_buffer && (cur_trans - prev_up) > state_buffer
        
        %set the range of the data-fitting window
        start_p = max([prev_up,cur_trans-target_window,1]);
        end_p = min([length(time),cur_trans+target_window,next_up-state_buffer]);
