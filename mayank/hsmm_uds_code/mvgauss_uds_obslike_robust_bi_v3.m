function [B,bad_model] = mvgauss_uds_obslike_robust_bi_v3(emiss,hmm,rel_prob,seg_number)
...
bad_down = find(emiss(:,1) > state_meanfun(:,1,2) & emiss(:,2) > state_meanfun(:,2,2) & more_likely_state==1);
...
bad_up = find(emiss(:,1) < state_meanfun(:,1,1) & emiss(:,2) < state_meanfun(:,2,1) & more_likely_state==2);
