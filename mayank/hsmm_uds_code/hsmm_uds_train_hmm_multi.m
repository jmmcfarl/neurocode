...
while delta_log_post > tolerance
    ...
    for i = 1:hmm.Nsegs
                [B,rob_model_inds{i}] = mvgauss_uds_obslike_robust_multi_v3(emiss(UDS_seg_inds{i},:),hmm,rel_prob,i);
%                 [B,rob_model_inds{i}] = mvgauss_uds_obslike_robust(emiss(UDS_seg_inds{i},:),hmm,i);
...
%         fixedmean_est = zeros(K,p-1); %if there is a nonstationary dimension the number of fixed dimensions is p-1
    ...
    if strcmp(hmm.meantype,'variable')
        ...
        for i = 1:hmm.Nsegs
            ...
            meanfun_est{i} = zeros(hmm.K,hmm.p,length(UDS_seg_inds{i}));
            ...
            for l=1:K
                ...
                to_conv_num = repmat(gamma{i}(:,l),1,hmm.p).*emiss(UDS_seg_inds{i},:);
                to_conv_num = interp1(to_use_inds,to_conv_num(to_use_inds,:),1:length(UDS_seg_inds{i}));
                if size(to_conv_num,2) > size(to_conv_num,1)
                    to_conv_num = to_conv_num';
                end
                ...
                to_conv_num = [flipud(to_conv_num(1:buff_len,:)); to_conv_num; flipud(to_conv_num(end-buff_len+1:end,:))];
                to_conv_den = [fliplr(to_conv_den(1:buff_len)) to_conv_den fliplr(to_conv_den(end-buff_len+1:end))];
                use_vec = logical(ones(size(to_conv_den)));
                use_vec(1:buff_len) = 0; use_vec(end-buff_len+1:end) = 0;
                
                cnum = conv2(to_conv_num,win_kern,'same');
                cden = conv(to_conv_den,win_kern,'same');              
                meanfun_est{i}(l,:,:) = (cnum(use_vec,:)./repmat(cden(use_vec)',1,hmm.p))';
...
%         end
    ...
    for l = 1:K %restimate state covariance matrices
        for i = 1:hmm.Nsegs
            curT = length(UDS_seg_inds{i});
            if strcmp(hmm.meantype,'variable')
                state_mean_est = squeeze(meanfun_est{i}(l,:,:))';
                if size(state_mean_est,2) > size(state_mean_est,1)
                    state_mean_est = state_mean_est';
                end
            else
        ...
        temp_var_est = o_sum/g_sum;
        for j = 1:hmm.p
            for k = j+1:hmm.p
                temp_var_est(k,j) = temp_var_est(j,k);
            end
        end
        var_est(l,:,:) = temp_var_est;
        %enforce symmetry of the covariance matrix estimate
...
%         end
    ...
    for l = 1:K
        ...
        if strcmp(hmm.meantype,'variable')
            for i = 1:hmm.Nsegs
                temp = squeeze(meanfun_est{i}(l,:,:));
                if size(temp,1) > size(temp,2)
                    temp = temp';
                end
                hmm.state(l).meanfun{i} = temp;
            end
        end
    end
    it = it + 1;
