function [hmm,gamma,hmm_window_post]=hsmm_uds_train_hsmm(hmm,emiss)
...
while delta_log_post > tolerance
    ...
    for i = 1:hmm.Nsegs
                ...
                [B,rob_model_inds{i}] = mvgauss_uds_obslike_robust_v3(emiss(UDS_seg_inds{i},:),hmm,rel_prob,i);
    ...
    if strcmp(hmm.meantype,'variable')
        ...
        for i = 1:hmm.Nsegs
            ...
            for l=1:K
                ...
                meanfun_est{i}(l,tot_prob{i}(l,:) < p_thresh) = nan; %set any estimates of meanfunction with insufficient posterior probability to nan
            end
            meandiff = [meandiff meanfun_est{i}(2,tot_prob{i}(l,:) >= p_thresh)-...
                meanfun_est{i}(tot_prob{i}(l,:) >= p_thresh)]; %compute a vector of difference of state meanfunctions
        end
