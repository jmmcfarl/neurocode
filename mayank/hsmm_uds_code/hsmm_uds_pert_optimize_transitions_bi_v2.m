function [new_hsmm_state_seq] = hsmm_uds_pert_optimize_transitions_bi_v2(new_obs,hmm,orig_state_seq,gamma,Fs_orig,Fs_new,pert_range)
...
if strcmp(meantype,'variable') %for variable state meanfunctions
    ...
    for i = 1:hmm.Nsegs
        ...
        temp_meanfun_d1{i} = zeros(hmm.K,curT(i));
        ...
        for l=1:hmm.K
            ...
            to_conv_num = gamma{i}(:,l)'.*new_obs_d(UDS_seg_inds{i},1)';
            ...
            temp_meanfun_d1{i}(l,:) = cnum(use_vec)./cden(use_vec);
            cur_tot_prob(l,:) = cden(use_vec)/length(win_kern);
            temp_meanfun_d1{i}(l,cur_tot_prob(l,:) < p_thresh) = nan; %set any estimates of meanfunction with insufficient posterior probability to nan
        end
        meandiff = [meandiff temp_meanfun_d1{i}(2,cur_tot_prob(l,:) >= p_thresh)-...
            temp_meanfun_d1{i}(1,cur_tot_prob(l,:) >= p_thresh)]; %compute a vector of difference of state meanfunctions
    end
    avgmeandiff = nanmean(meandiff);
    for i = 1:hmm.Nsegs
        temp_meanfun_d1{i}(1,isnan(temp_meanfun_d1{i}(1,:))) = temp_meanfun_d1{i}(2,isnan(temp_meanfun_d1{i}(1,:))) - avgmeandiff;
        temp_meanfun_d1{i}(2,isnan(temp_meanfun_d1{i}(2,:))) = temp_meanfun_d1{i}(1,isnan(temp_meanfun_d1{i}(2,:))) + avgmeandiff;
        ...
        meanfun_est_d1{i} = interp1(t_axis{i},temp_meanfun_d1{i}',t_axis_new{i});
    end
    ...
    for i = 1:hmm.Nsegs
        ...
        t_axis_new{i} = (1:curT_new(i))/Fs_new;
        cur_tot_prob = nan(hmm.K,curT(i));
        temp_meanfun_d2{i} = zeros(hmm.K,curT(i));
        win_kern = ones(round(hmm.windowSize*Fs_orig),1); %this is a convolution kernel
        buff_len = round(hmm.windowSize*Fs_orig); %this is a buffer window to handle edge effects during convolution
        
        for l=1:hmm.K
            %establish svectors for convolution
            to_conv_num = gamma{i}(:,l)'.*new_obs_d(UDS_seg_inds{i},2)';
            to_conv_den = gamma{i}(:,l)';
            ...
            cnum = conv(to_conv_num,win_kern,'same');
            cden = conv(to_conv_den,win_kern,'same');
            temp_meanfun_d2{i}(l,:) = cnum(use_vec)./cden(use_vec);
            cur_tot_prob(l,:) = cden(use_vec)/length(win_kern);
            temp_meanfun_d2{i}(l,cur_tot_prob(l,:) < p_thresh) = nan; %set any estimates of meanfunction with insufficient posterior probability to nan
        end
        meandiff = [meandiff temp_meanfun_d2{i}(2,cur_tot_prob(l,:) >= p_thresh)-...
            temp_meanfun_d2{i}(1,cur_tot_prob(l,:) >= p_thresh)]; %compute a vector of difference of state meanfunctions
    end
    avgmeandiff = nanmean(meandiff);
    for i = 1:hmm.Nsegs
        temp_meanfun_d2{i}(1,isnan(temp_meanfun_d2{i}(1,:))) = temp_meanfun_d2{i}(2,isnan(temp_meanfun_d2{i}(1,:))) - avgmeandiff;
        temp_meanfun_d2{i}(2,isnan(temp_meanfun_d2{i}(2,:))) = temp_meanfun_d2{i}(1,isnan(temp_meanfun_d2{i}(2,:))) + avgmeandiff;
        %         temp_meanfun{i} = [temp_meanfun{i}(:,1) temp_meanfun{i} temp_meanfun{i}(:,end)]';
        if t_axis{i}(2) ~= 0
            t_axis{i}(1) = 0;
        end
        ...
        meanfun_est_d2{i} = interp1(t_axis{i},temp_meanfun_d2{i}',t_axis_new{i});
    end
    ...
    for l = 1:hmm.K
        ...
        g_sum = 0;
        for i = 1:hmm.Nsegs
            mdiff{i}(:,:,l) = new_obs(UDS_seg_inds_new{i},:) - [meanfun_est_d1{i}(:,l) meanfun_est_d2{i}(:,l)];
        ...
        for i = 1:hmm.Nsegs
            state_llik{i}(l,:) = lmvnpdf(squeeze(mdiff{i}(:,:,l)),zeros(length(UDS_seg_inds_new{i}),p),squeeze(var_est(l,:,:)));
    ...
    for i = 1:hmm.Nsegs
        ...
        bad_model = [];
        bad_ups = find(new_obs(UDS_seg_inds_new{i},1) < meanfun_est_d1{i}(:,1) & new_obs(UDS_seg_inds_new{i},2) < meanfun_est_d2{i}(:,1) & most_likely_state'==2);
        bad_model = [bad_model; bad_ups];
        bad_downs = find(new_obs(UDS_seg_inds_new{i},1) > meanfun_est_d1{i}(:,2) & new_obs(UDS_seg_inds_new{i},2) > meanfun_est_d2{i}(:,2) & most_likely_state'==1);
...
elseif strcmp(meantype,'fixed') %%THIS STILL NEEDS TO BE UPDATED
    for i = 1:hmm.Nsegs
        t_axis{i} = (1:curT(i))/Fs_orig;
        t_axis_new{i} = (1:curT_new(i))/Fs_new;
    ...
    for i = 1:hmm.Nsegs
        [max_state_llik,most_likely_state] = max(state_llik{i});
        bad_model = [];
        bad_ups = find(new_obs(UDS_seg_inds_new{i},1) < mean_est{i}(1,1) & most_likely_state'==2);
        bad_model = [bad_model bad_ups];
        bad_downs = find(new_obs(UDS_seg_inds_new{i},1) > mean_est{i}(2,1) & most_likely_state'==1);
        bad_model = [bad_model bad_downs];
        %         bad_model = find(max_state_llik < min_mllik);
        if ~isempty(bad_model)
            for l = 1:hmm.K
