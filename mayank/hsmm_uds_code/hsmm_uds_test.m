...
close all
%specify path here
cur_dir = 'G:\hsmm_uds_toolbox';
cd(cur_dir)

%% Load test LFP data (Fs is sample-frequency =252Hz; LFP units are in z-score)
load ./test_lfp_data
...
params.movingwin = [50 10]; %sliding window parameters in sec ([window_width window_slide]) for computing time-varying state means
...
[bb_features,t_axis,Fsd] = hsmm_uds_get_lf_features(lfp,Fs,dsf_bb,bb_cf); %'low-frequency amplitude'
...
[hsmm_bb_state_seq] = hsmm_uds_pert_optimize_transitions_v2(bb_features,hsmm,hsmm_state_seq,hsmm.gamma,hsmm.Fs,Fs/dsf_bb,pert_range_bb);
