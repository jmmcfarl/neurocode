function hmm = hsmm_uds_train_hmm_bi(hmm,emiss)
...
p = 2; %dimensionality of observations
    ...
    if strcmp(hmm.meantype,'variable')
        ...
        for i = 1:hmm.Nsegs
            ...
            for l=1:K
                ...
                meanfun_est_d1{i}(l,cur_tot_prob(l,:) < p_thresh) = nan; %set any estimates of meanfunction with insufficient posterior probability to nan
            end
            meandiff = [meandiff meanfun_est_d1{i}(2,cur_tot_prob(l,:) >= p_thresh)-...
                meanfun_est_d1{i}(1,cur_tot_prob(l,:) >= p_thresh)]; %compute a vector of difference of state meanfunctions
        end
        ...
        for i = 1:hmm.Nsegs
            ...
            for l=1:K
                ...
                cur_tot_prob(l,:) = cden(use_vec)/length(win_kern);
                meanfun_est_d2{i}(l,cur_tot_prob(l,:) < p_thresh) = nan; %set any estimates of meanfunction with insufficient posterior probability to nan
            end
            meandiff = [meandiff meanfun_est_d2{i}(2,cur_tot_prob(l,:) >= p_thresh)-...
                meanfun_est_d2{i}(1,cur_tot_prob(l,:) >= p_thresh)]; %compute a vector of difference of state meanfunctions
        end
        avg_meandiff = nanmean(meandiff); %average difference of mean functions       
        %any instances where there was insufficient posterior probability
        %for one state are given by estimates extrapolated from the other
