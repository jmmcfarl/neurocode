function [hmm] = hsmm_uds_initialize_nstate(emiss,Fs,params,n)
...
hmm.K = n; %number of hidden states
hmm.p = p;
...
if hmm.p > 1 || strcmp(meantype,'fixed') %for fixed state means
    for k = 1:hmm.K
    hmm.state(k).fixedmean = obj.mu(state_order(k));
    end
end
...
%         hmm.state(3).meanfun{i} = (meanfuns{i}(:,1)+meanfuns{i}(:,2))/2;
%     end
% end
for k = 1:hmm.K
hmm.state(k).var = squeeze(obj.Sigma(:,:,state_order(k)));
end
