function [hmm] = hsmm_uds_initialize_3state(emiss,Fs,params)
...
hmm.K = 3; %number of hidden states
hmm.p = p;
...
if strcmp(meantype,'fixed')
    ...
    ustate_fixmean = obj.mu(state_order(end),:);
    dstate_var = squeeze(obj.Sigma(:,:,state_order(1)));
    ustate_var = squeeze(obj.Sigma(:,:,state_order(end)));
    mid_var = squeeze(obj.Sigma(:,:,state_order(2)));
...
if hmm.p > 1 || strcmp(meantype,'fixed') %for fixed state means
    ...
    hmm.state(2).fixedmean = (ustate_fixmean+dstate_fixmean)/2;
    hmm.state(3).fixedmean = ustate_fixmean;
end
if strcmp(meantype,'variable') %for variable state mean functions
    for i = 1:hmm.Nsegs
        hmm.state(1).meanfun{i} = meanfuns{i}(:,1);
        hmm.state(2).meanfun{i} = (meanfuns{i}(:,1)+meanfuns{i}(:,2))/2;
        hmm.state(3).meanfun{i} = meanfuns{i}(:,2);
    end
end
hmm.state(1).var = dstate_var;
hmm.state(2).var = ustate_var;
hmm.state(3).var = ustate_var;
