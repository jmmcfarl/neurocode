function [hmm] = hsmm_uds_initialize_bi(emiss,Fs,params)
...
p=2; %dimensionality of emissions vector (only works for d=1)
...
if strcmp(meantype,'variable')
    ...
    all_meandiffs = [];
    for i = 1:hmm.Nsegs
        [cur_peaks{i},state_t{i},emissdist_d1,emissrange_d1] = hsmm_uds_init_state_means(emiss(UDS_seg_inds{i},1),[hmm.windowSize hmm.windowSlide],Fs);
    ...
    for i = 1:hmm.Nsegs
        ...
        meanfuns_d1{i} = interp1(state_t{i},cur_peaks{i},time{i});
        %low-pass filter the state meanfunctions
        for k = 1:hmm.K
            meanfuns_d1{i}(:,k) = filtfilt(slid_b,slid_a,meanfuns_d1{i}(:,k));
        end
        ov_meanfuns_d1 = [ov_meanfuns_d1; meanfuns_d1{i}];
    end

    %for the second dimension
    all_meandiffs = [];
    for i = 1:hmm.Nsegs
        [cur_peaks{i},state_t{i},emissdist_d2,emissrange_d2] = hsmm_uds_init_state_means(emiss(UDS_seg_inds{i},2),[hmm.windowSize hmm.windowSlide],Fs);
        all_meandiffs = [all_meandiffs; diff(cur_peaks{i},1,2)];
    end
    ov_meanfuns_d2 = [];
    avg_meandiff = nanmean(all_meandiffs);
    for i = 1:hmm.Nsegs
        ...
        state_t{i} = [time{i}(1) state_t{i} (time{i}(end)+1)];
        cur_peaks{i} = [cur_peaks{i}(1,:); cur_peaks{i}; cur_peaks{i}(end,:)];
        meanfuns_d2{i} = interp1(state_t{i},cur_peaks{i},time{i});
        %low-pass filter the state meanfunctions
        for k = 1:hmm.K
            meanfuns_d2{i}(:,k) = filtfilt(slid_b,slid_a,meanfuns_d2{i}(:,k));
        end
        ov_meanfuns_d2 = [ov_meanfuns_d2; meanfuns_d2{i}];
    end
...
if strcmp(meantype,'fixed')
    ...
    temp_emiss(:,1) = emiss(UDS_samples,1)-ov_meanfuns_d1(:,1);
    temp_emiss(:,2) = emiss(UDS_samples,2)-ov_meanfuns_d2(:,1);
    ...
    temp_emiss = emiss(UDS_samples,:);
    temp_emiss(:,1) = emiss(UDS_samples,1)-ov_meanfuns_d1(:,2);
    temp_emiss(:,2) = emiss(UDS_samples,2)-ov_meanfuns_d2(:,2);
    temp_emiss(any(isnan(temp_emiss),2),:) = [];
    obj = gmdistribution.fit(temp_emiss,hmm.K,'Options',options);
...
if strcmp(meantype,'variable') %for variable state mean functions
    for i = 1:hmm.Nsegs
        hmm.state(1).meanfun{i}(:,1) = meanfuns_d1{i}(:,1);
        hmm.state(1).meanfun{i}(:,2) = meanfuns_d2{i}(:,1);
        hmm.state(2).meanfun{i}(:,1) = meanfuns_d1{i}(:,2);
        hmm.state(2).meanfun{i}(:,2) = meanfuns_d2{i}(:,2);
    end
end
