function hmm = hsmm_uds_train_hmm_nstate(hmm,emiss)
...
tolerance = 1e-4; %convergence tolerance for the likelihood
...
while delta_log_post > tolerance
    ...
    %parameters
    if ~exist('gamma','var')
        rel_prob = 1/hmm.K*ones(1,hmm.K);
    else
        rel_prob = zeros(1,hmm.K);
        for i = 1:hmm.Nsegs
...
%         flex_model_inds{i} = 1:length(UDS_seg_inds{i});
    ...
    if strcmp(hmm.meantype,'variable')
        ...
        for i = 1:hmm.Nsegs
            ...
            meanfun_est{i}(:,3) = (meanfun_est{i}(:,1) + meanfun_est{i}(:,2))/2;
        end
...
%         fixedmean_est(3) = (fixedmean_est(1)+fixedmean_est(2))/2;
    end
