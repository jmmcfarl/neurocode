function [desynch_times,desynch_ids,P,f,t] = hsmm_uds_locate_desynch_times(sig,Fs_orig,params,SO_thresh,HF_thresh)
...
%        desynch_ids: Nx2 matrix of the sample indices where
%        desynchronized epochs start and stop (relative to the original
%        signal vector sig)
%        P: Timexfrequency spectrogram matrix
...
winlength = movingwin(1);
winslide = movingwin(2);
...
%find an remove putative desynchronized epochs which are putative long down
%states
long_ds = [];
...
desynch_ids = round(desynch_times*Fs_orig);
