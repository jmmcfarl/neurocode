function f1 = plotfo1d_nopsc_subspace(fo,max_rows,cmap_type,basis,sdim)
...
for imod=1:nmods
    ...
    plot2drf(cur_k,sdim,mrange,to_transpose);
    ...
    if length(unique(thismod.k)) ~= 1
        if(isfield(thismod,'fox'))
            ...
            bound(1) = thismod.fox(find(cumdist < 0.005,1,'last'));
            bound(2) = thismod.fox(find(cumdist > 0.995,1,'first'));
