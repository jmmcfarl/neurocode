function [gnm] = fitGNM_filters_sepstim(gnm,X,spkbs,disp_type,its_per_round,optTol,progTol,silent)
...
[stimlen,klen]   = size(X{1}); %stimulus dimensions
...
[ll0, ll0p] = getLL_GNM_sepstim(gnm,X,spkbs,disp_type);
...
%     fprintf('Optimization round %d\n',n_opt_rounds);
    [gnm eflag] = fitGNM_filters_helper_sepstim(gnm,X,Robs,its_per_round,'lbgs',silent,optTol,progTol);
    if check_user_in == 1
...
for imod =1:length(gnm.mods);
    if max(abs(gnm.mods(imod).k)) > 0
        cur_stim_ind = gnm.mods(imod).stim_ind;
        cur_inputs = X{cur_stim_ind}*gnm.mods(imod).k;
