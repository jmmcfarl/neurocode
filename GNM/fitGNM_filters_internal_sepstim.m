function [LL, LLgrad] = fitGNM_filters_internal_sepstim(params, Robs, X, model, fprimes)
...
g = zeros(stimlen,1);
for n = 1:Nmods
    cur_stim_inds = model.mods(n).stim_ind;
    % Calculate convolution with internal receptive field
    gint{n} = X{cur_stim_inds} * params((n-1)*klen + (1:klen));
...
for n = 1:Nmods
    ...
    if klen <= chunk_size
        temp = X{cur_stim_inds}.*repmat(g,1,klen)*model.mods(n).w;
        ...
        for cc = 1:NChunks
            ...
            temp = X{cur_stim_inds}(:,cs:ce).*repmat(g,1,cur_chunklen)*model.mods(n).w;
