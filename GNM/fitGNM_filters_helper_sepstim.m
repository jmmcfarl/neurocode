function [fit1 exitflag] = fitGNM_filters_helper_sepstim(fit0, X, Robs, max_evals, method, silent, optTol, progTol)
...
if strcmp(fit0.spk_nl,'gauss')
 lambda = [lambda 0]/length(Robs);   
else
 lambda = [lambda 0]/sum(Robs);   
end

if max(lambda) > 0
    [params LL] = L1General2_PSSas(@(K) fitGNM_filters_internal_sepstim(K, Robs, X, fit0,fprimes),initial_params',lambda',options,silent);
    exitflag = 1;
else
    [params LL exitflag] = minFunc( @(K) fitGNM_filters_internal_sepstim(K, Robs, X, fit0,fprimes), initial_params', options );
end
...
% fit1.spk_theta = params(end) + fit0.spk_theta;
for n = 1:nmods
