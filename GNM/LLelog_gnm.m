function [ll grad] = LLelog_gnm(gnm,K,X,Robs,lamrange,lamrange2,llist, hold_const, nlxrange)
%
...
% internal = gnm.spk_beta*(kx-gnm.spk_theta);
internal = gnm.spk_beta*kx;
too_big = find(internal > 60);
% ekx = exp(gnm.spk_beta*(kx-gnm.spk_theta));
ekx = exp(gnm.spk_beta*kx);
r = gnm.spk_alpha*log(1+ekx);
...
residual = gnm.spk_alpha*gnm.spk_beta*(Robs./r - 1) .* ekx ./ (1+ekx);
residual(too_big) = gnm.spk_alpha*gnm.spk_beta*(Robs(too_big)./r(too_big)-1);

grad = residual'*X;
grad(end+1) = sum(residual);
...
ls = sum(Robs); %normalize by number of spikes
if ls > 0
  ll=-ll/ls;
  grad=-grad'/ls; %'
end
