...
imagedirectory_new_png = ['~/Patches_png_adjust'];
imagedirectory_good_png = [imagedirectory '/GoodPatches_png'];

addpath('/Users/James/Downloads/Images')
...
%%
% stim_set = randperm(stim.Nphoto);
% stim_set(stim_set < 320) = [];
stim_set = 1390:stim.Nphoto;
for i=stim_set
    ...
    for xi=1:gridx
        for yi=1:gridy
            if strcmp(mode,'conversion')
                ...
                imdata = imdata/image_scale(i)*stim.displayParams.whiteColor;
                imdata(imdata < stim.displayParams.blackColor) = stim.displayParams.blackColor;
                imdata(imdata > stim.displayParams.whiteColor) = stim.displayParams.whiteColor;
...
%                 imdatarescale(indx) = linspace(stim.displayParams.blackColor,stim.displayParams.whiteColor,numel(imdata));

%                 [val indx] = sort(imdata(:));
%                 imdatarescale = zeros(size(imdata));
%                 binN = 10; pixperbin = floor(length(imdata(:))/binN);
...
%                  f = highpassfilter(size(imdata),2/60,2); %cutoff freq at 2 cycle/degree, assuming 60 pix/degree
%                 im_filt = real(ifft2(fft2(imdata).*f));
% 
%                 %for filtered image
%                 [val indx] = sort(im_filt(:));
%                 imdatarescale = im_filt;
%                 imdatarescale(indx) = linspace(stim.displayParams.blackColor,stim.displayParams.whiteColor,numel(im_filt));
                
                %                 imdatarescale = zeros(size(im_filt));                
%                 binN = 20; pixperbin = floor(length(im_filt(:))/binN);
%                 lumscaleperbin = (stim.displayParams.whiteColor-stim.displayParams.blackColor)/binN;
%                 for ii=1:binN
%                     indxindx = (ii-1)*pixperbin + (1:pixperbin);
%                     pixels = im_filt(indx(indxindx));
%                     pixels = pixels - min(pixels); pixels = pixels/max(pixels);
%                     imdatarescale(indx(indxindx)) = pixels * lumscaleperbin + (ii-1)*lumscaleperbin + stim.displayParams.blackColor;
%                 end
%                 distorg = histc(im_filt(:),0:lumscaleperbin:stim.displayParams.whiteColor); distorg = distorg/sum(distorg(:));
%                 distnew = histc(imdatarescale(:),0:lumscaleperbin:stim.displayParams.whiteColor); distnew = distnew/sum(distnew(:));
%                 costnow = sum((distorg-distnew).^2);
%                 cost_filt(i,xi,yi) = costnow;
%                 
%                 fprintf('IMage-filt %d X %d Y %d Cost %3.4f\n',i,xi,yi,costnow);
%                 im_filt = imdatarescale;
...
%                 im_filt(im_filt>stim.displayParams.whiteColor)=stim.displayParams.whiteColor;
%                 im_filt(im_filt<stim.displayParams.blackColor)=stim.displayParams.blackColor;

                [Nx, Ny] = size(imdata);
                imdata = imdata(floor((0:Nx*taskParams.Xscale-1)/taskParams.Xscale)+1,floor((0:Ny*taskParams.Yscale-1)/taskParams.Yscale)+1);
%                 im_filt = im_filt(floor((0:Nx*taskParams.Xscale-1)/taskParams.Xscale)+1,floor((0:Ny*taskParams.Yscale-1)/taskParams.Yscale)+1);
...
%                 save(strcat(imagedirectory_new, '/', myimgfile, '_filt.x', num2str(xi), 'y', num2str(yi), '.mat'),'im_filt');
            else
                load(strcat(imagedirectory_new, '/', myimgfile, '.x', num2str(xi), 'y', num2str(yi), '.mat'),'imdata');
%                 load(strcat(imagedirectory_new, '/', myimgfile, '_filt.x', num2str(xi), 'y', num2str(yi), '.mat'),'im_filt');
            end
...
%                     figure(55);clf;imagesc(imdata);colormap gray; axis tight;
            ...
            cur_fname(4) = '1';
            imwrite(imdatapng,strcat(imagedirectory_new_png, '/', cur_fname, '.png'),'png');
%             imfiltpng = im_filt./stim.displayParams.whiteColor;
%             cur_fname(4) = '2';
%             imwrite(imfiltpng,strcat(imagedirectory_new_png, '/', cur_fname, '.png'),'png');
