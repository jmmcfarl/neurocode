...
cd '~/James_scripts/data_processing/Images'
load obj_params

cd(obj_directory)
obj_names = dir('./');
obj_names= obj_names(4:end);
tot_n_objs = length(obj_names);
...
for im = 1:n_imgs
    ...
    cur_fname = sprintf('info4%.4d.mat',im);
    load(cur_fname);
    
    n_objs = length(item_info);
    
    mask = zeros(Ny,Nx);
    
    for ob = 1:n_objs
        if strcmp(item_info(ob).item_type,'texture_patch')
            
        else %for objects
            
            cd(obj_directory)
            filename = strcat(item_info(ob).item_type,'.jpg');
            ...
            %identify object
            for ii = 1:tot_n_objs
                if strcmp(filename,obj_names(ii).name)
                    obj_id = ii;
                end
            end
            ...
            %implement rotation
            offset = 1;
            imdata_r = imrotate(imdata+offset,item_info(ob).rot_angle);
            imdata_r = imdata_r - offset;
            imdata_r(imdata_r == -1) = white;
            imdata = imdata_r;
            
            %identify object pixels
            outmask = (imdata >= white - obj_params(obj_id,1));
            ...
            boundary_pix = in_obj(D <= obj_params(obj_id,3));
            L(L > 1) = 0;
            L_old = L;
            L(boundary_pix) = 0;
            ...
            J = imadjust(imdata2(in_obj),stretchlim(imdata2(in_obj),[.025 .975]),[0 1],obj_params(obj_id,2));
            imdata(in_obj) = J*white;
            imdata(L~=1) = white;
            
            x_shift = item_info(ob).x_cent;
            y_shift = item_info(ob).y_cent;
            [Nyo,Nxo] = size(L);
