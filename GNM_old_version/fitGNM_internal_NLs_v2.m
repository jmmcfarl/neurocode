function [fo] = fitGNM_internal_NLs_v2(fo,Stim,linX,spkbs,silent,adj)
...
%%
nlmat    = reshape(fitp.k(1:NBFS*nmods),NBFS,nmods); %take output K vector and restructure into a matrix of NLBF coefs, one for each module
nlmat_fin = nlmat;
nlx = fo.mods(1).nlx';
for i=1:nmods;
...
linK = fitp.k((NBFS*nmods+1):end-1);
fo.linK = linK;

fin_nlys = nlmat_fin(:);
g = X(:,1:NBFS*nmods)*fin_nlys + linX*linK;
