function [LL, LLgrad] = fitGNM_filters_internal_v2(params, Robs, X, linX, model, fprimes)
...
g = g + b + linX*params((Nmods*klen+1):end-1);
...
if strcmp(model.spk_nl,'logexp')
    bgint = model.spk_beta*g;    
    expg = exp(bgint)';
    too_large = find(bgint > 50);
    r = model.spk_alpha*log(1+expg);
    r(too_large) = model.spk_alpha*bgint(too_large);
...
if min(r) < 1e-50
    r(r < 1e-50) = 1e-50; %minimum predicted rate
...
LLgrad((Nmods*klen+1):end-1) = residual*linX;
...
for n = 1:Nmods
    ...
    elseif model.stim_params.spatial_dims == 1
        ...
        if model.mods(n).lambda_d2T > 0
            lapl_oper = [1 -2 1];
            lapl{n} = zeros(size(kern_mat{n}));
            lapl{n}(2:end-1,:) = kern_mat{n}(1:end-2,:) + kern_mat{n}(3:end,:) - 2*kern_mat{n}(2:end-1,:);
            temp = lapl{n}(2:end-1,2:end-1);
            laplT_penalty(n) = model.mods(n).lambda_d2T*sum(temp(:).^2);
        end
...
if model.lambda_d2T_lin > 0
    lapl_oper = [1 -2 1];
    cur_kern = params((Nmods*klen+1):(Nmods*klen+flen*model.nlk));
%     cur_kern = model.linK;
    cur_kern = [0; cur_kern; 0];
    lapl_lin = conv(cur_kern,lapl_oper,'same');
    temp = lapl_lin(2:end-1);
    lapl_lin_pen = model.lambda_d2T_lin*sum(temp(:).^2);
else
    lapl_lin_pen = 0;
end
    
LL = LL - sum(smooth_penalty) - sum(l2_penalty) - sum(lapl_penalty) - ...
    sum(laplXT_penalty) - sum(laplT_penalty) - lapl_lin_pen;
...
for n = 1:Nmods
    ...
    elseif model.stim_params.spatial_dims == 1
        ...
        if model.mods(n).lambda_d2T > 0
            temp = zeros(size(lapl{n}));
            temp(2:end-1,:) = lapl{n}(1:end-2,:) + lapl{n}(3:end,:) - 2*lapl{n}(2:end-1,:);
            temp = temp(2:end-1,2:end-1);
            LLgrad_pen((n-1)*klen + (1:klen)) = LLgrad_pen((n-1)*klen + (1:klen)) + 2*model.mods(n).lambda_d2T*temp(:);
        end
...
if model.lambda_d2T_lin > 0
    temp = conv(lapl_lin,lapl_oper,'same');
    temp = temp(2:end-1);
    lapl_lin_grad_pen = 2*model.lambda_d2T_lin*temp(:);
    LLgrad_pen((Nmods*klen+1):(Nmods*klen+flen*model.nlk)) = LLgrad_pen((Nmods*klen+1):(Nmods*klen+flen*model.nlk)) + lapl_lin_grad_pen;
end
