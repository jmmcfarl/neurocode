...
load ./ns_40_phase_mod_data
ns_mod = stim_ind_phase_pfilt;
ns_dom = dom;
% figure
% hist(xv_phase_LL-xv_null_LL,100)

% load ./simsac_phase_mod_data
load ./simsac_phase_mod_data_first.mat
simsac_mod = stim_ind_phase_pfilt;
for cc = 1:96
    kern = reshape(simsac_mod(cc,:),nbins,length(wfreqs))';
    simsac_dom(cc,:) = (max(kern,[],2)-min(kern,[],2));
end

load ./simsac_phase_mod_data_second.mat
simsac_sec_mod = stim_ind_phase_pfilt;
for cc = 1:96
    kern = reshape(simsac_sec_mod(cc,:),nbins,length(wfreqs))';
    simsac_sec_dom(cc,:) = (max(kern,[],2)-min(kern,[],2));
end
...
for cc = 1:96
    ...
    subplot(2,2,3)
    pcolor(pax(1:end-1),wfreqs,reshape(ns_mod(cc,:),nbins,length(wfreqs))');shading flat;colorbar
    subplot(2,2,4)
    pcolor(pax(1:end-1),wfreqs,reshape(simsac_sec_mod(cc,:),nbins,length(wfreqs))');shading flat;colorbar
    pause
    clf
end
%%
...
for cc = 1:length(use_units)
    subplot(2,2,1)
    pcolor(pax(1:end-1),wfreqs,reshape(zscore(noise_mod(cc,:)),nbins,length(wfreqs))');shading flat;colorbar
    caxis([-2 2])
    set(gca,'yscale','log')
    
    subplot(2,2,2)
    pcolor(pax(1:end-1),wfreqs,reshape(zscore(ns_mod(cc,:)),nbins,length(wfreqs))');shading flat;colorbar
    caxis([-2 2])
    set(gca,'yscale','log')

    subplot(2,2,3)
    pcolor(pax(1:end-1),wfreqs,reshape(zscore(simsac_mod(cc,:)),nbins,length(wfreqs))');shading flat;colorbar
    caxis([-2 2])
    set(gca,'yscale','log')

    subplot(2,2,4)
    pcolor(pax(1:end-1),wfreqs,reshape(zscore(simsac_sec_mod(cc,:)),nbins,length(wfreqs))');shading flat;colorbar
    caxis([-2 2])
    set(gca,'yscale','log')
...
%%
save ns_40_phase_mod_data pax wfreqs nbins stim_ind* dom xv_*