...
for bb = 1:length(sim_sac_blocks)
    ...
    cur_use_trials = find(all_expt_id==sim_sac_blocks(bb) & all_trial_completed==1 ...
        & ismember(all_trial_seof,repeat_inds));
    ...
    clear interp_phasegrams interp_ampgrams unwr_phasegram
end
...
%% COMPUTE across-trial average model outputs
...
for cur_unit = 1:96;
    ...
    cur_spatial_mod_out = spatial_mod_out(full_stim_ids,cur_unit);
    cur_spatial_mod_out = interp1(full_t,cur_spatial_mod_out,all_expt_t);
    
    Tmat = [trial_Tmat bsxfun(@times,trial_Tmat,cur_spatial_mod_out)];
...
%         cur_inds(bad_set) = [];
        ...
        cur_to_ns_out = trial_Tmat(cur_inds,:)*stim_ind_to_ns_filt(cur_unit,:)';
        cur_to_out = Tmat(cur_inds,:)*to_filt(cur_unit,:)';
        cur_fullampphasemode_out = [Tmat(cur_inds,:) ampphase_set(cur_inds,use_set)]*ampphase_full_filt(cur_unit,:)';
        ...
        to_ns_mod_out{ss,cur_unit} = reshape(cur_to_ns_out,trial_dur,n_trials(ss));
        to_mod_out{ss,cur_unit} = reshape(cur_to_out,trial_dur,n_trials(ss));
        fullampmod_out{ss,cur_unit} = reshape(cur_fullampphasemode_out,trial_dur,n_trials(ss));

        sm_binned_spk_mat{ss,cur_unit} = reshape(all_sm_binned_spks(cur_inds,cur_unit),trial_dur,n_trials(ss));
        binned_spk_mat{ss,cur_unit} = reshape(all_binned_spks(cur_inds,cur_unit),trial_dur,n_trials(ss));
...
cur_unit = 3;
for ss = 1:length(repeat_inds)
    avg_phasemod_out(ss,:) = nanmean(phasemod_out{ss,cur_unit},2);
    avg_ampphasemod_out(ss,:) = nanmean(ampmod_out{ss,cur_unit},2);
    avg_to_ns_out(ss,:) = nanmean(to_ns_mod_out{ss,cur_unit},2);
    avg_to_out(ss,:) = nanmean(to_mod_out{ss,cur_unit},2);
