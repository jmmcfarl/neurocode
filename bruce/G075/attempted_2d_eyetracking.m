...
mod_data_name = 'Twod_eyecorr_mods';
anal_name = 'Twod_eyecorr';
...
cur_block_set = find(strcmp(temp,'image.serangeXseofRC') & expt_Fr == 2);
...
%     end
    ...
    load(sprintf('Expt%dClusterTimes.mat',cur_block));
    for cc = 1:n_probes
        all_spk_times{cc} = cat(1,all_spk_times{cc},Clusters{cc}.times' + cur_toffset);
    end
...
%         cur_xo(n_frames+1:end) = [];
        ...
        extra =length(cur_t_axis)- length(cur_Seeds);
...
used_inds = find(all_Seeds < 4e3 & all_tsince_start >= beg_buffer);
NT = length(used_inds);
...
NT = length(all_Seeds);
cd /media/NTlab_data1/Data/Images/Expt_1_9_13_imfolder/

all_im_patches = nan(NT,length(ypatch_inds),length(xpatch_inds));
unique_images = unique(all_Seeds);
for ii = 1:length(unique_images)
    
    filename = sprintf('IM000%.4d.pgm',unique_images(ii));
    if ~exist(filename,'file')
        filename = sprintf('IM000%.4d.png',unique_images(ii));
    end
...
full_X = create_time_embedding(norm_stim,full_stim_params);
...
for nn = 1:n_fix_inf_it
    ...
    if all(use_coils==0)
                lgamma = bsxfun(@plus,fix_LLs,leps_prior');
%         lgamma = fix_LLs;
...
%             cur_lA = bsxfun(@minus,cur_lA,logsumexp(cur_lA,2));
   ...
   [~,fix_post_max] = max(gamma,[],2);
    ...
    all_shift_stimmat = norm_stim;
    for i=1:NT
        all_shift_stimmat(used_inds(i),:) = norm_stim(used_inds(i),:)*Tshift_mat{all_fix_post_map(i)};
    end
    all_Xmat_fixcor = create_time_embedding(all_shift_stimmat,full_stim_params);
    
    %% REFIT ALL CELLS
    cur_X{1} = all_Xmat_fixcor(used_inds,use_kInds);
    cur_X{2} = Xblock(used_inds,:);
    if use_sac_kerns
        cur_X{3} = Xsac;
        cur_X{4} = Xmsac;
