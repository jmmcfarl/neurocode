...
for bb = 1:length(forty_blocks)
    ...
    cur_trials = find(all_expt_id==forty_blocks(bb) & all_trial_seof==0);
    
    expt_t_axis = [];
    t_since_start = [];
    for tt = 1:length(cur_trials)
        cur_t_edges = all_trial_start_times(cur_trials(tt)):desired_dt:all_trial_stop_times(cur_trials(tt));
...
%%
save noise_40_phase_mod_data pax wfreqs nbins stim_ind* dom xv_*