...
for cur_block = 1:4
    ...
    for s = 1:length(used_sac_times)
        ...
        for c = 1:n_units
            cur_dist = hist(Blocks{cur_block}.spktimes{c},cur_bin_centers);
            cur_dist([1 end]) = 0;
...
p = polyfit(Blocks{1}.suprobes*50,avg_delays,1);

figure
errorbar(Blocks{1}.suprobes*50,avg_delays,std(peak_delays)/2,'o','linewidth',1)
