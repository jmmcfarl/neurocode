...
%     sname = sprintf('image_patch_block%d_leye_50_dsf4',blockid);
    sname = sprintf('image_patch_block%d_leye_25_dsf6',blockid);
    load(sname)
...
%     eye_interp = interp1(eyets(1:end-1),reye_pos,recon_t);
    ...
    used_fix_start_inds = find(in_window(fix_start_inds)==1);
%     used_fix_start_inds = find(in_window(fix_start_inds)==1 & is_stim_filtered(fix_start_inds)' == 0);
    ...
    fprintf('analyzing %d fixations\n',n_used_fixs);
       
    
    cur_fix_image_set = zeros(n_used_fixs,size(ov_im_patch,2),size(ov_im_patch,3));
    cur_sta_out = zeros(n_used_fixs,1);
    spikecnt_mat = zeros(n_used_fixs,length(sac_trig_t));
    for i = 1:n_used_fixs
       cur_set = fix_start_inds(used_fix_start_inds(i)):fix_stop_inds(used_fix_start_inds(i)); 
       cur_avg_image = mean(ov_im_patch(cur_set,:,:));
       cur_avg_image = cur_avg_image - mean(cur_avg_image(:));
       cur_fix_image_set(i,:,:) = cur_avg_image;
       cur_sta_out(i,:) = sta(cellid,:)*cur_avg_image(:);     
       
       [~,nearest_stop] = min(abs(sac_stop_times-recon_t(fix_start_inds(used_fix_start_inds(i)))));
       cur_sac_stop_t = sac_stop_times(nearest_stop);
       spikecnt_mat(i,:) = histc(Blocks{blockid}.spktimes{cellid} - cur_sac_stop_t,sac_trig_t);

       if nearest_stop < length(sac_start_times)
           next_sac_start = sac_start_times(nearest_stop+1);
           fix_dur = next_sac_start-cur_sac_stop_t;
           fix_dur_dt = round(fix_dur/sac_dt);
           spikecnt_mat(i,fix_dur_dt:end) = nan;
       end
       
    end
    
    prc_bin_edges = prctile(cur_sta_out,linspace(0,100,6));
    clear cur_avg
    for i = 1:length(prc_bin_edges)-1
       cur_set = find(cur_sta_out >= prc_bin_edges(i) & cur_sta_out < prc_bin_edges(i+1));
       cur_avg(i,:) = nanmean(spikecnt_mat(cur_set,:));
    end
...
% 
% ml_sta3 = fitGLM_lexp(ml_sta2,X,spikebins{cellid}(:,1),'tots');
