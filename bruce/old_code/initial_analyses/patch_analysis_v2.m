...
%     used_times = find(in_window == 1 & fix_vec' == 1 & is_stim_filtered'==0);
    ...
    cur_patch_set = ov_im_patch(used_times,:,:);
    resh_patch_set = reshape(cur_patch_set,length(used_times),size(cur_patch_set,2)*size(cur_patch_set,3));
    cur_patch_set = cur_patch_set - repmat(mean(resh_patch_set,2),[1 size(cur_patch_set,2) size(cur_patch_set,3)]);
    cur_patch_set = cur_patch_set/std(cur_patch_set(:));
%     cur_patch_set = cur_patch_set./repmat(std(cur_patch_set),[length(used_times) 1 1]);
%     cur_patch_set = (cur_patch_set - mean(cur_patch_set(:)))/std(cur_patch_set(:));
    S = makeStimRows(cur_patch_set,nlags,0);
    X = [X; S];
...
plotfilterbank(sta',SDIM,1:SDIM^2)

%%
SDIM = 22;
for cellid = 1:10;
sta(cellid,:) = mean(X(spikebins{cellid}(:,1),:)) - mean(X);
end
save cur_sta_test sta
%%
utvcv = cov(X);
...
plotfilterbank(STCbvs,SDIM,1:SDIM^2)
...
% 
% ml_sta2 = fitGLM_lexp(ml_sta,X,spikebins{cellid}(:,1),'tots');
