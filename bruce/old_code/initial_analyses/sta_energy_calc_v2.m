...
% load surrogate_spikes_CCset
load surrogate_spikes_CCset2
%%
...
sac_win = [-0.05 0.05]; %time window around saccades to reject data
...
for cell_id = 1:length(spiketimes)
    
    %find set of used spikes
    spike_times_shifted = spiketimes{cell_id} + shift_time;
...
%
% spikes_binned = hist(spike_times_shifted,time_axis);
%
...
% noise_vals = [0 0.5 1 2];
noise_vals = [0];
for nn = 1:length(noise_vals)
    %     spktrg_avg = zeros(length(window_y),length(window_x),4);
    %     ov_avg = zeros(length(window_y),length(window_x),4);
    
    spktrg_avg = zeros(length(spiketimes),length(window_y),length(window_x),size(all_Energy_array,3),size(all_Energy_array,4));
    ov_avg = zeros(length(window_y),length(window_x),size(all_Energy_array,3),size(all_Energy_array,4));
    fprintf('Noise val %d of %d\n',nn,length(noise_vals));
    eye_pixr = eye_pix + randn(size(eye_pix))*noise_vals(nn)*Fsd;
    eye_grid = eye_pixr/grid_dx;
    ...
    for i = 1:length(used_samples)
        ...
        spktrg_avg = spktrg_avg + bsxfun(@times,spikes_binned(:,used_samples(i)),...
            ...
            size(all_Energy_array,3),size(all_Energy_array,4)),[length(spiketimes) 1 1 1 1]));
...
%             cur_image);
    ...
    spktrg_avg_cor(nn,:,:,:,:,:) = spktrg_avg - repmat(reshape(ov_avg,[1 size(ov_avg,1) size(ov_avg,2) size(ov_avg,3) size(ov_avg,4)]),[length(spiketimes) 1 1 1 1]);
    
end
%%
cd ~/Data/bruce/2_27_12/
% save spktrg_avg_energy_surCC_fullset spktrg_avg_cor
...
noise_id = 1;
cnt = 1;
for i = 1:size(spktrg_avg_cor,5)
    for j = 1:size(spktrg_avg_cor,6)
       subplot(size(spktrg_avg_cor,5),size(spktrg_avg_cor,6),cnt)
       imagesc(squeeze(spktrg_avg_cor(noise_id,cell_id,:,:,i,j)));
       caxis([-0.25 0.65]);
%        caxis([-0.025 0.055]);
       cnt = cnt + 1;
    end
