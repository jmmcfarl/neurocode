%%
clear all
cd ~/Analysis/bruce/G088/ET_final/
load binoc_datadump

load cur_best_binoc_mods
%up to sample 176400 uses dds = 12, then uses dds = 67
% sub_sample = 1:length(used_inds); %use all data
sub_sample1 = 176401:length(used_inds); %use only dense noise
sub_sample2 = 1:176400; %use only dense noise
% sub_sample = 176401:length(used_inds); %use only dense noise
% sub_sample = 1:176463; %use only dense noise

% used_inds = used_inds(sub_sample);
% Robs_mat = Robs_mat(sub_sample,:);

%this is a hack, but at least normalizes the SD of the stimulus to be
%constant (across the dense and sparse noise sections)
left_stim(sub_sample1,:) = left_stim(sub_sample1,:)/std(reshape(left_stim(sub_sample1,:),[],1));
left_stim(sub_sample2,:) = left_stim(sub_sample2,:)/std(reshape(left_stim(sub_sample2,:),[],1));
right_stim(sub_sample1,:) = right_stim(sub_sample1,:)/std(reshape(right_stim(sub_sample1,:),[],1));
right_stim(sub_sample2,:) = right_stim(sub_sample2,:)/std(reshape(right_stim(sub_sample2,:),[],1));
