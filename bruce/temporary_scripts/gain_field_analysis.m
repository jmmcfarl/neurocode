...
RobsNorm = Robs_mat;
for bb = 1:n_blocks
    cur_block_inds = find(all_blockvec(used_inds) == bb);
    RobsNorm(cur_block_inds,:) = bsxfun(@rdivide,RobsNorm(cur_block_inds,:),mean(RobsNorm(cur_block_inds,:)));
end
%%
parallel_EP = corrected_eye_vals_interp(:,[1 3]);
% parallel_EP = mean(parallel_EP(:,good_coils==1),2);
parallel_EP = parallel_EP(:,1);

% p_bin_edges = prctile(parallel_EP,linspace(0.5,99.5,30));
p_bin_edges = [-4.5 -1.5 1.5 4.5];
p_bin_cents = 0.5*p_bin_edges(1:end-1) + 0.5*p_bin_edges(2:end);
n_pbins = length(p_bin_cents);

%%
% back_shift = 0;
% EP_sig = [zeros(back_shift,1); parallel_EP(1:end-back_shift)];
% Xout = tb_rep(EP_sig(used_inds), p_bin_edges);
% 
% nim_stim_params = NMMcreate_stim_params(size(Xout,2));
% % nim_reg_params = NMMcreate_reg_params('lambda_d2T',500);
% nim_reg_params = NMMcreate_reg_params('lambda_L2',1);
% 
% cc = 99;
% mod = NMMinitialize_model(nim_stim_params,1,{'lin'},nim_reg_params);
% mod = NMMfit_filters(mod,Robs_mat(:,cc),Xout);
...
nTgroups = 5;
tperm(floor(nuse_trials/nTgroups)*nTgroups+1:end) = [];
tperm = reshape(tperm,nTgroups,[]);
for gg = 1:nTgroups
   TGinds{gg} = find(ismember(all_trialvec(used_inds),tperm(gg,:))); 
end
%%


% % gain_field_avgs = nan(n_chs,n_pbins);
% gain_field_avgs = nan(n_chs,n_pbins,nTgroups);
% for bb = 1:nTgroups
% %     cur_binds = find(all_blockvec(used_inds) == bb);
%     for pp = 1:n_pbins
%         cur_set = find(parallel_EP(used_inds(TGinds{bb})) >= p_bin_edges(pp) & parallel_EP(used_inds(TGinds{bb})) < p_bin_edges(pp+1));
%         gain_field_avgs(:,pp,bb) = nanmean(RobsNorm(TGinds{bb}(cur_set),:));
% %         cur_set = find(parallel_EP(used_inds) >= p_bin_edges(pp) & parallel_EP(used_inds) < p_bin_edges(pp+1));
% %         gain_field_avgs(:,pp) = nanmean(RobsNorm(cur_set,:));
% %         n_pts(pp) = length(cur_set);
%     end
% end
%  
% GF_avgs = squeeze(nanmean(gain_field_avgs,3));
% GF_sem = squeeze(nanstd(gain_field_avgs,[],3))/sqrt(nTgroups);

gain_field_avgs = nan(n_chs,n_pbins,n_blocks);
for bb = 1:n_blocks
    cur_binds = find(all_blockvec(used_inds) == bb);
    for pp = 1:n_pbins
        cur_set = find(parallel_EP(used_inds(cur_binds)) >= p_bin_edges(pp) & parallel_EP(used_inds(cur_binds)) < p_bin_edges(pp+1));
        gain_field_avgs(:,pp,bb) = nanmean(RobsNorm(cur_binds(cur_set),:));
%         cur_set = find(parallel_EP(used_inds) >= p_bin_edges(pp) & parallel_EP(used_inds) < p_bin_edges(pp+1));
%         gain_field_avgs(:,pp) = nanmean(RobsNorm(cur_set,:));
%         n_pts(pp) = length(cur_set);
    end
...
GF_avgs = squeeze(nanmean(gain_field_avgs,3));
GF_sem = squeeze(nanstd(gain_field_avgs,[],3))/sqrt(n_blocks);

%%
load ~/Data/bruce/general_array_data/array_pos_data.mat
ecc = sqrt(interp_x.^2 + interp_y.^2);
[~,eccord] = sort(ecc);
%%
close all
for cc = (n_probes+1):n_chs
    errorbar(p_bin_cents,GF_avgs(cc,:),GF_sem(cc,:),'ro-')
    hold on
%     plot(p_bin_cents,gain_field_avgs(cc,:),'o-')
    pause
    clf
end
...
%%
close all
cmap = jet(n_chs-n_probes);
for cc = (n_probes+1):n_chs
%     errorbar(p_bin_cents,GF_avgs(cc,:),GF_sem(cc,:),'o-','color',cmap(cc-n_probes,:))
    plot(p_bin_cents,GF_avgs(cc,:),'.-','color',cmap(cc-n_probes,:))
    hold on
end
plot(p_bin_cents,nanmean(GF_avgs(1:n_probes,:)),'k.-','linewidth',2);
