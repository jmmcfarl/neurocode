...
full_psth_eyeperm3 = tr_sub_psth_ec;
for ii = 1:length(rpt_taxis)
    temp = squeeze(tr_sub_psth_ec(:,ii,:));
    tempr = temp(b(:,ii),:);
%     tempr(not_usable(:,ii),:) = nan;
    full_psth_eyeperm1(:,ii,:) = tempr;
    
    temp = squeeze(tr_sub_psth_ec(:,ii,:));
    tempr(1:end-1,:) = temp(b(2:end,ii),:);
%     tempr(not_usable(2:end,ii),:) = nan;
    full_psth_eyeperm2(:,ii,:) = tempr;
    
    temp = squeeze(tr_sub_psth_ec(:,ii,:));
    tempr(2:end,:) = temp(b(1:end-1,ii),:);
%     tempr(not_usable(1:end-1,ii),:) = nan;
   full_psth_eyeperm3(:,ii,:) = tempr;
end
    ...
    for jj = 1:length(tr_SUs)
        if jj >= ii
            ...
            if sum(uset) >0
                ...
                Cmat_psth(ii,jj,:) = xcov(vec_psth_mod_rate(uset,ii),vec_psth_mod_rate(uset,jj),maxlag,'biased');
            end
            if sum(uset2) > 0
...
for ii = 1:length(tr_SUs)
    tot_var(ii) = Cmat_obs(ii,ii,maxlag+1);
    em_var(ii) = Cmat_eye(ii,ii,maxlag+1);
    psth_var(ii) = Cmat_psth(ii,ii,maxlag+1);
end
var_data.tot_var = tot_var;
var_data.psth_var = psth_var;
var_data.em_var = em_var;
var_data.np_em_var_frac = em_var./(em_var + psth_var);
...
%     Cmat_sac(ii,neighbs,maxlag+1) = nan;
    ...
    Cmat_eye(ii,ii,:) = nan;
    Cmat_psth(ii,ii,:) = nan;
    Cmat_psth_res(ii,ii,:) = nan;
    Cmat_eye_res(ii,ii,:) = nan;
    Cmat_mod_res(ii,ii,:) = nan;
