...
% max_shift = round(2*spatial_usfac);
dshift = 1;
...
% max_Dshift = round(2*spatial_usfac);
...
for nn = 1:n_fix_inf_it
    ...
    it_fix_post_mean(nn,:,1) = sum(bsxfun(@times,gamma,SH(:,1)'),2);
    it_fix_post_mean(nn,:,2) = sum(bsxfun(@times,gamma,SH(:,2)'),2);
    
    %back-project saccade-times
    all_fix_post_mean_cor = nan(NT,2);
    all_fix_post_mean_cor(~isnan(fix_ids),:) = squeeze(it_fix_post_mean(nn,fix_ids(~isnan(fix_ids)),:));
    ...
    for i = 1:NT
        all_shift_stimmat_left(used_inds(i),:) = shift_matrix_Nd(all_shift_stimmat_left(used_inds(i),:),-round(all_fix_post_mean_cor(i,1)),2);
        all_shift_stimmat_right(used_inds(i),:) = shift_matrix_Nd(all_shift_stimmat_right(used_inds(i),:),-round(all_fix_post_mean_cor(i,2)),2);
    end
    ...
    for ss = 1:length(tr_set)
        ...
        if nn > 1
            fprintf('Original: %.4f  Prev: %.4f  New: %.4f\n',all_mod_LLimp(cur_cell),it_LLimp(nn,cur_unit_ind),it_LLimp(nn+1,cur_unit_ind));
        else
...
for nn = 1:n_drift_inf_it
    ...
    all_Dshift_stimmat_left = all_shift_stimmat_left;
    all_Dshift_stimmat_right = all_shift_stimmat_right;
    for i = 1:NT
        all_Dshift_stimmat_left(used_inds(i),:) = shift_matrix_Nd(all_Dshift_stimmat_left(used_inds(i),:),-round(drift_post_mean(i,1)),2);
        all_Dshift_stimmat_right(used_inds(i),:) = shift_matrix_Nd(all_Dshift_stimmat_right(used_inds(i),:),-round(drift_post_mean(i,2)),2);
    end
    shift_Xmat = create_time_embedding([all_Dshift_stimmat_left all_Dshift_stimmat_right],stim_params_us);
    shift_Xmat = shift_Xmat(used_inds,:);
    ...
    for ss = 1:length(tr_set)
        ...
        if nn > 1
            fprintf('Original: %.4f  Prev: %.4f  New: %.4f\n',all_mod_LLimp(cur_cell),dit_LLimp(nn,cur_unit_ind),dit_LLimp(nn+1,cur_unit_ind));
        else
            fprintf('Original: %.4f  New: %.4f\n',all_mod_LLimp(cur_cell),dit_LLimp(nn+1,cur_cell));
        end
    end
...
fin_drift_corr = drift_post_mean*sp_dx;
min_fix_dur = 0.15;
fix_inds = [];
long_fix_inds = [];
for ii = 1:n_fixs
