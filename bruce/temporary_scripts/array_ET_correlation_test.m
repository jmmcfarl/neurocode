...
for ii = 1:NT
    all_stimmat_true(used_inds(ii),:) = shift_matrix_Nd(all_stimmat_true(used_inds(ii),:),-sim_eyepos_us_rnd(used_inds(ii)),2);
end
all_Xmat_us = create_time_embedding(all_stimmat_true,stim_params_us);
all_Xmat_us = all_Xmat_us(used_inds,use_kInds_up);
...
% init_d2XT = 5*ones(n_squared_filts+1,1);
...
%     cur_tr_inds = find(~isnan(Robs_mat(:,ss)));
%     Robs = Robs_mat(cur_tr_inds,ss);
%     
%     tr_X{1} = all_Xmat_us(cur_tr_inds,:);
...
%     full_mod_fit(ss) = gqm1;
%     full_mod_fitspkNL(ss) = NMMfit_logexp_spkNL(full_mod_fit(ss),Robs,tr_X);
% end
% 
% cd(anal_dir)
% save('full_quad_mods','full_mod_fit*');

%%
true_mods2 = true_mods(tr_set);
test_prate_out = nan(size(Robs_mat));
for ss = 1:length(tr_set)
    ss
    cur_tr_inds = find(~isnan(Robs_mat(:,ss)));
    Robs = Robs_mat(cur_tr_inds,ss);
    ...
    tr_X{1} = all_Xmat_us(cur_tr_inds,:);
    true_mods2(ss) = true_mods(tr_set(ss));
    true_mods2(ss).mods(4:end) = [];
    true_mods2(ss) = NMMfit_logexp_spkNL(true_mods2(ss),Robs,tr_X);
    
    [LL, penLL, pred_rate] = NMMmodel_eval(true_mods2(ss),[],tr_X);
    test_prate_out(cur_tr_inds,ss) = pred_rate;

%     [LL, penLL, pred_rate, G, gint] = NMMmodel_eval(true_mods2(ss),Robs,tr_X);
%     gint(:,2:end) = gint(:,2:end).^2;
%     cur_mod_weights = std(gint);
%     target_weight = std(sum(gint,2));
%     true_mods2(ss).mods(1).filtK = target_weight/cur_mod_weights(2)*true_mods2(ss).mods(2).filtK;
%     true_mods2(ss).mods(2).filtK(:) = 0;
%     true_mods2(ss).mods(3).filtK(:) = 0;
end
...
%%
% Robs_mat = nanzscore(Robs_mat);
% prate_mat = nanzscore(prate_mat);
...
%%
% Robs_mat(isnan(Robs_mat)) = 0;
% prate_mat(isnan(prate_mat)) = 0;
% Robs_tmat(isnan(Robs_tmat)) = 0;
% prate_tmat(isnan(prate_tmat)) = 0;
...
all_Ypos = [Y_pos Y_pos(su_probes)];
all_Xpos = [X_pos X_pos(su_probes)];
all_int_Ypos = [interp_y; interp_y(su_probes)];
all_int_Xpos = [interp_x; interp_x(su_probes)];
% [~,int_xord] = sort(all_int_Xpos(tr_set));
% [~,int_yord] = sort(all_int_Ypos(tr_set));

load ~/Analysis/bruce/G086/ET_final/tr_set_unit_data.mat
all_int_Ypos = weight_mean;
[~,int_yord] = sort(all_int_Ypos);

[~,xord] = sort(all_Xpos(tr_set));
% [~,yord] = sort(all_Ypos(tr_set(xord)));
% yord = xord(yord);
[~,yord] = sort(all_Ypos(tr_set));
mrates = mean(Robs_mat)/dt;
...
caxis([-0.45 0.45])
fname = [fig_dir 'sim_noisecorr_act.pdf'];
...
h = figure;
plot(stim_corrmat(:),corrected_rate_corrs(:),'k.');
...
figufy(h);
fname = [fig_dir 'sim_noisecorr_act_scatter.pdf'];
exportfig(fname,'width',fig_width,'height',rel_height*fig_width,'fontmode','scaled','fontsize',1);
close;
