...
if ~isempty(cc_uinds)
    ...
    stim_mod_signs = [1 1 1 1 -1 -1 -1];
    stim_Xtargs = [ones(1,length(stim_mod_signs))];
    stim_NL_types = {'lin','quad','quad','quad','quad','quad','quad'};
...
fig_dir = '/home/james/Desktop/grantfigs/modFigs/';

poss_Enames = {'G086','G087','G088','G089','G091','G093','G095'}'

for ee = 1:length(poss_Enames)
Ename = poss_Enames{ee};
Enum = str2num(Ename(2:end));
moddir = sprintf('~/Analysis/bruce/%s/models',Ename);
cd(moddir);
load corrected_models
for cc = 97:length(ModData)
    
    if ~isempty(ModData(cc).rectGQM)
    f1 = NMMdisplay_model_pax(ModData(cc).rectGQM,[],[],1,p_ax);
    
    nfilts = length(ModData(cc).rectGQM.mods);
    modFilts = reshape([ModData(cc).rectGQM.mods(:).filtK],[flen use_nPix_us nfilts]);
    filt_tempkern = squeeze(std(modFilts,[],2));
    qfilts = find(strcmp('quad',{ModData(cc).rectGQM.mods(:).NLtype}));
    efilts = find([ModData(cc).rectGQM.mods(:).sign] == 1);
    ifilts = find([ModData(cc).rectGQM.mods(:).sign] == -1);
    
    f2 = figure;
    subplot(2,1,1);
    plot(t_ax,filt_tempkern(:,efilts),'b');
    hold on
    plot(t_ax,filt_tempkern(:,ifilts),'r');
    xlim([0 max(t_ax)]);
    xlabel('Time lag (ms)');
    
    subplot(2,1,2);
    plot(t_ax,mean(filt_tempkern(:,efilts),2),'b','linewidth',2);
    hold on
    plot(t_ax,mean(filt_tempkern(:,ifilts),2),'r','linewidth',2);
    xlim([0 max(t_ax)]);
    xlabel('Time lag (ms)');
    
            n_columns = max(round(sqrt(nfilts/2)),1);
        n_rows = ceil(nfilts/n_columns);

    
    fname = [fig_dir sprintf('E%d_C%d_rectmod.pdf',Enum,cc)];
fig_width = 8*n_columns;
rel_height = 0.5*n_rows/n_columns;
figufy(f1.stim_filts);
exportfig(f1.stim_filts,fname,'width',fig_width,'height',rel_height*fig_width,'fontmode','scaled','fontsize',1);
close(f1.stim_filts);

    fname = [fig_dir sprintf('E%d_C%d_tempkerns.pdf',Enum,cc)];
fig_width = 4;
rel_height = 2;
figufy(f2);
