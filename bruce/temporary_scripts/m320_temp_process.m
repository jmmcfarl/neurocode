...
params.trial_dur = 4;
params.pix_offset = -16; %overall offset in stimulus
...
include_expts = {'rls.smoothPursuit'};
...
for ee = 1:n_blocks;
    ...
    cur_use_pix = (1:params.full_nPix) + buffer_pix + params.pix_offset; %use central pixels
...
%get the set of rials where drift is either non-existent or parallel to the
%bar stim
% used_trials = find(all_trial_exvals(:,1) == 0 | (all_trial_exvals(:,2) == 170 | all_trial_exvals(:,2) == 350)); 
used_trials = find(all_trial_exvals(:,2) == 170 | all_trial_exvals(:,2) == 350); 
used_inds = find(all_tsince_start >= params.beg_buffer & (params.trial_dur-all_tsince_start) >= params.end_buffer);
used_inds(~ismember(all_trialvec(used_inds),used_trials)) = [];
...
trial_data = create_struct_from_mats('start_times',all_trial_start_times,'end_times',all_trial_end_times,'se',all_trial_Se,...
    'Ff',all_trial_Ff,'block_nums',all_trial_blocknums,'nrpt_frames',all_trial_nrptframes,'wi',all_trial_wi,...
    'drift_vel',all_trial_exvals(:,1),'drift_dir',all_trial_exvals(:,2),'init_fx',all_trial_exvals(:,3));
