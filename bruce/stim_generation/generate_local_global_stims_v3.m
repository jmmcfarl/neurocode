close all
clear all
ssac_dir = '~/Data/bruce/Expt_1_8_13_imfolder';
...
for cur_seq = 1:N_sim_seqs
    ...
    for cur_fix = 1:fix_per_seq/2
        ...
        myimgfile = strcat(ssac_dir,sprintf('/IM%.7d.png',cur_id));
...
cur_id = 1000*(cur_seq) + (cur_fix-1)*ims_per_fix + ii;
myimgfile = strcat(save_dir,sprintf('/IM%.7d.pgm',cur_id));
...
cur_id = 1000*(cur_seq+3) + (cur_fix-1)*ims_per_fix + ii;
myimgfile = strcat(save_dir,sprintf('/IM%.7d.pgm',cur_id));
