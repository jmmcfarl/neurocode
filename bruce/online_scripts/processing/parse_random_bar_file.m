function [trial_ids,seed_nums,frame_trialid,frame_nums,left_pix,right_pix,trial_ds,has_mtrS] = parse_random_bar_file(fname)
...
frame_lines = setdiff(1:n_lines,[ds_lines; mtrS_lines; trial_id_lines]);
line_lengths = cellfun(@length,data);
colon_locs = cell2mat(strfind(data(frame_lines),':'));
n_pix = line_lengths(frame_lines) - colon_locs;
max_npix = max(n_pix);
...
for ii = 1:n_frames
    ...
    pix_data(ii,(1:n_pix(ii)) + buffers(ii)) = ...
        data{frame_lines(ii)}(colon_locs(ii)+1:end);
end
