function [] = align_rls_data(dat_dir,Expts,block_nums)

%%

if nargin < 3 || isempty(block_nums)
    block_nums = 1:length(Expts);
end

load([dat_dir '/stim_data.mat']);

%%
exclude_expt_types = {'rls.orXme'};
%%
...
for ii = 1:n_expts
    ...
    if ~isempty(Expts{ii})
        if ~strcmp(Expts{ii}.Header.expname,exclude_expt_types)
            ...
            %%
            fprintf('\n');
            sname = [dat_dir sprintf('/Expt%d_stim',block_nums(ii))];
