function nim = NMMinitialize_model_rowsumtest( stim_params, mod_signs, NL_types, reg_params, Xtargets, init_filts, spk_NL_type)
%
% Usage: nim = NMMinitialize_model( stim_params, mod_signs, NL_types, <reg_params>, <Xtargets>, <init_filts>, <spk_NL_type>)
%
% Creates a new NIM
% Inputs: 
...
if nargin < 6 || isempty(init_filts)
    ...
    for ii = 1:Nmods
        init_filts{ii} = randn(stim_params(Xtargets(ii)).stim_dims)/prod(stim_params(Xtargets(ii)).stim_dims) * 0.1;
    end
else
