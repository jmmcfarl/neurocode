function nim_out = NMMfit_filters_rowsumtest( nim, Robs, Xstims, Gmults, targets, silent, desired_optim_params, regmat_custom )
%
% Usage: nim_out = NMMfit_filters( nim, Robs, Xstims, <Gmults>, <targets>, <silent>, <desired_optim_params>, <regmat_custom> )
%
...
cnt = 0;
for ii = 1:Ntargets
    sdim = size(nim.mods(targets(ii)).filtK);
    filtLen = prod(sdim);
    cur_kern = params((1:filtLen) + cnt);
    nim_out.mods(targets(ii)).filtK = reshape(cur_kern,sdim);
    cnt = cnt + filtLen;
end

% [LL, penLL, ~, G, gint, fgint] = NMMmodel_eval_rowsumtest( nim_out, Robs, Xstims, Gmults, regmat_custom );
...
for ii = 1:Ntargets
    ...
    filtDim(ii,:) = nim.stim_params(nim.mods(tar).Xtarget).stim_dims;
    filtLen(ii) = prod(filtDim(ii,:));
    
    if filtDim(ii,2) > 1
        ktemp = reshape(params(NKtot+(1:filtLen(ii))),[1 filtDim(ii,:)]);
        ks{ii} = ktemp(:);
        NKtot = NKtot + filtLen(ii);
        temp = sum(bsxfun(@times,Xstims{nim.mods(tar).Xtarget},ktemp),3);
        if strcmp(nim.mods(tar).NLtype,'quad')
            fgint = sum(temp.^2,2);
            gint(:,ii) = sum(temp,2);
...
for ii = 1:Ntargets
    ...
    if any(strcmp(nim.mods(tar).NLtype,{'quad'}))
        % compute f'(gint)
        fpg = 2*gint(:,ii);
        ...
        if use_chunking %compute LL gradient wrt stim filter in pieces
            for cc = 1:NChunks
                ...
                ce = min(filtLen(ii),cs+chunk_size); %chunk end ind
                temp = bsxfun( @times, Xstims{nim.mods(tar).Xtarget}(:,cs:ce), fpg ) * nim.mods(tar).sign;
                LLgrad((placeholder+cs):(placeholder+ce)) = residual'*temp;
            end
        else
            LLgrad(placeholder+(1:filtLen(ii))) = residual'*bsxfun( @times, Xstims{nim.mods(tar).Xtarget}, fpg ) * nim.mods(tar).sign;
        end
        
    elseif strcmp(nim.mods(tar).NLtype,'lin')
        % For 'lin and threshlin NLs can use a faster method for computing LL grad
