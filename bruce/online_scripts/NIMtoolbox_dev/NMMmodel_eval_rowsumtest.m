function [LL, penLL, pred_rate, G, gint, fgint, nullLL] = NMMmodel_eval_rowsumtest( nim, Robs, Xstims, Gmults, regmat_custom)
%
...
for n = 1:Nmods
    ...
    filtDim(n,:) = nim.stim_params(nim.mods(n).Xtarget).stim_dims;
    filtLen(n) = prod(filtDim(n,:));
    
    if filtDim(n,2) > 1
        ktemp = reshape(nim.mods(n).filtK,[1 filtDim(n,:)]);
        NKtot = NKtot + filtLen(n);
        temp = sum(bsxfun(@times,Xstims{nim.mods(n).Xtarget},ktemp),3);
        if strcmp(nim.mods(n).NLtype,'quad')
            fgint(:,n) = sum(temp.^2,2);
        ...
        NKtot = NKtot + filtLen(n);
        fgint(:,n) = Xstims{nim.mods(n).Xtarget} * nim.mods(n).filtK;
...
%         error('No support for 2-spatial dims yet');
    ...
    if nim.mods(n).reg_params.lambda_L1 > 0
        sparse_penalty(n) = nim.mods(n).reg_params.lambda_L1*sum(cur_kern);
    end
