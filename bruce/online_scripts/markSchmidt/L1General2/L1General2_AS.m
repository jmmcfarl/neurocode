...
%     'maxIter',20,'suffDec',1e-4,'corrections',100);
[verbose,optTol,progTol,maxIter,suffDec,corrections] = ...
    myProcessOptions(options,'verbose',2,'optTol',1e-4,'progTol',1e-6,...
    'maxIter',25,'suffDec',1e-4,'corrections',100);
...
p = length(w);
[f,g] = pseudoGrad(funObj,w,lambda); %compute initial value of f and gradient
funEvals = 1;
...
%% Main loop
i = 1; %iteration count
W = lambda == 0 | w ~= 0; % Initial working set is coefs with non-zero pen, that are not currently at 0
addStep = 0;
oldAdd = [0 0];
while 1
    optCond = max(abs(g(W)));
    while optCond >= optTol
        ...
        if i == 1 %if first iteration
            d(W) = -g(W); %set direction to negative gradient over working set
            Y = zeros(p,0); %initialize gradient diffs
            S = zeros(p,0); %initialize coef diffs
            sigma = 1;
            t = min(1,1/sum(abs(g)));
        else
            y = g-g_old;
        ...
        xi = sign(w); %sign of coefs
        xi(w==0) = sign(-g(w==0)); %for zero values take sign of the negative grad
        ...
        w_new = orthantProject(w+t*d,xi); %increment w by t*d, but set any w's that change sign to 0
        [f_new,g_new] = pseudoGrad(funObj,w_new,lambda); %evaluate f and g with new W vals
        funEvals = funEvals+1;
        
        % Line search along projection arc
        while f_new > f + suffDec*g'*(w_new-w) || ~isLegal(f_new) %check armijo condition
            t_old = t;
...
%                     fprintf('Halving Step Size\n');
    ...
    % Update working set
    [maxVal,maxInd] = max(abs(g));  %maximum coordinate of gradient
    W = w ~=0;
    addStep = W(maxInd)==0;
    W(maxInd) = 1;
