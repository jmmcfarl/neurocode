...
if bar_ori == 0
    anal_name = 'monoc_eyecorr_hbar_Fin2_loose';
    mod_data_name = 'monoc_eyecorr_hbar_mods_Fin';
else
    anal_name = 'monoc_eyecorr_vbar_Fin2';
...
%%
smooth_eyepos = corrected_eye_vals_interp(:,2);
...
for ii = 1:n_fixs
    ...
    if length(cur_inds) > eye_smooth_sig*5;
        smooth_eyepos(used_inds(cur_inds)) = jmm_smooth_1d_cor(smooth_eyepos(used_inds(cur_inds)),eye_smooth_sig,2);
    end
...
for ii = 1:n_fixs
    ...
    if length(cur_inds)*dt >= min_fix_dur & ismember(used_inds(cur_inds(1)),usable_inds)
        ...
        measured_driftS(cur_inds) = measured_seqS(cur_inds) - measured_fix_avgS(ii);
     end
end
