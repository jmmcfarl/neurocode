...
eps_prior_sigma = 0.1; %0.125 start
...
cur_post_mean(:,1) = sum(bsxfun(@times,Post,SH(:,1)'),2);
cur_post_mean(:,2) = sum(bsxfun(@times,Post,SH(:,2)'),2);
...
mod_spkNL_params = nan(n_tr_chs,3);
for ss = 1:n_tr_chs
    cur_Xtargs = [ref_mod{1}(tr_set(ss)).mods(:).Xtarget];
    cur_k = [ref_mod{1}(tr_set(ss)).mods(cur_Xtargs == 1).filtK];
    n_used_filts = size(cur_k,2);
    filt_bank(ss,:,1:n_used_filts) = cur_k;
    mod_spkNL_params(ss,:) = ref_mod_spkNL{1}(tr_set(ss)).spk_NL_params;
    lin_kerns(ss,:) = ref_mod{1}(tr_set(ss)).mods(cur_Xtargs == 2).filtK;
    sac_kerns(ss,:) = ref_mod{1}(tr_set(ss)).mods(cur_Xtargs == 3).filtK;
    msac_kerns(ss,:) = ref_mod{1}(tr_set(ss)).mods(cur_Xtargs == 4).filtK;
end
filt_bank = permute(filt_bank,[2 1 3]);
filt_bank = reshape(filt_bank,[flen 2*use_nPix_up n_tr_chs n_squared_filts+1]);
...
it_post_mean(:,1) = sum(bsxfun(@times,Post,SH(:,1)'),2);
it_post_mean(:,2) = sum(bsxfun(@times,Post,SH(:,2)'),2);

% cur_diff = bsxfun(@minus,cur_post_mean,shifts).^2;
% cur_post_std = sqrt(sum(cur_diff.*Post,2));

it_all_fix_post_mean = it_post_mean(fix_ids,:);
% all_fix_post_std = cur_post_std(fix_ids);

%back-project saccade-times
it_all_fix_post_mean_cor = all_fix_post_mean;
%align jumps to saccade times
for i = length(saccade_start_inds):-1:1
    cur_inds = saccade_start_inds(i):(saccade_start_inds(i)+sac_shift);
    it_all_fix_post_mean_cor(cur_inds,:) = repmat(it_all_fix_post_mean(cur_inds(end),:),length(cur_inds),1);
    cur_back_inds = (saccade_start_inds(i)-(flen-sac_shift)):(saccade_start_inds(i)-1);
    it_all_fix_post_mean_cor(cur_back_inds,:) = repmat(it_all_fix_post_mean(saccade_start_inds(i),:),length(cur_back_inds),1);
end
...
subplot(2,1,1); hold on
            plot(all_t_axis(used_inds(uu))-bt,all_fix_post_mean(uu,1)*sp_dx,'r');
                 plot(all_t_axis(used_inds(uu))-bt,corrected_interp_eyevals(used_inds(uu),1),'k')
             xlim([0 dur]);
            ylim([-0.5 0.5]);
            xlabel('Time (s)','fontsize',14)
            ylabel('Orthoganol position (deg)','fontsize',14)
subplot(2,1,2); hold on
                 plot(all_t_axis(used_inds(uu))-bt,all_fix_post_mean(uu,2)*sp_dx,'r');
                plot(all_t_axis(used_inds(uu))-bt,corrected_interp_eyevals(used_inds(uu),2),'k')
            xlim([0 dur]);
            ylim([-0.5 0.5]);
            xlabel('Time (s)','fontsize',14)
            ylabel('Orthoganol position (deg)','fontsize',14)
...
measured_fixL = nan(n_fixs,2);

for ii = 1:n_fixs
    cur_inds = jump_inds(ii):(jump_inds(ii+1)-1);
    measured_fixL(ii,:) = nanmedian(corrected_interp_eyevals(used_inds(cur_inds),1:2));
end
...
inferred_pos = inferred_eyepos_cor;
