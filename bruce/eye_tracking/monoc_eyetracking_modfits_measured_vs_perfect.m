...
eye_data_name = 'monoc_eyecorr_vbar';
...
rid = [];
if ~ismember(1,use_types)
    rid = [rid; grayback_gs_expts];
end
if ~ismember(2,use_types)
    rid = [rid; imback_gs_expts];
end
if ~ismember(3,use_types)
    rid = [rid; sim_sac_expts];
end
cur_block_set(rid) = [];
...
for ii = 1:NT
    all_stim_mat_cor(used_inds(ii),:) = shift_matrix_Nd(all_stim_mat(used_inds(ii),:),...
        -inferred_eyepos_cor((ii)),2);
end
all_Xmat_cor2 = create_time_embedding(all_stim_mat_cor,stim_params);
...
for ss = 1:length(su_probes)
    ...
    if ~isempty(cur_used_inds)
       ...
       proj_mat = sta_cor'/(sta_cor*sta_cor')*sta_cor;
        ...
        big_eval_cor(ss) = evs(end);
 
       spike_cond_stim = all_Xmat_cor2(cur_used_inds(spikebins),:);
        sta_cor2      = mean(spike_cond_stim) - mean(all_Xmat_cor2(cur_used_inds,:));
        stacor2_norm(ss) = norm(sta_cor2);
         sta_cor2 = sta_cor2/norm(sta_cor2);
       proj_mat = sta_cor2'/(sta_cor2*sta_cor2')*sta_cor2;
        stim_proj = all_Xmat_cor2(cur_used_inds,:) - all_Xmat_cor2(cur_used_inds,:)*proj_mat;
        % stim_proj = stim_emb;
        stvcv = cov(stim_proj(spikebins,:));  utvcv = cov(stim_proj);
        [evecs,evals] = eig(stvcv-utvcv); evs   = diag(evals);
        stcs_cor2  = evecs(:,[nneg:-1:1,length(evs)-npos+1:end]); stcs_cor2  = stcs_cor2(:,end:-1:1);
        big_eval_cor2(ss) = evs(end);
        ...
        sua_data(ss).stcs_cor2 = stcs_cor2;
        sua_data(ss).sta_cor2 = sta_cor2;
