...
measured_driftR = nan(size(used_inds));
...
fix_prior_sigma = 0.15;
noise_sigma = 0.075;
post_var = 1/(1/noise_sigma^2 + 1/fix_prior_sigma^2);


post_means = post_var*measured_fix_avg/noise_sigma^2;
mdiff = bsxfun(@minus,post_means,shifts*sp_dx);
leps_prior = -(mdiff).^2/(2*post_var);
leps_prior = bsxfun(@minus,leps_prior,logsumexp(leps_prior)); %normalize

shift_dx = shifts'*sp_dx;
measured_fix_deltas = nan(n_fixs,1);
measured_fix_deltas(2:end) = diff(measured_fix_avg);
post_mean_delta = post_var*measured_fix_deltas/noise_sigma^2;
...
for nn = 1:n_fix_inf_it
    ...
    lalpha(1,:) = leps_prior(1,:) + fix_LLs(1,:);
    for t=2:n_fixs
        cur_lA = -pdist2(shift_dx+post_mean_delta(t),shift_dx).^2/(2*post_var);
    ...
    for t=n_fixs-1:-1:1
        cur_lA = -pdist2(shift_dx+post_mean_delta(t+1),shift_dx).^2/(2*post_var);
        cur_lA = bsxfun(@minus,cur_lA,logsumexp(cur_lA,2)); %normalize
...
cur_lA = -pdist2(shift_dx+post_mean_drift(ff)*drift_dsf,shift_dx).^2/(2*post_drift_var*drift_dsf);
cur_lA = bsxfun(@minus,cur_lA,logsumexp(cur_lA,2)); %normalize

shift_dx = shifts*sp_dx;
shift_dx_edges = [shift_dx-dshift*sp_dx/2 shift_dx(end)+dshift*sp_dx/2];
gprobs = diff(erf(shift_dx_edges/sqrt(post_drift_var)));

shift_dx = (0:dshift:2*max_shift)*sp_dx;
shift_dx_edges = [shift_dx-dshift*sp_dx/2 shift_dx(end)+dshift*sp_dx/2];
gprobs = diff(erf(shift_dx_edges/sqrt(post_drift_var)));
% gprobs = gprobs(n_shifts:end);
temp = toeplitz(gprobs);
temp = log(bsxfun(@rdivide,temp,sum(temp,2)));
eps = -1e3;
temp(temp < eps) = eps;

temp = toeplitz(gprobs,zeros(1,n_shifts));


%overall prior on shifts
drift_leps_prior = -(shifts*sp_dx).^2/(2*drift_jump_sigma^2);
drift_leps_prior = bsxfun(@minus,drift_leps_prior,logsumexp(drift_leps_prior)); %normalize
...
%         end
% 
%         cur_lA = -pdist2(shift_dx+post_mean_drift(ff),shift_dx).^2/(2*post_drift_var);
%         cur_lA = bsxfun(@minus,cur_lA,logsumexp(cur_lA,2)); %normalize
% 
%         tset = find(fix_ids==ff)';
%         ntset = length(tset);
%         talpha = zeros(ntset,n_shifts);
%         tbeta = zeros(ntset,n_shifts);
% 
%         cur_LL_set = frame_LLs(tset,:);        
%         talpha(1,:) = drift_leps_prior + cur_LL_set(1,:);
%         for t = 2:ntset
