...
full_corr_mean = drift_post_mean(1,:)';
% full_corr_std = drift_post_std(1,:)';
full_corr_mean = smooth(full_corr_mean,3);
...
for tt = 1:n_trials
    ...
    if ~isempty(uu)
        ...
        if dur > 3.5
            hold on
            plot(all_t_axis(used_inds(uu))-bt,full_corr_mean(uu)*sp_dx,'k')
