...
mod_data_name = 'Twod_eyecorr_mods_rds';
anal_name = 'Twod_eyecorr_rds';
...
cur_block_set(cur_block_set == 1 | cur_block_set == 6 | cur_block_set == 12) = [];
n_blocks = length(cur_block_set);

full_nPix = 64;
use_nPix = 30;
[XX,YY] = meshgrid((-full_nPix/2+1):full_nPix/2);
use_pix = find(abs(XX) <= floor(use_nPix/2) & abs(YY) <= floor(use_nPix/2));
SDIM = sqrt(length(use_pix));
%% COMPUTE TRIAL DATA
cd(data_dir);
...
for ee = 1:n_blocks;
    ...
    for tt = 1:n_trials
        ...
        n_frames = ceil(size(left_stim_mats{use_trials(tt)},1)/2);
            ...
            cur_stim_mat = double(left_stim_mats{use_trials(tt)}(1:2:2*use_frames,use_pix));
            
            cur_stim_mat((end-fixed_delay+1):end,:) = [];
            cur_stim_mat = [nan(fixed_delay,length(use_pix)); cur_stim_mat];
            
            cur_stim_mat = reshape(cur_stim_mat',[SDIM SDIM use_frames]);
            cur_stim_mat = permute(cur_stim_mat,[3 2 1]);
...
all_stim_mat_cor(all_stim_mat == -1) = 0;
all_stim_mat_cor(all_stim_mat == 0) = 1;
all_stim_mat_cor(all_stim_mat == 1) = -1;
clear all_
% all_stim_mat_cor = all_stim_mat_cor - mean(all_stim_mat_cor(:));
full_stim_params = NMMcreate_stim_params([flen SDIM SDIM],dt);
full_X = create_time_embedding(all_stim_mat_cor,full_stim_params);
use_kInds = 1:flen*SDIM^2;
...
%     temp = reshape(sta(ss,:),[flen SDIM SDIM]);
...
init_optim_p.optTol = 1e-4; init_optim_p.progTol = 1e-6; init_optim_p.MaxIter = 500;
...
init_d2XT = [10*ones(n_squared_filts+1,1); 0;];
...
for xx = 1:length(x_shifts)
    tempx = spdiags( ones(full_SDIM,1), -x_shifts(xx), full_SDIM, full_SDIM);
    xshift_mat{xx} = kron(Iy, kron(tempx, It));
end
yshift_mat = cell(length(y_shifts),1);
for yy = 1:length(y_shifts)
    tempy = spdiags( ones(full_SDIM,1), -y_shifts(yy), full_SDIM, full_SDIM);
...
for xx = 1:n_shifts
    tempx = spdiags(ones(full_SDIM,1), -SH(xx,1), full_SDIM, full_SDIM);
    xshift = kron(Iy,tempx);
    tempy = spdiags(ones(full_SDIM,1),-SH(xx,2),full_SDIM,full_SDIM);
...
% 
% %shift matrices for images (non-time-embedded)    
% n_Tshifts = length(shifts);
% Tshift_mat = cell(n_shifts,1);
% for xx = 1:n_Tshifts
%     Tshift_mat{xx} = kron(Iy,spdiags( ones(full_SDIM,1), -Tshifts(xx), full_SDIM, full_SDIM));
% end
...
elseif use_coils(1) == 1
    ...
    measured_fix_deltas(2:end,2) = diff(measured_fix_avg(:,3));
...
%     post_mean_drift(:,2) = post_drift_var*measured_drift(:,3)/drift_noise_sigma^2;
