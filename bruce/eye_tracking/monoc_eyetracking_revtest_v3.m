...
anal_name = 'monoc_eyecorr_vbar_v3';
...
cd(anal_dir);
save(anal_name,'fpost_*','fshift_cor','ref_mod','tr_set','poss_*','all_mod*');
...
it_sac_delta_corr =  corr(measured_delta_pos(:),inferred_delta_pos(:),'type','spearman');
...
for tt = 1:n_trials
    ...
    if ~isempty(uu)
        ...
        dur = et-bt;
        if dur > 3
            shadedErrorBar(all_t_axis(used_inds(uu))-bt,it_fpost_mean{end}(uu)*sp_dx,it_fpost_std{2}(uu)*sp_dx);
            hold on
            plot(all_t_axis(used_inds(uu))-bt,corrected_interp_eyevals(used_inds(uu),2),'r')
            % plot(all_t_axis(used_inds(uu))-bt,corrected_interp_eyevals(used_inds(uu),4),'color',[0.2 0.8 0.2])
            plot(all_t_axis(used_inds(uu))-bt,corrected_interp_eyevals(used_inds(uu),4)-median(corrected_interp_eyevals(used_inds(uu),4)),'color',[0.2 0.8 0.2])
%             plot(all_t_axis(used_inds(uu))-bt,fpost_mean(uu)*sp_dx,'b')
            xlim([0 dur]);
            ylim([-0.5 0.5]);
