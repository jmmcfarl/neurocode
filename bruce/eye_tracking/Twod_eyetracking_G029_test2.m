...
data_dir = ['/media/NTlab_data1_/Data/bruce/' Expt_name];
...
mod_data_name = 'Twod_eyecorr_mods_Rinit';
anal_name = 'Twod_eyecorr_Rinit';
...
for ii = 1:length(unique_images)
    ...
    for j = 1:length(cur_samp_set)
        ...
        ypatch_inds_adj = round(ypatch_inds - all_yo(cur_samp_set(j))/sp_dx);
        xpatch_inds_adj = round(xpatch_inds - all_xo(cur_samp_set(j))/sp_dx);
...
all_im_patches = bsxfun(@minus,all_im_patches,mean(all_im_patches));
all_im_patches = bsxfun(@rdivide,all_im_patches,std(all_im_patches));
...
%         cur_drift_mean = drift_dsf*mean(reshape(cur_drift_mean,[drift_dsf nt_pts]));
   ...
   drift_post_std(nn,:,1) = sqrt(sum(cur_xdiff.*gamma,2));
    ...
    drift_post_mean(nn,:,1) = interp1(find(~isnan(fix_ids)),drift_post_mean(nn,~isnan(fix_ids),1),1:NT);
    drift_post_mean(nn,:,2) = interp1(find(~isnan(fix_ids)),drift_post_mean(nn,~isnan(fix_ids),2),1:NT);
    drift_post_std(nn,:,1) = interp1(find(~isnan(fix_ids)),drift_post_std(nn,~isnan(fix_ids),1),1:NT);
    drift_post_std(nn,:,2) = interp1(find(~isnan(fix_ids)),drift_post_std(nn,~isnan(fix_ids),2),1:NT);
...
%     all_post_cor = round((fix_post_cor+drift_post_cor)) + max_Tshift + 1;
    ...
    for i=1:NT
        all_dshift_stimmat(used_inds(i),:,:) = shift_matrix_Nd(all_dshift_stimmat(used_inds(i),:,:),-squeeze(round(drift_post_mean(nn,i,1))),3);
        all_dshift_stimmat(used_inds(i),:,:) = shift_matrix_Nd(all_dshift_stimmat(used_inds(i),:,:),-round(drift_post_mean(nn,i,2)),2);
    end
...
%     end
    ...
    for tt = 1:n_trials
        ...
        if ~isempty(uu)
            ...
            if dur > 2
                hold on
                h1=shadedErrorBar(all_t_axis(used_inds(uu))-bt,fin_fix_corr(uu,2),fin_fix_std(uu,2),{'color','m'});
%                 plot(all_t_axis(used_inds(uu))-bt,drift_post_mean(nn,uu,2)*sp_dx,'k')
                            h2=shadedErrorBar(all_t_axis(used_inds(uu))-bt,fin_tot_corr(uu,2),fin_tot_std(uu,2),{'color','k'});
