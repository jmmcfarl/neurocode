...
true_mods_spkNL = it_mods_spkNL{end};
true_mods = it_mods{end};
load(true_moddata_name,'all_mod*');

%%
true_pos_seq = round(it_fix_post_mean(end,:));
...
%%
sim_err_seq = true_pos_seq(randperm(length(true_pos_seq)));
sim_err_seq_cor = sim_err_seq(fix_ids);
for i = length(saccade_start_inds):-1:1
    cur_inds = saccade_start_inds(i):(saccade_start_inds(i)+sac_shift);
...
%%
all_Xmat_us = create_time_embedding(all_stimmat_up,stim_params_us);
all_Xmat_us_err = create_time_embedding(all_stimmat_up_err,stim_params_us);
...
cur_used_blocks = tr_blocks(su_used_blocks(tr_blocks,target_su)); %blocks when SU
...
Robs = all_binned_spikes(used_inds(cur_tr_inds),su_probes(target_su));
...
[LL, penLL, pred_rate] = NMMmodel_eval(true_mods_spkNL(96+target_su), Robs, tr_X);

sim_spikes = poissrnd(pred_rate);

Xtargs = [true_mods(96+target_su).mods(:).Xtarget];
tr_X{1} = all_Xmat_us_err(used_inds(cur_tr_inds),use_kInds_up);
new_mod = true_mods(96+target_su);
stim_filts = find(Xtargs == 1);
for i = 1:length(stim_filts)
   new_mod.mods(i).reg_params = init_reg_params; 
end
new_mod = NMMfit_filters(true_mods(96+target_su), sim_spikes, tr_X);
[LL, penLL, pred_rate, G, gint] = NMMmodel_eval(new_mod,Robs,tr_X);
new_mod = NMMadjust_regularization(new_mod,find(Xtargs==1),'lambda_d2XT',base_lambda_d2XT./var(gint)');
new_mod = NMMadjust_regularization(new_mod,find(Xtargs==1),'lambda_L1',base_lambda_L1./std(gint)');
new_mod = NMMfit_filters( true_mods(96+target_su), sim_spikes, tr_X);
...
post_mod = new_mod;
pre_mod = it_mods{1}(96+target_su);
fin_mod = it_mods{end}(96+target_su);
...
for ii = 1:n_stimfilts
    ...
    imagesc(pix_axis,lag_axis,squeeze(uncor_filts(:,:,ii))); caxis([-uc_max_vals(ii) uc_max_vals(ii)]);
    line(pix_axis([1 end]),lag_axis(best_lags_fin([ii ii])),'color','r');
    xlabel('Position (deg)','fontsize',10);
    ylabel('Lag (s)','fontsize',10);
    set(gca,'xtick',-0.5:0.25:0.5,'fontsize',8,'fontname','arial');
    ...
    imagesc(pix_axis,lag_axis,squeeze(fin_filts(:,:,ii))); caxis([-fmax_vals(ii) fmax_vals(ii)]);
    line(pix_axis([1 end]),lag_axis(best_lags_fin([ii ii])),'color','r');
    xlabel('Position (deg)','fontsize',10);
    ylabel('Lag (s)','fontsize',10);
    set(gca,'xtick',-0.5:0.25:0.5,'fontsize',8,'fontname','arial');
    ...
    plot(pix_axis,spatial_profiles_cor(:,ii));
    hold on
    plot(pix_axis,spatial_profiles_uncor(:,ii),'r');
    plot(pix_axis,spatial_profiles_fin(:,ii),'k');
    xlim(pix_axis([1 end]));
    xlabel('Position (deg)','fontsize',10);
...
fname = 'sim_measured_compare_examp';
