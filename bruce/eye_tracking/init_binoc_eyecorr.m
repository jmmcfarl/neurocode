...
init_mods_fname = [save_dir '/init_mods_f10' '.mat'];

full_klen = flen*2*nPix; half_klen = flen*nPix;
stim_params = NIMcreate_stim_params([flen 2*nPix],dt,1,1,length(cur_expt_set)-1);
stim_params1 = NIMcreate_stim_params([flen nPix],dt,1,1,length(cur_expt_set)-1);
...
ex = ones(nPix,1)*sqrt(XTmix(1));
D1t = spdiags([et -2*et et], [-1 0 1], flen, flen)';
D1x = spdiags([ex -2*ex ex], [-1 0 1], nPix, nPix)';
It = speye(flen);Ix = speye(nPix(1));
...
if refit_mods == 1 || ~exist(init_mods_fname,'file')
    ...
    for cc = 1:96
        ...
        fit0(cc,1) = NIMfit_filters(fit0(cc,1),Robs,Xmat(tr_inds,:),Xexpt(tr_inds,:),[],silent,optim_params,full_L2_mats); %fit stimulus filters
        if xv_frac > 0
        ...
        fit0(cc,2) = NIMadjust_regularization(fit0(cc,2),[length(mod_signs)],'lambda_L1',0);
        
        fit0(cc,2) = NIMfit_filters(fit0(cc,2),Robs,Xmat(tr_inds,:),Xexpt(tr_inds,:),[],silent,optim_params,full_L2_mats); %fit stimulus filters
...
for cc = 1:96
    fprintf('Unit %d of %d\n',cc,96);
    init_mod(cc) = NIMfit_filters(init_mod(cc),all_binned_spks(tr_inds,cc),Xmat(tr_inds,:),Xexpt(tr_inds,:),[],silent,optim_params,full_L2_mats); %fit stimulus filters
...
save(init_mods_fname,'fit0','xv_imp','null_xvLL','xvLL','init_mod','null_mod');
% else
%     for cc = 1:96
%         fprintf('Unit %d of %d\n',cc,96);
...
lin_filt_bank = zeros(96,full_klen);
quad_filt_bank = zeros(96,full_klen);
...
left_eye_inds = false(2*nPix,1); left_eye_inds(1:nPix) = true;
...
resh_X = reshape(use_Xmat',[flen 2*nPix NT]);
...
for XV = 1:length(poss_xv_set)
    ...
    ref_mod{1}(xv_cell) = NIMfit_filters(ref_mod{1}(xv_cell),all_binned_spks(tr_inds,xv_cell),X_sh,Xexpt(tr_inds,:),[],silent,optim_params,full_L2_mats); %fit stimulus filters
...
if any(~ismember(tr_set,poss_xv_set))
    ...
    for cc = 1:length(cur_fit_set)
        ...
        ref_mod{1}(cur_unit) = NIMfit_filters(ref_mod{1}(cur_unit),all_binned_spks(tr_inds,cur_unit),X_sh,Xexpt(tr_inds,:),[],silent,optim_params,full_L2_mats); %fit stimulus filters
