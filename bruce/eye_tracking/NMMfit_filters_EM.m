function nim_out = NMMfit_filters_EM( nim, Robs, Xstims, gamma, shifts, corrected_X, Gmults, targets, silent, desired_optim_params, regmat_custom,sign_con)
%
...
if (nargin < 7) || (length(Gmults) < Nmods)
    Gmults{Nmods} = [];
end
...
if use_con == 0 %if no constraints
    ...
    if max(lambda_L1) > 0
        if exist('L1General2_PSSas','file') == 2
            ...
            [params] = L1General2_PSSas(@(K) LLfit_filters_internal(nim, K, Robs, Xstims,Xspkhst,gamma,shift_mat,corrected_X,Gmults,L2_mats,targets,nt_gout,fprimes),...
        ...
        if exist('minFunc','file') == 2 %try to use Mark Schmidt's optimizer
            ...
            [params] = minFunc( @(K) LLfit_filters_internal( nim, K, Robs, Xstims, Xspkhst,gamma,shift_mat, corrected_X,Gmults, L2_mats, targets, nt_gout, fprimes ), initial_params, optim_params);
            ...
            [params] = fminunc( @(K) LLfit_filters_internal( nim, K, Robs, Xstims, Xspkhst,gamma,shift_mat, corrected_X,Gmults, L2_mats, targets, nt_gout, fprimes ), initial_params, optim_params);
    ...
    if exist('minConf_TMP','file') == 2 && isempty(A) && isempty(Aeq)
        ...
        [params] = minConf_TMP( @(K) LLfit_filters_internal( nim, K, Robs, Xstims, Xspkhst,gamma,shift_mat, corrected_X,Gmults, L2_mats, targets, nt_gout, fprimes ),...
        ...
        optim_params.optTol = 1e-4;
        optim_params.progTol = 1e-6;
        [params] = fmincon( @(K) LLfit_filters_internal( nim, K, Robs, Xstims, Xspkhst,gamma,shift_mat, corrected_X,Gmults, L2_mats, targets, nt_gout, fprimes ),...
...
function [LL, LLgrad] = LLfit_filters_internal(nim, params, Robs, Xstims, Xspkhst, gamma, shift_mat,corrected_X,Gmults, L2_mats, targets, nt_gout, fprimes)
%
...
Xtargets = [nim.mods(targets).Xtarget];
stim_targets = find(Xtargets == 1);
NKtot = 0;  filtLen = zeros(Ntargets,1);  
base_ks = cell(Ntargets,1);
gint = cell(Ntargets,1);
base_gint = nan(length(Robs),Ntargets);
for ii = 1:length(targets)
    ...
    base_ks{ii} = params(NKtot+(1:filtLen(ii)));
    NKtot = NKtot + filtLen(ii);
    
    if ismember(ii,stim_targets)
        ks = zeros(length(base_ks{ii}),n_shifts);
        for n = 1:n_shifts
            ks(:,n) = shift_mat{n} * base_ks{ii};
        end
        
        gint{ii} = Xstims{nim.mods(tar).Xtarget} * ks;
        base_gint(:,ii) = sum(gamma.*gint{ii},2);
    else
        gint{ii} = Xstims{nim.mods(tar).Xtarget} * base_ks{ii};
        base_gint(:,ii) = gint{ii};
    end
...
if strcmp(nim.spk_NL_type,'linear') % use MSE as cost function 
    Nspks = length(Robs);
    LL = - bsxfun(@minus,r,Robs).^2 ;
else
    Nspks = sum(Robs);
    LL = bsxfun(@times,log(r),Robs) - r;
end
LL = sum(sum(LL.*gamma));

r = sum(gamma.*r,2);
expg = sum(gamma.*expg,2);
...
% %'residual' = (R/r - 1)*F'[] where F[.] is the spk NL
% if strcmp(nim.spk_NL_type,'logexp')
%     residual = nim.spk_NL_params(3)*nim.spk_NL_params(2)*(bsxfun(@rdivide,Robs,r) - 1) .* expg ./ (1+expg);
%     residual(too_large) = nim.spk_NL_params(3)*nim.spk_NL_params(2)*(bsxfun(@rdivide,Robs(too_large),r(too_large)) - 1);
% elseif strcmp(nim.spk_NL_type,'exp')
%     residual = Robs - r;
% elseif strcmp(nim.spk_NL_type,'linear')
%     residual = 2*(Robs - r);    
...
% %now use point estimate approximation (posterior mean)
% residual = sum(residual.*gamma,2);

%initialize LL gradient
LLgrad = zeros(length(params),1);
% 
% % Calculate derivatives with respect to constant term (theta)
% LLgrad(end) = sum(sum(residual));
...
%         if isempty(Gmults{tar})
if ismember(ii,stim_targets)
            LLgrad(placeholder+(1:filtLen(ii))) = residual'*corrected_X * nim.mods(tar).sign;
else
...
%         end
        ...
        if strcmp(nim.mods(tar).NLtype,'nonpar')
            fpg = piececonst_process(base_gint(:,ii),fprimes{ii}, nim.mods(tar).NLx);
        elseif strcmp(nim.mods(tar).NLtype,'quad')
            fpg = 2*base_gint(:,ii);
        elseif strcmp(nim.mods(tar).NLtype,'threshlin')
            fpg = base_gint(:,ii) >= 0;
...
%         end
        if ismember(ii,stim_targets)
        LLgrad(placeholder+(1:filtLen(ii))) = (fpg.*residual)' * corrected_X* nim.mods(tar).sign;
        else
...
for ii = 1:Ntargets
    ...
    if nim.mods(tar).reg_params.lambda_dT > 0
        deriv_penalty(ii) = deriv_penalty(ii) + nim.mods(tar).reg_params.lambda_dT*sum((L2_mats.L2_dT{nim.mods(tar).Xtarget} * base_ks{ii}).^2);
        cur_grad_pen = 2*nim.mods(tar).reg_params.lambda_dT*(L2_mats.L2_dT{nim.mods(tar).Xtarget}' * L2_mats.L2_dT{nim.mods(tar).Xtarget} * base_ks{ii});
    ...
    if nim.mods(tar).reg_params.lambda_d2T > 0
        smooth_penalty(ii) = smooth_penalty(ii) + nim.mods(tar).reg_params.lambda_d2T*sum((L2_mats.L2_d2T{nim.mods(tar).Xtarget} * base_ks{ii}).^2);
        cur_grad_pen = 2*nim.mods(tar).reg_params.lambda_d2T*(L2_mats.L2_d2T{nim.mods(tar).Xtarget}' * L2_mats.L2_d2T{nim.mods(tar).Xtarget} * base_ks{ii});
        LLgrad_pen(placeholder+(1:filtLen(ii))) = LLgrad_pen(placeholder+(1:filtLen(ii))) + cur_grad_pen;
    end
    ...
    if nim.stim_params(nim.mods(tar).Xtarget).stim_dims(2) > 1
        ...
        if nim.mods(tar).reg_params.lambda_dX > 0
            deriv_penalty(ii) = deriv_penalty(ii) + nim.mods(tar).reg_params.lambda_dX*sum((L2_mats.L2_dX{nim.mods(tar).Xtarget} * base_ks{ii}).^2);
            cur_grad_pen = 2*nim.mods(tar).reg_params.lambda_dX*(L2_mats.L2_dX{nim.mods(tar).Xtarget}' * L2_mats.L2_dX{nim.mods(tar).Xtarget} * base_ks{ii});
            LLgrad_pen(placeholder+(1:filtLen(ii))) = LLgrad_pen(placeholder+(1:filtLen(ii))) + cur_grad_pen;
        end
        if nim.mods(tar).reg_params.lambda_d2X > 0
            smooth_penalty(ii) = smooth_penalty(ii) + nim.mods(tar).reg_params.lambda_d2X*sum((L2_mats.L2_d2X{nim.mods(tar).Xtarget} * base_ks{ii}).^2);
            cur_grad_pen = 2*nim.mods(tar).reg_params.lambda_d2X*(L2_mats.L2_d2X{nim.mods(tar).Xtarget}' * L2_mats.L2_d2X{nim.mods(tar).Xtarget} * base_ks{ii});
            LLgrad_pen(placeholder+(1:filtLen(ii))) = LLgrad_pen(placeholder+(1:filtLen(ii))) + cur_grad_pen;
        end
        if nim.mods(tar).reg_params.lambda_d2XT > 0
            smooth_penalty(ii) = smooth_penalty(ii) + nim.mods(tar).reg_params.lambda_d2XT*sum((L2_mats.L2_d2XT{nim.mods(tar).Xtarget} * base_ks{ii}).^2);
            cur_grad_pen = 2*nim.mods(tar).reg_params.lambda_d2XT*(L2_mats.L2_d2XT{nim.mods(tar).Xtarget}' * L2_mats.L2_d2XT{nim.mods(tar).Xtarget} * base_ks{ii});
            LLgrad_pen(placeholder+(1:filtLen(ii))) = LLgrad_pen(placeholder+(1:filtLen(ii))) + cur_grad_pen;
        end
    end
    ...
    if nim.mods(tar).reg_params.lambda_custom > 0
        custom_penalty(ii) = custom_penalty(ii) + nim.mods(tar).reg_params.lambda_custom*sum((L2_mats.custom * base_ks{ii}).^2);
        cur_grad_pen = 2*nim.mods(tar).reg_params.lambda_custom*(L2_mats.custom' * L2_mats.custom * base_ks{ii});
        LLgrad_pen(placeholder+(1:filtLen(ii))) = LLgrad_pen(placeholder+(1:filtLen(ii))) + cur_grad_pen;
    end
    
    if nim.mods(tar).reg_params.lambda_L2 > 0
        ridge_penalty(ii) = nim.mods(tar).reg_params.lambda_L2*(base_ks{ii}' * base_ks{ii});
        LLgrad_pen(placeholder+(1:filtLen(ii))) = LLgrad_pen(placeholder+(1:filtLen(ii))) + 2*nim.mods(tar).reg_params.lambda_L2*base_ks{ii};
    end
