...
anal_name = 'dualroi_eyecorr_mods';
...
flen = 10;
use_nPix = 24;
stim_params = NIMcreate_stim_params([flen 2*use_nPix],dt);
Fr = 1;
bar_ori = [0 90];
min_trial_dur = 0.75;
...
for ee = 1:length(cur_block_set);
    ...
    n_frames = nan(n_trials,1);
    for tt = 1:n_trials
        cur_stim_times = Expts{cur_block}.Trials(use_trials(tt)).Start/1e4;
        cur_stim_ori = Expts{cur_block}.Trials(use_trials(tt)).or;
        n_frames(tt) = size(left_stim_mats{use_trials(tt)},1);
        if n_frames > 0
            if length(cur_stim_times) == 1
                cur_stim_times = (cur_stim_times:dt*Fr:(cur_stim_times + (n_frames(tt)-1)*dt*Fr))';
        ...
        if ~any(isnan(left_stim_mats{use_trials(tt)}(:))) && n_frames(tt) > min_trial_dur/dt
            use_frames = min(length(cur_stim_times),n_frames(tt));
            if use_frames ~= length(cur_stim_ori)
                error('Ori-stim mismatch');
            end
...
full_stim_mat = zeros(length(all_stim_times),2*size(all_stim_mat,2));
full_stim_mat(hor_stim_inds,1:use_nPix) = all_stim_mat(hor_stim_inds,:);
full_stim_mat(ver_stim_inds,(use_nPix+1):end) = all_stim_mat(ver_stim_inds,:);
all_Xmat = create_time_embedding(full_stim_mat,stim_params);
...
for ss = 1:length(su_probes)
    ...
    if ~isempty(cur_used_inds)
        ...
        sua_data(ss).npos_stc = find(cur_evec_diff(1:use_nPix*flen/2) < stc_thresh,1,'last');
        sua_data(ss).nneg_stc = find(cur_evec_diff(end:-1:end-use_nPix*flen/2) < stc_thresh,1,'last')-1;
        ...
        d2XT = [lambda_d2XT; repmat(sq_sc_fac*sqrt(lambda_d2XT),n_squared_filts,1); 0];
        ...
        fit0 = NMMfit_filters(fit0,Robs(sub_sample),cur_Xmat,[],[],silent,optim_p,L2_mat); %fit stimulus filters
