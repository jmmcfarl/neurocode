...
mod_data_name = 'monoc_eyecorr_LP_mods_us4';
anal_name = 'monoc_eyecorr_LP_us4';
...
%%
n_blocks = length(bar_expts);
cur_block_set = bar_expts;

%% load overall su data
load ./CellList.mat
max_sunum = max(max(max(CellList(bar_expts,:,:))));
poss_suprobes = 1:max_sunum;

probe_has_su = zeros(length(bar_expts),n_probes);
for ii = poss_suprobes
    curset = find(CellList(bar_expts,:,:) == ii);
...
all_trial_end_times = [];
all_bin_edge_pts = [];
all_mu_times = cell(n_probes,1);
all_su_times = cell(length(su_block),1);
...
for ee = 1:n_blocks;
    ...
    load(fname);
    for cc = 1:n_probes
        if probe_has_su(ee,cc) == 0
            all_mu_times{cc} = cat(1,all_mu_times{cc},Clusters{cc}.times'+cur_toffset);
        end
    end
    for cc = 1:length(su_block)
        temp = find(su_block{cc}==ee);
        if ~isempty(temp)
            if su_extra{cc}(temp) == 1
                all_su_times{cc} = cat(1,all_su_times{cc},Clusters{su_probe{cc}(temp)}.times'+cur_toffset);
            elseif su_extra{cc}(temp)==2
                all_su_times{cc} = cat(1,all_su_times{cc},Clusters{su_probe{cc}(temp)}.next{1}.times'+cur_toffset);
            end
        end
    end
...
%for only-MU probes
for cc = 1:n_probes
    if ~isempty(all_mu_times{cc})
    [cur_spkhist,cur_bin_inds] = histc(all_mu_times{cc},all_t_bin_edges);
    cur_spkhist(all_bin_edge_pts) = [];
    all_binned_mua(:,cc) = cur_spkhist;
    end
end

all_binned_sua = nan(length(all_t_axis),length(su_block));
for ss = 1:length(su_block)
    if ~isempty(all_su_times{ss})
        [cur_spkhist,cur_bin_inds] = histc(all_su_times{ss},all_t_bin_edges);
        cur_spkhist(all_bin_edge_pts) = [];
        all_binned_sua(:,ss) = cur_spkhist;
    end
end
...
if ~exist(['./' mod_data_name '.mat'],'file') || recompute_init_mods == 1
    tot_nUnits = length(su_block) + n_probes;
    all_mod_SU = zeros(tot_nUnits,1);
    for ss = 1:n_probes;
        if ~all(probe_has_su(:,ss)==1)
        ...
        tr_X{1} = all_Xmat(used_inds(tr_inds),:);
        tr_X{2} = Xblock(used_inds(tr_inds),:);
        xv_X{1} = all_Xmat(used_inds(xv_inds),:);
        ...
        tr_X{1} = all_Xmat_us(used_inds(tr_inds),:);
        xv_X{1} = all_Xmat_us(used_inds(xv_inds),:);
    ...
    for ss = 1:length(su_block);
        ...
        all_mod_SU(ss+n_probes) = mode(su_probe{ss});
        all_mod_SUind(ss+n_probes) = ss;
        ...
        if ~isempty(cur_tr_inds)
            ...
            tr_X{1} = all_Xmat_us(used_inds(cur_tr_inds),:);
            xv_X{1} = all_Xmat_us(used_inds(cur_xv_inds),:);
            %spatial up-sampling of filter estimates
            base_filts = reshape([gqm1.mods(find(init_Xtargs == 1)).filtK],[flen n_bar_pos n_squared_filts+1]);
            if spatial_usfac == 2
                base_filts_up = zeros(flen,use_nPix_us,n_squared_filts+1);
...
for ss = 1:n_tr_chs
    if all_mod_SU(tr_set(ss)) == 0
        ...
        Robs_mat(:,ss) = all_binned_sua(used_inds,all_mod_SUind(tr_set(ss)));
    end
end
...
for nn = 1:n_fix_inf_it
    ...
    %% RECOMPUTE XMAT
    all_shift_barmat_up = all_bar_mat_up;
    for i=1:NT
        all_shift_barmat_up(used_inds(i),:) = shift_matrix_Nd(all_bar_mat_up(used_inds(i),:),-round(all_fix_post_mean_cor(i)),2);
    end
    all_Xmat_up_fixcor = create_time_embedding(all_shift_barmat_up,stim_params_us);
    ...
    for ss = 1:length(tr_set)
        ...
        it_mods{nn+1}(cur_cell) = NMMfit_filters(it_mods{nn+1}(cur_cell),Robs_mat(cur_tr_uset,cur_unit_ind),...
            tr_X,[],[],silent,init_optim_p); %fit stimulus filters
    ...
    if use_LOOXV == 1
        for xv = 1:length(su_inds)
            ...
            all_Xmat_up_fixcor_LOO = create_time_embedding(all_shift_barmat_up,stim_params_us);
            all_Xmat_up_fixcor_LOO = all_Xmat_up_fixcor_LOO(used_inds,:);
            
            %% REFIT XV CELLS
            cur_X{1} = all_Xmat_up_fixcor_LOO;
...
post_mean_drift = post_drift_var*measured_drift_avg/drift_noise_sigma^2;
...
et_params = struct('beg_buffer',beg_buffer,'end_buffer',end_buffer,'min_trial_dur',min_trial_dur,'bar_ori',bar_ori,...
    'use_nPix',n_bar_pos,'flen',flen,'dt',dt,'drift_jump_sigma',drift_jump_sigma,...
