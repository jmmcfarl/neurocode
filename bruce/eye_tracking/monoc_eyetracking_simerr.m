...
anal_name = 'monoc_eyecorr_hbar_simerr';
mod_data_name = 'monoc_eyecorr_hbar_mods_simerr';
true_eyedata_name = 'monoc_eyecorr_hbar_v4';
...
cd(anal_dir)
load(true_eyedata_name,'it_all_fix_post_mean_cor','it_all_fix_post_std')
true_fix_post_mean = nan(n_fixs,1);
true_fix_post_std = nan(n_fixs,1);
for ii = 1:n_fixs
    cur_inds = jump_inds(ii);
    true_fix_post_mean(ii) = it_all_fix_post_mean_cor(cur_inds);
    true_fix_post_std(ii) = it_all_fix_post_std(cur_inds);
end

for ii = 1:NT
    all_stimmat_up(used_inds(ii),:) = shift_matrix_Nd(all_stimmat_up(used_inds(ii),:),-round(it_all_fix_post_mean_cor(ii)),2);
    all_stim_mat(used_inds(ii),:) = shift_matrix_Nd(all_stim_mat(used_inds(ii),:),-round(it_all_fix_post_mean_cor(ii)/spatial_usfac),2);
end

%% create simulated eye-tracking error 
sim_std = 0.3;
sim_err_sequence = randn(n_fixs,1)*sim_std;
sim_err_sequence = round(sim_err_sequence/sp_dx);

sim_err_tseq = sim_err_sequence(fix_ids);
sim_err_tseq_cor = sim_err_tseq;
...
for i = length(saccade_start_inds):-1:1
    ...
    sim_err_tseq_cor(cur_inds) = sim_err_tseq(cur_inds(end));
    cur_back_inds = (saccade_start_inds(i)-(flen-sac_shift)):(saccade_start_inds(i)-1);
    sim_err_tseq_cor(cur_back_inds) = sim_err_tseq(saccade_start_inds(i));
end

all_stim_mat_err = all_stim_mat;
all_stimmat_up_err = all_stimmat_up;
for ii = 1:NT
   all_stimmat_up_err(used_inds(ii),:) = shift_matrix_Nd(all_stimmat_up(used_inds(ii),:),-sim_err_tseq_cor(ii),2);
   all_stim_mat_err(used_inds(ii),:) = shift_matrix_Nd(all_stim_mat(used_inds(ii),:),-round(sim_err_tseq_cor(ii)/spatial_usfac),2);
end
...
eps_prior_sigma = 0.2; %0.125 start
...
%%
min_std = 0.05;
ww_std = true_fix_post_std;
ww_std(ww_std < min_std) = min_std;
ww = 1./ww_std;
ww = ww/sum(ww);

mx = sum(ww.*-cur_post_mean);
my = sum(ww.*sim_err_sequence);
cov_xy = sum(ww.*(-cur_post_mean - mx).*(sim_err_sequence - my));
cov_xx = sum(ww.*(-cur_post_mean - mx).*(-cur_post_mean - mx));
cov_yy = sum(ww.*(sim_err_sequence - my).*(sim_err_sequence - my));

weight_r2 = cov_xy./sqrt(cov_xx*cov_yy);
%%
...
lPost = bsxfun(@minus,lPost,logsumexp(lPost,2));
Post = exp(lPost);
it_cur_post_mean = sum(bsxfun(@times,Post,shifts),2);
cur_diff = bsxfun(@minus,it_cur_post_mean,shifts).^2;
it_cur_post_std = sqrt(sum(cur_diff.*Post,2));

it_all_fix_post_mean = it_cur_post_mean(fix_ids);
it_all_fix_post_std = it_cur_post_std(fix_ids);
...
mx = sum(ww.*-it_cur_post_mean);
my = sum(ww.*sim_err_sequence);
cov_xy = sum(ww.*(-it_cur_post_mean - mx).*(sim_err_sequence - my));
cov_xx = sum(ww.*(-it_cur_post_mean - mx).*(-it_cur_post_mean - mx));
cov_yy = sum(ww.*(sim_err_sequence - my).*(sim_err_sequence - my));

it_weight_r2 = cov_xy./sqrt(cov_xx*cov_yy);

%%
close all
f1 = figure();
f2 = figure();
for ss = 90:length(tr_set)
...
all_shift_stimmat_up = all_stimmat_up_err;
for i=1:NT
    all_shift_stimmat_up(used_inds(i),:) = shift_matrix_Nd(all_stimmat_up_err(used_inds(i),:),-round(it_all_fix_post_mean_cor(i)),2);
end
all_Xmat_up_fixcor = create_time_embedding(all_shift_stimmat_up,stim_params_us);
all_Xmat_up_fixcor = all_Xmat_up_fixcor(used_inds,:);
...
for ss = 1:length(cur_fit_set)
    ...
    fprintf('Original: %.4f  New: %.4f\n',all_mod_LLimp(tr_cell),ref_LL_imp(2,tr_cell));
    
    %refit spk NL
    ref_mod_spkNL{2}(tr_cell) = NMMfit_logexp_spkNL(ref_mod{2}(tr_cell),Robs_mat(cur_uset,cur_unit_ind),cur_X);
...
mod_spkNL_params = nan(n_tr_chs,3);
for ss = 1:n_tr_chs
    cur_mod = ref_mod{2}(tr_set(ss));
    cur_mod_spkNL = ref_mod_spkNL{2}(tr_set(ss));
    cur_Xtargs = [cur_mod.mods(:).Xtarget];
    cur_k = [cur_mod.mods(cur_Xtargs == 1).filtK];
    n_used_filts = size(cur_k,2);
    filt_bank(ss,:,1:n_used_filts) = cur_k;
...
lPost = bsxfun(@minus,lPost,logsumexp(lPost,2));
Post = exp(lPost);
it2_cur_post_mean = sum(bsxfun(@times,Post,shifts),2);
cur_diff = bsxfun(@minus,it_cur_post_mean,shifts).^2;
it2_cur_post_std = sqrt(sum(cur_diff.*Post,2));

it2_all_fix_post_mean = it2_cur_post_mean(fix_ids);
it2_all_fix_post_std = it2_cur_post_std(fix_ids);

%back-project saccade-times
it2_all_fix_post_mean_cor = it2_all_fix_post_mean;
%align jumps to saccade times
for i = length(saccade_start_inds):-1:1
    cur_inds = saccade_start_inds(i):(saccade_start_inds(i)+sac_shift);
    it2_all_fix_post_mean_cor(cur_inds) = it2_all_fix_post_mean(cur_inds(end));
    cur_back_inds = (saccade_start_inds(i)-(flen-sac_shift)):(saccade_start_inds(i)-1);
    it2_all_fix_post_mean_cor(cur_back_inds) = it2_all_fix_post_mean(saccade_start_inds(i));
end
...
mx = sum(ww.*-it2_cur_post_mean);
my = sum(ww.*sim_err_sequence);
cov_xy = sum(ww.*(-it2_cur_post_mean - mx).*(sim_err_sequence - my));
cov_xx = sum(ww.*(-it2_cur_post_mean - mx).*(-it2_cur_post_mean - mx));
cov_yy = sum(ww.*(sim_err_sequence - my).*(sim_err_sequence - my));

it2_weight_r2 = cov_xy./sqrt(cov_xx*cov_yy);

%%
all_shift_stimmat_up = all_stimmat_up_err;
for i=1:NT
    all_shift_stimmat_up(used_inds(i),:) = shift_matrix_Nd(all_stimmat_up_err(used_inds(i),:),-round(it2_all_fix_post_mean_cor(i)),2);
end
all_Xmat_up_fixcor = create_time_embedding(all_shift_stimmat_up,stim_params_us);
all_Xmat_up_fixcor = all_Xmat_up_fixcor(used_inds,:);

%%
for ss = 92:length(cur_fit_set)
    tr_cell = cur_fit_set(ss);
    fprintf('Refitting model for tr cell %d of %d\n',ss,length(cur_fit_set));
    cur_unit_ind = find(tr_set == tr_cell);
    cur_uset = find(~isnan(Robs_mat(:,cur_unit_ind)));
    ...
    ref_mod{3}(tr_cell) = ref_mod{2}(tr_cell);
    ref_mod{3}(tr_cell) = NMMfit_filters(ref_mod{3}(tr_cell),Robs_mat(cur_uset,cur_unit_ind),...
        cur_X,[],[],silent); %fit stimulus filters
    
    ref_LL_imp(3,tr_cell) = (ref_mod{3}(tr_cell).LL_seq(end) - all_nullmod(tr_cell).LL_seq(end))/log(2);
    
    fprintf('Original: %.4f  New: %.4f\n',all_mod_LLimp(tr_cell),ref_LL_imp(3,tr_cell));
    
    
end

%%
data = load(true_eyedata_name,'ref_mod','tr_set');
orig_ref_mod = data.ref_mod;
orig_tr_set = data.tr_set;
orig_flen = 10;
orig_nPix_up = 48;
...
for ss = 90:length(tr_set)
    ...
    if ~isempty(orig_ref_mod{end}(tr_set(ss)).LL_seq)
    fin_mod = orig_ref_mod{end}(tr_set(ss));
    xtargs = [fin_mod.mods(:).Xtarget];
    kmat = [fin_mod.mods(xtargs == 1).filtK];
    figure(f3); clf
    subplot(2,2,1)
    imagesc(reshape(kmat(:,1),orig_flen,orig_nPix_up));
    ca = max(abs(kmat(:,1))); caxis([-ca ca]);
    for ii = 1:(size(kmat,2)-1)
        subplot(2,2,2+ii)
        imagesc(reshape(kmat(:,ii+1),orig_flen,orig_nPix_up));
        ca = max(abs(kmat(:,ii+1))); caxis([-ca ca]);
    end
    end
    fprintf('Cell %d of %d\n',ss,length(tr_set));
...
init_mod = all_mod_fits(tr_set(ss));
xtargs = [init_mod.mods(:).Xtarget];
init_kmat = [init_mod.mods(xtargs == 1).filtK];

fin_mod = ref_mod{end}(tr_set(ss));
xtargs = [fin_mod.mods(:).Xtarget];
fin_kmat = [fin_mod.mods(xtargs == 1).filtK];

orig_mod = orig_ref_mod{end}(tr_set(ss));
...
cur_X{3} = Xsac(cur_uset,:);
cur_X{4} = Xmsac(cur_uset,:);
[LL, penLL, pred_rate, G, init_gint] = NMMmodel_eval( init_mod, Robs_mat(cur_uset,cur_unit_ind), cur_X);
[LL, penLL, pred_rate, G, fin_gint] = NMMmodel_eval( fin_mod, Robs_mat(cur_uset,cur_unit_ind), cur_X);

n_stimfilts = size(init_kmat,2);
for ii = 1:n_stimfilts
    subplot(n_stimfilts,4,(ii-1)*4+1)
    imagesc(reshape(orig_kmat(:,ii),orig_flen,orig_nPix_up));
    ca = max(abs(orig_kmat(:,ii))); caxis([-ca ca]);
    set(gca,'xtick',[],'ytick',[]);
    if ii == 1
        title('Original','fontsize',10);
    end
    subplot(n_stimfilts,4,(ii-1)*4+2)
    imagesc(reshape(init_kmat(:,ii),flen,use_nPix_up));
    ca = max(abs(init_kmat(:,ii))); caxis([-ca ca]);
    set(gca,'xtick',[],'ytick',[]);
    if ii == 1
    ...
    imagesc(reshape(fin_kmat(:,ii),flen,use_nPix_up));
    ca = max(abs(fin_kmat(:,ii))); caxis([-ca ca]);
    set(gca,'xtick',[],'ytick',[]);
    if ii == 1
    ...
    h_x = linspace(prctile(fin_gint(:,ii),0.25),prctile(fin_gint(:,ii),99.75),100);
    [h_y] = hist(fin_gint(:,ii),h_x);
    h_yunc = hist(init_gint(:,ii),h_x);
    if ii == 1
        cur_mody = h_x;
    else
        cur_mody = h_x.^2;
