...
anal_name = 'monoc_eyecorr_hbar_simerr_sig_0p15';
...
it_mod_fits{1} = all_mod_fits;
it_mod_fits_spkNL{1} = all_mod_fits_withspkNL;
n_iter = 5;
for nn = 2:(n_iter+1)
    ...
    for ss = 1:n_tr_chs
        cur_Xtargs = [it_mod_fits{nn-1}(tr_set(ss)).mods(:).Xtarget];
        cur_k = [it_mod_fits{nn-1}(tr_set(ss)).mods(cur_Xtargs == 1).filtK];
        n_used_filts = size(cur_k,2);
        filt_bank(ss,:,1:n_used_filts) = cur_k;
        mod_spkNL_params(ss,:) = it_mod_fits_spkNL{nn-1}(tr_set(ss)).spk_NL_params;
        lin_kerns(ss,:) = it_mod_fits{nn-1}(tr_set(ss)).mods(cur_Xtargs == 2).filtK;
        sac_kerns(ss,:) = it_mod_fits{nn-1}(tr_set(ss)).mods(cur_Xtargs == 3).filtK;
        msac_kerns(ss,:) = it_mod_fits{nn-1}(tr_set(ss)).mods(cur_Xtargs == 4).filtK;
    end
    ...
    it_post_mean(nn,:) = sum(bsxfun(@times,Post,shifts),2);
    cur_diff = bsxfun(@minus,it_post_mean(nn,:)',shifts).^2;
    it_post_std(nn,:) = sqrt(sum(cur_diff.*Post,2));
    
    all_fix_post_mean = it_post_mean(nn,fix_ids);
    all_fix_post_std = it_post_std(nn,fix_ids);
    ...
    %% COMPUTE WEIGHTED AND UNWEIGHTED CORRELATION
    
    mx = sum(ww.*-it_post_mean(nn,:)');
    my = sum(ww.*sim_err_sequence);
    cov_xy = sum(ww.*(-it_post_mean(nn,:)' - mx).*(sim_err_sequence - my));
    cov_xx = sum(ww.*(-it_post_mean(nn,:)' - mx).*(-it_post_mean(nn,:)' - mx));
    cov_yy = sum(ww.*(sim_err_sequence - my).*(sim_err_sequence - my));
    
    it_weight_r2(nn) = cov_xy./sqrt(cov_xx*cov_yy);
    it_r2(nn) = corr(-it_post_mean(nn,:)',sim_err_sequence);
    it_r2_sp(nn) = corr(-it_post_mean(nn,:)',sim_err_sequence,'type','spearman');
    ...
    for ss = 1:length(cur_fit_set)
        ...
        it_mod_fits{nn}(tr_cell) = it_mod_fits{nn-1}(tr_cell);
        it_mod_fits{nn}(tr_cell) = NMMfit_filters(it_mod_fits{nn}(tr_cell),Robs_mat(cur_uset,cur_unit_ind),...
            cur_X,[],[],silent); %fit stimulus filters
        
        ref_LL_imp(nn,tr_cell) = (it_mod_fits{nn}(tr_cell).LL_seq(end) - all_nullmod(tr_cell).LL_seq(end))/log(2);
        
        if nn > 2
            fprintf('Original: %.4f  Prev: %.4f  New: %.4f\n',all_mod_LLimp(tr_cell),ref_LL_imp(nn-1,tr_cell),ref_LL_imp(nn,tr_cell));
        else
            fprintf('Original: %.4f  New: %.4f\n',all_mod_LLimp(tr_cell),ref_LL_imp(nn,tr_cell));
        end
        
        %refit spk NL
        it_mod_fits_spkNL{nn}(tr_cell) = NMMfit_logexp_spkNL(it_mod_fits{nn}(tr_cell),Robs_mat(cur_uset,cur_unit_ind),cur_X);
    end
...
%%
cd(anal_dir)
save(anal_name,'sim_err_sequence','it_*','ref_*','sim_std','all_mod*','tr_set');
%%
close all
figure;
plot(0:5,it_r2,'ro-','markersize',5)
hold on
plot(0:5,it_weight_r2,'ko-','markersize',5);
% plot(0:5,it_r2_sp,'bo-','markersize',5);
legend('unweighted','weighted','Spearman','Location','Southeast');
xlabel('Iteration number','fontsize',12,'fontname','arial');
ylabel('Correlation','fontsize',12,'fontname','arial');
set(gca,'fontsize',10,'fontname','arial');
box off
fillPage(gcf,'papersize',[4 4]);

LL_imp = bsxfun(@rdivide,ref_LL_imp,all_mod_LLimp);
figure
errorbar(0:5,nanmean(LL_imp(:,tr_set),2),nanstd(LL_imp(:,tr_set),[],2)/sqrt(length(tr_set)),'o-')
xlim([0 5])
xlabel('Iteration number','fontsize',12,'fontname','arial');
ylabel('LL improvement','fontsize',12,'fontname','arial');
set(gca,'fontsize',10,'fontname','arial');
box off
fillPage(gcf,'papersize',[4 4]);
...
fin_mod = it_mod_fits{end}(tr_set(ss));
