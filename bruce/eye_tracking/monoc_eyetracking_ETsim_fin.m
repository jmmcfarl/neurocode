...
anal_name = 'monoc_eyecorr_hbar_ETsim_g25';
mod_data_name = 'monoc_eyecorr_hbar_mods_ETsim_g25';
...
fix_prior_sigma = 0.2*sim_gain;
drift_sigma = 0.01*sim_gain; %.015
drift_jump_sigma = 0.06*sim_gain; %0.05 start
...
max_shift = round(18*spatial_usfac);
dshift = 1;
...
if bar_ori == 0
    sim_eyepos = corrected_interp_eyevals(:,2);
else
    sim_eyepos = corrected_interp_eyevals(:,1);
end
sim_eyepos = sim_eyepos*sim_gain;

max_sim_pos = max_shift*sp_dx*0.9;
sim_eyepos(sim_eyepos > max_sim_pos) = max_sim_pos;
sim_eyepos(sim_eyepos < -max_sim_pos) = -max_sim_pos;
sim_eyepos(isnan(sim_eyepos)) = 0;
sim_eyepos_us = round(sim_eyepos/sp_dx);
sim_eyepos = round(sim_eyepos/(sp_dx*spatial_usfac));

all_stimmat_true = all_stimmat_up;
for ii = 1:NT
    all_stimmat_true(ii,:) = shift_matrix_Nd(all_stimmat_up(ii,:),-sim_eyepos_us(ii),2);
end
...
it_leps_sigma(1) = fix_prior_sigma;
for nn = 1:n_fix_inf_it
    ...
    Post = exp(lPost);
    it_leps_sigma(nn) = sqrt(mean(sum(bsxfun(@times,Post,shifts*sp_dx).^2,2)./sum(Post,2)));
    leps_prior = it_leps_sigma(nn);
...
%%
cd(anal_dir)
anal_name = 'monoc_eyecorr_hbar_ETsim_g3';
load(anal_name);
...
fin_fix_corr = it_fix_post_mean(end,fix_ids)*sp_dx;
fin_fix_std = it_fix_post_std(end,fix_ids)*sp_dx;

fin_tot_corr = fin_fix_corr + fin_drift_corr;
fin_tot_std = sqrt(fin_fix_std.^2 + fin_drift_std.^2);

fin_tot_err = fin_tot_corr - sim_eyepos_us(used_inds)'*sp_dx;

%%
cd(anal_dir)
anal_name = 'monoc_eyecorr_hbar_ETsim_g3_lin';
load(anal_name);

lin_drift_corr = drift_post_mean(end,:)*sp_dx;
lin_drift_std = drift_post_std(end,:)*sp_dx;
...
for ii = 1:n_fixs
    ...
    fix_durs(ii) = length(cur_inds)*dt;
    if length(cur_inds) > sac_shift
        lin_drift_corr(cur_inds(1:end-sac_shift+1)) = lin_drift_corr(cur_inds(sac_shift:end));
        lin_drift_std(cur_inds(1:end-sac_shift+1)) = lin_drift_std(cur_inds(sac_shift:end));
    end
end

lin_fix_corr = it_fix_post_mean(end,fix_ids)*sp_dx;
lin_fix_std = it_fix_post_std(end,fix_ids)*sp_dx;

lin_tot_corr = lin_fix_corr + lin_drift_corr;
lin_tot_std = sqrt(lin_fix_std.^2 + lin_drift_std.^2);

lin_tot_err = lin_tot_corr - sim_eyepos_us(used_inds)'*sp_dx;

%%
cd(anal_dir)
anal_name = 'monoc_eyecorr_hbar_ETsim_g3_lingauss';
load(anal_name);

gauss_drift_corr = drift_post_mean(end,:)*sp_dx;
gauss_drift_std = drift_post_std(end,:)*sp_dx;
...
for ii = 1:n_fixs
    ...
    fix_durs(ii) = length(cur_inds)*dt;
    if length(cur_inds) > sac_shift
        gauss_drift_corr(cur_inds(1:end-sac_shift+1)) = gauss_drift_corr(cur_inds(sac_shift:end));
        gauss_drift_std(cur_inds(1:end-sac_shift+1)) = gauss_drift_std(cur_inds(sac_shift:end));
    end
end

gauss_fix_corr = it_fix_post_mean(end,fix_ids)*sp_dx;
gauss_fix_std = it_fix_post_std(end,fix_ids)*sp_dx;

gauss_tot_corr = gauss_fix_corr + gauss_drift_corr;
gauss_tot_std = sqrt(gauss_fix_std.^2 + gauss_drift_std.^2);

gauss_tot_err = gauss_tot_corr - sim_eyepos_us(used_inds)'*sp_dx;
%%
used_sim_eyepos = sim_eyepos_us(used_inds);
...
cjump_inds = sort([saccade_start_inds(use_ss) + 4; trial_start_inds(use_tt)]);
ejump_inds = nan(length(cjump_inds),1);
n_fixs = length(cjump_inds)-1;
for ii = 1:n_fixs
    ejump_inds(ii) = alljump_inds(find(alljump_inds > cjump_inds(ii),1))-1;
end

fix_durs = (ejump_inds-cjump_inds)*dt;
used_fixs = find(fix_durs > 0.15);

used_fix_inds = [];
for i = 1:length(used_fixs)
    used_fix_inds = [used_fix_inds cjump_inds(used_fixs(i)):ejump_inds(used_fixs(i))];
end

max_sim_pos = 34;
used_fix_inds(abs(sim_eyepos_us(used_inds(used_fix_inds))) >= max_sim_pos) = [];

max_eye_speed = 1;
used_fix_inds(abs(interp_eye_speed(used_inds(used_fix_inds))) >= max_eye_speed) = [];

fin_sure_inds = used_fix_inds(fin_tot_std(used_fix_inds) <= 0.05);
lin_sure_inds = used_fix_inds(lin_tot_std(used_fix_inds) <= 0.05);
...
figure
xx = linspace(0,1,500);
lin_nn = hist(abs(lin_tot_err(used_fix_inds)),xx);
lin_nn = cumsum(lin_nn)/sum(lin_nn);
fin_nn = hist(abs(fin_tot_err(used_fix_inds)),xx);
fin_nn = cumsum(fin_nn)/sum(fin_nn);
gauss_nn = hist(abs(gauss_tot_err(used_fix_inds)),xx);
gauss_nn = cumsum(gauss_nn)/sum(gauss_nn);
plot(xx,1-lin_nn,xx,1-fin_nn,'r',xx,1-gauss_nn,'k')
    ...
    if ~isempty(uu)
        ...
        if dur > 3.5
            ...
            h3=shadedErrorBar(all_t_axis(used_inds(uu))-bt,fin_tot_corr(uu),fin_tot_std(uu),{'color','r'});
                h3=plot(all_t_axis(used_inds(uu))-bt,sim_eyepos_us(used_inds(uu))*sp_dx,'k','linewidth',2);
            xlim([0 dur]);
            ylim([-0.75 0.75]);
