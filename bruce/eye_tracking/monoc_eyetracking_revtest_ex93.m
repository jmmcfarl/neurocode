...
full_stim_params(2) = NMMcreate_stim_params(size(Xblock,2));
...
lin_kerns = nan(n_tr_chs,size(Xblock,2));
...
cd(anal_dir);
save(anal_name,'post_*','shift_cor','ref_mod');
...
%% save inferred eye position
cd(anal_dir);
save(anal_name,'it_*','ref_mod','all_mod*','ref_*','tr_set');
    
end

%% BACK-PROJECT INFERRED JUMPS TO SACCADE TIMES
inferred_eyepos = it_fpost_mean{4}*sp_dx;
...
%% save inferred eye position
cd(anal_dir);
save(anal_name,'inferred_*','it_*','ref_mod','ref_*','all_mod*','tr_set');
...
interp_filt_eye_vals = interp1(all_eye_ts,all_filt_eyevals,all_t_axis);
