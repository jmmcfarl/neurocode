...
%%
anal_name = 'monoc_eyecorr_hbar_finf';
anal_name = 'monoc_eyecorr_hbar_finf_sup';
cd(anal_dir)
load(anal_name);
...
if bar_ori == 0
    ...
    measured_seqL_orth = corrected_interp_eyevals(used_inds,1);
    measured_seqR_orth = corrected_interp_eyevals(used_inds,3);
else
    measured_seqL = corrected_interp_eyevals(used_inds,1);
    measured_seqR = corrected_interp_eyevals(used_inds,3);
    measured_seqL_orth = corrected_interp_eyevals(used_inds,2);
    measured_seqR_orth= corrected_interp_eyevals(used_inds,4);
end
usable_inds = find(~ismember(all_blockvec(used_inds),ignore_blocks));
measured_seqL = measured_seqL - median(measured_seqL(usable_inds));
measured_seqR = measured_seqR - median(measured_seqR(usable_inds));
measured_seqL_orth = measured_seqL_orth - median(measured_seqL_orth(usable_inds));
measured_seqR_orth = measured_seqR_orth - median(measured_seqR_orth(usable_inds));
...
inferred_post_pos = fin_tot_corr(saccade_start_inds(use_ss) + 5);
inferred_delta_pos = (inferred_post_pos - inferred_pre_pos);
inferred_pre_pos_fo = fin_fix_corr(saccade_start_inds(use_ss)-2);
inferred_post_pos_fo = fin_fix_corr(saccade_start_inds(use_ss) + 5);
inferred_delta_pos_fo = (inferred_post_pos_fo - inferred_pre_pos_fo);

u = find(~isnan(measured_delta_posA) & ~isnan(inferred_delta_pos));
[sac_corrs,sac_pvals] = corr([measured_delta_posL(u)' measured_delta_posR(u)'],inferred_delta_pos(u)','type','spearman');

u_micro = find(~isnan(measured_delta_posA) & ~isnan(inferred_delta_pos) & use_micros');
% u_micro = u_micro(isis(use_ss(u_micro)) < 0.2);
[msac_corrs,msac_pvals] = corr([measured_delta_posL(u_micro)' measured_delta_posR(u_micro)'],inferred_delta_pos(u_micro)','type','spearman');

u = find(~isnan(measured_delta_posA) & ~isnan(inferred_delta_pos) & use_nonmicros');
[nmsac_corrs,nmsac_pvals] = corr([measured_delta_posL(u)' measured_delta_posR(u)'],inferred_delta_pos(u)','type','spearman');
...
for tt = sim_sac_trials
    ...
    if ~isempty(uu)
        ...
        if dur > 3.5
            ...
            hold on
%             if bar_ori == 0
                h3=plot(all_t_axis(used_inds(uu))-bt,corrected_interp_eyevals(used_inds(uu),2),'k','linewidth',2);
