...
elseif recompute_init_mods == 2
    fprintf('Initializing with pre-computed models and re-fitting\n');
    load(mod_data_name);
...
for nn = 1:n_fix_inf_it
    ...
    for ss = 1:length(tr_set)
        ...
        %         expLL = NMMmodel_eval_EM( it_mods_spkNL{nn+1}(cur_cell),Robs_mat(cur_tr_uset,cur_unit_ind), ...
        %             get_Xcell_tInds(uncor_X,cur_tr_uset), gamma(cur_tr_uset,:),shifts);
        ...
        %         expxvLL = NMMmodel_eval_EM(it_xvmods_spkNL{nn+1}(cur_cell),Robs_mat(cur_xv_uset,cur_unit_ind), ...
        %             get_Xcell_tInds(uncor_X,cur_xv_uset), gamma(cur_xv_uset,:),shifts);
...
dit_xv_LLimp(1,:) = it_LLimp(n_fix_inf_it+1,:);
dit_xv_xvLLimp(1,:) = it_xvLLimp(n_fix_inf_it+1,:);
    ...
    if use_LOOXV == 1
        for xv = 1:length(su_inds)
            ...
            for xx = 1:length(shifts)
                ...
                for ff = 2:(n_squared_filts+1)
                    gfuns = gfuns + (cur_stim_shift*filt_bank(ff,:)').^2;
                end
            ...
            gamma = exp(lgamma);
            drift_post_mean_xv(xv,:,nn) = sum(bsxfun(@times,gamma,shifts),2);
            drift_post_std_xv(xv,:,nn) = sqrt(sum(bsxfun(@times,gamma,shifts.^2),2) - squeeze(drift_post_mean_xv(xv,:,nn))'.^2);
            drift_post_mean_cor = squeeze(drift_post_mean_xv(xv,:,nn));
            ...
            dit_xv_mods{nn+1}(cur_cell) = dit_mods{nn}(cur_cell);
            dit_xv_mods{nn+1}(cur_cell) = NMMfit_filters(dit_xv_mods{nn+1}(cur_cell),Robs_mat(cur_tr_uset,cur_unit_ind),...
                tr_X,[],[],silent); %fit stimulus filters
            
            %refit spk NL
            dit_xv_mods_spkNL{nn+1}(cur_cell) = NMMfit_logexp_spkNL(dit_xv_mods{nn+1}(cur_cell),Robs_mat(cur_tr_uset,cur_unit_ind),tr_X);
            
            newLL = NMMmodel_eval(dit_xv_mods_spkNL{nn+1}(cur_cell),Robs_mat(cur_tr_uset,cur_unit_ind),tr_X);
            
            dit_xv_xvmods{nn+1}(cur_cell) = dit_xvmods{nn}(cur_cell);
            dit_xv_xvmods{nn+1}(cur_cell) = NMMfit_filters(dit_xv_xvmods{nn+1}(cur_cell),Robs_mat(cur_xv_uset,cur_unit_ind),...
                xv_X,[],[],silent); %fit stimulus filters
            %refit spk NL
            dit_xv_xvmods_spkNL{nn+1}(cur_cell) = NMMfit_logexp_spkNL(dit_xv_xvmods{nn+1}(cur_cell),Robs_mat(cur_xv_uset,cur_unit_ind),xv_X);
            
            newxvLL = NMMmodel_eval(dit_xv_xvmods_spkNL{nn+1}(cur_cell),Robs_mat(cur_xv_uset,cur_unit_ind),xv_X);
            
            dit_xv_LLimp(nn+1,cur_cell) = (newLL - null_LL(cur_cell))/log(2);
            dit_xv_xvLLimp(nn+1,cur_cell) = (newxvLL - null_xvLL(cur_cell))/log(2);
        end
    end
