...
anal_name = 'monoc_eyecorr_hbar_fin_EM';
...
it_mods_soft{1} = all_mod_fits;
it_mods_spkNL_soft{1} = all_mod_fits_withspkNL;
it_xvmods{1} = all_xvmod_fits;
it_xvmods_spkNL{1} = all_xvmod_fits_withspkNL;
it_xvmods_soft{1} = all_xvmod_fits;
it_xvmods_spkNL_soft{1} = all_xvmod_fits_withspkNL;
it_LLimp(1,:) = all_mod_LLimp;
it_xvLLimp(1,:) = all_mod_xvLLimp;
it_expLLimp(1,:) = all_mod_LLimp;
it_expxvLLimp(1,:) = all_mod_xvLLimp;
it_LLimp_soft(1,:) = all_mod_LLimp;
it_xvLLimp_soft(1,:) = all_mod_xvLLimp;
it_expLLimp_soft(1,:) = all_mod_LLimp;
it_expxvLLimp_soft(1,:) = all_mod_xvLLimp;
for nn = 1:n_fix_inf_it
    ...
    for ss = 1:n_tr_chs
        cur_Xtargs = [it_mods_soft{nn}(tr_set(ss)).mods(:).Xtarget];
        cur_k = [it_mods_soft{nn}(tr_set(ss)).mods(cur_Xtargs == 1).filtK];
        n_used_filts = size(cur_k,2);
        filt_bank(ss,:,1:n_used_filts) = cur_k;
        mod_spkNL_params(ss,:) = it_mods_spkNL_soft{nn}(tr_set(ss)).spk_NL_params;
        lin_kerns(ss,:) = it_mods_soft{nn}(tr_set(ss)).mods(cur_Xtargs == 2).filtK;
        if use_sac_kerns
            sac_kerns(ss,:) = it_mods_soft{nn}(tr_set(ss)).mods(cur_Xtargs == 3).filtK;
            msac_kerns(ss,:) = it_mods_soft{nn}(tr_set(ss)).mods(cur_Xtargs == 4).filtK;
        end
    end
    ...
    for i = length(saccade_start_inds):-1:1
        ...
        cor_fixids(cur_inds) = fix_ids(cur_inds(end));
...
%         all_shift_stimmat_up(used_inds(i),:) = shift_matrix_Nd(all_stimmat_up(used_inds(i),:),-post_max(i),2);
    ...
    cor_X = cur_X;
    cor_X{1} = all_Xmat_up_fixcor(used_inds,use_kInds_up);
    
    optim_p.progTol = 1e-7;
    optim_p.optTol = 1e-4;
    silent = 1;
    for ss = 1:length(tr_set)
        ...
        cur_tr_uset = find(~isnan(Robs_mat(tr_inds,cur_unit_ind)));
        cur_xv_uset = find(~isnan(Robs_mat(xv_inds,cur_unit_ind)));
        
        tr_X = get_Xcell_tInds(cur_X,cur_tr_uset);
        xv_X = get_Xcell_tInds(cur_X,cur_xv_uset);
        tr_cX = get_Xcell_tInds(cor_X,cur_tr_uset);
        xv_cX = get_Xcell_tInds(cor_X,cur_xv_uset);
        
        it_mods_soft{nn+1}(cur_cell) = it_mods_soft{nn}(cur_cell);
        it_mods_soft{nn+1}(cur_cell) = NMMfit_filters_EM(it_mods_soft{nn+1}(cur_cell),Robs_mat(cur_tr_uset,cur_unit_ind),...
            tr_X,gamma(tr_inds(cur_tr_uset),:),shifts,all_Xmat_up_fixcor(tr_inds(cur_tr_uset),use_kInds_up),[],[],silent,optim_p); %fit stimulus filters
        ...
        it_mods{nn+1}(cur_cell) = NMMfit_filters(it_mods{nn+1}(cur_cell),Robs_mat(cur_tr_uset,cur_unit_ind),...
            tr_cX,[],[],silent,optim_p); %fit stimulus filters
               
        %refit spk NL
        it_mods_spkNL_soft{nn+1}(cur_cell) = NMMfit_logexp_spkNL(it_mods_soft{nn+1}(cur_cell),Robs_mat(cur_tr_uset,cur_unit_ind),tr_cX);
        it_mods_spkNL{nn+1}(cur_cell) = NMMfit_logexp_spkNL(it_mods{nn+1}(cur_cell),Robs_mat(cur_tr_uset,cur_unit_ind),tr_cX);
        
        new_expLL = NMMmodel_eval_EM(it_mods_spkNL{nn+1}(cur_cell),Robs_mat(cur_tr_uset,cur_unit_ind),tr_X,gamma(cur_tr_uset,:),shifts);
        new_LL = NMMmodel_eval(it_mods_spkNL{nn+1}(cur_cell),Robs_mat(cur_tr_uset,cur_unit_ind),tr_cX);
        new_expLL_soft = NMMmodel_eval_EM(it_mods_spkNL_soft{nn+1}(cur_cell),Robs_mat(cur_tr_uset,cur_unit_ind),tr_X,gamma(cur_tr_uset,:),shifts);
        new_LL_soft = NMMmodel_eval(it_mods_spkNL_soft{nn+1}(cur_cell),Robs_mat(cur_tr_uset,cur_unit_ind),tr_cX);

        it_xvmods_soft{nn+1}(cur_cell) = it_xvmods_soft{nn}(cur_cell);
        it_xvmods_soft{nn+1}(cur_cell) = NMMfit_filters_EM(it_xvmods_soft{nn+1}(cur_cell),Robs_mat(cur_xv_uset,cur_unit_ind),...
            xv_X,gamma(xv_inds(cur_xv_uset),:),shifts,all_Xmat_up_fixcor(xv_inds(cur_xv_uset),use_kInds_up),[],[],silent,optim_p); %fit stimulus filters
        ...
        it_xvmods{nn+1}(cur_cell) = NMMfit_filters(it_xvmods{nn+1}(cur_cell),Robs_mat(cur_xv_uset,cur_unit_ind),...
            xv_cX,[],[],silent,optim_p); %fit stimulus filters
        
        
        %refit spk NL
        it_xvmods_spkNL_soft{nn+1}(cur_cell) = NMMfit_logexp_spkNL(it_xvmods_soft{nn+1}(cur_cell),Robs_mat(cur_xv_uset,cur_unit_ind),xv_cX);       
        it_xvmods_spkNL{nn+1}(cur_cell) = NMMfit_logexp_spkNL(it_xvmods{nn+1}(cur_cell),Robs_mat(cur_xv_uset,cur_unit_ind),xv_cX);       

        new_expxvLL = NMMmodel_eval_EM(it_xvmods_spkNL{nn+1}(cur_cell),Robs_mat(cur_xv_uset,cur_unit_ind),xv_X,gamma(cur_xv_uset,:),shifts);
        new_xvLL = NMMmodel_eval(it_xvmods_spkNL{nn+1}(cur_cell),Robs_mat(cur_xv_uset,cur_unit_ind),xv_cX);
        new_expxvLL_soft = NMMmodel_eval_EM(it_xvmods_spkNL_soft{nn+1}(cur_cell),Robs_mat(cur_xv_uset,cur_unit_ind),xv_X,gamma(cur_xv_uset,:),shifts);
        new_xvLL_soft = NMMmodel_eval(it_xvmods_spkNL_soft{nn+1}(cur_cell),Robs_mat(cur_xv_uset,cur_unit_ind),xv_cX);
        
        it_LLimp(nn+1,cur_cell) = (new_LL - null_LL(cur_cell))/log(2);
        it_xvLLimp(nn+1,cur_cell) = (new_xvLL - null_xvLL(cur_cell))/log(2);
        it_expLLimp(nn+1,cur_cell) = (new_expLL - null_LL(cur_cell))/log(2);
        it_expxvLLimp(nn+1,cur_cell) = (new_expxvLL - null_xvLL(cur_cell))/log(2);
        it_LLimp_soft(nn+1,cur_cell) = (new_LL_soft - null_LL(cur_cell))/log(2);
        it_xvLLimp_soft(nn+1,cur_cell) = (new_xvLL_soft - null_xvLL(cur_cell))/log(2);
        it_expLLimp_soft(nn+1,cur_cell) = (new_expLL_soft - null_LL(cur_cell))/log(2);
        it_expxvLLimp_soft(nn+1,cur_cell) = (new_expxvLL_soft - null_xvLL(cur_cell))/log(2);
        
        if nn > 1
            fprintf('Original: %.4f  Prev: %.4f  New: %.4f\n',all_mod_LLimp(cur_cell),it_LLimp(nn,cur_cell),it_LLimp(nn+1,cur_cell));
            fprintf('Original Exp: %.4f  Prev: %.4f  New: %.4f\n',all_mod_LLimp(cur_cell),it_expLLimp(nn,cur_cell),it_expLLimp(nn+1,cur_cell));
        else
            fprintf('Original: %.4f  New: %.4f\n',all_mod_LLimp(cur_cell),it_LLimp(nn+1,cur_cell));
            fprintf('Original Exp: %.4f  New: %.4f\n',all_mod_LLimp(cur_cell),it_expLLimp(nn+1,cur_cell));
        end
        if nn > 1
            fprintf('XV Original: %.4f  Prev: %.4f  New: %.4f\n',all_mod_xvLLimp(cur_cell),it_xvLLimp(nn,cur_cell),it_xvLLimp(nn+1,cur_cell));
            fprintf('XV Original Exp: %.4f  Prev: %.4f  New: %.4f\n',all_mod_xvLLimp(cur_cell),it_expxvLLimp(nn,cur_cell),it_expxvLLimp(nn+1,cur_cell));
        else
            fprintf('XV Original: %.4f  New: %.4f\n',all_mod_xvLLimp(cur_cell),it_xvLLimp(nn+1,cur_cell));
            fprintf('XV Original Exp: %.4f  New: %.4f\n',all_mod_xvLLimp(cur_cell),it_expxvLLimp(nn+1,cur_cell));
        end
    end
end
