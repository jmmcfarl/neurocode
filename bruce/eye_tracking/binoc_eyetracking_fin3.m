...
anal_name = 'binoc_eyecorr_vbar_left_Fin';
mod_data_name = 'binoc_eyecorr_vbar_mods_left_Fin';
...
if ~exist(['./' mod_data_name '.mat'],'file') || recompute_init_mods
    ...
    for ss = 1:96;
        ...
        if ~isempty(cur_tr_inds)
            ...
            if xv_frac > 0
                xvLL = NMMmodel_eval(all_mod_fits_withspkNL(ss),Robsxv,xv_X);
    ...
    for ss = 1:length(su_probes);
        ...
        if ~isempty(cur_tr_inds)
            ...
            if xv_frac > 0
                xvLL = NMMmodel_eval(all_mod_fits_withspkNL(ss+96),Robsxv,xv_X);
...
et_params = struct('beg_buffer',beg_buffer,'end_buffer',end_buffer,'min_trial_dur',min_trial_dur,'bar_ori',bar_ori,...
    ...
    'drift_dsf',drift_dsf,'fix_prior_sigma',fix_prior_sigma,'n_fix_inf_it',n_fix_inf_it,'use_sac_kerns',use_sac_kerns,'shifts',shifts,...
