...
anal_name = 'dualori_eyecorr';
mod_data_name = 'dualroi_eyecorr_mods';
eye_data_name = 'dualori_eyecorr_eyeonly';
use_right_eye = false;
%%
stim_fs = 100; %in Hz
dt = 0.01;
...
for ii = 1:n_trials
    ...
    if ~isempty(cur_trial_inds)
        ...
        %         end
        cur_out = find(abs(corrected_interp_eyevals(cur_trial_inds,2)) >= orth_thresh | ...
            abs(corrected_interp_eyevals(cur_trial_inds,1)) >= orth_thresh,1,'first');
        if ~isempty(cur_out)
...
full_X{1} = full_Xmat(used_inds,use_kInds);
...
hor_filt_bank = filt_bank(:,1:use_nPix,:,:);
ver_filt_bank = filt_bank(:,(1:use_nPix) + use_nPix,:,:);
...
cur_Xmat = full_Xmat(used_inds,use_kInds);

%precompute LL at all shifts for all units
frame_LLs = nan(NT,n_shifts);
for xx = 1:n_shifts
    fprintf('Shift %d of %d\n',xx,n_shifts);
    cur_Hfilt_shift = shift_matrix_Nd(hor_filt_bank,SH(xx,1),2);
    cur_Hfilt_shift = reshape(cur_Hfilt_shift,[flen*use_nPix n_tr_chs 3]);
    cur_Vfilt_shift = shift_matrix_Nd(ver_filt_bank,SH(xx,2),2);
    cur_Vfilt_shift = reshape(cur_Vfilt_shift,[flen*use_nPix n_tr_chs 3]);
...
lalpha=zeros(use_NT,n_shifts);
lbeta = zeros(use_NT,n_shifts);
lscale=zeros(use_NT,1); %initialize rescaling parameters
...
lbeta(use_NT,:)=log(ones(1,n_shifts)) - lscale(use_NT);
for t=use_NT-1:-1:1
    if mod(t,100)==0
        fprintf('%d of %d\n',t,use_NT);
    end
...
gamma = exp(lgamma);
clear fpost_mean
fpost_mean(:,1) = sum(bsxfun(@times,gamma,SH(:,1)'),2);
fpost_mean(:,2) = sum(bsxfun(@times,gamma,SH(:,2)'),2);

% fpost_std = sqrt(sum(bsxfun(@times,gamma,shifts.^2),2) - fpost_mean.^2);
[max_post,max_loc] = max(lgamma,[],2);
fshift_cor = SH(max_loc,:);
...
ms = 10;
close all
for ii = tail_len+1:cur_NT
    clf;
    plot(fpost_mean(ii,2)*sp_dx,fpost_mean(ii,1)*sp_dx,'ko','markersize',ms,'linewidth',2)
    xlim([-1 1]); ylim([-1 1]);
    hold on
    plot(fpost_mean(ii-tail_len:ii,2)*sp_dx,fpost_mean(ii-tail_len:ii,1)*sp_dx,'k-');
    plot(corrected_interp_eyevals(used_inds(ii),1),corrected_interp_eyevals(used_inds(ii),2),'ro','markersize',ms,'linewidth',2)
    plot(corrected_interp_eyevals(used_inds(ii-tail_len:ii),1),corrected_interp_eyevals(used_inds(ii-tail_len:ii),2),'r-')
   plot(corrected_interp_eyevals(used_inds(ii),3),corrected_interp_eyevals(used_inds(ii),4),'bo','markersize',ms,'linewidth',2)
    plot(corrected_interp_eyevals(used_inds(ii-tail_len:ii),3),corrected_interp_eyevals(used_inds(ii-tail_len:ii),4),'b-')
...
%find hor and ver inds within used inds and make appropriate corrections to
%those segments
stim_mat_cor = all_stim_mat;
for ii = 1:NT
   stim_mat_cor(used_inds(ii),:) = shift_matrix_Nd(all_stim_mat(used_inds(ii),:),-fshift_cor(ii,1),2); 
end
...
for ss = 1:length(tr_set)
    ...
    left_ref_mod{1}(tr_cell) = NMMfit_filters(left_ref_mod{1}(tr_cell),Robs_mat(cur_uset,cur_unit_ind),...
        cur_X,[],find(Xtargs ~= 1),silent); %fit stimulus filters
