...
%%
spatial_usfac = 2;
full_nPix_us = spatial_usfac*full_nPix - 1;
all_stimmat_up = zeros(size(all_stim_mat,1),full_nPix_us);
all_stimmat_up(:,1) = all_stim_mat(:,1);
for ii = 1:size(all_stim_mat,2)-1
    all_stimmat_up(:,2*ii+1) = all_stim_mat(:,ii+1);
end
...
[Xinds_up,Tinds] = meshgrid(1:1/spatial_usfac:full_nPix,1:flen);
...
nmm_stim_params2 = NMMcreate_stim_params([flen spatial_usfac*use_nPix-1],dt);
...
for ss = 1:96;
    ...
    if ~isempty(cur_used_inds)
        ...
        init_stim_filts = zeros(flen*(use_nPix*spatial_usfac-1),n_squared_filts+1);
        init_stim_filts(use_kInds_back,:) = base_filts;
...
use_nPix_up = use_nPix*spatial_usfac-1;
