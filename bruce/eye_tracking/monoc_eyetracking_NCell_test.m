...
for nn = 1:length(Ncells)
    if Ncells(nn) > 0
        ...
        tr_set = usable_units(rperm(1:Ncells(nn)));
    else
        tr_set = usable_units(all_mod_SU(usable_units) == 0);
    end
    Ncell_tr_set{nn} = tr_set;
    ...
    Post = exp(lPost);
    fix_post_mean(nn,:) = sum(bsxfun(@times,Post,shifts),2);
    % cur_post_std = sqrt(sum(bsxfun(@times,Post,shifts.^2),2) - cur_post_mean.^2);
    cur_diff = bsxfun(@minus,fix_post_mean(nn,:)',shifts).^2;
    fix_post_std(nn,:) = sqrt(sum(cur_diff.*Post,2));
    
    all_fix_post_mean = fix_post_mean(nn,fix_ids);
    all_fix_post_std = fix_post_std(nn,fix_ids);
    ...
    for ss = 1:length(cur_fit_set)
        ...
        ref_mod{nn}(tr_cell) = all_mod_fits(tr_cell);
    ...
    mod_spkNL_params = nan(n_tr_chs,3);
    for ss = 1:n_tr_chs
        cur_mod = ref_mod{nn}(tr_set(ss));
        cur_mod_spkNL = ref_mod_spkNL{nn}(tr_set(ss));
    ...
    it_all_fix_post_mean = it_fix_post_mean(nn,fix_ids);
    it_all_fix_post_std = it_fix_post_std(nn,fix_ids);
...
for nn = 1:length(Ncells);
mx = sum(ww.*it_fix_post_mean(1,:));
my = sum(ww.*it_fix_post_mean(nn,:));
cov_xy = sum(ww.*(it_fix_post_mean(1,:) - mx).*(it_fix_post_mean(nn,:) - my));
cov_xx = sum(ww.*(it_fix_post_mean(1,:) - mx).*(it_fix_post_mean(1,:) - mx));
cov_yy = sum(ww.*(it_fix_post_mean(nn,:) - my).*(it_fix_post_mean(nn,:) - my));

it2_weight_r2(nn) = cov_xy./sqrt(cov_xx*cov_yy);
end