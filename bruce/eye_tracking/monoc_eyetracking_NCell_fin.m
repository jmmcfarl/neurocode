...
if bar_ori == 0
    ...
    anal_name = 'monoc_eyecorr_hbar_finf_Ncell_5c';
    %     anal_name = 'monoc_eyecorr_hbar_finf_noLOOXV';
else
    mod_data_name = 'monoc_eyecorr_vbar_mods_fin5';
    anal_name = 'monoc_eyecorr_vbar_finf_Ncell_5c';
    %     anal_name = 'monoc_eyecorr_vbar_finf_noLOOXV';
end

% try_Ncells = [80 70 60 50 40 30 20 10];
% Ncells{1} = 'all';
% Ncells{2} = 'good';
% Ncells{3} = 'multi';
% for i = 4:(4+length(try_Ncells)-1)
%     Ncells{i} = try_Ncells(i-3);
% end
try_Ncells = [5];
Ncells{1} = 5;
n_rpts = 5;
...
%%
it_LLimp = nan(n_rpts,length(Ncells),n_fix_inf_it,length(all_mod_SU));
it_fix_post_mean = nan(n_rpts,length(Ncells),n_fix_inf_it,n_fixs);
it_fix_post_std = nan(n_rpts,length(Ncells),n_fix_inf_it,n_fixs);
for cc = 1:length(Ncells)
    ...
    for rr = 1:cur_nrpts
        ...
        if strcmp(Ncells{cc},'all')
            tr_set = find(all_mod_LLimp ~= 0);
        elseif strcmp(Ncells{cc},'multi')
            tr_set = usable_units(all_mod_SU(usable_units) == 0);
        elseif strcmp(Ncells{cc},'good')
            tr_set = usable_units;
        else
            rperm = randperm(length(usable_units));
            tr_set = usable_units(rperm(1:Ncells{cc}));
        end
        Ncell_tr_set{cc,rr} = tr_set;
        n_used_sus = sum(all_mod_SU(tr_set) ~= 0);
        n_used_mus = sum(all_mod_SU(tr_set) == 0);
        ...
        it_mods{cc}{1} = all_mod_fits;
        it_mods_spkNL{cc}{1} = all_mod_fits_withspkNL;
        for nn = 1:n_fix_inf_it
            ...
            for ss = 1:n_tr_chs
                cur_Xtargs = [it_mods{cc}{nn}(tr_set(ss)).mods(:).Xtarget];
                cur_k = [it_mods{cc}{nn}(tr_set(ss)).mods(cur_Xtargs == 1).filtK];
                n_used_filts = size(cur_k,2);
                filt_bank(ss,:,1:n_used_filts) = cur_k;
                mod_spkNL_params(ss,:) = it_mods_spkNL{cc}{nn}(tr_set(ss)).spk_NL_params;
                lin_kerns(ss,:) = it_mods{cc}{nn}(tr_set(ss)).mods(cur_Xtargs == 2).filtK;
                if use_sac_kerns
                    sac_kerns(ss,:) = it_mods{cc}{nn}(tr_set(ss)).mods(cur_Xtargs == 3).filtK;
                    msac_kerns(ss,:) = it_mods{cc}{nn}(tr_set(ss)).mods(cur_Xtargs == 4).filtK;
                end
            end
            ...
            Post = exp(lPost);
            it_fix_post_mean(rr,cc,nn,:) = sum(bsxfun(@times,Post,shifts),2);
            cur_diff = bsxfun(@minus,squeeze(it_fix_post_mean(rr,cc,nn,:)),shifts).^2;
            it_fix_post_std(rr,cc,nn,:) = sqrt(sum(cur_diff.*Post,2));
            
            %back-project saccade-times
            all_fix_post_mean_cor = squeeze(it_fix_post_mean(rr,cc,nn,fix_ids));
            ...
            for ss = 1:length(tr_set)
                ...
                it_mods{cc}{nn+1}(cur_cell) = it_mods{cc}{nn}(cur_cell);
                it_mods{cc}{nn+1}(cur_cell) = NMMfit_filters(it_mods{cc}{nn+1}(cur_cell),Robs_mat(cur_tr_uset,cur_unit_ind),...
                    tr_X,[],[],silent); %fit stimulus filters
                
                %refit spk NL
                it_mods_spkNL{cc}{nn+1}(cur_cell) = NMMfit_logexp_spkNL(it_mods{cc}{nn+1}(cur_cell),Robs_mat(cur_tr_uset,cur_unit_ind),tr_X);
                newLL = NMMmodel_eval(it_mods_spkNL{cc}{nn+1}(cur_cell),Robs_mat(cur_tr_uset,cur_unit_ind),tr_X);
                
                it_LLimp(rr,cc,nn+1,cur_cell) = (newLL - null_LL(cur_cell))/log(2);
                if nn > 1
                    fprintf('Original: %.4f  Prev: %.4f  New: %.4f\n',all_mod_LLimp(cur_cell),it_LLimp(rr,cc,nn,cur_cell),it_LLimp(rr,cc,nn+1,cur_cell));
                else
                    fprintf('Original: %.4f  New: %.4f\n',all_mod_LLimp(cur_cell),it_LLimp(rr,cc,nn+1,cur_cell));
                end
            end
        end
        
        cd(anal_dir)
        save(anal_name,'it_*','Ncell*');
...
%%
cd(anal_dir);
anal_name = 'monoc_eyecorr_hbar_finf_Ncell';
load(anal_name);
anal_name = 'monoc_eyecorr_hbar_finf_Ncell_5c';
temp = load(anal_name,'it_fix_post_mean');
new_pt = (temp.it_fix_post_mean);
it_fix_post_mean = cat(2,it_fix_post_mean,new_pt);
Ncells{end+1} = 5;
%%
cd ~/Analysis/bruce/summary_analysis/eyetrack_figs/

fix_blocks = all_blockvec(jump_inds(1:end-1));
used_fixs = find(~ismember(fix_blocks,ignore_blocks));

best_seq = squeeze(it_fix_post_mean(1,2,3,used_fixs));

b_ax = linspace(-0.3,0.3,501);
clear diff_dist
for cc = 1:length(Ncells)
    for rr = 1:n_rpts
        cur_avg_seq = squeeze(it_fix_post_mean(rr,cc,end,used_fixs));
        cur_diff = (cur_avg_seq - best_seq)*sp_dx;
        
        best_mad(rr,cc) = nanmedian(abs(cur_diff));
        best_rms(rr,cc) = sqrt(nanmean(cur_diff.^2));
        diff_dist(rr,cc,:) = hist(cur_diff,b_ax);
        diff_dist(rr,cc,:) = cumsum(diff_dist(rr,cc,:))/sum(diff_dist(rr,cc,:));
        
    end
end

mad_inf_PF = median(abs(best_seq))*sp_dx;
measured_fix_avg = nan(n_fixs,1);
for ii = 1:n_fixs
    cur_inds = jump_inds(ii):(jump_inds(ii+1)-1);
    measured_fix_avg(ii) = median(corrected_interp_eyevals(used_inds(cur_inds),2));
end
mad_inf_ET = median(abs(measured_fix_avg(used_fixs) - best_seq*sp_dx));

used = [3:12];
nn = [85 80:-10:10 5];
figure
errorbar(nn,nanmean(best_mad(:,used)),nanstd(best_mad(:,used)),'o-','linewidth',2);
line([0 90],[mad_inf_PF mad_inf_PF],'color','r','linestyle','--');
line([0 90],[mad_inf_ET mad_inf_ET],'color','k','linestyle','--');
xlim([0 90]);
xlabel('Number of units','fontsize',12);
ylabel('MAD from best estimate (deg)','fontsize',12);
set(gca,'fontsize',10,'fontname','arial');
box off
fillPage(gcf,'papersize',[4 4]);
fname = 'NCell_MAD';

avg_diff_dists = squeeze(nanmean(diff_dist));
figure; hold on
plot(b_ax,avg_diff_dists(3,:));
plot(b_ax,avg_diff_dists(7,:),'r');
plot(b_ax,avg_diff_dists(10,:),'g');
xlim([-0.2 0.2]);
xlabel('Difference from best estimate (deg)','fontsize',12);
ylabel('Cumulative probability','fontsize',12);
set(gca,'fontsize',10,'fontname','arial');
nn = hist(best_seq*sp_dx,b_ax); 
% nn = nn/max(nn);
nn = cumsum(nn)/sum(nn);
plot(b_ax,nn,'k');
legend('Multi-unit','50 units','20 units','Best inferred');
fillPage(gcf,'papersize',[4 4]);
fname = 'NCell_cumdist';
