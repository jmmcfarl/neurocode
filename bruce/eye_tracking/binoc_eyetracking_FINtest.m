...
if bar_ori == 0
    mod_data_name = 'binoc_eyecorr_hbar_mods_FinFin';
    anal_name = 'binoc_eyecorr_hbar_FinFin';
else
    mod_data_name = 'binoc_eyecorr_vbar_mods_FinFin';
    anal_name = 'binoc_eyecorr_vbar_FinFin';
end
...
if spatial_usfac == 2
    all_Lstimmat_up = zeros(size(all_Lstim_mat,1),full_nPix_us);
    all_Rstimmat_up = zeros(size(all_Rstim_mat,1),full_nPix_us);
    for ii = 1:size(all_Lstim_mat,2)
        all_Lstimmat_up(:,2*(ii-1)+1) = all_Lstim_mat(:,ii);
        all_Lstimmat_up(:,2*(ii-1)+2) = all_Lstim_mat(:,ii);
        all_Rstimmat_up(:,2*(ii-1)+1) = all_Rstim_mat(:,ii);
        all_Rstimmat_up(:,2*(ii-1)+2) = all_Rstim_mat(:,ii);
    end
elseif spatial_usfac == 1
    all_Lstimmat_up = all_Lstim_mat;
    all_Rstimmat_up = all_Rstim_mat;
else
    error('Unsupported spatial Up-sample factor!');
end
stim_params_us = NMMcreate_stim_params([flen full_nPix_us],dt);
all_LXmat = create_time_embedding(all_Lstim_mat,stim_params);
all_LXmat_us = create_time_embedding(all_Lstimmat_up,stim_params_us);
all_RXmat = create_time_embedding(all_Rstim_mat,stim_params);
all_RXmat_us = create_time_embedding(all_Rstimmat_up,stim_params_us);
...
atr_X{1} = all_LXmat(used_inds,use_kInds);
atr_X{2} = all_RXmat(used_inds,use_kInds);
atr_X{3} = Xblock(used_inds,:);
axv_X{1} = all_LXmat(used_inds,use_kInds);
axv_X{2} = all_RXmat(used_inds,use_kInds);
axv_X{3} = Xblock(used_inds,:);
...
init_stim_params(3) = NMMcreate_stim_params([n_blocks 1],dt);

fin_stim_params(1) = NMMcreate_stim_params([flen use_nPix_us],dt);
fin_stim_params(2) = NMMcreate_stim_params([flen use_nPix_us],dt);
fin_stim_params(3) = NMMcreate_stim_params([n_blocks 1],dt);
if use_sac_kerns
    fin_stim_params(4) = NMMcreate_stim_params(n_sac_bins,dt);
    fin_stim_params(5) = NMMcreate_stim_params(n_sac_bins,dt);
end

null_stim_params = fin_stim_params(3:end);
...
n_squared_filts = 2;
mod_signs = ones(1,2*n_squared_filts+3);
NL_types = [{'lin'} repmat({'quad'},1,n_squared_filts) {'lin'} repmat({'quad'},1,n_squared_filts) {'lin'}];
init_d2XT = [ones(n_squared_filts+1,1); ones(n_squared_filts+1,1); 0];
init_L2 = [zeros(n_squared_filts+1,1); zeros(n_squared_filts+1,1); block_L2];
init_reg_params = NMMcreate_reg_params('lambda_d2XT',init_d2XT,'lambda_L2',init_L2);
init_Xtargs = [ones(n_squared_filts+1,1); 2*ones(n_squared_filts+1,1); 3];
...
if ~exist(['./' mod_data_name '.mat'],'file') || recompute_init_mods == 1
    ...
    for ss = 1:n_probes;
        ...
        if ~isempty(cur_tr_inds)
            ...
            tr_X = get_Xcell_tInds(atr_X,cur_tr_inds);
            xv_X = get_Xcell_tInds(axv_X,cur_xv_inds);            
            if use_sac_kerns
            ...
            null_mod = NMMfit_filters(null_mod,Robs,tr_X(3:end));
            ...
            tr_X{1} = all_LXmat_us(used_inds(cur_tr_inds),use_kInds_up);
            xv_X{1} = all_LXmat_us(used_inds(cur_xv_inds),use_kInds_up);
            tr_X{2} = all_RXmat_us(used_inds(cur_tr_inds),use_kInds_up);
            xv_X{2} = all_RXmat_us(used_inds(cur_xv_inds),use_kInds_up);
            %spatial up-sampling of filter estimates
            stim_filts = find(ismember(init_Xtargs,[1 2]));
            base_filts = reshape([gqm1.mods(stim_filts).filtK],[flen use_nPix 2*(n_squared_filts+1)]);
            if spatial_usfac == 2
            ...
            init_filts{end} = gqm1.mods(find(init_Xtargs==3)).filtK;
            for ii = 1:2*(n_squared_filts+1)
            ...
            gqm2 = NMMadjust_regularization(gqm2,stim_filts,'lambda_d2XT',base_lambda_d2XT./var(gint)');
            gqm2 = NMMadjust_regularization(gqm2,stim_filts,'lambda_L1',base_lambda_L1./std(gint)');
            if use_sac_kerns
                gqm2 = NMMadd_NLinput(gqm2,{'lin'},1,4,zeros(n_sac_bins,1),sac_reg_params); %sac term
                gqm2 = NMMadd_NLinput(gqm2,{'lin'},1,5,zeros(n_sac_bins,1),sac_reg_params); %nsac term
            end
            gqm2 = NMMfit_filters(gqm2,Robs,tr_X,[],[],silent);
 
            xvLL = NMMmodel_eval(gqm2,Robsxv,xv_X);
            null_xvLL(ss) = NMMmodel_eval(null_mod,Robsxv,xv_X(3:end));
            ...
            tr_X{1} = all_LXmat_us(used_inds(cur_tr_inds),use_kInds_up);
            tr_X{2} = all_RXmat_us(used_inds(cur_tr_inds),use_kInds_up);            
            tr_X{3} = Xblock(used_inds(cur_tr_inds),:);
            if use_sac_kerns
                tr_X{4} = Xsac(cur_tr_inds,:);
                tr_X{5} = Xmsac(cur_tr_inds,:);
            end
            null_mod = NMMfit_filters(null_mod,Robs,tr_X(3:end));
            ...
            [null_LL(ss),~,null_prate] = NMMmodel_eval(null_mod,Robs,tr_X(3:end));
    ...
    for ss = 1:length(su_probes);
        ...
        if ~isempty(cur_tr_inds)
            ...
            xvLL = NMMmodel_eval(gqm2,Robsxv,xv_X);
            null_xvLL(ss+n_probes) = NMMmodel_eval(null_mod,Robsxv,xv_X(3:end));
           ...
           [LL,~,pred_rate] = NMMmodel_eval(all_mod_fits_withspkNL(ss+n_probes),Robs,tr_X);
            [null_LL(ss+n_probes),~,null_prate] = NMMmodel_eval(null_mod,Robs,tr_X(3:end));
