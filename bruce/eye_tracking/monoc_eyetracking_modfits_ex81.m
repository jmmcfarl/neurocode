...
for ss = 1:length(su_probes)
    ...
    if ~isempty(cur_used_inds)
        ...
        sta = sta/norm(sta);
        abs_spike_cond_stim = abs(all_Xmat(cur_used_inds(spikebins),:));
        abs_sta      = mean(abs_spike_cond_stim) - mean(abs(all_Xmat(cur_used_inds,:)));
...
for ss = 1:96
    ...
    if ~isempty(cur_used_inds)
        ...
        mua_data(ss).npos_stc = find(cur_evec_diff > stc_thresh,1,'first')-1;
        mua_data(ss).nneg_stc = find(cur_evec_diff(end:-1:1) > stc_thresh,1,'first')-1;
...
for ss = 1:length(su_probes)
    ...
    cur_abs_sta = sua_data(ss).abs_sta; abs_ca = max(abs(cur_abs_sta));
    subplot(3,3,1)
    imagesc(reshape(cur_sta,flen,use_nPix)); caxis([-ca ca]);
    subplot(3,3,2)
    imagesc(reshape(cur_abs_sta,flen,use_nPix)); caxis([-abs_ca abs_ca]);
...
for ss = 1:96
    ...
    cur_abs_sta = mua_data(ss).abs_sta; abs_ca = max(abs(cur_abs_sta));
    subplot(3,3,1)
    imagesc(reshape(cur_sta,flen,use_nPix)); caxis([-ca ca]);
     subplot(3,3,2)
    imagesc(reshape(cur_abs_sta,flen,use_nPix)); caxis([-abs_ca abs_ca]);
