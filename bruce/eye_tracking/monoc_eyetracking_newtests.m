...
% use_kInds_up = find(Xinds_up(:) >= cur_use_pix(1)-1/spatial_usfac & Xinds_up(:) <= cur_use_pix(end) & Tinds(:) == 5);
% use_kInds_back = find(ismember(Xinds_up(use_kInds_up),cur_use_pix));
...
% sac_shift = round(0/dt);
min_post_sac_dur = round(0.05/dt);
...
for i = 1:length(saccade_start_inds)
    ...
    if next_trial - saccade_start_inds(i) > min_post_sac_dur
...
all_stim_mat_cor = all_stimmat_up;
for ii = 1:NT
    all_stim_mat_cor(used_inds(ii),:) = shift_matrix_Nd(all_stimmat_up(used_inds(ii),:),...
        -round(corrected_interp_eyevals(used_inds(ii),2)/sp_dx),2);
end
all_Xmat_cor = create_time_embedding(all_stim_mat_cor,stim_params_us);
% all_Xmat_cor = all_Xmat_cor(used_inds,:);
all_Xmat_us = all_Xmat_cor;
%% EVALUATE LLS, FIT SPK NLS, EVALUATE NULL_LLS AND THRESHOLD LL IMP VALUES TO GET USABLE UNITS
nmm_stim_params2 = NMMcreate_stim_params([flen spatial_usfac*use_nPix],dt);
...
%         init_stim_filts = reshape(init_stim_filts,use_nPix*spatial_usfac*1,n_squared_filts+1);
        ...
        new_fit_up = NMMinitialize_model(nmm_stim_params2,mod_signs,NL_types,[],Xtargs,init_filts); %initialize NIM
...
filt_bank = reshape(filt_bank,[flen use_nPix_up n_tr_chs 3]);
...
resh_X = reshape(all_Xmat_us(used_inds,:)',[flen full_nPix_us NT]);
...
close all
uu = 1:10e4;
% shadedErrorBar(all_t_axis(used_inds(uu)),fpost_mean(uu)*sp_dx,fpost_std(uu)*sp_dx);
% hold on
% plot(all_eye_ts,corrected_eyevals(:,2),'r')
shadedErrorBar(uu,fpost_mean(uu)*sp_dx,fpost_std(uu)*sp_dx);
hold on
plot(corrected_interp_eyevals(used_inds,2),'r')
plot(uu,fpost_mean_cor(uu)*sp_dx,'b','linewidth',2);
...
fshift_cor_cor = fshift_cor;

jump_inds = sort([trial_start_inds; saccade_start_inds; length(used_inds)]);

%align jumps to saccade times
for i = 1:length(jump_inds)-1
    cur_inds = (jump_inds(i)):(jump_inds(i+1)-1);
    if length(cur_inds) > sac_shift
        fpost_mean_cor(cur_inds(1:(end-sac_shift+1))) = fpost_mean(cur_inds(sac_shift:end));
        fpost_mean_cor(cur_inds((end-sac_shift+1):end)) = fpost_mean(cur_inds(end));
        fshift_cor_cor(cur_inds(1:(end-sac_shift+1))) = fshift_cor(cur_inds(sac_shift:end));
        fshift_cor_cor(cur_inds((end-sac_shift+1):end)) = fshift_cor(cur_inds(end));
    end
end
...
% for ss = 1:length(cur_fit_set)
 for ss =(length(cur_fit_set)-6):length(cur_fit_set)shg
   ...
   tr_cell = cur_fit_set(ss);
    ...
    fprintf('Original: %.4f  New1: %.4f \n',all_mod_LLimp(tr_cell),ref_LL_imp(1,tr_cell));
    
%     cur_X{1} = all_Xmat_cor(cur_uset,:);
%     
%     ref_mod2{1}(tr_cell) = all_mod_fits(tr_cell);
%     ref_mod2{1}(tr_cell) = NMMfit_filters(ref_mod2{1}(tr_cell),Robs_mat(cur_uset,cur_unit_ind),...
%         cur_X,[],[],silent); %fit stimulus filters
%     
%     ref_LL_imp2(1,tr_cell) = (ref_mod2{1}(tr_cell).LL_seq(end) - all_nullmod(tr_cell).LL_seq(end))/log(2);
%     
%     fprintf('Original: %.4f  New1: %.4f  New2: %.4f\n',all_mod_LLimp(tr_cell),ref_LL_imp(1,tr_cell),ref_LL_imp2(1,tr_cell));
