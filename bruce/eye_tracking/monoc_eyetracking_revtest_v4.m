...
%% LOOP OVER USABLE SUS AND DO XVAL ESIMATION
poss_sus = find(all_mod_SU(tr_set) > 0);
for xv = 1:length(poss_sus)
    fprintf('Setting aside SU %d of %d\n',xv,length(poss_sus));
    cur_tr_set = setdiff(1:length(tr_set),poss_sus(xv));
    
    cur_cell_LLs = nan(NT,n_shifts);
    cur_filt_bank = squeeze(filt_bank(:,:,poss_sus(xv),:));
    for xx = 1:length(shifts)
        %         fprintf('Shift %d of %d\n',xx,n_shifts);
        cur_filt_shift = shift_matrix_Nd(cur_filt_bank,shifts(xx),2);
        cur_filt_shift = reshape(cur_filt_shift,[klen n_squared_filts+1]);
        
        %outputs of stimulus models at current X-matrix shift
        gfuns = ones(NT,1);
        gfuns = bsxfun(@times,gfuns,mod_spkNL_params(poss_sus(xv),1)');
        gfuns = gfuns + cur_Xmat*squeeze(cur_filt_shift(:,1));
        for ff = 2:(n_squared_filts+1)
            gfuns = gfuns + (cur_Xmat*squeeze(cur_filt_shift(:,ff))).^2;
        end
        ...
        %add contributions from extra lin kernels
        gfuns = gfuns + block_out(:,poss_sus(xv)) + sac_out(:,poss_sus(xv)) + msac_out(:,poss_sus(xv));
        
        %incorporate beta
        gfuns = bsxfun(@times,gfuns,mod_spkNL_params(poss_sus(xv),2)');
        ...
        %incorporate alpha
        pred_rate = bsxfun(@times,pred_rate,mod_spkNL_params(poss_sus(xv),3)');
        
        pred_rate(pred_rate < 1e-20) = 1e-20;
        
        cur_cell_LLs(:,xx) = Robs_mat(:,poss_sus(xv)).*log(pred_rate) - pred_rate;
        %         LLs(:,:,xx) = Robs_mat.*log(pred_rate) - pred_rate;
    end
    cur_cell_LLs(isnan(cur_cell_LLs)) = 0;
    cur_frame_LLs = frame_LLs - cur_cell_LLs;
    ...
    fix_post_mean(xv,:) = cur_post_mean(fix_ids);
    fix_post_std(xv,:) = cur_post_std(fix_ids);
    
    %back-project saccade-times
    fix_post_mean_cor(xv,:) = fix_post_mean(xv,:);
    ...
    for i = length(saccade_start_inds):-1:1
        ...
        fix_post_mean_cor(xv,cur_inds) = fix_post_mean(xv,cur_inds(end));
        cur_back_inds = (saccade_start_inds(i)-(flen-sac_shift)):(saccade_start_inds(i)-1);
        fix_post_mean_cor(xv,cur_back_inds) = fix_post_mean(xv,saccade_start_inds(i));
    end
    
    %% RECOMPUTE XMAT
    all_shift_stimmat_up = all_stimmat_up;
    for i=1:NT
        all_shift_stimmat_up(used_inds(i),:) = shift_matrix_Nd(all_stimmat_up(used_inds(i),:),-round(fix_post_mean_cor(xv,i)),2);
    end
    ...
    xv_ref_mod{1}(xv) = all_mod_fits(tr_cell);
    xv_ref_mod{1}(xv) = NMMfit_filters(xv_ref_mod{1}(xv),Robs_mat(cur_uset,cur_unit_ind),...
        cur_X,[],[],silent); %fit stimulus filters
    
    xv_ref_LL_imp(1,xv) = (xv_ref_mod{1}(xv).LL_seq(end) - all_nullmod(tr_cell).LL_seq(end))/log(2);
    
    fprintf('Original: %.4f  New: %.4f\n',all_mod_LLimp(tr_cell),xv_ref_LL_imp(1,xv));
    xv_ref_mod_spkNL{1}(xv) = NMMfit_logexp_spkNL(xv_ref_mod{1}(xv),Robs_mat(cur_uset,cur_unit_ind),cur_X);
...
for xv = 1:length(poss_sus)
    ...
    use_inds = find(~any(isnan(cur_cell_LLs),2));
    cur_frame_LLs = frame_LLs;
    cur_frame_LLs(use_inds,:) = cur_frame_LLs(use_inds,:) - cur_cell_LLs(use_inds,:);
    ...
    it_fix_post_mean(xv,:) = cur_post_mean(fix_ids);
    it_fix_post_std(xv,:) = cur_post_std(fix_ids);
    
    %back-project saccade-times
    it_fix_post_mean_cor(xv,:) = it_fix_post_mean(xv,:);
    %align jumps to saccade times
    for i = length(saccade_start_inds):-1:1
        cur_inds = saccade_start_inds(i):(saccade_start_inds(i)+sac_shift);
        it_fix_post_mean_cor(xv,cur_inds) = it_fix_post_mean(xv,cur_inds(end));
        cur_back_inds = (saccade_start_inds(i)-(flen-sac_shift)):(saccade_start_inds(i)-1);
        it_fix_post_mean_cor(xv,cur_back_inds) = it_fix_post_mean(xv,saccade_start_inds(i));
    end
    
    %% RECOMPUTE XMAT
    all_shift_stimmat_up = all_stimmat_up;
    for i=1:NT
        all_shift_stimmat_up(used_inds(i),:) = shift_matrix_Nd(all_stimmat_up(used_inds(i),:),-round(it_fix_post_mean_cor(xv,i)),2);
    end
    all_Xmat_up_fixcor = create_time_embedding(all_shift_stimmat_up,stim_params_us);
    all_Xmat_up_fixcor = all_Xmat_up_fixcor(used_inds,:);
    ...
    it_xv_ref_mod{1}(xv) = xv_ref_mod{1}(xv);
    it_xv_ref_mod{1}(xv) = NMMfit_filters(it_xv_ref_mod{1}(xv),Robs_mat(cur_uset,cur_unit_ind),...
        cur_X,[],[],silent); %fit stimulus filters
    
    it_xv_ref_LL_imp(1,xv) = (it_xv_ref_mod{1}(xv).LL_seq(end) - all_nullmod(tr_cell).LL_seq(end))/log(2);
    
    fprintf('Original: %.4f  Prev: %.4f  New: %.4f\n',all_mod_LLimp(tr_cell),xv_ref_LL_imp(1,xv),it_xv_ref_LL_imp(1,xv));

     it_xv_ref_mod_spkNL{1}(xv) = NMMfit_logexp_spkNL(it_xv_ref_mod{1}(xv),Robs_mat(cur_uset,cur_unit_ind),cur_X);
...
for xv = 1:length(poss_sus)
    ...
    for ii = 1:NT
        %     resh_X_sh(:,:,ii) = shift_matrix_Nd(resh_X(:,:,ii), -shift_cor(xv,ii),2);
        resh_X_sh(:,:,ii) = shift_matrix_Nd(resh_X(:,:,ii), -round(post_mean(xv,ii)),2);
    end
    ...
    xv_ref_mod{2}(xv) = it_xv_ref_mod{1}(xv);
    xv_ref_mod{2}(xv) = NMMfit_filters(xv_ref_mod{2}(xv),Robs_mat(cur_uset,cur_unit_ind),...
        cur_X,[],[],silent); %fit stimulus filters
    
    xv_ref_LL_imp(2,xv) = (xv_ref_mod{2}(xv).LL_seq(end) - all_nullmod(tr_cell).LL_seq(end))/log(2);
    
    fprintf('Original: %.4f  Prev: %.4f  New: %.4f\n',all_mod_LLimp(tr_cell),it_xv_ref_LL_imp(1,xv),xv_ref_LL_imp(2,xv));
    xv_ref_mod_spkNL{2}(xv) = NMMfit_logexp_spkNL(xv_ref_mod{2}(xv),Robs_mat(cur_uset,cur_unit_ind),cur_X);
end
...
%back-project saccade-times
fpost_mean_cor = fpost_mean;
%align jumps to saccade times
for i = length(saccade_start_inds):-1:1
    cur_inds = saccade_start_inds(i):(saccade_start_inds(i)+sac_shift);
    cur_inds(cur_inds > NT) = [];
    fpost_mean_cor(cur_inds) = fpost_mean(cur_inds(end));
    cur_back_inds = (saccade_start_inds(i)-(flen-sac_shift)):(saccade_start_inds(i)-1);
    fpost_mean_cor(cur_back_inds) = fpost_mean(saccade_start_inds(i));
end
...
cd(anal_dir);
save(anal_name,'fpost_*','fshift*','xv_*','post_*','ref_mod*','tr_set','poss_*','all_mod*','fix_*','ref_LL*','fix_*','it_*');
...
fix_LL_imp = it_ref_LL_imp(1,tr_set)./all_mod_LLimp(tr_set);
full_LL_imp = ref_LL_imp(2,tr_set)./all_mod_LLimp(tr_set);
fix_xvLL_imp = it_xv_ref_LL_imp./all_mod_LLimp(tr_set(poss_sus));
full_xvLL_imp = xv_ref_LL_imp(2,:)./all_mod_LLimp(tr_set(poss_sus));

close all
subplot(2,1,1)
plot(all_mod_LLimp(tr_set),it_ref_LL_imp(1,tr_set),'.');
hold on
plot(all_mod_LLimp(tr_set(poss_sus)),it_xv_ref_LL_imp,'ro','markersize',4);
xlim([0 0.5]); ylim([0 0.5]);
line([0 0.5],[0 0.5],'color','k');
xlabel('pre LL (bits/spk)','fontsize',10)
ylabel('Post LL (bits/spk)','fontsize',10)
box off
title('Fixation corrections','fontsize',10);
legend('All cells','Leave-one-out','Location','SouthEast');
subplot(2,1,2)
plot(it_ref_LL_imp(1,tr_set),ref_LL_imp(2,tr_set),'.');
hold on
plot(it_xv_ref_LL_imp,xv_ref_LL_imp(2,:),'ro','markersize',4);
xlim([0 0.5]); ylim([0 0.5]);
line([0 0.5],[0 0.5],'color','k');
xlabel('pre LL (bits/spk)','fontsize',10)
ylabel('Post LL (bits/spk)','fontsize',10)
box off
title('Drift corrections','fontsize',10);
legend('All cells','Leave-one-out','Location','SouthEast');
fillPage(gcf,'papersize',[4 6]);

%%
fix_corr_mean = it_all_fix_post_mean_cor;
fix_corr_std = it_all_fix_post_std; %will want to use corrected versions of the uncertainty too!
full_corr_mean = it_all_fix_post_mean_cor + fpost_mean_cor2;
full_corr_std = sqrt(it_all_fix_post_std.^2 + fpost_std.^2);
...
inferred_pos = fpost_mean_cor + it_all_fix_post_mean_cor;
% inferred_pos = it_all_fix_post_mean_cor;
...
sac_delta_corrL =  corr(measured_delta_posL(:),inferred_delta_pos(:),'type','spearman');
sac_delta_corrR =  corr(measured_delta_posR(:),inferred_delta_pos(:),'type','spearman');
sac_delta_corrA =  corr(measured_delta_posA(:),inferred_delta_pos(:),'type','spearman');

measured_delta_corrLR = corr(measured_delta_posL(:),measured_delta_posR(:),'type','spearman');

%PLOT AGREEMENT BETWEEN MEAUSRED SAC AMPS



%%

inferred_seq = (it_all_fix_post_mean_cor+fpost_mean_cor)*sp_dx;
if bar_ori == 0
...
inferred_drift = nan(size(fpost_mean));
inferred_fix = nan(size(fpost_mean));
measured_driftL = nan(size(fpost_mean));
measured_driftR = nan(size(fpost_mean));
measured_fixL = nan(size(fpost_mean));
measured_fixR = nan(size(fpost_mean));
inferred_driftV = nan(size(fpost_mean));
inferred_fix_avg = nan(n_fixs,1);
measured_fix_avgL = nan(n_fixs,1);
measured_fix_avgR = nan(n_fixs,1);
for ii = 1:n_fixs
    cur_inds = cjump_inds(ii):(cjump_inds(ii+1)-1);
    cur_inf = inferred_seq(cur_inds) - median(inferred_seq(cur_inds));
    inferred_fix(cur_inds) = median(inferred_seq(cur_inds));
    ...
    inferred_fix_avg(ii) = mean(inferred_seq(cur_inds));
    measured_fix_avgL(ii) = mean(measured_seqL(cur_inds));
    measured_fix_avgR(ii) = mean(measured_seqR(cur_inds));
    
    inferred_driftV(cur_inds(2:end)) = diff(inferred_seq(cur_inds))/dt;
end
  
drift_corrL = corr(inferred_drift(:),measured_driftL(:),'type','spearman');
drift_corrR = corr(inferred_drift(:),measured_driftR(:),'type','spearman');
drift_meas_corrLR = corr(measured_driftL(:),measured_driftR(:),'type','spearman');

%%
close all
x1_fix = linspace(-0.5,0.5,250);
x1_drift = linspace(-0.1,0.1,250);

[f1] = ksdensity(inferred_fix,x1_fix,'width',0.02);
[f2] = ksdensity(measured_fixL,x1_fix,'width',0.02);
[f3] = ksdensity(inferred_drift,x1_drift,'width',0.0025);
[f4] = ksdensity(measured_driftL,x1_drift,'width',0.0025);

subplot(2,1,1)
plot(x1_fix,f1,x1_fix,f2,'r','linewidth',1.5)
legend('Inferred','Measured');
xlim([-0.5 0.5]);
set(gca,'xtick',-0.5:0.25:0.5);
xlabel('Position (deg)','fontsize',10);
ylabel('Probability density','fontsize',10);
title('Fixation','fontsize',10);
set(gca,'fontsize',8,'fontname','arial');
box off

subplot(2,1,2)
plot(x1_drift,f3,x1_drift,f4,'r','linewidth',1.5)
legend('Inferred','Measured');
xlim([-0.1 0.1]);
xlabel('Position (deg)','fontsize',10);
ylabel('Probability density','fontsize',10);
title('Drift','fontsize',10);
set(gca,'fontsize',8,'fontname','arial');
box off
fillPage(gcf,'papersize',[4 6]);
...
% %%
% uu = 88;
% target_unit = tr_set(uu);
% init_optim_p.optTol = 1e-5; init_optim_p.progTol = 1e-8; init_optim.p.maxIter = 50;
% 
% cur_unit_ind = find(tr_set == target_unit);
% cur_uset = find(~isnan(Robs_mat(:,cur_unit_ind)));
% 
% cur_X{1} = X_sh(cur_uset,use_kInds_up);
% cur_X{2} = Xblock(used_inds(cur_uset),:);
...
% cur_X{4} = Xmsac(cur_uset,:);
% 
% new_mod = ref_mod{end}(target_unit);
% new_mod = NMMadd_NLinput(new_mod,'quad',-1,1);
% new_mod.mods(end).reg_params.lambda_L1 = 0;
% new_mod = NMMfit_filters(new_mod,Robs_mat(cur_uset,cur_unit_ind),...
%     cur_X,[],[],0,init_optim_p); %fit stimulus filters
% [LL, penLL, pred_rate, G, gint] = NMMmodel_eval(new_mod,Robs_mat(cur_uset,cur_unit_ind),cur_X);
% new_mod = NMMadjust_regularization(new_mod,length(new_mod.mods),'lambda_L1',base_lambda_L1./std(gint)');
% new_mod = NMMfit_filters(new_mod,Robs_mat(cur_uset,cur_unit_ind),...
%     cur_X,[],[],0); %fit stimulus filters
% 
