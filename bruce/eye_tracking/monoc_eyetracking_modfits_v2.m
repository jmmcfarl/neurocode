...
anal_name = 'monoc_eyecorr_hbar_mods2';
...
for ss = 1:length(su_probes)
    ...
    if ~isempty(cur_used_inds)
        ...
        fit0 = NMMinitialize_model(nmm_stim_params,mod_signs,NL_types,[],Xtargets,init_filts); %initialize NIM
        ...
        fit0 = NMMadjust_regularization(fit0,find(Xtargets==1),'lambda_d2XT',base_lambda_d2XT./var(gint)');
        fit0 = NMMadjust_regularization(fit0,find(Xtargets==1),'lambda_L1',base_lambda_L1./std(gint)');
...
save(anal_name,'sua_data','mua_data','modfit_used_blocks','base_*');
