...
stpt = round(0*Fsd);
n_wins = 7;
ept = stpt + round(Fsd*0.5*n_wins);
windur = round(Fsd*0.5);
avg_trig_spec_reshape = permute(avg_trig_spec_cond,[2 1 3]);
avg_trig_spec_reshape = reshape(avg_trig_spec_reshape(stpt+1:ept,:,:),[windur n_wins length(C) length(wfreqs)]);
...
avg_trig_sig_reshape = reshape(avg_trig_sig_reshape(stpt+1:ept,:),[windur n_wins length(C)]);
avg_trig_sig_win = squeeze(mean(avg_trig_sig_reshape,2));

sem_trig_sig_reshape = permute(trig_sig_cond,[3 1 2]);
sem_trig_sig_reshape = reshape(sem_trig_sig_reshape(stpt+1:ept,:),[windur n_wins length(use_lfps) length(C)]);
