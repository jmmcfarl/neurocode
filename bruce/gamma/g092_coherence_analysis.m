...
for i = 2:length(use_lfps)
    for j = 1:(i-1)
        pair_ind(cnt,1) = use_lfps(i);
        pair_ind(cnt,2) = use_lfps(j);
        cnt = cnt + 1;
    end
end
pair_x_sep = abs(interp_x(pair_ind(:,1)) - interp_x(pair_ind(:,2)));
pair_y_sep = abs(interp_y(pair_ind(:,1)) - interp_y(pair_ind(:,2)));
tot_sep = sqrt(pair_x_sep.^2 + pair_y_sep.^2);
[~,xord] = sort(pair_x_sep);
[~,yord] = sort(pair_y_sep);
[~,tord] = sort(tot_sep);
%%
start_buffer = round(0.15*Fsd);
params.Fs = Fsd;
params.tapers = [4 7];
params.fpass = [0 130];
movingwin = [1.75 1.75];
...
for cc = 1:length(C)
    ...
    [Cmn,Phimn,Smn,Smm,f] = coherencyc_unequal_length_trials(all_V, movingwin, params, sMarkers(cur_trials,:) );
...
for cc = 2:length(C)
    C(cc,:)
    subplot(2,1,1)
    plot(f,squeeze(mean(all_Cmn(1,:,:),3)),'k')
    hold on
    plot(f,squeeze(mean(all_Cmn(cc,:,:),3)));
    subplot(2,1,2)
    plot(f,squeeze(mean(log(all_Smm(1,:,:)),3)),'k')
    hold on
    plot(f,squeeze(mean(log(all_Smm(cc,:,:)),3)));
    pause
    clf
end
...
cd('gamma');
save cond_trig_avg_gamma use_lfps wfreqs lags Fsd cond_* C ov_*
