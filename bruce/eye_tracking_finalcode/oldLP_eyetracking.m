...
max_Dshift = round(5*spatial_usfac);
...
init_stim_params = NMMcreate_stim_params([flen n_bar_pos],dt);
init_stim_params(2) = NMMcreate_stim_params([n_blocks 1],dt);

fin_stim_params(1) = NMMcreate_stim_params([flen use_nPix_tb],dt);
...
n_stim_filts = n_squared_filts+1;
...
if ~exist(['./' mod_data_name '.mat'],'file') || recompute_init_mods == 1
    ...
    for ss = 1:n_probes;
        ...
        if ~isempty(cur_tr_inds)
            ...
            tr_X{1} = all_Xmat(used_inds(cur_tr_inds),:);
            tr_X{2} = Xblock(used_inds(cur_tr_inds),:);
            xv_X{1} = all_Xmat(used_inds(cur_xv_inds),:);
            ...
            tr_X{1} = all_Xmat_tb(used_inds(cur_tr_inds),:);
            xv_X{1} = all_Xmat_tb(used_inds(cur_xv_inds),:);
            %spatial up-sampling of filter estimates
            base_filts = reshape([gqm1.mods(find(init_Xtargs == 1)).filtK],[flen n_bar_pos n_squared_filts+1]);
            if spatial_usfac/spatial_tbspace == 2
                base_filts_up = zeros(flen,use_nPix_tb,n_squared_filts+1);
                for ii = 1:n_bar_pos
                    base_filts_up(:,2*(ii-1)+1,:) = 0.5*base_filts(:,ii,:);
                    base_filts_up(:,2*(ii-1)+2,:) = 0.5*base_filts(:,ii,:);
                end
            elseif spatial_usfac/spatial_tbspace == 1
                base_filts_up = base_filts;
            elseif spatial_usfac/spatial_tbspace == 4
                base_filts_up = zeros(flen,use_nPix_tb,n_squared_filts+1);
                for ii = 1:n_bar_pos
                    base_filts_up(:,4*(ii-1)+(1:4),:) = repmat(0.25*base_filts(:,ii,:),[1 4 1]);
                end
            else
                error('unsupported')
            end
            base_filts_up = reshape(base_filts_up,use_nPix_tb*flen,n_squared_filts+1);
            ...
            fprintf('Unit %d LLimp: %.4f\n',ss,all_mod_xvLLimp(ss));
    ...
    for ss = 1:length(su_probes);
       ...
       cur_tr_inds = tr_inds(~isnan(all_binned_sua(used_inds(tr_inds),ss)));
        ...
        elseif spatial_usfac/spatial_tbspace == 4
            ...
            fprintf('Unit %d LLimp: %.4f\n',ss+n_probes,all_mod_xvLLimp(ss+n_probes));
...
for nn = 1:n_fix_inf_it
    ...
    filt_bank = zeros(n_tr_chs,klen_us,n_stim_filts);
    ...
    for ss = 1:n_tr_chs
        ...
        if spatial_tbspace > 1
            cur_k = reshape([it_mods{nn}(tr_set(ss)).mods(cur_Xtargs == 1).filtK],[flen use_nPix_tb n_stim_filts]);
            cur_k = cur_k*tent_basis;
            cur_k = reshape(cur_k,[klen_us n_stim_filts]);
        else
    ...
    all_shift_stimmat_up = all_bar_mat_up;
    for i=1:NT
        all_shift_stimmat_up(used_inds(i),:) = all_bar_mat_up(used_inds(i),:)*Tshift_mat{all_fix_post_mean_cor(i)};
    end
    if spatial_tbspace > 1
       all_shift_stimmat_up = all_shift_stimmat_up*tent_basis';
    end
    all_Xmat_up_fixcor = create_time_embedding(all_shift_stimmat_up,stim_params_tb);
    all_Xmat_up_fixcor = all_Xmat_up_fixcor(used_inds,:);
    
    %% REFIT ALL CELLS
    cur_X{1} = all_Xmat_up_fixcor;
    cur_X{2} = Xblock(used_inds,:);
    if use_sac_kerns
        cur_X{3} = Xsac;
        cur_X{4} = Xmsac;
    ...
    if use_LOOXV == 1
        for xv = 1:length(su_inds)
            ...
            for ss = 1:length(cur_uset);
                ...
                if spatial_tbspace > 1
                    cur_k = reshape([it_mods_LOO{xv,nn}(tr_set(ss)).mods(cur_Xtargs == 1).filtK],[flen use_nPix_tb n_stim_filts]);
                    cur_k = cur_k*tent_basis;
                    cur_k = reshape(cur_k,[klen_us n_stim_filts]);
                else
                    cur_k = [it_mods_LOO{xv,nn}(tr_set(ss)).mods(cur_Xtargs == 1).filtK];
                end
                n_used_filts = size(cur_k,2);
                filt_bank(ss,:,1:n_used_filts) = cur_k;
...
%     if use_LOOXV
        ...
        all_Xmat_up_fixcor = all_Xmat_up_fixcor(used_inds,:);
%     else
%         all_Xmat_up_fixcor = create_time_embedding(all_bar_mat_up,stim_params_us);
%         all_Xmat_up_fixcor = all_Xmat_up_fixcor(used_inds,:);
%     end
    ...
    for ss = 1:n_tr_chs
        ...
        if spatial_tbspace > 1
            cur_k = reshape([dit_mods{nn}(tr_set(ss)).mods(cur_Xtargs == 1).filtK],[flen use_nPix_tb n_stim_filts]);
            cur_k = cur_k*tent_basis;
            cur_k = reshape(cur_k,[klen_us n_stim_filts]);
        else
    ...
    %RECOMPUTE XMAT
            all_shift_stimmat_up = all_bar_mat_up;
    for i=1:NT
        all_shift_stimmat_up(used_inds(i),:) = all_bar_mat_up(used_inds(i),:)*Tshift_mat{all_post_cor(i)};
    end
    if spatial_tbspace > 1
        all_shift_stimmat_up = all_shift_stimmat_up*tent_basis';
    end
    cur_X{1} = create_time_embedding(all_shift_stimmat_up,stim_params_tb);
    cur_X{1} = cur_X{1}(used_inds,:);
    ...
    if use_LOOXV == 1
        for xv = 1:length(su_inds)
            ...
            for ss = 1:n_uset
                ...
                if spatial_tbspace > 1
                    cur_k = reshape([dit_mods_LOO{xv,nn}(tr_set(cur_uset(ss))).mods(cur_Xtargs == 1).filtK],[flen use_nPix_tb n_stim_filts]);
                    cur_k = cur_k*tent_basis;
                    cur_k = reshape(cur_k,[klen_us n_stim_filts]);
                else
...
et_params = struct('beg_buffer',beg_buffer,'end_buffer',end_buffer,'min_trial_dur',min_trial_dur,'bar_ori',bar_ori,...
    'n_bar_pos',n_bar_pos,'flen',flen,'dt',dt,'drift_jump_sigma',drift_jump_sigma,'drift_prior_sigma',drift_prior_sigma,...
...
%% COMPUTE RELATIVE LOOXV LL improvements for SUs
...
%     fin_mod = it_mods_LOO{su_ind,end}(tr_set(ss));
    ...
    fprintf('Cell %d of %d\n',ss,length(tr_set));
    fprintf('Original: %.4f  Fin: %.4f\n',all_mod_LLimp(tr_set(ss)),dit_LLimp_LOO(su_ind,end,tr_set(ss)));
