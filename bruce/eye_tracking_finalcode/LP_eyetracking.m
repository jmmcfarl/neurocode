...
drift_noise_sigma = 0.0005;
drift_prior_sigma = 0.006; %0.004
...
et_sp_dx = sp_dx;
cd(anal_dir);
save(anal_name,'it_*','drift_post_*','fix_ids','dit_*','et_used_inds','et_tr_set','et_saccade_inds','et_params','et_tr_trials','et_xv_trials','et_sp_dx');
...
fin_drift_std = interp1(find(~isnan(fix_ids)),fin_drift_corr(~isnan(fix_ids)),1:NT);
...
u = find(~isnan(fin_tot_corr));
[tot_corrs,tot_pvals] = corr([measured_seqL(u) measured_seqR(u)],fin_tot_corr(u)','type','spearman');
