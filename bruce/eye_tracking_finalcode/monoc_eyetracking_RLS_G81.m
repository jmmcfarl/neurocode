...
for ee = 1:n_blocks;
    ...
    for tt = 1:n_trials
        cur_stim_Op = Expts{cur_block}.Trials(use_trials(tt)).Op;
        cur_stim_ph = Expts{cur_block}.Trials(use_trials(tt)).ph;
        cur_stim_times = Expts{cur_block}.Trials(use_trials(tt)).Start/1e4;
                cur_t_edges = [cur_stim_times; cur_stim_times(end) + dt*Fr];
        cur_t_axis = 0.5*cur_t_edges(1:end-1) + 0.5*cur_t_edges(2:end);
        n_frames = length(cur_stim_Op);
        ...
        if n_frames > min_trial_dur/dt
            ...
            all_stim_pos = [all_stim_pos; cur_stim_Op(:)];
...
%%
unique_pos = unique(all_stim_pos);
all_stim_sign = ones(size(all_t_axis));
all_stim_sign(all_stim_ph == 0) = -1;
all_stim_mat = zeros(length(all_t_axis),use_nPix);
for ii = 1:use_nPix
    cur_set = all_stim_pos == unique_pos(ii);
   all_stim_mat(cur_set,ii) = all_stim_sign(cur_set); 
end
%%
