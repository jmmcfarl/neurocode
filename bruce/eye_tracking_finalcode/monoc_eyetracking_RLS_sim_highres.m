...
orig_mod_name = 'monoc_eyecorr_hbar';
mod_data_name = 'monoc_eyecorr_hbar_sim_mods';
old_anal_name = 'monoc_eyecorr_hbar_sim';
anal_name = 'monoc_eyecorr_hbar_sim_highres';
hrmod_data_name = 'monoc_eyecorr_hbar_sim_mods_hres2';
true_moddata_name = 'monoc_eyecorr_hbar_mods';
true_data_name = 'monoc_eyecorr_hbar_highres2';
...
sim_stim_params = NMMcreate_stim_params([flen sim_nPix_us],dt);
...
all_Xmat_true = all_Xmat_true(used_inds,use_kInds_sim);

%%
load(true_data_name,'dit_mods','et_tr_set');
true_mods = dit_mods{2};
tr_set = et_tr_set;
n_tr_chs = length(tr_set);

load(true_moddata_name,'all_mod_SU*','all_nullmod');
...
%% LOAD IN BASE ESTIMATION OF EYE POSITION
load(old_anal_name,'it_fix*','drift_post_mean','dit_mods');
...
for ss = 1:n_tr_chs
    ...
    if ~isempty(cur_tr_inds) && nansum(Robs) > 0
        ...
        base_filts = reshape([old_best_mods(ss).mods(find(init_Xtargs == 1)).filtK],[flen use_nPix*2 n_squared_filts+1]);
        ...
        all_mod_fits(tr_set(ss)).spk_NL_params(1) = old_best_mods(ss).spk_NL_params(1);
...
cd(anal_dir);
save(anal_name,'sim_eyepos*','it_*','drift_post_*','fix_ids','dit_*','et_used_inds','et_tr_set','et_saccade_inds','et_params','et_tr_trials','et_xv_trials');
...
err_m = fin_tot_corr - sim_eyepos(used_inds)';
err_z = err_m./fin_tot_std;
...
for tt = 1:n_trials
    ...
    if ~isempty(uu)
        ...
        if dur > 3.5
            ...
            plot(all_t_axis(used_inds(uu)) - bt,sim_eyepos(used_inds(uu)),'b','linewidth',2)
            ...
            subplot(2,1,2)
            plot(all_t_axis(used_inds(uu))-bt,err_z(uu),'k','linewidth',2);
            xlim([0 dur]);
...
%%
n_bins = 100;
err_mag_b = linspace(0,0.025,n_bins);
...
unc_cdist = histc(fin_tot_std(fix_inds),err_mag_b);
unc_cdist = unc_cdist/max(unc_cdist);
% unc_cdist = cumsum(unc_cdist)/sum(~isnan(fin_tot_std(fix_inds)));

temp = randn(length(fin_tot_std),1).*fin_tot_std';
...
legend('Error','Normal','Uncertainty');
...
fig_width = 4;
rel_height = 1.6;
fname = ['/home/james/Desktop/' 'sim_error_analysis.pdf'];
