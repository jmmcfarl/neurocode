...
if bar_ori == 0
    ...
    anal_name = 'eyecorr_hbar';
else
    mod_data_name = 'eyecorr_vbar_mods';
    anal_name = 'eyecorr_vbar';
end
% mod_data_name = strcat(use_eye,'_',mod_data_name);
mod_data_name = strcat(use_eye,'_',mod_data_name,'_full');
anal_name = strcat(use_eye,'_',anal_name,'_full2');
...
max_shift = round(14*spatial_usfac);
dshift = 1;
...
init_stim_params = NMMcreate_stim_params([flen use_nPix*2],dt);
...
if ~exist(['./' mod_data_name '.mat'],'file') || recompute_init_mods == 1
    ...
    for ss = 1:n_probes;
        ...
        if ~isempty(cur_tr_inds)
            ...
            base_filts = reshape([gqm1.mods(find(init_Xtargs == 1)).filtK],[flen 2*use_nPix n_squared_filts+1]);
            if spatial_usfac == 2
                base_filts_up = zeros(flen,2*use_nPix_us,n_squared_filts+1);
                for ii = 1:use_nPix*2
            ...
            gqm2 = NMMadjust_regularization(gqm2,find(init_Xtargs==1),'lambda_d2XT',base_lambda_custom./var(gint)');
            ...
            gqm2 = NMMfit_filters(gqm2,Robs,tr_X,[],[],silent,[],L2_mat);
...
for xx = 1:n_shifts
    tempF = spalloc(2*full_nPix_us,2*full_nPix_us,2*full_nPix_us);
    temp = spdiags( ones(full_nPix_us,1), -shifts(xx), full_nPix_us, full_nPix_us);
    tempF(1:full_nPix_us,1:full_nPix_us) = temp;
    tempF(full_nPix_us+1:end,full_nPix_us+1:end) = temp;
    temp = kron(tempF,It);
...
for xx = 1:n_Tshifts
    tempF = spalloc(2*full_nPix_us,2*full_nPix_us,2*full_nPix_us);
    temp = spdiags( ones(full_nPix_us,1), -Tshifts(xx), full_nPix_us, full_nPix_us);
    tempF(1:full_nPix_us,1:full_nPix_us) = temp;
    tempF(full_nPix_us+1:end,full_nPix_us+1:end) = temp;    
    Tshift_mat{xx} = tempF;
end
...
%%
usable_inds = 1:length(used_inds);
...
for ss = sbeg:length(tr_set)
    ...
    figure(f1); clf
    subplot(2,2,1)
    imagesc(reshape(kmat(:,1),flen,2*use_nPix_us));
    ca = max(abs(kmat(:,1))); caxis([-ca ca]);
    for ii = 1:(size(kmat,2)-1)
        subplot(2,2,2+ii)
        imagesc(reshape(kmat(:,ii+1),flen,2*use_nPix_us));
...
close all
Nframe_test = 5e3;
dds = expt_dds(cur_block_set(1))/100;
poss_dxs = -10:10;
% cc = 46;
...
for xx = 1:length(poss_dxs);
  xx  
    ...
    left_stim = rand_stimmat_up;
    right_stim = shift_matrix_Nd(left_stim,poss_dxs(xx),2);
    sim_Xmat = create_time_embedding([left_stim right_stim],stim_params_us);
    sim_Xmat = sim_Xmat(:,use_kInds_up);
    
    for cc = 1:length(tr_set)
        
        cur_mod = it_mods{1}(tr_set(cc));
        cur_modr = dit_mods{end}(tr_set(cc));
        mod_offset = mean(cur_mod(:).mods(4).filtK) + cur_mod.spk_NL_params(1);
        mod_offsetr = mean(cur_modr(:).mods(4).filtK) + cur_modr.spk_NL_params(1);
        cur_stim_filts = [cur_mod(:).mods(1:3).filtK];
        cur_stim_filtsr = [cur_modr(:).mods(1:3).filtK];
        
        filt_outs = sim_Xmat*cur_stim_filts;
        filt_outs(:,2:3) = filt_outs(:,2:3).^2;
        
        mod_out = log(1+exp(sum(filt_outs,2) + mod_offset));
        sim_disp_tune(cc,xx) = mean(mod_out);
        
        filt_outs = sim_Xmat*cur_stim_filtsr;
        filt_outs(:,2:3) = filt_outs(:,2:3).^2;
        mod_out = log(1+exp(sum(filt_outs,2) + mod_offsetr));
        sim_disp_tuner(cc,xx) = mean(mod_out);
    end
end
rel_disp_tuner = bsxfun(@rdivide,sim_disp_tuner,mean(sim_disp_tuner,2));
rel_disp_tune = bsxfun(@rdivide,sim_disp_tune,mean(sim_disp_tune,2));
% figure
% plot(poss_dxs*sp_dx,sim_disp_tune/dt);
% hold on
% plot(poss_dxs*sp_dx,sim_disp_tuner/dt,'r');

%%
close all
for cc = 97:length(tr_set)
    
    NMMdisplay_model(dit_mods{end}(tr_set(cc)),[],[],1)
    
    figure
    plot(poss_dxs*sp_dx,rel_disp_tune(cc,:))
    hold on
    plot(poss_dxs*sp_dx,rel_disp_tuner(cc,:),'r')
    pause
    close all
end
...
left_stim = all_shift_stimmat_up(:,1:full_nPix_us);
right_stim = all_shift_stimmat_up(:,(full_nPix_us+1):end);

buffer_pix = floor((full_nPix - use_nPix)/2);
%repeat for up-sampled versions of the Xmatrix
[Xinds_up,~] = meshgrid(1/spatial_usfac:1/spatial_usfac:full_nPix,1);
cur_use_pix = (1/spatial_usfac:1/spatial_usfac:use_nPix) + buffer_pix;
use_kInds_up = find(ismember(Xinds_up(:),cur_use_pix));

left_stim = left_stim(:,use_kInds_up);
right_stim = right_stim(:,use_kInds_up);

%%
for bb = 1:n_blocks
    cur_set = find(all_blockvec == bb);
    left_stim(cur_set,:) = left_stim(cur_set,:)/std(reshape(left_stim(cur_set,:),[],1));
    right_stim(cur_set,:) = right_stim(cur_set,:)/std(reshape(right_stim(cur_set,:),[],1));
end    
%%
%%
% clear all
...
cc = 10; %SUs are cc > 96 (99 is best)
...
GQM = NMMfit_filters(GQM,cur_Robs,cur_Xmat,[],[],0,[],L2_mat_us);

base_lambda_custom = 25;
base_lambda_L1 = 4;
[LL, penLL, pred_rate, G, gint] = NMMmodel_eval(GQM,cur_Robs,cur_Xmat);
GQM = NMMadjust_regularization(GQM,[],'lambda_d2XT',base_lambda_custom./var(gint)');
...
Nframe_test = 5e3; %about 5000 RLS frames seems to be sufficient
...
for xx = 1:length(poss_dxs)
    ...
    left_stim = rand_stimmat_up;
    right_stim = shift_matrix_Nd(left_stim,poss_dxs(xx),2);
    left_X = create_time_embedding(left_stim,NMMcreate_stim_params([flen full_nPix_us]));
    right_X = create_time_embedding(right_stim,NMMcreate_stim_params([flen full_nPix_us]));
    sim_Xmat = [left_X(:,use_kInds_up) right_X(:,use_kInds_up)];
    
    %compute average firing rate output predicted by the model
    [~,~,mod_out] = NMMmodel_eval(GQM,[],sim_Xmat);
