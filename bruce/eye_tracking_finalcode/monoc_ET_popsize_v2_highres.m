...
%         hrmod_data_name =sprintf('popsize_v2_%d_it%d_hres_mods',Nunits,rr);
        
        load(cur_data_name,'dit_mods','tr_set','it_fix_post_mean','drift_post_mean','it_fix_post_std');
...
%         post_mean_drift(isnan(post_mean_drift)) = 0;
        ...
        for ss = 1:n_tr_chs
            ...
            if ~isempty(cur_tr_inds) && nansum(Robs) > 0
                ...
                all_mod_fits(tr_set(ss)).mods(4).filtK = old_best_mods(tr_set(ss)).mods(4).filtK;
                all_mod_fits(tr_set(ss)).mods(5).filtK = old_best_mods(tr_set(ss)).mods(5).filtK;
        ...
        cur_hres_data_name = sprintf('popsize_v2_%d_it%d_highres',Nunits,rr);
        save(cur_hres_data_name,'fin_tot_corr','fin_tot_std');
