...
%        anal_name = 'monoc_eyecorr_hbar.mat';
       ...
       if exist(anal_name,'file') && use_ori(1)
           hor_data = load(anal_name,'it_mods*','it_R2','dit_mods*','dit_R2_LOO','it_R2_LOO','dit_R2','et_tr_set');
...
%        anal_name = 'monoc_eyecorr_vbar.mat';
       ...
       if exist(anal_name,'file') && use_ori(2)
           ver_data = load(anal_name,'it_mods*','it_R2','dit_mods*','dit_R2_LOO','it_R2_LOO','dit_R2','et_tr_set');
       ...
       for ii = 1:length(tot_use_sus)
           ...
           if cur_hor_imp > cur_ver_imp
            cur_hor_R2fract = hor_data.it_R2(1,hor_su_inds(cur_hor_loc))/sum(hor_data.it_R2(1,:));
              cur_su_data(ii).usable = true;
               ...
               cur_su_data(ii).before_mod = hor_data.it_mods_spkNL{1}(hor_su_inds(cur_hor_loc));
               cur_su_data(ii).after_mod = hor_data.dit_mods_spkNL_LOO{cur_hor_loc,end}(hor_su_inds(cur_hor_loc));
           ...
           elseif cur_ver_imp > cur_hor_imp
             ...
             cur_su_data(ii).usable = true;
               ...
               cur_su_data(ii).before_mod = ver_data.it_mods_spkNL{1}(ver_su_inds(cur_ver_loc));
               cur_su_data(ii).after_mod = ver_data.dit_mods_spkNL_LOO{cur_ver_loc,end}(ver_su_inds(cur_ver_loc));
...
%         anal_name = 'monoc_eyecorr.mat';
      if exist(anal_name,'file')
           load(anal_name,'it_mods*','it_R2','dit_mods*','dit_R2_LOO','it_R2_LOO','dit_R2','et_tr_set');
       ...
       for ii = 1:length(su_numbers)
          ...
          if ~isempty(cur_loc);
               ...
               cur_su_data(ii).usable = true;
               cur_su_data(ii).ori = 0;
               cur_su_data(ii).before_mod = it_mods_spkNL{1}(su_inds(cur_loc));
               cur_su_data(ii).after_mod = dit_mods_spkNL_LOO{cur_loc,end}(su_inds(cur_loc));
...
%% compute filter properties before and after
close all
...
%% COMPUTE spatial phase sensitivity and weighted avg filter props
...
%up-sample bars
    ...
    for ii = 1:length(su_set)
        ...
        %compute output of uncorrected model
        uncor_filters = [all_su_data(su_set(ii)).before_mod.mods(Xtargs==1).filtK];
        offset = all_su_data(su_set(ii)).before_mod.spk_NL_params(1);
        beta = all_su_data(su_set(ii)).before_mod.spk_NL_params(2);
        alpha = all_su_data(su_set(ii)).before_mod.spk_NL_params(3);
        
        offset = offset + mean(all_su_data(su_set(ii)).before_mod.mods(4).filtK);
        ...
        %compute model output rate for negative of stimulus
        filt_outr = -test_stim*uncor_filters;
        filt_outr(:,2:end) = filt_outr(:,2:end).^2;
        rate_out_rev = alpha*log(1+exp(beta*(sum(filt_outr,2) + offset)));
        
        %phase-reversal modulation for uncorrected model
        prm_before(su_set(ii)) = mean(abs(rate_out_rev - rate_out))/mean(rate_out);
...
%% COMPUTE DATA FOR BEFORE-AFTER COMPARISONS
thresh_R2 = 0;
...
%% CREATE PRM BEFORE-AFTER FIGURE
mSize = 12;
b = robustfit(prm_before(used_sus(jbeUnits)),prm_after(used_sus(jbeUnits)),[],[],'off');
h5 = figure(); hold on
plot(prm_before(used_sus(jbeUnits)),prm_after(used_sus(jbeUnits)),'k.','markersize',mSize);
maxlim = max(max(xlim(),ylim()));
xlim([0 maxlim]); ylim([0 maxlim]);
xl = xlim(); xx = linspace(xl(1),xl(2),100);
plot(xx,b*xx,'k--');
line([0 maxlim],[0 maxlim],'color','k','linestyle','--');
xlabel('PRM before'); ylabel('PRM after');
set(gca,'xtick',[0:0.2:xl(2)],'ytick',[0:0.2:xl(2)]);
...
figufy(h5);
fname = [fig_dir 'PRM_before_after4.pdf'];
