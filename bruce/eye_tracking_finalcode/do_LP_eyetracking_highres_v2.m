function [] = do_LP_eyetracking_highres_v2(Expt_name,use_coils)
...
old_anal_name = 'monoc_eyecorr2';
anal_name = 'monoc_eyecorr_highres4';
hrmod_data_name = 'monoc_eyecorr_mods_hres2';
...
if any(use_coils > 0)
   anal_name = [anal_name '_Cprior'];
    hrmod_data_name = [hrmod_data_name '_Cprior'];
end
if use_measured_pos == 1
    mod_data_name = [mod_data_name '_Cinit2'];
    hrmod_data_name = [hrmod_data_name '_Cinit2'];
...
fix_noise_sigma = 0.075; %0.1
% drift_noise_sigma = 0.006;
% drift_prior_sigma = 0.006; %0.003
drift_noise_sigma = 0.004;
drift_prior_sigma = 0.004; %0.003
...
if spatial_usfac > 1
    ...
    if length(use_kInds_up)/flen < use_nPix_us
        use_kInds_up = find(Xinds_up(:) >= cur_use_pix(1)-1/spatial_usfac & Xinds_up(:) <= cur_use_pix(end) + 2/spatial_usfac);
    end
    cnt = 2;
    while length(use_kInds_up)/flen < use_nPix_us
        ...
        if length(use_kInds_up)/flen < use_nPix_us
            use_kInds_up = find(Xinds_up(:) >= cur_use_pix(1)-cnt/spatial_usfac & Xinds_up(:) <= cur_use_pix(end) + (cnt+1)/spatial_usfac);
        end
        cnt = cnt + 1;
    end
else
...
%         end
    ...
    best_fix_std = it_fix_post_std(end,:);
%     dit_mods{1} = it_mods{end};
%     dit_mods_spkNL{1} = it_mods_spkNL{end};
%     dit_LLimp(1,:) = it_LLimp(end,:);
%     dit_R2(1,:) = it_R2(end,:);
%     dit_dev(1,:) = it_dev(end,:);
...
%             cur_lA(cur_lA < eps) = eps;
    ...
    uset = find(~isnan(drift_post_std(nn,:)));
    drift_post_mean(nn,:) = interp1(uset,drift_post_mean(nn,uset),1:NT);
    drift_post_std(nn,:) = interp1(uset,drift_post_std(nn,uset),1:NT);
