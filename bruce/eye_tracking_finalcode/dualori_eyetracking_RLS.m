...
for nn = 1:n_fix_inf_it
    ...
    for xx = 1:n_shifts
        ...
        cur_stim_shift = [all_ver_Xmat_us*shift_mat{SH_inds(xx,1)} all_hor_Xmat_us*shift_mat{SH_inds(xx,2)}];
...
%     fix_post_mean_ind(:,1) = sum(bsxfun(@times,gamma,SH_inds(:,1)'),2);
%     fix_post_mean_ind(:,2) = sum(bsxfun(@times,gamma,SH_inds(:,2)'),2);
...
%     %back-project saccade-times
%     all_fix_post_mean_inds = nan(NT,2);
%     all_fix_post_mean_inds(~isnan(fix_ids),:) = fix_post_mean_ind(fix_ids(~isnan(fix_ids)),:);
%     all_fix_post_mean_inds = round(interp1(find(~isnan(fix_ids)),all_fix_post_mean_inds(~isnan(fix_ids),:),1:NT));
%     
...
%%
measured_seqL = corrected_eye_vals_interp(used_inds,[1 2]);
measured_seqR = corrected_eye_vals_interp(used_inds,[3 4]);
% measured_seqL = smooth_eyepos(used_inds,[1 2]);
% measured_seqR = smooth_eyepos(used_inds,[3 4]);
...
measured_fix_avgL = nan(n_fixs,2);
measured_fix_avgR = nan(n_fixs,2);
for ii = 1:n_fixs
    cur_inds = fix_start_inds(ii):fix_stop_inds(ii);
    if length(cur_inds)*dt >= min_fix_dur
        ...
        measured_driftL(cur_inds,:) = bsxfun(@minus,measured_seqL(cur_inds,:),measured_fix_avgL(ii,:));
        measured_driftR(cur_inds,:) = bsxfun(@minus,measured_seqR(cur_inds,:),measured_fix_avgR(ii,:));
    end
end

u = find(all(~isnan(measured_driftL),2) & all(~isnan(inferred_drift),2));
[drift_corrs,drif_pvals] = corr([measured_driftL(u,:) measured_driftR(u,:)],inferred_drift(u,:),'type','spearman');
u = find(all(~isnan(measured_fix_avgL),2) & all(~isnan(inferred_fix_avg),2));
...
for tt = 1:n_trials
    ...
    if ~isempty(uu)
        ...
        if dur > 2
            ...
            h3=plot(all_t_axis(used_inds(uu))-bt,corrected_eye_vals_interp(used_inds(uu),plot_dim),'r','linewidth',2);
