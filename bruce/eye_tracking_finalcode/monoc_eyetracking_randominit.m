...
if bar_ori == 0
    mod_data_name = 'monoc_eyecorr_hbar_mods_randinit';
    anal_name = 'monoc_eyecorr_hbar_randinit2';
else
    mod_data_name = 'monoc_eyecorr_vbar_mods_randinit';
    anal_name = 'monoc_eyecorr_vbar_randinit2';
end
...
rand_fixs = randn(n_fixs,1)*rand_fix_std;

for ii = 1:n_fixs
    cur_inds = fix_start_inds(ii):fix_stop_inds(ii);
    init_eyepos(cur_inds) = rand_fixs(ii);
end
...
all_stimmat_up_init = all_stimmat_up;
all_stimmat_init = all_stim_mat;
for ii = 1:NT
    all_stimmat_up_init(ii,:) = shift_matrix_Nd(all_stimmat_up(ii,:),-init_eyepos_rnd(ii),2);
    all_stimmat_init(ii,:) = shift_matrix_Nd(all_stim_mat(ii,:),-init_eyepos_ds(ii),2);
end
all_Xmat_us = create_time_embedding(all_stimmat_up_init,stim_params_us);
all_Xmat = create_time_embedding(all_stimmat_init,stim_params);
...
%%
cd(anal_dir);
load(anal_name,'it_fix_*','drift_post*');
...
rinit_fin_tot_corr = fin_fix_corr + fin_drift_corr;
rinit_tot_std = sqrt(fin_fix_std.^2 + fin_drift_std.^2);

%%
orig_anal_name = 'monoc_eyecorr_hbar2';
load(orig_anal_name,'it_fix_*','drift_post*');
...
zinit_fin_tot_corr = fin_fix_corr + fin_drift_corr;
zinit_tot_std = sqrt(fin_fix_std.^2 + fin_drift_std.^2);
...
% for tt = [96 137 154 179 376 409]
    ...
    if ~isempty(uu)
        ...
        if dur > 3.5
            ...
            hold on;
            h1=shadedErrorBar(all_t_axis(used_inds(uu))-bt,rinit_fin_tot_corr(uu),rinit_tot_std(uu),{'color','m'});
            h2=shadedErrorBar(all_t_axis(used_inds(uu))-bt,zinit_fin_tot_corr(uu),zinit_tot_std(uu),{'color','k'});
