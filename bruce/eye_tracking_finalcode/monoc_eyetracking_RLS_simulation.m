...
data_name = 'monoc_eyecorr_hbar_sim'
else
    true_moddata_name = 'monoc_eyecorr_vbar_mods';
%     true_eyedata_name = 'monoc_eyecorr_vbar';
    true_eyedata_name = 'monoc_eyecorr_vbar_highres';
    mod_data_name = 'monoc_eyecorr_vbar_sim_mods';
data_name = 'monoc_eyecorr_vbar_sim'
end
true_upsamp_fac = 2;
...
if use_smooth_eyepos
    data_name = [data_name '_coilbased'];
end
%%
xv_frac = 0;
...
drift_noise_sigma = 0.005*sim_gain;
drift_prior_sigma = 0.005*sim_gain; %.004 may be best here
...
inferred_eyepos = (fin_fix_corr + fin_drift_corr)/true_upsamp_fac;
...
if use_smooth_eyepos
    ...
    %maximum initial corrections
    max_sim_pos = max_shift*(sp_dx/true_upsamp_fac);
    sim_eyepos(sim_eyepos > max_sim_pos) = max_sim_pos;
    sim_eyepos(sim_eyepos < - max_sim_pos) = -max_sim_pos;
    sim_eyepos_us_rnd = round(sim_eyepos/(sp_dx));
...
%%
cd(anal_dir)
save(data_name,'sim_gain','dit_*','it_*','all_mod_*','tr_set','drift_*','it_fix_*','sim_*','use_*');
