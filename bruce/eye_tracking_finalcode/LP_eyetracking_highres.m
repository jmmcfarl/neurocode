...
old_anal_name = 'monoc_eyecorr';
anal_name = 'monoc_eyecorr_highres';
hrmod_data_name = 'monoc_eyecorr_mods_hres';
...
cnt = 1;
while length(use_kInds_up) < spatial_usfac*length(use_kInds)
    use_kInds_up = find(Xinds_up(:) >= cur_use_pix(1)-cnt/spatial_usfac & Xinds_up(:) <= cur_use_pix(end) + (cnt-1)/spatial_usfac);
    if length(use_kInds_up) == spatial_usfac*length(use_kInds)
        break
    end
...
load(mod_data_name);
load(old_anal_name,'dit_mods','et_tr_set','it_fix_post_mean','drift_post_mean');
