...
if bar_ori == 0
    ...
    true_data = 'monoc_eyecorr_hbar';
else
    true_mod_data = 'monoc_eyecorr_vbar_mods';
    true_data = 'monoc_eyecorr_vbar';
end
    ...
    for rr = 1:Nrpts
        ...
        for nn = 1:n_fix_inf_it
            ...
            for ss = 1:length(et_tr_set)
                ...
                it_mods{nn+1}(cur_cell) = NMMfit_filters(it_mods{nn+1}(cur_cell),full_Robs_mat(cur_tr_uset,cur_unit_ind),...
                    tr_X,[],[],silent); %fit stimulus filters
                
                %refit spk NL
                it_mods_spkNL{nn+1}(cur_cell) = NMMfit_logexp_spkNL(it_mods{nn+1}(cur_cell),full_Robs_mat(cur_tr_uset,cur_unit_ind),tr_X);
                
                [newLL,~,new_prate] = NMMmodel_eval(it_mods_spkNL{nn+1}(cur_cell),full_Robs_mat(cur_tr_uset,cur_unit_ind),tr_X);
                [~,~,null_prate] = NMMmodel_eval(all_nullmod(cur_cell),full_Robs_mat(cur_tr_uset,cur_unit_ind),tr_X(2:end));
                [it_R2(nn+1,cur_cell),it_dev(nn+1,cur_cell)] = pseudo_r2(full_Robs_mat(cur_tr_uset,cur_unit_ind),new_prate,null_prate);
        ...
        for nn = 1:n_drift_inf_it
            ...
            for ss = 1:length(et_tr_set)
                ...
                dit_mods{nn+1}(cur_cell) = NMMfit_filters(dit_mods{nn+1}(cur_cell),full_Robs_mat(cur_tr_uset,cur_unit_ind),...
                    tr_X,[],[],silent); %fit stimulus filters
                
                dit_mods_spkNL{nn+1}(cur_cell) = NMMfit_logexp_spkNL(dit_mods{nn+1}(cur_cell),full_Robs_mat(cur_tr_uset,cur_unit_ind),tr_X);
                
                [newLL,~,new_prate] = NMMmodel_eval(dit_mods_spkNL{nn+1}(cur_cell),full_Robs_mat(cur_tr_uset,cur_unit_ind),tr_X);
                [~,~,null_prate] = NMMmodel_eval(all_nullmod(cur_cell),full_Robs_mat(cur_tr_uset,cur_unit_ind),tr_X(2:end));
                [dit_R2(nn+1,cur_cell),dit_dev(nn+1,cur_cell)] = pseudo_r2(full_Robs_mat(cur_tr_uset,cur_unit_ind),new_prate,null_prate);
