function [] = do_LP_eyetracking(Expt_name,use_LOOXV,use_coils)
...
for nn = 1:n_drift_inf_it
    ...
    uset = find(~isnan(drift_post_mean(nn,:)));
    ...
    if use_LOOXV == 1
        for xv = 1:length(su_inds)
            ...
            uset = find(~isnan(drift_post_mean_LOO(xv,nn,:)));
            drift_post_mean_LOO(xv,nn,:) = interp1(uset,squeeze(drift_post_mean_LOO(xv,nn,uset)),1:NT);
            drift_post_std_LOO(xv,nn,:) = interp1(uset,squeeze(drift_post_std_LOO(xv,nn,uset)),1:NT);
