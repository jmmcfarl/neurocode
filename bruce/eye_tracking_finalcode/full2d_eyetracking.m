...
dsfrac = 2; %spatial down-sample factor
flen = 1;

%if using single time-delay, make stim-mat at optimal delay
if flen > 1
    fixed_delay = 0;
else
    fixed_delay = 2;
...
fix_noise_sigma = sqrt(2*0.15^2); %0.1
fix_prior_sigma = sqrt(2*0.15^2); %0.2
drift_noise_sigma = 0.01; %0.01
drift_prior_sigma = 0.01; %0.01
drift_jump_sigma = sqrt(2*0.075^2); %0.075
drift_dsf = 2;
...
used_inds = used_inds(~isnan(all_Seeds(used_inds))); %dont use repeat trials
NT = length(used_inds);
...
if ~exist(['./' mod_data_name '.mat'],'file') || recompute_init_mods == 1
    ...
    for ss = 1:n_probes
        ...
        if ~isempty(cur_tr_inds)
            ...
            xv_X{1} = full_X(cur_xv_inds,use_kInds);
            xv_X{2} = Xblock(cur_xv_inds,:);
...
usable_units = find(~isnan(all_mod_LLimp));
...
for nn =1:n_fix_inf_it
    ...
    for xx = 1:n_shifts
        ...
        cur_stim_shift = full_X*shift_mat{xx};
    ...
    cur_xdiff = bsxfun(@minus,squeeze(it_fix_post_mean(nn,:,1)),SH(:,1)).^2';
    it_fix_post_std(nn,:,1) = sqrt(sum(cur_xdiff.*gamma,2));
    cur_ydiff = bsxfun(@minus,squeeze(it_fix_post_mean(nn,:,2)),SH(:,2)).^2';
    it_fix_post_std(nn,:,2) = sqrt(sum(cur_ydiff.*gamma,2));
...
%     all_fix_post_map = round(all_fix_map);
    ...
    for i=1:NT
        ...
        all_shift_stimmat(used_inds(i),:,:) = shift_matrix_Nd(all_shift_stimmat(used_inds(i),:,:),-round(all_fix_post_mean_cor(i,1)),3);
        all_shift_stimmat(used_inds(i),:,:) = shift_matrix_Nd(all_shift_stimmat(used_inds(i),:,:),-round(all_fix_post_mean_cor(i,2)),2);
    end
    ...
    for ss = 1:length(tr_set)
        ...
        [newxvLL] = NMMmodel_eval(it_mods_spkNL{nn+1}(cur_cell),Robs_mat(cur_xv_uset,cur_unit_ind),xv_X);
    ...
    clear cur_X tr_X xv_X
    
end

%% NOW INFER DRIFT CORRECTIONS
clear full_X
all_Xmat_up_fixcor = create_time_embedding(all_shift_stimmat,full_stim_params);
...
%     end
    ...
    for ff = 1:n_fixs
        ...
        if ntset > drift_dsf
            ...
            if mod(ntset,drift_dsf) ~= 0
                ...
                cur_drift_mean = cat(1,cur_drift_mean,nan(dangling_pts,2));
            end
            cur_LL_set = reshape(cur_LL_set,[drift_dsf nt_pts n_Dshifts]);
            cur_LL_set = squeeze(sum(cur_LL_set,1));
            
            cur_drift_mean = drift_dsf*reshape(nanmean(reshape(cur_drift_mean,[drift_dsf nt_pts 2])),nt_pts,2);
    ...
    drift_post_mean(nn,isnan(drift_post_mean(nn,:,1)),:) = 0;
...
for i=1:NT
    all_dshift_stimmat(used_inds(i),:,:) = shift_matrix_Nd(all_dshift_stimmat(used_inds(i),:,:),-round(drift_post_cor(i,1)),3);
    all_dshift_stimmat(used_inds(i),:,:) = shift_matrix_Nd(all_dshift_stimmat(used_inds(i),:,:),-round(drift_post_cor(i,2)),2);
end
...
% for tt = [108 244 318 541 725]
    ...
    if ~isempty(uu)
        ...
        if dur > 3
            ...
            hold on
            h1=shadedErrorBar(all_t_axis(used_inds(uu))-bt,fin_tot_corr(uu,1),fin_tot_std(uu,1),{'color','k'},0);
            hh1=shadedErrorBar(all_t_axis(used_inds(uu))-bt,fin_tot_corr(uu,2),fin_tot_std(uu,2),{'color','m'},0);
            h2=plot(all_t_axis(used_inds(uu))-bt,corrected_eye_vals_interp(used_inds(uu),2),'r');
            h3=plot(all_t_axis(used_inds(uu))-bt,corrected_eye_vals_interp(used_inds(uu),1),'b');
    ...
    for ss = 1:length(tr_set)
        ...
        kmat = reshape(kmat,[5 23 23 3]);
        kmat = squeeze(mean(abs(kmat),4));
        spatial_profile(ss,:,:) = squeeze(mean(kmat));
    end
    
    
    for ss = 1:length(tr_set)
        
    imagesc(xax(RF_xpatch_inds),yax(RF_ypatch_inds),squeeze(spatial_profile(ss,:,:)));
    set(gca,'ydir','normal');
    pause
    end
