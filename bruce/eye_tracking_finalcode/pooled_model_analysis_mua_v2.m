...
for ee = 1:n_expts
   ...
   if cur_expt_name(1) == 'G'       
       ...
       if cur_expt_num == 86
       anal_name = 'monoc_eyecorr_hbar_fullXV.mat';
       else
...
%        if cur_expt_num == 86
%        anal_name = 'monoc_eyecorr_vbar_fullXV.mat';
%        else
       ...
       for ii = 1:length(tot_use_mus)
           ...
           if cur_hor_imp > cur_ver_imp
               ...
               if cur_hor_loc > 96
                   ...
                   cur_su_data(ii).n_spikes = cur_hor_SU_data.mua_data(cur_hor_loc).n_spikes;
                   cur_su_data(ii).mean_rate = cur_hor_SU_data.mua_data(cur_hor_loc).mean_rate;
               end
...
%        anal_name = 'monoc_eyecorr3_Cprior.mat';
...
load pooled_mu_data_v4.mat %this is final data set
%% COMPUTE SIMPLICITY
% n_frames = 1e5;
% flen = 12;
...
screen_X = -weight_mean'.*sind(expt_oris) + rf_pos(1,:);
screen_Y = weight_mean'.*cosd(expt_oris) + rf_pos(2,:);

jbe_hori = jbeUnits(expt_oris(jbeUnits) == 0);
jbe_vert = jbeUnits(expt_oris(jbeUnits) == 90);
screen_X(jbe_hori) = all_X(jbe_hori)';
screen_Y(jbe_vert) = all_Y(jbe_vert)';

rf_eccs = sqrt(screen_X.^2 + screen_Y.^2);
...
for ii = 1:length(cur_set)
   ...
   fprintf('Ov mean %.4f\n\n',weight_mean(used_units(cur_set(ii))));
...
allUnits = find(after_sq_gR2 >= thresh_R2 | after_lin_gR2 >= thresh_R2);
% nonallUnits = setdiff(1:length(used_units),allUnits);
...
% 
% fname = [fig_dir 'modimp_vs_RFwidth_overlayNEW.pdf'];
...
fig_width = 3.27; rel_height = 0.8;
figufy(h);
fname = [fig_dir 'RFwidth_vs_eccNEW.pdf'];
exportfig(h,fname,'width',fig_width,'height',rel_height*fig_width,'fontmode','scaled','fontsize',1);
close(h);
