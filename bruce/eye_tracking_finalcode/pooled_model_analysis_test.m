...
for ee = 1:n_expts
    ...
    for ii = 1:length(su_set)
        ...
        cv_before(su_set(ii)) = mean(abs(rate_out_rev - rate_out))/mean(rate_out);
       ...
       filt_out_var = std(filt_out);
        ...
        filt_outr(:,2:end) = filt_outr(:,2:end).^2;
        rate_out_rev = alpha*log(1+exp(beta*(sum(filt_outr,2) + offset)));
        cv_after(su_set(ii)) = mean(abs(rate_out_rev - rate_out))/mean(rate_out);
...
b = robustfit(cv_before(jbeUnits),cv_after(jbeUnits),[],[],'off');
h5 = figure(); hold on
plot(cv_before(jbeUnits),cv_after(jbeUnits),'k.','markersize',mSize);
maxlim = max(max(xlim(),ylim()));
xlim([0 maxlim]); ylim([0 maxlim]);
xl = xlim(); xx = linspace(xl(1),xl(2),100);
plot(xx,b*xx,'k--');
...
figufy(h5);
fname = [fig_dir 'PRM_before_after2.pdf'];
