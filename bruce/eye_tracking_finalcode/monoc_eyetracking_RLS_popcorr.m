...
if use_measured_pos == 1
    mod_data_name = [mod_data_name '_Cinit3'];
    anal_name = [anal_name '_Cinit3'];
end
...
%%
spk_flen = 5;
spike_params = NMMcreate_stim_params([spk_flen n_tr_chs],dt);
XRobs = create_time_embedding(Robs_mat,spike_params);
XRobs(isnan(XRobs)) = 0;
XfRobs = flipud(create_time_embedding(flipud(Robs_mat),spike_params));
XfRobs(isnan(XfRobs)) = 0;

[spk_CC,spk_FF] = meshgrid(1:n_tr_chs,1:spk_flen);
uset = find(spk_FF > 1);
XRobs = [XRobs(:,uset) XfRobs(:,uset)];
cur_X{5} = XRobs;

%%
spike_params = NMMcreate_stim_params([(spk_flen-1)*2 n_tr_chs],dt);
spike_regparams = NMMcreate_reg_params('lambda_L2',0.1);
NspkP = (spk_flen-1)*2*n_tr_chs;
silent = 1;
for ss = 1:length(tr_set)
    fprintf('Fitting model for unit %d/%d\n',ss,length(tr_set));
    cur_inds = find(~isnan(Robs_mat(:,ss)));
    tr_X = get_Xcell_tInds(cur_X,cur_inds);
    new_mod(ss) = dit_mods{end}(tr_set(ss));
    new_mod(ss).stim_params(5) = spike_params;
    new_mod(ss) = NMMadd_NLinput(new_mod(ss),'lin',1,5,0.01*randn(NspkP,1),spike_regparams);
    new_mod(ss) = NMMfit_filters(new_mod(ss),Robs_mat(cur_inds,ss),tr_X,[],length(new_mod(ss).mods),silent);
    
    new_mod_spkNL(ss) = NMMfit_logexp_spkNL(new_mod(ss),Robs_mat(cur_inds,ss),tr_X);
...
clear it_* 
it_mods{1}(tr_set) = new_mod;
it_mods_spkNL{1}(tr_set) = new_mod_spkNL;
...
for nn = 1:n_fix_inf_it
    ...
    for ss = 1:n_tr_chs
        ...
        spk_kerns(ss,:) = it_mods{nn}(tr_set(ss)).mods(cur_Xtargs == 5).filtK;
    end
...
dit_mods{1}(tr_set) = new_mod;
dit_mods_spkNL{1}(tr_set) = new_mod_spkNL;
...
for nn = 1:n_drift_inf_it
    ...
    for ss = 1:n_tr_chs
        ...
        spk_kerns(ss,:) = dit_mods{nn}(tr_set(ss)).mods(cur_Xtargs == 5).filtK;
    end
    filt_bank = permute(filt_bank,[2 1 3]);
...
%%
cd(anal_dir)
load(anal_name,'it_fix_post_*','drift_post_*');

true_fix_corr = nan(NT,1);
true_fix_std = nan(NT,1);
true_fix_corr(~isnan(fix_ids)) = it_fix_post_mean(end,fix_ids(~isnan(fix_ids)));
true_fix_corr = interp1(find(~isnan(fix_ids)),true_fix_corr(~isnan(fix_ids)),1:NT);
true_fix_std(~isnan(fix_ids)) = it_fix_post_std(end,fix_ids(~isnan(fix_ids)));
true_fix_std = interp1(find(~isnan(fix_ids)),true_fix_std(~isnan(fix_ids)),1:NT);

true_fix_corr = true_fix_corr*sp_dx;
true_fix_std = true_fix_std*sp_dx;

true_drift_corr = drift_post_mean(end,:)*sp_dx;
true_drift_std = drift_post_std(end,:)*sp_dx;
min_fix_dur = 0.15;
fix_inds = [];
long_fix_inds = [];
for ii = 1:n_fixs
    cur_inds = fix_start_inds(ii):fix_stop_inds(ii);
    if length(cur_inds) > sac_shift
        true_drift_corr(cur_inds(1:end-sac_shift+1)) = true_drift_corr(cur_inds(sac_shift:end));
        true_drift_std(cur_inds(1:end-sac_shift+1)) = true_drift_std(cur_inds(sac_shift:end));
    end
end

true_tot_corr = true_fix_corr + true_drift_corr;
true_tot_std = sqrt(true_fix_std.^2 + true_drift_std.^2);
...
for tt = 1:n_trials
    ...
    if ~isempty(uu)
        ...
        if dur > 3.5
            hold on
            h1=shadedErrorBar(all_t_axis(used_inds(uu))-bt,true_tot_corr(uu),true_tot_std(uu),{'color','m'});
