function [LL,LLgrad] = LLinternal_alphas_full4( mod, params, Robs, Xmat, Xsac_mat, L2_mats,lambda,lambda_L2)
   ...
   sac_Xmat = reshape(bsxfun(@times,Xmat,sac_emb),[NT klen]);
    ...
    for ii = 1:Nmods
        ...
        if strcmp(mod.mods(ii).NLtype,'lin')
            LLgrad(pp) = LLgrad(pp) + (residual.*gint(:,ii)* mod.mods(ii).sign)' *(sac_Xmat*all_Filts(:,ii));            
        else
            if strcmp(mod.mods(ii).NLtype,'nonpar')
            ...
            LLgrad(pp) = LLgrad(pp) + (fpg.*residual*mod.mods(ii).sign)' * (sac_Xmat*all_Filts(:,ii));
        end
    end
