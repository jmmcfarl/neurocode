...
[~, ~, ~, tot_G,ind_Gints,fgint] = NMMmodel_eval(cur_GQM, cur_Robs, all_Xmat_shift);
...
n_Gbins = 35;
Gtick = linspace(my_prctile(g_tot(any_sac_inds),0.1),my_prctile(g_tot(any_sac_inds),100-1),n_Gbins);
Xtick = -(backlag-1/2):(1):(forlag+1/2);

base_out = log(1+exp(Gtick + cur_GQM.spk_NL_params(1)));
...
sac_dep_out = log(1+exp(bsxfun(@plus,bsxfun(@times,sac_dep_beta,Gtick),sac_dep_theta)));
...
h2=figure;
subplot(2,1,1);
imagesc(slags*dt,Gtick,sac_dep_out'); set(gca,'ydir','normal'); caxis([0 1]);
yl = ylim();
...
subplot(2,1,2)
imagesc(slags*dt,Gtick,sacStimProc(cc).gsac_TB_rate); set(gca,'ydir','normal'); caxis([0 1]);
yl = ylim();
line(slags(mmmloc([1 1]))*dt,yl,'color','w');
line(slags(mmmloc([2 2]))*dt,yl,'color','w');
xlabel('Time (s)');
...
h3=figure;
subplot(2,1,1);
plot(Gtick,sac_dep_out(mmmloc(1),:)/dt); hold on
...
ylabel('Firing rate (Hz)');
subplot(2,1,2);
plot(Gtick,sac_dep_out(mmmloc(2),:)/dt); hold on
