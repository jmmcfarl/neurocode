    ...
    if ~isempty(cc_uinds)
        ...
        %% FIT (eye-corrected) STIM-PROCESSING MODEL
                ...
                if mode(expt_dds(cur_block_set)) == 12
                    ...
                    base_lambda_L1 = 5;
                    base_qlambda_d2XT = 200;
                    base_qlambda_L1 = 5;
                end
                init_sub_fact = 0.5; %fraction of data to use for initial filter estimation
                init_lambda_d2XT = 10;
                ...
                eqfilts = find(strcmp('quad',stim_NL_types) & stim_Xtargs == 1 & stim_mod_signs == 1);
                iqfilts = find(strcmp('quad',stim_NL_types) & stim_Xtargs == 1 & stim_mod_signs == -1);
                qfilts = [eqfilts iqfilts];
                linfilts = find(strcmp('lin',stim_NL_types) & stim_Xtargs == 1);
                ...
                %fit initial filters and estimate their magnitudes to set
                %regularization strengths
                init_fitN = ceil(length(cc_uinds)*init_sub_fact);
                ...
                vgint = var(gint);
                sgint = std(gint);
                init_mod = NMMadjust_regularization(init_mod,linfilts,'lambda_d2XT',base_lambda_d2XT./vgint(linfilts));
                init_mod = NMMadjust_regularization(init_mod,linfilts,'lambda_L1',base_lambda_L1./sgint(linfilts));
                if ~isempty(qfilts)
                ...
                fullGQM = NMMfit_filters(init_mod,cur_Robs(cur_tr_inds),all_Xmat_shift(cur_tr_inds,:),[],[],silent);
        
                %fit a sequence of submodels
                excGQM = fullGQM;
                excGQM.mods(iqfilts) = [];
                excGQM = NMMfit_filters(excGQM,cur_Robs(cur_tr_inds),all_Xmat_shift(cur_tr_inds,:),[],[],silent);
                inhGQM = fullGQM;
                inhGQM.mods(eqfilts) = [];
                inhGQM = NMMfit_filters(inhGQM,cur_Robs(cur_tr_inds),all_Xmat_shift(cur_tr_inds,:),[],[],silent);
                linGQM = fullGQM;
                linGQM.mods(qfilts) = [];
                linGQM = NMMfit_filters(linGQM,cur_Robs(cur_tr_inds),all_Xmat_shift(cur_tr_inds,:),[],[],silent);
        
                %% use cross-validated LLs to select the best GQM
                if ~isempty(cur_xv_inds)
                    [fullGQM_xvLL, ~, ~, ~, ~, ~, nullxvLL] = NMMmodel_eval(fullGQM, cur_Robs(cur_xv_inds), all_Xmat_shift(cur_xv_inds,:));
                    [excGQM_xvLL] = NMMmodel_eval(excGQM, cur_Robs(cur_xv_inds), all_Xmat_shift(cur_xv_inds,:));
                    [inhGQM_xvLL] = NMMmodel_eval(inhGQM, cur_Robs(cur_xv_inds), all_Xmat_shift(cur_xv_inds,:));
                    [linGQM_xvLL] = NMMmodel_eval(linGQM, cur_Robs(cur_xv_inds), all_Xmat_shift(cur_xv_inds,:));
        
                    fullGQM.xvLLimp = (fullGQM_xvLL-nullxvLL)/log(2);
                    excGQM.xvLLimp = (excGQM_xvLL-nullxvLL)/log(2);
                    inhGQM.xvLLimp = (inhGQM_xvLL-nullxvLL)/log(2);
                    linGQM.xvLLimp = (linGQM_xvLL-nullxvLL)/log(2);
                end
                all_mods = [linGQM excGQM inhGQM fullGQM];
                all_mod_xvLL = [linGQM_xvLL excGQM_xvLL inhGQM_xvLL fullGQM_xvLL];
                [best_xvLL,best_ind] = max(all_mod_xvLL);
                bestGQM = all_mods(best_ind);
                bestGQM.xvLLimp = (best_xvLL-nullxvLL)/log(2);
                ...
                if fit_rect_quad
                    ...
                    rectGQM.mods(end).filtK = -rectGQM.mods(end).filtK;
                    rectGQM.mods(1).NLtype = 'threshlin'; rectGQM.mods(end).NLtype = 'threshlin';
                    stim_NL_types = {rectGQM.mods(:).NLtype};
                    stim_mod_signs = [rectGQM.mods(:).sign];
                    rectGQM = NMMfit_filters(rectGQM,cur_Robs(cur_tr_inds),all_Xmat_shift(cur_tr_inds,:),[],[],silent);
                    [rectGQM_xvLL] = NMMmodel_eval(rectGQM, cur_Robs(cur_xv_inds), all_Xmat_shift(cur_xv_inds,:));
                    rectGQM.xvLLimp = (rectGQM_xvLL-nullxvLL)/log(2);
                end
        
                %% Refit model parameters on the full data set
                if ~isempty(cur_xv_inds)
                    bestGQM = NMMfit_filters(bestGQM,cur_Robs,all_Xmat_shift,[],[],silent);
                    [bestGQM_LL, ~, ~, ~, ~, ~, nullLL] = NMMmodel_eval(bestGQM, cur_Robs, all_Xmat_shift);
                    bestGQM.LLimp = (bestGQM_LL - nullLL)/log(2);
                    if fit_rect_quad
                        rectGQM = NMMfit_filters(rectGQM,cur_Robs,all_Xmat_shift,[],[],silent);
                        rectGQM.LLimp = (rectGQM.LL_seq(end)-nullLL)/log(2);
                    end
                end
        
        %% Store data
                fullLL = nansum(cur_Robs.*log(cur_Robs) - cur_Robs);
                fullxvLL = nansum(cur_Robs(cur_xv_inds).*log(cur_Robs(cur_xv_inds)) - cur_Robs(cur_xv_inds));
                unit_data.fullLL = fullLL;
                unit_data.nullLL = nullLL*nansum(cur_Robs);
                ...
                ModData(cc).fullGQM = fullGQM;
                ModData(cc).excGQM = excGQM;
                ModData(cc).inhGQM = inhGQM;
                ModData(cc).linGQM = linGQM;
                if fit_rect_quad
        ...
        %%
        clear tune_props
        bestGQM = ModData(cc).bestGQM;
        rectGQM = ModData(cc).rectGQM;
        
        stim_filters = [bestGQM.mods(:).filtK];
        stim_mod_signs = [bestGQM.mods(:).sign];
        mod_stim_params = bestGQM.stim_params;
        filt_data = get_filter_properties_v2(stim_filters,mod_stim_params,sp_dx);
        tune_props.filt_data = filt_data;
        
        [~,~,best_pred_rate,~,~,filt_outs] = NMMmodel_eval(bestGQM,cur_Robs,all_Xmat_shift);
        [~,~,rev_pred_rate] = NMMmodel_eval(bestGQM,cur_Robs,-all_Xmat_shift);
        ...
        rect_stim_filters = [bestGQM.mods([1 end]).filtK];
        ...
        %         rf_orth_avg = -ov_RF_pos(1).*sind(bar_ori) - ov_RF_pos(2).*cosd(bar_ori); %avg RF position (in eye coords, so flip hori comp for numbers stored in screen coords)
        % %         rf_orth_avg = -ov_RF_pos(1).*sind(bar_ori) + ov_RF_pos(2).*cosd(bar_ori); %avg RF position (in eye coords, so flip hori comp for numbers stored in screen coords)
        %         tune_props.RF_orthPos = tune_props.RF_mean + rf_orth_avg;
        %
        %         %for RF component parallel to bar stim, take avg RF position estimates for
        %         %each probe
        %         if strcmp(rec_type,'UA')
        %             tune_props.RF_parPos = -all_X(cc).*cosd(bar_ori) + all_Y(cc).*sind(bar_ori);
        %         else
        %             tune_props.RF_parPos = -ov_RF_pos(1)'.*cosd(bar_ori) + ov_RF_pos(2).*sind(bar_ori);
        %         end
...
%%
modFitParams = struct('bar_ori',bar_ori,'use_MUA',use_MUA,'fit_uncor',fit_unCor,'fit_rect_quad',fit_rect_quad,...
    ...
    'init_sub_fact',init_sub_fact,'init_lambda_d2XT',init_lambda_d2XT);
