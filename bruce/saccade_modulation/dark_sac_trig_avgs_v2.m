...
backlag = round(0.45/dt);
forwardlag = round(0.75/dt);
...
%     end
% end
% all_mua_rate_norm(isnan(all_mua_rate_norm)) = 0;
...
[light_msac_in_avg,lags] = get_event_trig_avg(all_mua_rate_norm,sac_start_inds(light_msac_in),backlag,forwardlag,nboot);
[light_gsac_avg,lags] = get_event_trig_avg(all_mua_rate_norm,sac_start_inds(light_gsacs),backlag,forwardlag,nboot);
[light_gsac_in_avg,lags] = get_event_trig_avg(all_mua_rate_norm,sac_start_inds(light_gsac_in),backlag,forwardlag,nboot);
...
%%
% uset = find(sum(all_binned_mua) > 1e4);

bad_units = [16 31 62 63 64 92];
uset = setdiff(1:96,bad_units);

figure;
subplot(2,1,1)
h1=shadedErrorBar(lags*dt,mean(light_gsac_avg(:,uset)'),std(light_gsac_avg(:,uset)')/sqrt(length(uset)));
hold on
h2=shadedErrorBar(lags*dt,mean(light_gsac_in_avg(:,uset)'),std(light_gsac_in_avg(:,uset)')/sqrt(length(uset)),{'color','b'});
h3=shadedErrorBar(lags*dt,mean(light_msac_avg(:,uset)'),std(light_msac_avg(:,uset)')/sqrt(length(uset)),{'color','r'});
h4=shadedErrorBar(lags*dt,mean(light_msac_in_avg(:,uset)'),std(light_msac_in_avg(:,uset)')/sqrt(length(uset)),{'color','g'});
xlim([-0.2 0.4]);
legend([h1.mainLine h2.mainLine h3.mainLine h4.mainLine],{'Big','Big-onscreen','Micro','Micro-onscreen'});
xlabel('Time (s)');
...
hold on
h2=shadedErrorBar(lags*dt,mean(dark_msac_avg(:,uset)'),std(dark_msac_avg(:,uset)')/sqrt(length(uset)),{'color','r'});
legend([h1.mainLine h2.mainLine],{'Big','Micro'});
xlim([-0.2 0.4]);
xlabel('Time (s)');
ylabel('Relative rate');
title('Dark');
...
hold on
for ii = 1:n_amp_bins
    shadedErrorBar(lags*dt,nanmean(light_sac_amp_avg(ii,:,uset),3),nanstd(light_sac_amp_avg(ii,:,uset),[],3)/sqrt(length(uset)),{'color',cmap(ii,:)})
end
subplot(2,1,2)
hold on
for ii = 1:n_amp_bins
    shadedErrorBar(lags*dt,nanmean(dark_sac_amp_avg(ii,:,uset),3),nanstd(dark_sac_amp_avg(ii,:,uset),[],3)/sqrt(length(uset)),{'color',cmap(ii,:)})
end
