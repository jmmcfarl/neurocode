...
[mua_data.msac_burst_avg,lags] = get_event_trig_avg(all_mua_rate_norm,sac_start_inds(msac_bursts),backlag,forwardlag,nboot,used_trialvec,0);
...
[mua_data.msac_peak_avg,lags] = get_event_trig_avg(all_mua_rate_norm,sac_peak_inds(micro_set),backlag,forwardlag,nboot,used_trialvec,0);
[mua_data.gsac_peak_avg,lags] = get_event_trig_avg(all_mua_rate_norm,sac_peak_inds(gsac_set),backlag,forwardlag,nboot,used_trialvec,0);
...
nboot = [];
[sua_data.msac_burst_avg,lags] = get_event_trig_avg_v2(all_sua_rate_norm,sac_start_inds(msac_bursts),backlag,forwardlag,nboot,used_trialvec,0);
[sua_data.msac_end_avg,lags] = get_event_trig_avg_v2(all_sua_rate_norm,sac_stop_inds(micro_set),backlag,forwardlag,nboot,used_trialvec,0);
[sua_data.gsac_end_avg,lags] = get_event_trig_avg_v2(all_sua_rate_norm,sac_stop_inds(gsac_set),backlag,forwardlag,nboot,used_trialvec,0);
[sua_data.msac_peak_avg,lags] = get_event_trig_avg_v2(all_sua_rate_norm,sac_peak_inds(micro_set),backlag,forwardlag,nboot,used_trialvec,0);
[sua_data.gsac_peak_avg,lags] = get_event_trig_avg_v2(all_sua_rate_norm,sac_peak_inds(gsac_set),backlag,forwardlag,nboot,used_trialvec,0);
...
sua_trig_avgs = struct('msac',sua_msacs,'gsac',sua_gsacs,'simsac',sua_simsacs,'msac_gray',sua_msacs_gray,'gsac_gray',sua_gsacs_gray,'msac_im',sua_msac_im,'gsac_im',sua_gsac_im);

%%
sua_data.SU_numbers = SU_numbers;
sua_data.probe_numbers = SU_probes;

sua_data.avg_rates = nanmean(all_binned_sua(used_inds,:));
sua_data.tot_nspikes = nansum(all_binned_sua(used_inds,:));
sua_data.N_used_blocks = sum(~isnan(Clust_data.SU_block_probes),2);
sua_data.N_used_samps = nansum(~isnan(all_binned_sua(used_inds,:)));

block_rates = nan(length(cur_block_set),length(SU_probes));
for ii = 1:length(cur_block_set)
    block_rates(ii,:) = nanmean(all_binned_sua(all_blockvec == ii,:));
end
sua_data.stability_cv = nanstd(block_rates)./nanmean(block_rates);

sua_data.SU_Lratios = Clust_data.SU_Lratios;
sua_data.SU_isodists = Clust_data.SU_isodists;
sua_data.SU_refracts = Clust_data.SU_refract;
sua_data.SU_dprimes = Clust_data.SU_dprimes;
%%
