function [LL,LLgrad] = LLinternal_alphas( mod, params, Robs, Xmat, Jmat)
...
for ii = 1:Nmods
    ...
    gint(:,ii) = (Xmat+params(1)*JXmat)* all_Filts(:,ii);
...
for ii = 1:Nmods
    if strcmp(mod.mods(ii).NLtype,'lin')
        ...
        LLgrad(1) = LLgrad(1) + (residual.*gint(:,ii)* mod.mods(ii).sign)' *(JXmat*all_Filts(:,ii));
        ...
        LLgrad(1) = LLgrad(1) + (fpg.*residual*mod.mods(ii).sign)' * (JXmat*all_Filts(:,ii));
    end
end
