...
%%
mua_gsac_avgs = [mua_data(:).gsac_avg];
mua_msac_avgs = [mua_data(:).msac_avg];

figure
imagesc(lags*dt,1:24,mua_gsac_avgs');
xlim([-0.15 0.35]);
colorbar;
caxis([0.7 1.3])
%%
...
% cd(anal_dir);
% load(anal_name,'dit_mods','et_tr_set');
% 
% %%
% stim_dims = dit_mods{end}(et_tr_set(1)).stim_params(1).stim_dims;
% stim_dt = dit_mods{end}(et_tr_set(1)).stim_params(1).dt;
% 
% for ee = 1:length(et_tr_set)
%     cur_filts = reshape([dit_mods{end}(et_tr_set(ee)).mods(1:3).filtK],[stim_dims(1) stim_dims(2) 3]);
%     cur_tempk = squeeze(std(cur_filts,[],2));
%     avg_tempfilt(ee,:) = mean(cur_tempk,2);
% end
