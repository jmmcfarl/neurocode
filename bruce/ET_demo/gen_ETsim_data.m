...
save_dir = ['~/James_scripts/bruce/ET_demo'];
...
orig_moddata_name = 'monoc_eyecorr_hbar2.mat';
...
load(orig_moddata_name,'dit_mods');
orig_mods = dit_mods{end}(tr_set);
...
%% GET RID OF BLOCK KERNEL
for ii = 1:n_tr_chs
    rid = find(ismember([orig_mods(ii).mods(:).Xtarget],[2 3 4]));
    addfac = nanmean(true_mods(tr_set(ii)).mods(rid(1)).filtK);
    true_mods(tr_set(ii)).mods(rid) = [];
    true_mods(tr_set(ii)).stim_params([2 3 4]) = [];
    true_mods(tr_set(ii)).spk_NL_params(1) = true_mods(tr_set(ii)).spk_NL_params(1) + addfac;
        
    orig_mods(ii).mods(rid) = [];
    orig_mods(ii).stim_params([2 3 4]) = [];
    orig_mods(ii).spk_NL_params(1) = orig_mods(ii).spk_NL_params(1) + addfac;
end
...
% make Robs_mat
true_rates = nan(length(used_inds),n_tr_chs);
for ss = 1:n_tr_chs
...
%     end
%     true_rates(cur_use,ss) = pred_rate(cur_use);
true_rates(:,ss) = pred_rate;
end
Robs_mat = poissrnd(true_rates);
% Robs_xv = poissrnd(true_rates);

%% LIMIT DATA TO ONLY COMPLETED TRIALS
% trial_durs = (trial_end_inds - trial_start_inds)*dt;
% used_trials = find(trial_durs > 3.73);
% use_subset = find(ismember(all_trialvec(used_inds),used_trials));

T = tabulate(all_trialvec(used_inds));
used_trials = find(T(:,2)*dt > 3.73);
use_subset = find(ismember(all_trialvec(used_inds),T(used_trials,1)));

NT = length(use_subset);

trial_start_inds = [1; find(diff(all_trialvec(used_inds(use_subset))) ~= 0) + 1];
trial_end_inds = [find(diff(all_trialvec(used_inds(use_subset))) ~= 0); NT];

saccade_start_inds = find(ismember(used_inds(use_subset),interp_sac_start_inds));
used_saccade_set = find(ismember(interp_sac_start_inds,used_inds(use_subset)));
%nearest index in the used data set of the saccade stop time
saccade_stop_inds = round(interp1(used_inds(use_subset),1:NT,interp_sac_stop_inds(used_saccade_set)))';
saccade_stop_inds(isnan(saccade_stop_inds)) = NT;

sim_eyepos = sim_eyepos(used_inds(use_subset));
% t_axis = all_t_axis(used_inds(use_subset));
% t_since_start = all_tsince_start(used_inds(use_subset));
stim_mat = all_stim_mat(used_inds(use_subset),:);
true_rates = true_rates(use_subset,:);
Robs_mat = Robs_mat(use_subset,:);

%% CHANGE DATA FORMAT FOR STORING
stim_mat = int8(stim_mat);
Robs_mat = uint8(Robs_mat);
%%
cd(save_dir)
save(sim_data_name,'sim_eyepos','Robs_mat','orig_mods','saccade_start_inds','saccade_stop_inds','dt','trial_*inds','stim_mat')
