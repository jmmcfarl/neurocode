...
for t = 1:96
    ...
    for cur_freq = 1:length(wfreqs);
        fprintf('Freq %d of %d\n',cur_freq,length(wfreqs));
        cur_inds = (cur_freq-1)*nbins+(1:nbins);
%         Xmat =  [Tmat(:,cur_inds) bsxfun(@times,Tmat(:,cur_inds),mod_out)];
        Xmat =  [Tmat(:,cur_inds) mod_out];
        ...
        l1_pen = 1000;
        clear lamrange*
%         lamrange2 = [l2_pen 1 nbins 1;5*l2_pen nbins+1 2*nbins 1];
%         lamrange = [l1_pen 1 nbins 1;2*l1_pen nbins+1 2*nbins 1];
        lamrange2 = [l2_pen 1 nbins 1];
        lamrange = [l1_pen 1 nbins 1];
        [fitp_phase,grad] = GLMsolve_jmm(Xmat,spkbs,K0,silent,lamrange,lamrange2,[],[],[],[],0);
        phase_kern(cur_freq,:) = fitp_phase.k;
    end
...
%         Xmat =  [Tmat(:,cur_inds) bsxfun(@times,Tmat(:,cur_inds),mod_out)];
        ...
        pred_rate = Xmat*phase_kern(cur_freq,1:end-1)' + phase_kern(cur_freq,end);
        pred_rate = log(1+exp(pred_rate));
        xv_phase_LL(t,cur_freq) = -sum(xv_Robs.*log(pred_rate) - pred_rate)/sum(xv_Robs);
    end
    xv_phase_LL(t,:) = xv_phase_LL(t,:) - xv_so_LL(t);
...
%%
% save extp1_stim_alpha_models xv_* ind_kern dep_kern wfreqs pax
