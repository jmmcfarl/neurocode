...
load ./expt1_lfp_alpha_phase_delay_all_v2.mat
...
stim_params = [npbins length(wfreqs)];
...
for t = 1:96
    ...
    cur_alpha_amp = squeeze(all_interp_amps(uset(tr_inds),:,nearest_lfps(t)));
    cur_alpha_amp = nanzscore(cur_alpha_amp);
    ...
    Pgmat = bsxfun(@times,Pmat,mod_out);
    Xmat =  [Pmat Pgmat];
...
%     stim_ind_pfilt(t,:) = fitp.k(1:npbins*length(wfreqs));
%     stim_dep_pfilt(t,:) = fitp.k((npbins*length(wfreqs)+1):2*npbins*length(wfreqs));

    new_gain = Pgmat*stim_dep_pfilt(t,:)';
    new_offset = Pmat*stim_ind_pfilt(t,:)';
    
    Xmat = [bsxfun(@times,cur_alpha_amp,new_gain) new_gain bsxfun(@times,cur_alpha_amp,new_offset) new_offset];
    
    klen = size(Xmat,2);
    K0 = zeros(klen+1,1);
    lamrange2 = [3000 1 length(wfreqs) 0];
    lamrange2 = [lamrange2; 10000 length(wfreqs)+2 length(wfreqs)*2+1 0];
    [fitp_ap] = GLMsolve_jmm(Xmat,spkbs,K0,silent,[],lamrange2,[],[],[],[],0);
    stim_dep_afilt(t,:) = fitp_ap.k(1:length(wfreqs));
    stim_ind_afilt(t,:) = fitp_ap.k((length(wfreqs)+2):(2*length(wfreqs)+1));
    ...
    cur_alpha_amp = squeeze(all_interp_amps(uset(xv_inds),:,nearest_lfps(t)));
    cur_alpha_amp = nanzscore(cur_alpha_amp);
...
%     xv_po_LL(t) = -sum(xv_Robs.*log(xv_pred_rate) - xv_pred_rate)/sum(xv_Robs);
    ...
    xv_pred_ap = Xmat*fitp_ap.k(1:end-1) + fitp_ap.k(end);
    xv_pred_ap = log(1+exp(xv_pred_ap));
    xv_ap_LL(t) = -sum(xv_Robs.*log(xv_pred_ap) - xv_pred_ap)/sum(xv_Robs);
...
%%
save extp1_stim_alpha_models_v2 xv_* stim_* wfreqs pax 