...
%%
cd ~/Data/bruce/7_15_12/G029
save oned_noise_mapping_v2 *profile unique* *bin* 
%%
...
for i = 1:96
    ...
    mean_y_old(i) = 0.5*cur_par_y(parloc) + 0.5*cur_orth_y(orthloc);
     mean_x(i) = cur_par_so(parloc)*cos(vec_angles(cur_set_par)) + cur_orth_so(orthloc)*cos(vec_angles(cur_set_orth));
     mean_y(i) = cur_par_so(parloc)*sin(vec_angles(cur_set_par)) + cur_orth_so(orthloc)*sin(vec_angles(cur_set_orth));
...
save oned_fixation_fits_v2 std_* *_profile mean_* gpred_* gfit_* dom_* parallel_* orth_*
