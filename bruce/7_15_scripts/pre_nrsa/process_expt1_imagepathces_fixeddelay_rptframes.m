...
%            del_bins = [del_bins cur_repeat_frames(cc)+cc-1]; 
%         end
...
for i = 1:tot_images
    ...
    for j = 1:length(cur_samp_set)
        ...
        ypatch_inds_adj = round(ypatch_inds - full_yo_vec(cur_samp_set(j))*Fsd);
        xpatch_inds_adj = round(xpatch_inds - full_xo_vec(cur_samp_set(j))*Fsd);
...
save('Expt1_newcompiled_data_fixeddelay_d1p25_34_v2.mat','-v7.3','full*','dt','Fsd','RF_patch*','xax','yax','*_inds','dsfrac');
%%
