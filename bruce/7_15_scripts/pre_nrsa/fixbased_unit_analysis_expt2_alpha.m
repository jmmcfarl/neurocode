...
for i = 1:n_fixs
    cur_set = full_fix_starts(i):full_fix_ends(i);
...
max_phase = 2*pi*7;
nbins = 62;
tr_inds = tr_inds(phase_since_trial(tr_inds) <= max_phase);
...
for t = 1:96
    ...
    klen = size(ov_Xmat,2);
    K0 = zeros(klen,1);
    lamrange2 = [l2_dep 1 nbins;l2_ind (nbins+1) 2*nbins];
    llist = [];
    [fitp,grad] = GLMsolve_jmm(ov_Xmat, spksN, K0, silent, [], lamrange2, [], [],llist, [], 0);
    fstim_dep_kern(t,:) = fitp.k(1:nbins);
    fstim_ind_kern(t,:) = fitp.k((nbins+1):2*nbins);
    fblock_kern(t,:) = fitp.k((2*nbins+1):end-1);
...
save expt2_unit_tempmods_alpha_v4 LLf fstim* fblock* fov_* tax nbins dtd trig_avg_rate
%%
close all
for t = 1:96
...
%     plot(tent_centers,ostim_dep_kern(t,:));
    hold on
    plot(tax/(2*pi),fstim_dep_kern(t,:),'r')
    xlim([0 6])
    subplot(3,1,2)
%     plot(tent_centers,ostim_ind_kern(t,:));
    hold on
    plot(tax/(2*pi),fstim_ind_kern(t,:),'r')
    xlim([0 6])
    subplot(3,1,3)
    plot(tax/(2*pi),trig_avg_rate(t,:)/dtd,'k')
    xlim([0 6])
    t
    pause
    clf
