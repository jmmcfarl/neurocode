...
%         cur_xo = (0:(ims_per_trial-1))*jump_sizes(completed_trials(i))*sin(degtorad(Trial_angles(completed_trials(i))));
%         cur_yo = (0:(ims_per_trial-1))*jump_sizes(completed_trials(i))*cos(degtorad(Trial_angles(completed_trials(i))));
...
rand_xo_vec = rand(size(all_xo_vec))*12-6;
rand_yo_vec = rand(size(all_xo_vec))*12-6;
...
for i = 1:tot_images
    ...
    for j = 1:length(cur_samp_set)
        ...
        xpatch_inds_adj = round(xpatch_inds - all_xo_vec(cur_samp_set(j))*Fsd);
        cur_im = IMAGE(ypatch_inds_adj,xpatch_inds_adj);
        all_im_patches_yshift(cur_samp_set(j),:,:) = cur_im;
        
        ypatch_inds_adj = round(ypatch_inds - all_yo_vec(cur_samp_set(j))*Fsd);
        xpatch_inds_adj = round(xpatch_inds - rand_xo_vec(cur_samp_set(j))*Fsd);
        cur_im = IMAGE(ypatch_inds_adj,xpatch_inds_adj);
        all_im_patches_xshift(cur_samp_set(j),:,:) = cur_im;
    end
end
...
%initialize model
defmod.lambda_dT = 0;
defmod.lambda_d2X = 100000; %3000
defmod.lambda_L1x = 800; %40
...
test_gnm = createGNM(init_kerns,init_signs,kern_types,defmod,stim_params);
test_gnm = fitGNM_filters(test_gnm,fullX,spikebins,'none',[],1e-4,1e-8);
