...
%     corrected_right(:,2) = bsxfun(@plus,X*by_right(2:end),by_right(1));
    ...
    pool_dx_l = [pool_dx_l sac_data(:).dx_left];
    pool_dx_r = [pool_dx_r sac_data(:).dx_right];
    pool_dy_l = [pool_dy_l sac_data(:).dy_left];
    pool_dy_r = [pool_dy_r sac_data(:).dy_right];
...
eye_dx = nan(length(used_fixs),2);
eye_dy = nan(length(used_fixs),2);
for i = 2:length(used_fixs)
    if ~ismember(fix_start_inds(i)-1,trial_flips)
       eye_dx(i,1) = all_eyepos(fix_start_inds(i),1)-all_eyepos(fix_stop_inds(i-1),1);
       eye_dx(i,2) = all_eyepos(fix_start_inds(i),3)-all_eyepos(fix_stop_inds(i-1),3);
       eye_dy(i,1) = all_eyepos(fix_start_inds(i),2)-all_eyepos(fix_stop_inds(i-1),2);
       eye_dy(i,2) = all_eyepos(fix_start_inds(i),4)-all_eyepos(fix_stop_inds(i-1),4);       
    end
end
...
%     %     fullX(cur_set,:) = reshape(all_im_patches(cur_inds{i}-fixed_delay,:,:),fix_len(i),sdim^2);
...
full_inblink = all_inblink(used_inds);
...
save('Expt1_newcompiled_data_flen6_d1p25_full.mat','-v7.3','full*','dt','Fsd','RF_patch*','xax','yax','*_inds','dsfrac','eye_*');
%%
