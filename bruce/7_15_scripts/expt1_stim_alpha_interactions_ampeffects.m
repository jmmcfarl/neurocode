...
load ./expt1_lfp_alpha_phase_delay_all_sameelec_fullfreq.mat all_interp_* nearest_lfps new_* desired_dt wfreqs frac
...
reg_params2.dl2_ind = 500;
reg_params2.dl2_freq_ind = 30;
reg_params2.l1_ind = 0.5;
...
n_amp_bins = 15;
amp_prctile_bins = linspace(1,99,n_amp_bins);
...
for t = 1:96
    ...
    cur_amps = squeeze(all_interp_amps(uset(cur_tr_inds),:,nearest_lfps(t)));
    cur_amps = nanzscore(cur_amps);
...
%     Xmat =  [Pmat(:,(23-1)*nbins + (1:nbins))];
    ...
    phase_outs = nan(length(cur_tr_inds),length(wfreqs));
    for ww = 1:length(wfreqs)
        cur_inds = (ww-1)*nbins + (1:nbins);
        phase_outs(:,ww) = Pmat(:,cur_inds)*stim_ind_filt_po(t,cur_inds)';
    end
    
    Amat = [];
    cur_amp_bins = nan(length(wfreqs),n_amp_bins);
    for ww = 1:length(wfreqs)
        cur_amp_bins(ww,:) = prctile(cur_amps(:,ww),amp_prctile_bins);
        cur_tb = tbrep(cur_amps(:,ww),cur_amp_bins(ww,:));
        Amat = [Amat bsxfun(@times,cur_tb,phase_outs(:,ww))];
    end
    
    cur_stim_params = [n_amp_bins length(wfreqs)];
    klen = size(Amat,2);
    K0 = zeros(klen+1,1);
    [fitp_amp] = fit_GLM_phase_model_const(Amat, sum(phase_outs,2),Robs, K0, 1, cur_stim_params, reg_params2,0,NL_type);
    stim_ind_afilt_po(t,:) = fitp_amp.k(1:n_amp_bins*length(wfreqs));
    ...
    Xmat =  [Amat cur_stim_mod_out];
    klen = size(Xmat,2);
    K0 = zeros(klen+1,1);
    [fitp_amps] = fit_GLM_phase_model_const(Xmat,sum(phase_outs,2), Robs, K0, 1, cur_stim_params, reg_params2,0,NL_type);
    stim_ind_afilt_pos(t,:) = fitp_amps.k(1:n_amp_bins*length(wfreqs));
    ...
    cur_amps = squeeze(all_interp_amps(uset(xv_inds),:,nearest_lfps(t)));
    cur_amps = nanzscore(cur_amps);
    ...
    phase_outs = nan(length(xv_inds),length(wfreqs));
    for ww = 1:length(wfreqs)
        cur_inds = (ww-1)*nbins + (1:nbins);
        phase_outs(:,ww) = Pmat(:,cur_inds)*stim_ind_filt_po(t,cur_inds)';
    end
    ...
    Xmat =  [Amat];
    xv_pred_amp = Xmat*fitp_amp.k(1:end-1) + sum(phase_outs,2)+fitp_amp.k(end);
    if NL_type == 0
        xv_pred_amp = log(1+exp(xv_pred_amp));
    else
        xv_pred_amp = exp(xv_pred_amp);
    end
    xv_amp_LL(t) = -sum(xv_Robs.*log(xv_pred_amp) - xv_pred_amp)/sum(xv_Robs);
    ...
    Xmat =  [Amat cur_stim_mod_out];
    xv_pred_amps = Xmat*fitp_amps.k(1:end-1) + sum(phase_outs,2)+fitp_amps.k(end);
    if NL_type == 0
        xv_pred_amps = log(1+exp(xv_pred_amps));
    else
        xv_pred_amps = exp(xv_pred_amps);
    end
    xv_amps_LL(t) = -sum(xv_Robs.*log(xv_pred_amps) - xv_pred_amps)/sum(xv_Robs);
...
%%
save extp1_stim_alpha_models_same_elec_ampdep2 xv_* wfreqs pax stim_* *_prctiles prctile*
...
amp_rel_LL = xv_amp_LL(ucells) - xv_null_LL(ucells);
pos_rel_LL = xv_pos_LL(ucells) - xv_null_LL(ucells);
amps_rel_LL = xv_amps_LL(ucells) - xv_null_LL(ucells);
% pg_rel_LL = xv_pg_LL(ucells)-xv_null_LL(ucells);
so_rel_LL = xv_so_LL(ucells) - xv_null_LL(ucells);

figure
boxplot([-amps_rel_LL(:) -pos_rel_LL(:) -so_rel_LL(:) -amp_rel_LL(:) -po_rel_LL(:)])
hold on
% plot([-amps_rel_LL(:) -pos_rel_LL(:) -so_rel_LL(:) -amp_rel_LL(:) -po_rel_LL(:)]','o-')
%%
% ucells = 1:96;
% ucells = find(xv_so_LL < xv_null_LL);
% ucells(ismember(ucells,single_units)) = [];
...
avg_phase_amps = reshape(mean(stim_ind_afilt_pos(ucells,:)),n_amp_bins,length(wfreqs))';
figure
subplot(2,1,1)
pcolor(doub_pax*180/pi,wfreqs,[avg_phase_pos avg_phase_pos]);shading flat
subplot(2,1,2)
pcolor(amp_prctile_bins,wfreqs,avg_phase_amps);shading flat
...
for t =1:96
    ...
    amp_kern = reshape(stim_ind_afilt_pos(t,:),n_amp_bins,length(wfreqs))';
    subplot(3,2,[1 2])
    pcolor(pax(1:end-1)*180/pi,wfreqs,phase_kern);shading flat;colorbar
    subplot(3,2,[3 4])
    pcolor(amp_prctile_bins,wfreqs,amp_kern);shading flat;colorbar
    subplot(3,2,5)
