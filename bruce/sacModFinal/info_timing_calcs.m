...
for ii = 1:length(info_slags)
    ...
    cur_X(scramb_lags) = shuf_X(scramb_lags);
...
%     cur_X(rXis_during(cur_set,:)) = shuf_X(rXis_during(cur_set,:));
    ...
    base_LL_imp_before(ii) = nansum(base_unshuf_LLseq - base_shuf_LLseq)/sum(cur_Robs(cur_set));
end
...
for ii = 1:length(info_slags)
    ...
    base_LL_imp_after(ii) = nansum(base_unshuf_LLseq - base_shuf_LLseq)/sum(cur_Robs(cur_set));
...
for ii = 1:length(info_slags)
    ...
    base_LL_imp_during(ii) = nansum(base_unshuf_LLseq - base_shuf_LLseq)/sum(cur_Robs(cur_set));
