...
offset_d2T = post_sacMod.opt_d2T;
offset_L2 = post_sacMod.opt_L2;
...
for jj = 1:length(poss_d2T)
    ...
    %kernel
    reg_params = NMMcreate_reg_params('lambda_d2T',[offset_d2T repmat(modSeq_d2T,1,length(mod_signs)-1)]',...
        'lambda_L2',[offset_L2 zeros(1,length(mod_signs)-1)]','boundary_conds',repmat([Inf Inf Inf],length(mod_signs),1));
...
subspace_optL2 = poss_d2T(optloc);
...
%used data
reg_params = NMMcreate_reg_params('lambda_d2T',[offset_d2T repmat(subspace_optL2,1,length(mod_signs)-1)]',...
    'lambda_L2',[offset_L2 zeros(1,length(mod_signs)-1)]','boundary_conds',repmat([Inf Inf Inf],length(mod_signs),1));
init_mod = NMMinitialize_model(cur_stim_params,mod_signs,NL_types,reg_params,Xtargs);
init_mod.spk_NL_params = stim_mod.spk_NL_params;
...
subspace_mod.opt_L2 = subspace_optL2;
