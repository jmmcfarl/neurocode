...
for ii = 1:length(slags)
    temp = find(Xsac(:,ii) == 1);
    gsac_fix_disp(ii) = nanmean(fix_disp(temp));
end

msac_fix_disp = nan(length(slags));
for ii = 1:length(slags)
    temp = find(Xmsac(:,ii) == 1);
    msac_fix_disp(ii) = nanmean(fix_disp(temp));
end

fix_disp_data.gsac_fix_disp = gsac_fix_disp;
fix_disp_data.msac_fix_disp = msac_fix_disp;
