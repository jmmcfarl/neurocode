...
%     used_fix_start_inds = find(in_window(fix_start_inds)==1 & is_stim_filtered(fix_start_inds)' == 0);
    ...
    for i = 1:n_used_fixs
       cur_set = fix_start_inds(used_fix_start_inds(i)):fix_stop_indas(used_fix_start_inds(i)); 
       cur_avg_image = nanmean(ov_im_patch(cur_set,:,:));
       
%        cur_fix_image_set(i,:,:) = cur_avg_image;
       cur_fix_image_set(i,:,:) = cur_avg_image - nanmean(cur_avg_image(:));
       
       cur_fix_spkcnt_set(i,:) = sum(spike_bin_vecs(:,cur_set),2);
    end
...
defmod.SDIM = SDIM;
defmod.locLambda = 0;
defmod.lambda_dX = 500000;
defmod.lambda_dT = 0;
defmod.lambda_L1x = 1000;
basis = 'pix';
...
% 
% ml_sta2 = fitGLM_lexp(ml_sta,Xmat,spikebins{cellid}(:,1),'tots',100);
