...
[b_alpha,a_alpha] = butter(2,[4 15]/niqf);
...
for blockid = 1:4;
    ...
    lfp_samps_alpha = angle(hilbert(lfp_samps_alpha));
    ...
    clear temp tampgram
    interp_alpha_phase = interp1(lfp_timed,lfp_samps_alpha,all_model_time_axis(cur_all_model));
    
    %%
    used_tpoints = find(~isnan(interp_ampgram(1,:,1)));
    all_ampgrams = cat(2,all_ampgrams,interp_ampgram(:,used_tpoints,:));
    all_alpha_phase = [all_alpha_phase; interp_alpha_phase(used_tpoints,:)];
...
for cc = 1:length(use_lfps)
    ...
    for ww = 1:length(wfreqs)
        %         beta = nlinfit(all_alpha_phase(:,cc),squeeze(all_ampgrams(cc,:,ww))',cosfun,BETA0);
        beta = lsqcurvefit(cosfun,BETA0,all_alpha_phase(:,cc),squeeze(all_ampgrams(cc,:,ww))',LB,UB);
        pref_alpha_phase(cc,ww) = beta(2);
        pref_alpha_amp(cc,ww) = beta(1);
    end
end
%%
X_resh = reshape(X,NT,SDIM^2);
...
%         beta(cc,ww,:) = smooth_regress_2d(squeeze(fix_avg_amps(cc,:,ww))',pred_mat,init_beta,smooth_lambda,sparse_lambda);
...
for cc = 1:length(use_lfps)
    for ww = 1:length(wfreqs)
        fprintf('Freq %d of %d\n',ww,length(wfreqs));
        total_out = zscore(squeeze(fft_pred_out(use_lfps(cc),ww,all_model_fixids(all_used_inds)))');
        phase_out = 1+cos(all_alpha_phase(:,cc) - pref_alpha_phase(cc,ww));
        
%         Y = all_ampgrams(cc,:,ww)';
%         XX = [Tmat(all_used_inds,:) bsxfun(@times,Tmat(all_used_inds,:),total_out') bsxfun(@times,Tmat(all_used_inds,:),phase_out)];
%         init_params = randn(3*length(tax),1);%
%         [B_stimmod_alphaphase(cc,ww,:)] = smoothed_regression(Y,XX,init_params,[1e3 2e3 2e3],[1 length(tax); length(tax)+1 2*length(tax);2*length(tax)+1 3*length(tax)],0);
        Y = all_ampgrams(cc,:,ww)';
        XX = [Tmat(all_used_inds,:) bsxfun(@times,Tmat(all_used_inds,:),total_out') total_out'.*phase_out];
        init_params = randn(2*length(tax)+1,1);%
        [B_stimmod_alphaphase(cc,ww,:)] = smoothed_regression(Y,XX,init_params,[1e3 2e3],[1 length(tax); length(tax)+1 2*length(tax);],0);
    end
...
save fourier_mod_fits beta per_dim tax B_stimmod wfreqs scales lfp_gabor_params
%%
for ww = 1:length(wfreqs);
    fprintf('Freq: %.3f\n',wfreqs(ww));
    for cc = 1:24
