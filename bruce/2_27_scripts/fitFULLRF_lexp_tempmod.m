...
temporal_set = length(initial_params)+(1:length(fit0.sacmod));
...
if max(lambda) > 0
    [params LL] = L1General2_PSSas(@(K) FULLBF_lexp_LLinternal_tempmod(K, Robs, X, T, fit0,filter_set,temporal_set),initial_params',lambda',options,0);
    exitflag = 1;
else
    [params LL exitflag] = minFunc( @(K) FULLBF_lexp_LLinternal_tempmod(K, Robs, X, T, fit0,filter_set,temporal_set), initial_params', options );
end
...
fit1.sacmod = params(temporal_set)';