...
all_sac_locs_left = [];
all_sac_locs_right = [];
all_sac_locs_left_nt = [];
all_sac_locs_right_nt = [];
all_stim_nums = [];
all_fix_vervar = [];
all_sac_dx = [];
all_sac_dy = [];
...
%     load(sprintf('lemM232.5%d.em.hor.mat',blockid))
    ...
    for i = 1:length(Expt.Header.CRrates)
        Eyedt(i) = Expt.Header.CRrates(i); % resolution of eye signal (sec)
    end
    if length(unique(Eyedt)) > 1
        error('Different Eye dts')
    else
        Eyedt = unique(Eyedt);
    end
    eyets = [];
    ...
    leye_pos = [];
    for i = 1:n_trials
       EyeStartT(i) = Expt.Trials(i).Start/10000;
       EyeEndT(i) = Expt.Trials(i).End/10000;
       eyets = [eyets EyeStartT(i):Eyedt:(EyeEndT(i)-Eyedt)];
       reye_pos = [reye_pos; Expt.Trials(i).Eyevals.rh Expt.Trials(i).Eyevals.rv];
       leye_pos = [leye_pos; Expt.Trials(i).Eyevals.lh Expt.Trials(i).Eyevals.lv];
    end
    ...
    EyeFs = 1/Eyedt;
    [bf,af] = butter(2,35/(EyeFs/2),'high');
    vergence = reye_pos - leye_pos;
    vervel = [0 0;diff(vergence)]/Eyedt;
    vervel_filt = filtfilt(bf,af,vervel);
    ...
    for t = 1:n_stims
        ...
        if t < n_stims %for the last image, take the end time as the block end time
            ...
            endT = min(Blocks{blockid}.blocktimes(2,end),eyets(end));
        end
        ...
        cur_mua_binned = zeros(14,length(Tax));
        for cellid = 1:14
            spikes_binned = histc(Blocks{blockid}.mutimes{cellid},time_bin_edges);
            spikes_binned(end) = [];
            cur_mua_binned(cellid,:) = spikes_binned;
        end
        
        spike_bin_vecs = [spike_bin_vecs; cur_spikes_binned'];
        mua_bin_vecs = [mua_bin_vecs; cur_mua_binned'];
        recon_t = [recon_t Tax];
        stim_num = [stim_num ones(size(Tax))*t];
    end
    spike_bin_vecs = spike_bin_vecs';
    mua_bin_vecs = mua_bin_vecs';
    
    % interpolate eye signal onto stimulus time axis
    eye_interp_right = interp1(eyets,reye_pos,recon_t);
    eye_interp_left = interp1(eyets,leye_pos,recon_t);
    ...
    fix_stop_times = recon_t(fix_stop_inds);
    ...
    used_fix_start_inds = find(in_window(fix_start_inds));
    %     used_fix_start_inds = find(in_window(fix_start_inds) & fix_skips'==0);
    ...
    cur_fix_muacnt_set = zeros(n_used_fixs,14);
    cur_fix_loc_left = zeros(n_used_fixs,2);
    cur_fix_loc_right = zeros(n_used_fixs,2);
    cur_fix_loc_left_nt = zeros(n_used_fixs,2);
    cur_fix_loc_right_nt = zeros(n_used_fixs,2);
    cur_fix_stim_nums = zeros(n_used_fixs,1);
    cur_fix_disp_var = zeros(n_used_fixs,2);
    for i = 1:n_used_fixs
        ...
        cur_set = fix_start_inds(used_fix_start_inds(i)) : (fix_stop_inds(used_fix_start_inds(i))); %use only window at begenning of fixation
        cur_fix_loc_right(i,:) = mean(eye_interp_right(cur_set,:));
        cur_fix_loc_left(i,:) = mean(eye_interp_left(cur_set,:));
        
        cur_fix_stim_nums(i) = stim_num(cur_set(1));
        
        cur_set_eyets = find(eyets >= fix_start_times(used_fix_start_inds(i)) & eyets <= fix_stop_times(used_fix_start_inds(i)));
        cur_set_eyets(cur_set_eyets > size(vervel_filt,1)) = [];
        cur_fix_disp_var(i,:) = var(vervel_filt(cur_set_eyets,:));
        
        cur_set_nt = (fix_start_inds(used_fix_start_inds(i))+round(0.15/stimres)) : (fix_stop_inds(used_fix_start_inds(i))); %use only window at begenning of fixation
        cur_fix_loc_right_nt(i,:) = mean(eye_interp_right(cur_set_nt,:));
        cur_fix_loc_left_nt(i,:) = mean(eye_interp_left(cur_set_nt,:));
        
        cur_set = fix_start_inds(used_fix_start_inds(i)) : (fix_start_inds(used_fix_start_inds(i))+round(min_fix_dur/stimres)); %use only window at begenning of fixation
       cur_fix_spkcnt_set(i,:) = sum(spike_bin_vecs(:,cur_set),2); %total spike counts within fixation window
        cur_fix_muacnt_set(i,:) = sum(mua_bin_vecs(:,cur_set),2); %total spike counts within fixation window
    end
    ...
    %     good_fixs = 1:length(vert_disp);
    good_fixs = find(cur_fix_disp_var(:,1) < 2);
    n_bad_fixs(blockid) = length(setdiff(1:length(vert_disp),good_fixs));
    n_good_fixs(blockid) = length(good_fixs);
    
    
    spk_cnts = [spk_cnts; cur_fix_spkcnt_set(good_fixs,:)];
    mua_cnts = [mua_cnts; cur_fix_muacnt_set(good_fixs,:)];
    ...
    all_sac_locs_left_nt = [all_sac_locs_left_nt; cur_fix_loc_left_nt(good_fixs,:)];
    all_sac_locs_right_nt = [all_sac_locs_right_nt; cur_fix_loc_right_nt(good_fixs,:)];
    all_sac_peaklocs = [all_sac_peaklocs; sac_peakloc(fix_prev_sac_inds(used_fix_start_inds(good_fixs)))'];
    all_sac_amps = [all_sac_amps; sac_amps(fix_prev_sac_inds(used_fix_start_inds(good_fixs)))];
    all_sac_dx = [all_sac_dx; sac_dx(fix_prev_sac_inds(used_fix_start_inds(good_fixs)))];
    all_sac_dy = [all_sac_dy; sac_dy(fix_prev_sac_inds(used_fix_start_inds(good_fixs)))];
    ...
    all_fix_vervar = [all_fix_vervar; cur_fix_disp_var(good_fixs,:)];
    ...
    all_stim_nums = [all_stim_nums; cur_fix_stim_nums(good_fixs)];
end


%%
cd /Users/James/Data/bruce/2_27_12/stimrecon
save fixation_data_v4 all_* block* spk_* mua_* n_bad_fixs n_good_fixs