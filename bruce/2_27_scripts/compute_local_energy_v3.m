    ...
    for xx = 1:length(gridX)
        for yy = 1:length(gridY)
            ...
            for ii = 1:length(hor_freqrange)
                hor_energy(yy,xx,ii) = sum(abs(F(hor_freqrange{ii}).^2));
                ver_energy(yy,xx,ii) = sum(abs(F(ver_freqrange{ii}).^2));
                diagur_energy(yy,xx,ii) = sum(abs(F(diagur_freqrange{ii}).^2));
                diagul_energy(yy,xx,ii) = sum(abs(F(diagul_freqrange{ii}).^2));
            end
        end
    end
...
%     subplot(3,2,3)
%     imagesc(squeeze(hor_energy(:,:,1)));colormap(gray); set(gca,'ydir','normal')
%     subplot(3,2,4)
%     imagesc(squeeze(ver_energy(:,:,1)));colormap(gray); set(gca,'ydir','normal')
%     subplot(3,2,5);
%     imagesc(squeeze(diagur_energy(:,:,1)));colormap(gray); set(gca,'ydir','normal')
%     subplot(3,2,6);
%     imagesc(squeeze(diagul_energy(:,:,1)));colormap(gray); set(gca,'ydir','normal')

%     figure
%         subplot(3,2,1)
%     imagesc(IMAGE);colormap(gray); set(gca,'ydir','normal')
%     subplot(3,2,3)
%     imagesc(squeeze(hor_energy(:,:,3)));colormap(gray); set(gca,'ydir','normal')
%     subplot(3,2,4)
%     imagesc(squeeze(ver_energy(:,:,3)));colormap(gray); set(gca,'ydir','normal')
%     subplot(3,2,5);
%     imagesc(squeeze(diagur_energy(:,:,3)));colormap(gray); set(gca,'ydir','normal')
%     subplot(3,2,6);
%     imagesc(squeeze(diagul_energy(:,:,3)));colormap(gray); set(gca,'ydir','normal')
...
%%
cd ~/Data/bruce/2_27_12/stimrecon/
save all_image_Energyarrays_v2_block4 all_Energy_array grid* dsfrac Fsd sigx sigy *_freqrange bandpass*