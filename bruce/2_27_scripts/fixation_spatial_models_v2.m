...
%     glm0.image_type = '2d';
%     glm0 = normalizeRFs_full(glm0,Xmat(blockids==blockid,:));
%     curset = find(blockids==blockid);
%     offset = curset(1)-1;
%     ml_sta(blockid) = fitGLM_lexp(glm0,Xmat(curset,:),...
%         spikebins{cellid}(ismember(spikebins{cellid},curset))-offset,'tots')
% end
...
ml_sta_all = fitGLM_lexp(glm0,Xmat,spikebins{cellid},'tots',200);
%%
k_vec = get_k_mat(ml_sta_all);
k_mat = reshape(k_vec,SDIM,SDIM);
spatial_output = Xmat*k_vec;
...
k_pert = nan(length(delta_x),length(delta_y),length(k_vec));
...
k_pert = reshape(k_pert,length(delta_x)*length(delta_y),size(Xmat,2));

min_x = min(ml_sta_all.mods(1).nlx);
poss_spatial_outputs = Xmat*k_pert';
poss_spatial_outputs = 1/ml_sta_all.mods(1).beta*log(1+exp(ml_sta_all.mods(1).beta*poss_spatial_outputs)) - ...
    1/ml_sta_all.mods(1).beta*log(1+exp(ml_sta_all.mods(1).beta*min_x));


NT = size(poss_spatial_outputs,1);
K0 = ones(size(poss_spatial_outputs));
K0 = K0(:);
K0 = [K0; ml_sta_all.const];
...
llist = [];
hold_const = [];
[bestk,LL,exitflag,output] = minFunc(@(K) LLelog_tempvar(K,poss_spatial_outputs,spk_cnts(:,cellid),lamrange,lamrange2,llist,hold_const),K0,options);

LLelog_tempvar(K0,poss_spatial_outputs,spk_cnts(:,cellid),[],[],[],[]);




%%
Xmat_resh = reshape(Xmat,size(Xmat,1),SDIM,SDIM);
for i = 1:size(Xmat,1)
    i
    Xmat_resh(i,:,:) = dist_shift2d(squeeze(Xmat_resh(i,:,:)),-sm_max_deltaXs(i),2,0);
    Xmat_resh(i,:,:) = dist_shift2d(squeeze(Xmat_resh(i,:,:)),-sm_max_deltaYs(i),1,0);
end
Xmat_new = reshape(Xmat_resh,size(Xmat,1),size(Xmat,2));

%%
ml_sta_revised = fitGLM_lexp(ml_sta_all,Xmat_new,spikebins{cellid},'tots',200);
