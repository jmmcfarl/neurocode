...
%     nlxrange = [tax];
    ...
    [fitp,grad] = GLMsolve_jmm( Xmat(tr_inds,:), spkbs, W0, silent, lamrange, lamrange2, Pcon, Pmono, llist, hold_const, NLtype);
...
%     sac_hist_xvLL(c) = sum(-Robs_xv.*log(cur_predrate_xv) + cur_predrate_xv)/sum(Robs_xv);
    ...
    lamrange2 = [slope2_lambda 1 length(tax) 0;
        4*slope2_lambda length(tax)+1 length(tax)*2 0];
%     nlxrange = [tax tax];
    Xmat = Tmat;
    ...
    [fitp,grad] = GLMsolve_jmm( Xmat(tr_inds,:), spkbs, W0, 1, lamrange, lamrange2,Pcon, Pmono, llist, [], NLtype);
...
save gabor_tempmodfits_new stim_mod* sac_* null* tax 

%%
for i = 1:18
    i
subplot(2,1,1)
plot(tax,stim_mod_sachist(i,:))
subplot(2,1,2)
plot(tax,stim_mod_tot(i,:))
pause
clf
end
...
for i = 1:n_stims
    ...
    cur_trg_avg(i,:) = smooth(cur_trg_avg(i,:),10,'lowess');
end
