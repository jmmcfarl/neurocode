...
% ROI = find(tempx >= -1.5 & tempx <= 1.5 & tempy >= -1.5 & tempy <= 1.5);
...
ov_sigma = 3;
ov_pen = exp(-(dX.^2+dY.^2)/(2*ov_sigma^2));
A = bsxfun(@times,A,ov_pen');
...
dX_seqs = zeros(n_iter+1,size(Xmat,1));
dY_seqs = zeros(n_iter+1,size(Xmat,1));
% cur_state_seq = ones(size(Xmat,1),1)*(K+1)/2;

% cur_Amat = shiftdim(repmat(A,[1 1 NT]),1);
% cur_Amat = repmat(reshape(A,[1 size(A,1) size(A,2)]),[NT 1 1]);
...
for n = 2:n_iter+1
    ...
    for cellid = 1:10
        ...
        poss_spatial_outputs = Xmat(:,ROI)*k_pert(:,ROI)';
    ...
    %%
%     if n > 2 %if this isn't the first iteration
%         disp('compute time-varying transition matrix');
%         for t = 2:NT
%             loc1 = [(dX+dX(cur_state_seq(t-1))) (dY+dY(cur_state_seq(t-1)))];
%             loc2 = [(dX+dX(cur_state_seq(t))) (dY+dY(cur_state_seq(t)))];
%             loc2(max(abs(loc2),[],2) > max_pert) = Inf;
%             cur_distmat = pdist2(loc1,loc2);
%             cur_Amat(t,:,:) = w1*c1*exp(-cur_distmat.^2/(2*sigma1^2)) + ...
%                 w2*c2*exp(-cur_distmat.^2/(2*sigma2^2));
%         end
%     end
...
% save modfits_avgeye_25_ds4_v2 mod_fit dsfrac Fsd state_seq dX_seqs dY_seqs gamma loglik all_sac_* all_fix_* blockids all_stim_*
% 
%%
figure
it_num = 10;
