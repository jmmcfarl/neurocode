...
for blockid = 1:4;
    ...
    temp_smbinned = nan(n_used_cells,length(cur_all_taxis));
    for i = 1:size(blocktimes,2)
        ...
        bad_tinds = find(isnan(cur_tinds));
        cur_taxis(bad_tinds) = []; cur_tinds(bad_tinds) = [];
        for c = 1:n_used_cells
            ...
            temp(end) = 0;
            temp_sm = jmm_smooth_1d_cor(temp,sm_win);
            
            temp_binned(c,cur_tinds) = temp;
            temp_smbinned(c,cur_tinds) = temp_sm;
        end
    end
    spikes_binned = [spikes_binned temp_binned];
    spikes_smbinned = [spikes_smbinned temp_smbinned];
end
...
for i = 1:n_fixs
    ...
    fix_trg_smmat(:,i,1:length(cur_set)) = spikes_smbinned(:,cur_set);    
end
fix_trg_avgs = squeeze(nanmean(fix_trg_mat,2));
fix_trg_smavgs = squeeze(nanmean(fix_trg_smmat,2));
fix_trg_avgs_n = bsxfun(@rdivide,fix_trg_avgs,ov_avg_rate);
fix_trg_smavgs_n = bsxfun(@rdivide,fix_trg_smavgs,ov_avg_rate);

big_sacs = find(all_sac_amps > 1);
sm_sacs = find(all_sac_amps < 1);

n_fixs = length(sm_sacs);
smfix_trg_mat = nan(n_used_cells,n_fixs,length(lags));
smfix_trg_smmat = nan(n_used_cells,n_fixs,length(lags));
for i = 1:n_fixs
    cur_set = (all_fix_start_inds(sm_sacs(i))-backwin):all_fix_stop_inds(sm_sacs(i));
    cur_set(length(lags)+1:end) = [];
    
    smfix_trg_mat(:,i,1:length(cur_set)) = spikes_binned(:,cur_set);    
    smfix_trg_smmat(:,i,1:length(cur_set)) = spikes_smbinned(:,cur_set);    
end
smfix_trg_avgs = squeeze(nanmean(smfix_trg_mat,2));
smfix_trg_smavgs = squeeze(nanmean(smfix_trg_smmat,2));
smfix_trg_avgs_n = bsxfun(@rdivide,smfix_trg_avgs,ov_avg_rate);
smfix_trg_smavgs_n = bsxfun(@rdivide,smfix_trg_smavgs,ov_avg_rate);

n_fixs = length(big_sacs);
bifix_trg_mat = nan(n_used_cells,n_fixs,length(lags));
bifix_trg_smmat = nan(n_used_cells,n_fixs,length(lags));
for i = 1:length(sm_sacs)
    cur_set = (all_fix_start_inds(big_sacs(i))-backwin):all_fix_stop_inds(big_sacs(i));
    cur_set(length(lags)+1:end) = [];
    
    bifix_trg_mat(:,i,1:length(cur_set)) = spikes_binned(:,cur_set);    
    bifix_trg_smmat(:,i,1:length(cur_set)) = spikes_smbinned(:,cur_set);    
end
bifix_trg_avgs = squeeze(nanmean(bifix_trg_mat,2));
bifix_trg_smavgs = squeeze(nanmean(bifix_trg_smmat,2));
bifix_trg_avgs_n = bsxfun(@rdivide,bifix_trg_avgs,ov_avg_rate);
bifix_trg_smavgs_n = bsxfun(@rdivide,bifix_trg_smavgs,ov_avg_rate);

sac_sz_diff = bifix_trg_smavgs_n-smfix_trg_smavgs_n;
