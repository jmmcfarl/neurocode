...
for c = 1:9
    ...
    %     lambdas = [0.25 0.3 0.4 0.6 0.85 1.2]*Fsd;
    %     sigmas = [0.5];
    phases = [0 pi/2 pi 3*pi/2];
    lambdas = [0.25 0.4 0.6 0.9]*Fsd;
    sigmas = [0.5];
%     dxs = [-0.05 0 0.05]*Fsd;
%     dys = [-0.05 0 0.05]*Fsd;
    dxs = [0]*Fsd;
    dys = [0]*Fsd;
    n_gabors = length(orientations)*length(phases)*length(lambdas)*length(sigmas)*length(dxs)*length(dys);
