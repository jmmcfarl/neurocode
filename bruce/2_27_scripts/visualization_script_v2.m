...
for blockid = 2;
    ...
    %% indices of fixation start/stop relative to eye-tracking data and LFP
    %% data
    ...
    all_fix_start_lfpinds = round(interp1(lfp_timed,1:length(lfp_timed),all_fix_start_times(cur_block_fixs)));
    all_fix_stop_lfpinds = round(interp1(lfp_timed,1:length(lfp_timed),all_fix_stop_times(cur_block_fixs)));
    
    %% COMPUTE GAMMA POWER WITHIN EACH LATE FIXATION WINDOW
    %SOME PROBLEM HERE STILL
    ...
    for i = 1:length(long_enough)
        cur_set = (all_fix_start_lfpinds(long_enough(i))+round(Fsd*0.15)):all_fix_stop_lfpinds(long_enough(i));
        cur_set(round(Fsd*0.1):end) = [];
        gamma_pow(long_enough(i),:) = nanmean(lfp_sampsd_hfamp(cur_set,:));
    end
%     cur_filt_stims = find(all_stim_filtered(cur_block_fixs)==1);
%     gamma_pow(cur_filt_stims,:) = nan;
    usable = find(~isnan(gamma_pow(:,2)));
    [sorted_gamma_pow,gamma_ord] = sort(gamma_pow(usable,2),'descend');
    gamma_ord = usable(gamma_ord);
    ...
    %% CYCLE THROUGH FIXATIONS IN DESIRED ORDER
    cd ~/Data/bruce/2_27_12/ExptA
    for i = 1:length(gamma_ord)
        
        %% load and preprocess image
        fprintf('Image %d of %d\n',all_stim_nums(cur_block_fixs(gamma_ord(i))),n_stims);
        clear filename
        if stimID(all_stim_nums(cur_block_fixs(gamma_ord(i)))) <10
            filename = ['ExptA0000' num2str(stimID(all_stim_nums(cur_block_fixs(gamma_ord(i))))) '.png'];
        elseif stimID(all_stim_nums(cur_block_fixs(gamma_ord(i)))) >=10 && stimID(all_stim_nums(cur_block_fixs(gamma_ord(i)))) <100
            filename = ['ExptA000' num2str(stimID(all_stim_nums(cur_block_fixs(gamma_ord(i))))) '.png'];
        else
            filename = ['ExptA00' num2str(stimID(all_stim_nums(cur_block_fixs(gamma_ord(i))))) '.png'];
        end
        ...
        cur_eye_samps = all_fix_start_inds(gamma_ord(i)):all_fix_stop_inds(gamma_ord(i));
        ...
        figure(1)
        xlim([all_fix_start_times(cur_block_fixs(gamma_ord(i)))-lfp_backwin all_fix_start_times(cur_block_fixs(gamma_ord(i)))+lfp_forwin])
        input('');
    end
