...
sigx = 0.4*Fsd; %sigma for Gaussian
sigy = 0.4*Fsd;

win_x = round(-2*sigx):round(2*sigx);
win_y = round(-2*sigy):round(2*sigy);
...
%         if ~exist(fname,'file')
            ...
            cur_im_decomp(i,:,:,:) = zeros(length(gridY),length(gridX),length(used_freqs));
            for xx = 1:length(gridX)
                for yy = 1:length(gridY)
                    ...
                    cur_im_decomp(i,yy,xx,:) = abs(F(used_freqs));
                end
            end
...
%         spec_mat = zeros(length(used_inds),length(used_gridy),length(used_gridx),length(used_freqs));
        for ii = 1:length(used_inds)
%             spec_mat(ii,:,:,:) = cur_im_decomp(used_gridy+eye_pos_grid(used_inds(ii),2),...
%                 used_gridx+eye_pos_grid(used_inds(ii),1),:);
            cur_set = squeeze(cur_im_decomp(i,used_gridy+eye_pos_grid(used_inds(ii),2),...
                used_gridx+eye_pos_grid(used_inds(ii),1),:));
    ...
    block_avg_pow(blockid,:) = mean(mean(mean(cur_im_decomp(:,used_gridy,used_gridx,:))));
...
%%
save right_eye_powspectra_data spk_trg_avg_sub block_ov_avg block_* freq* used_* grid* used*


%%
block_ov_avg_norm = bsxfun(@rdivide,block_ov_avg,reshape(mean(block_avg_pow),[1 1 1 length(used_freqs)]));
block_spktrg_avg_norm = bsxfun(@rdivide,block_spk_trg_avg,reshape(mean(block_avg_pow),[1 1 1 1 length(used_freqs)]));
block_spktrg_avg_norm_sub = bsxfun(@minus,block_spktrg_avg_norm,reshape(block_ov_avg_norm,[4 1 length(used_gridy) length(used_gridx) length(used_freqs)]));
%%
