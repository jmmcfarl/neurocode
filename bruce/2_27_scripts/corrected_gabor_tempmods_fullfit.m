...
for c = 1:n_used_cells
    ...
    sac_thresh = 1;
    min_sac_amp = 0.5;
    micro_sacs = find(all_sac_amps(used_fixs) < sac_thresh & all_sac_amps(used_fixs) > min_sac_amp);
    macro_sacs = find(all_sac_amps(used_fixs) >= sac_thresh);
    micro_set = tr_inds(ismember(all_model_fixids(tr_inds),micro_sacs));
    macro_set = tr_inds(ismember(all_model_fixids(tr_inds),macro_sacs));
    
%     un_spk_cnts = length(unique(spikes_binned(micro_set,c))) - 1;
%     micro_spkbs = [];
%     for i = 1:un_spk_cnts
%         curset = find(spikes_binned(micro_set,c) == i);
%         micro_spkbs = [micro_spkbs; repmat(curset,i,1)];
%     end
%     un_spk_cnts = length(unique(spikes_binned(macro_set,c))) - 1;
%     macro_spkbs = [];
%     for i = 1:un_spk_cnts
%         curset = find(spikes_binned(macro_set,c) == i);
%         macro_spkbs = [macro_spkbs; repmat(curset,i,1)];
%     end
...
%     llist = [l1_pen (length(tax)+1):size(Xmat,2)];
%     [fitp,grad] = GLMsolve_jmm( Xmat(micro_set,:), micro_spkbs, W0, 1, lamrange, lamrange2,Pcon, Pmono, llist, [], NLtype,1e-5,nlxrange);
%     stim_mod_const_micro(c) = fitp.k(end);
%     stim_mod_sachist_micro(c,:) = fitp.k(1:length(tax));
%     stim_mod_tot_micro(c,:) = fitp.k(length(tax)+1:length(tax)*2);
%     
%     llist = [l1_pen (length(tax)+1):size(Xmat,2)];
%     [fitp,grad] = GLMsolve_jmm( Xmat(macro_set,:), macro_spkbs, W0, 1, lamrange, lamrange2,Pcon, Pmono, llist, [], NLtype,1e-5,nlxrange);
%     stim_mod_const_macro(c) = fitp.k(end);
%     stim_mod_sachist_macro(c,:) = fitp.k(1:length(tax));
%     stim_mod_tot_macro(c,:) = fitp.k(length(tax)+1:length(tax)*2);
...
%         Xmat = Tmat;
%         Xmat = [Xmat bsxfun(@times,Tmat,total_out)];
% %         Xmat = [Xmat bsxfun(@times,Tmat,mean_out)];
%         Xmat = [Xmat bsxfun(@times,Tmat,cont_out)];
% %         Xmat = [Xmat bsxfun(@times,Tmat,loc_mean)];
%         Xmat = [Xmat bsxfun(@times,Tmat,loc_cont)];
%                 llist = [];
%     [fitp,grad] = GLMsolve_jmm( Xmat(tr_inds,:), spkbs, W0, 1, lamrange, lamrange2,Pcon, Pmono, llist, [], NLtype,1e-5,nlxrange);
%         sep_mod_const(c) = fitp.k(end);
%         sep_mod_sachist(c,:) = fitp.k(1:length(tax));
%         sep_mod_stim(c,:) = fitp.k(length(tax)+1:length(tax)*2);
% %         sep_mod_mean(c,:) = fitp.k(length(tax)*2+1:length(tax)*3);
%         sep_mod_cont(c,:) = fitp.k(length(tax)*2+1:length(tax)*3);
% %         sep_mod_locmean(c,:) = fitp.k(length(tax)*4+1:length(tax)*5);
%         sep_mod_loccont(c,:) = fitp.k(length(tax)*3+1:length(tax)*4);
%         cur_genfun = Xmat(tr_inds,:)*fitp.k(1:end-1) + sep_mod_const(c);
%         cur_predrate = log(1+exp(cur_genfun));
%         sep_mod_LL(c) = sum(-Robs.*log(cur_predrate) + cur_predrate)/sum(Robs);
% %         cur_genfun_xv = Xmat(xv_inds,:)*fitp.k(1:end-1) + sep_mod_const(c);
% %         cur_predrate_xv = log(1+exp(cur_genfun_xv));
% %         sep_mod_xvLL(c) = sum(-Robs_xv.*log(cur_predrate_xv) + cur_predrate_xv)/sum(Robs_xv);
%     
