...
for i = 1:NT
    for n = 1:n_used_cells
        gabor_phase1 = conv2(squeeze(Xmat_resh(i,:,:)),squeeze(gabor_bank(n,1,:,:)),'same');
        gabor_phase2 = conv2(squeeze(Xmat_resh(i,:,:)),squeeze(gabor_bank(n,2,:,:)),'same');
...
sigma = 4;
dist_pen = exp(-dist_mat.^2/(2*sigma.^2));
dist_pen = dist_pen./sum(dist_pen);
for i = 1:K
    A(i,:) = dist_pen;
end
...
for n = 1:n_used_cells
    ...
    temp = squeeze(R2vals(n,poss_vals,poss_vals));
    [best_R2(n),best_loc(n)] = max(temp(:));
    [best_loc_i(n),best_loc_j(n)] = ind2sub([length(poss_vals) length(poss_vals)],best_loc(n));
end
best_loc_i = poss_vals(best_loc_i);
best_loc_j = poss_vals(best_loc_j);
