...
%     lfp_samps_alpha = filtfilt(b_alpha,a_alpha,lfp_samps);
%     lfp_samps_alpha_ph = angle(hilbert(lfp_samps_alpha));
%     lfp_samps_alpha = downsample(lfp_samps_alpha,dsf);
%     lfp_samps_alpha_ph = downsample(lfp_samps_alpha_ph,dsf);
%     lfp_samps_alpha = zscore(lfp_samps_alpha);
...
%     fix_trig_lfpalpha_mat = nan(24,cur_n_fixs,length(lags));
%     fix_trig_lfpalpha_ph_mat = nan(24,cur_n_fixs,length(lags));
%     fix_trig_lfp_mat_p = nan(24,cur_n_fixs,length(lags));
%     fix_trig_lfphf_mat_p = nan(24,cur_n_fixs,length(lags));
%     fix_trig_lfpgam_mat_p = nan(24,cur_n_fixs,length(lags));
%     fix_trig_lfpalpha_mat_p = nan(24,cur_n_fixs,length(lags));
%     fix_trig_lfpalpha_ph_mat_p = nan(24,cur_n_fixs,length(lags));
    for i = 1:cur_n_fixs
...
%         fix_trig_lfpalpha_mat(:,i,1:length(cur_set)) = lfp_samps_alpha(cur_set,:)';
%         fix_trig_lfpalpha_ph_mat(:,i,1:length(cur_set)) = lfp_samps_alpha_ph(cur_set,:)';
        
%         if i > 1
%             cur_set = (cur_fix_stop_inds(i-1)-backlag):(cur_fix_stop_inds(i-1)+forwardlag);
%             fix_trig_lfp_mat_p(:,i,1:length(cur_set)) = lfp_sampsd(cur_set,:)';
%             fix_trig_lfphf_mat_p(:,i,1:length(cur_set)) = lfp_samps_hf(cur_set,:)';
%             fix_trig_lfpgam_mat_p(:,i,1:length(cur_set)) = lfp_samps_gam(cur_set,:)';
% %             fix_trig_lfpalpha_mat_p(:,i,1:length(cur_set)) = lfp_samps_alpha(cur_set,:)';
% %             fix_trig_lfpalpha_ph_mat_p(:,i,1:length(cur_set)) = lfp_samps_alpha_ph(cur_set,:)';
%         end
    end
...
%     all_lfpalpha_mat = cat(2,all_lfpalpha_mat,fix_trig_lfpalpha_mat);
%     all_lfpalpha_ph_mat = cat(2,all_lfpalpha_ph_mat,fix_trig_lfpalpha_ph_mat);
    
%     all_lfp_mat_p = cat(2,all_lfp_mat_p,fix_trig_lfp_mat_p);
%     all_lfphf_mat_p = cat(2,all_lfphf_mat_p,fix_trig_lfphf_mat_p);
%     all_lfpgam_mat_p = cat(2,all_lfpgam_mat_p,fix_trig_lfpgam_mat_p);
%     all_lfpalpha_mat_p = cat(2,all_lfpalpha_mat_p,fix_trig_lfpalpha_mat_p);
%     all_lfpalpha_ph_mat_p = cat(2,all_lfpalpha_ph_mat_p,fix_trig_lfpalpha_ph_mat_p);
