...
if bar_ori == 0
    ...
    orig_anal_name = 'monoc_eyecorr_hbar';
    anal_name = 'monoc_eyecorr_hbar_static';
else
    mod_data_name = 'monoc_eyecorr_vbar_mods';
    orig_anal_name = 'monoc_eyecorr_vbar';
    anal_name = 'monoc_eyecorr_vbar_static';
end
...
use_sl_trials = find(all_trial_sl == 0);
static_trials = find(all_trial_sl == 50);
used_static_inds = used_inds(ismember(all_trialvec(used_inds),static_trials));
% used_inds = used_inds(ismember(all_trialvec(used_inds),use_sl_trials));
used_inds = used_inds(ismember(all_trialvec(used_inds),static_trials));
NT = length(used_inds);
...
par_thresh = 4;
orth_thresh = 1;
[out_of_range] = detect_bad_fixation(corrected_eye_vals_interp,all_trialvec,used_inds,par_thresh,orth_thresh,[0 1]);
...
all_stim_flips = [];
poss_flip_times = 0:0.5:4;
for ii = 1:length(poss_flip_times)
    all_stim_flips = cat(1,all_stim_flips,1+find(all_tsince_start(used_inds(1:end-1)) < poss_flip_times(ii) & all_tsince_start(used_inds(2:end)) >= poss_flip_times(ii)));
end
Xstim = zeros(NT,n_sac_bins);
for ii = 1:n_sac_bins
    cur_target = all_stim_flips + sac_bincents(ii);
    uu = find(cur_target > 1 & cur_target < NT);
    cur_target = cur_target(uu);
    Xstim(cur_target,ii) = 1;
end
...
for ss = 1:length(tr_set);
    ...
    cur_mod = dit_mods{end}(cur_cell);
    cur_mod.stim_params = fin_stim_params;
    cur_mod = NMMadd_NLinput(cur_mod,'lin',1,5);
    cur_mod = NMMadjust_regularization(cur_mod,7,'lambda_d2T',sac_d2t);
    cur_mod = NMMfit_filters(cur_mod,Robs_mat(cur_tr_uset,cur_unit_ind),tr_X,[],4:7,silent);
    init_mod(cur_cell) = cur_mod;
    init_mod_spkNL(cur_cell) = NMMfit_logexp_spkNL(cur_mod,Robs_mat(cur_tr_uset,cur_unit_ind),tr_X);
end
...
clear it_fix_post_*
it_mods{1} = init_mod;
it_mods_spkNL{1} = init_mod_spkNL;
...
for nn = 1:n_fix_inf_it
    ...
    if use_sac_kerns
        ...
        stim_kerns = nan(n_tr_chs,n_sac_bins);
    end
    ...
    for ss = 1:n_tr_chs
        ...
        if use_sac_kerns
            ...
            stim_kerns(ss,:) = it_mods{nn}(tr_set(ss)).mods(cur_Xtargs == 5).filtK;
    end
    ...
    for ss = 1:length(tr_set)
        ...
        it_mods{nn+1}(cur_cell) = NMMfit_filters(it_mods{nn+1}(cur_cell),Robs_mat(cur_tr_uset,cur_unit_ind),...
            tr_X,[],4:7,silent); %fit stimulus filters
...
for xv = 1:length(su_inds)
    ...
    LL_uncor_simsac(xv) = NMMmodel_eval(it_mods_spkNL{1}(cur_cell),Robs_mat(cur_tr_uset,cur_unit_ind),tr_X);
    LL_uncor_simsac(xv) = LL_uncor_simsac(xv) - null_LL(tr_set(su_inds(xv)));
    
end

fname = [anal_dir anal_name '_simsacimps'];
save(fname,'LL_cor_simsac','LL_uncor_simsac');
%%
