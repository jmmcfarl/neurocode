...
%JMM changed for compatibility 7-4-13
NSx.MetaTags = struct('FileTypeID',[],'Label',[],'ChannelCount',[],'SamplingFreq',[], 'Resolution', [], ...
    'ChannelID',[],'DateTime',[],'CreateDateTime',[], 'Comment', [], 'FileSpec', [], ...
    'Timestamp', [], 'DataPoints', [], 'openNSxver', [], 'Filename', [], 'FilePath', [], ...
    'FileExt', [], 'HeaderOffset', []);
...
%     NSx.MetaTags.TimeRes       = 30000;
    NSx.MetaTags.Resolution       = 30000; %7-4-13 JMM
...
%     NSx.MetaTags.SamplingLabel = char(BasicHeader(7:22))';
    NSx.MetaTags.Label = char(BasicHeader(7:22))'; %7-4-13 JMM
    ...
    %% ADDED BY JMM 7-4-13 for compatibility
    NSx.MetaTags.ElecLabel = char(zeros(96,16));
    for ch = 1:ChannelCount
        maxDig = double([NSx.ElectrodesInfo(ch).MaxDigiValue]);
        maxAna = double([NSx.ElectrodesInfo(ch).MaxAnalogValue]);
        AnalogUnits = NSx.ElectrodesInfo(ch).AnalogUnits;
        
        if (strcmpi(AnalogUnits(1:2), 'uV')) % micro volts
            NSx.MetaTags.VoltsPerDigit(ch) = maxAna / maxDig * 1e-6;
        elseif (strcmpi(AnalogUnits(1:2), 'mV')) % milli volts
            NSx.MetaTags.VoltsPerDigit(ch) = maxAna / maxDig * 1e-3;
        else
            disp('Wrong unit');
            fclose(FID);
            clear variables;
        ...
        NSx.MetaTags.ElecLabel(ch,:) = NSx.ElectrodesInfo(ch).Label;
    end
...
%     NSx.MetaTags.DateTimeRaw = t.';
    NSx.MetaTags.CreateDateTime = t.'; %7-4-13 JMM
