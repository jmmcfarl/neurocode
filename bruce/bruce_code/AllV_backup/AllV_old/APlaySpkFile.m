...
%                 Expt.Stimvals.(fn{nf}) = prctile([AllTrials.(fn{nf})(igood)],50);
                Expt.Stimvals.(fn{nf}) = my_prctile([AllTrials.(fn{nf})(igood)],50);
            elseif isnumeric(nv)
        ...
        if isfield(AllTrials,'Stimseq') && length(AllTrials.Stimseq{igood(k)}) > 1 && ...
            ...
            if isfield(AllTrials,'Phaseseq') & length(AllTrials.Phaseseq{igood(k)}) > 0
                ...
                Trials(k).rph = rphasevals(AllTrials.Phaseseq{igood(k)}+1);
                    ...
                    %hack fix by James to set bad phase values to NAN
                    Trials(k).ph = nan(size(AllTrials.Phaseseq{igood(k)}));
                    uset = AllTrials.Phaseseq{igood(k)}+1 <= length(phasevals);
                    Trials(k).ph(uset) = phasevals(AllTrials.Phaseseq{igood(k)}(uset)+1);
