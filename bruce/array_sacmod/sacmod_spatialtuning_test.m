...
for ss = 1:length(su_probes)
   %% 
    ...
    if ~isempty(cur_used_inds)
        ...
        reg_params = NMMcreate_reg_params('lambda_d2XT',[25;25;0],'boundary_conds',[Inf 0;Inf 0;0 0],...
            'mixing_prop',[0.1 1;0.1 1;0 0],'lambda_d2T',[0;0;100]);
        su_curfit(ss) = NMMinitialize_model(stim_params,[1 1 1],{'lin','lin','lin'},reg_params,[1 2 3]);
        su_curfit(ss) = NMMfit_filters(su_curfit(ss),Robs,cur_X,[],[],silent);
      
        su_curfit(ss) = NMMadjust_regularization(su_curfit(ss),[1 2],'lambda_L1',[2;2]);
        su_curfit(ss) = NMMfit_filters(su_curfit(ss),Robs,cur_X,[],[],silent);
...
%     cur_used_blocks = cur_used_blocks(ismember(cur_used_blocks,cur_blocks)); %blocks when SU
    ...
    if ~isempty(cur_used_inds)
        ...
        mu_curfit(ss) = NMMfit_filters(mu_curfit(ss),Robs,cur_X,[],[],silent);
      
        mu_curfit(ss) = NMMadjust_regularization(mu_curfit(ss),[1 2],'lambda_L1',[2;2]);
        mu_curfit(ss) = NMMfit_filters(mu_curfit(ss),Robs,cur_X,[],[],silent);
...
%         tuning_width_fun = sum(bsxfun(@times,temp3,xlocs'));
    ...
    plot(mu_trig_avg(ss,:));
    subplot(2,2,4)
    plot(mu_curfit(ss).mods(3).filtK);
    end
    pause
end
