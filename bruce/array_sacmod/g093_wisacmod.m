...
for cc = 1:96
    all_spike_rate(:,cc) = jmm_smooth_1d_cor(all_binned_spikes(:,cc),sm_sig);
end
...
gsac_trial_nums = all_trialvec(gsac_inds);
msac_trial_nums = all_trialvec(msac_inds);
simsac_trial_nums = all_trialvec(all_sim_sacs);
...
for cc = 1:96
    ...
    if ~isempty(su_probe_ind)
        ...
        cur_use_inds = used_inds(ismember(all_blockvec(used_inds),cur_used_blocks));
    else
        cur_use_inds = used_inds;
    end
    
    %general averages
    [mua_msac_avgs(cc,:),lags] = get_event_trig_avg(all_spike_rate_norm(:,cc),interp_sac_start_inds(used_msacs),backlag,forwardlag,nboot,used_trialvec,0,cur_use_inds);
    [mua_gsac_avgs(cc,:),lags] = get_event_trig_avg(all_spike_rate_norm(:,cc),interp_sac_start_inds(used_gsacs),backlag,forwardlag,nboot,used_trialvec,0,cur_use_inds);
    [mua_simsac_avgs(cc,:),lags] = get_event_trig_avg(all_spike_rate_norm(:,cc),all_sim_sacs,backlag,forwardlag,nboot,used_trialvec,0,cur_use_inds);
            
    %wi specific
    for ww = 1:3
        cur_trialset = find(all_trial_wi == un_wi_vals(ww));
        cur_trial_inds = cur_use_inds(ismember(all_trialvec(cur_use_inds),cur_trialset));
        mua_trialset_avg(cc,ww) = mean(all_binned_spikes(cur_trial_inds,cc));
        mua_trialset_norm_avg(cc,ww) = mean(all_spike_rate_norm(cur_trial_inds,cc));
        cur_msacs = msac_inds(ismember(msac_trial_nums,cur_trialset));
        cur_gsacs = gsac_inds(ismember(gsac_trial_nums,cur_trialset));
        cur_simsacs = all_sim_sacs(ismember(simsac_trial_nums,cur_trialset));
        [mua_msac_wi_avgs(cc,ww,:),lags] = get_event_trig_avg(all_spike_rate_norm(:,cc),cur_msacs,backlag,forwardlag,nboot,used_trialvec,0,cur_use_inds);
        [mua_gsac_wi_avgs(cc,ww,:),lags] = get_event_trig_avg(all_spike_rate_norm(:,cc),cur_gsacs,backlag,forwardlag,nboot,used_trialvec,0,cur_use_inds);
        [mua_simsac_wi_avgs(cc,ww,:),lags] = get_event_trig_avg(all_spike_rate_norm(:,cc),cur_simsacs,backlag,forwardlag,nboot,used_trialvec,0,cur_use_inds);
    end
...
figure
shadedErrorBar(lags*dt,squeeze(nanmean(mua_gsac_wi_avgs(:,1,:))),squeeze(nanstd(mua_gsac_wi_avgs(:,1,:)))/sqrt(96),{'color','b'});
hold on
shadedErrorBar(lags*dt,squeeze(nanmean(mua_gsac_wi_avgs(:,2,:))),squeeze(nanstd(mua_gsac_wi_avgs(:,2,:)))/sqrt(96),{'color','r'});
shadedErrorBar(lags*dt,squeeze(nanmean(mua_gsac_wi_avgs(:,3,:))),squeeze(nanstd(mua_gsac_wi_avgs(:,3,:)))/sqrt(96),{'color','k'});

figure
shadedErrorBar(lags*dt,squeeze(nanmean(mua_msac_wi_avgs(:,1,:))),squeeze(nanstd(mua_msac_wi_avgs(:,1,:)))/sqrt(96),{'color','b'});
hold on
shadedErrorBar(lags*dt,squeeze(nanmean(mua_msac_wi_avgs(:,2,:))),squeeze(nanstd(mua_msac_wi_avgs(:,2,:)))/sqrt(96),{'color','r'});
shadedErrorBar(lags*dt,squeeze(nanmean(mua_msac_wi_avgs(:,3,:))),squeeze(nanstd(mua_msac_wi_avgs(:,3,:)))/sqrt(96),{'color','k'});

figure
shadedErrorBar(lags*dt,squeeze(nanmean(mua_simsac_wi_avgs(:,1,:))),squeeze(nanstd(mua_simsac_wi_avgs(:,1,:)))/sqrt(96),{'color','b'});
hold on
shadedErrorBar(lags*dt,squeeze(nanmean(mua_simsac_wi_avgs(:,2,:))),squeeze(nanstd(mua_simsac_wi_avgs(:,2,:)))/sqrt(96),{'color','r'});
shadedErrorBar(lags*dt,squeeze(nanmean(mua_simsac_wi_avgs(:,3,:))),squeeze(nanstd(mua_simsac_wi_avgs(:,3,:)))/sqrt(96),{'color','k'});

mua_gsac_wi_norm_avgs = bsxfun(@rdivide,mua_gsac_wi_avgs,mua_trialset_norm_avg);
mua_msac_wi_norm_avgs = bsxfun(@rdivide,mua_msac_wi_avgs,mua_trialset_norm_avg);
mua_simsac_wi_norm_avgs = bsxfun(@rdivide,mua_simsac_wi_avgs,mua_trialset_norm_avg);

figure
shadedErrorBar(lags*dt,squeeze(nanmean(mua_gsac_wi_norm_avgs(:,1,:))),squeeze(nanstd(mua_gsac_wi_norm_avgs(:,1,:)))/sqrt(96),{'color','b'});
hold on
shadedErrorBar(lags*dt,squeeze(nanmean(mua_gsac_wi_norm_avgs(:,2,:))),squeeze(nanstd(mua_gsac_wi_norm_avgs(:,2,:)))/sqrt(96),{'color','r'});
shadedErrorBar(lags*dt,squeeze(nanmean(mua_gsac_wi_norm_avgs(:,3,:))),squeeze(nanstd(mua_gsac_wi_norm_avgs(:,3,:)))/sqrt(96),{'color','k'});

figure
shadedErrorBar(lags*dt,squeeze(nanmean(mua_msac_wi_norm_avgs(:,1,:))),squeeze(nanstd(mua_msac_wi_norm_avgs(:,1,:)))/sqrt(96),{'color','b'});
hold on
shadedErrorBar(lags*dt,squeeze(nanmean(mua_msac_wi_norm_avgs(:,2,:))),squeeze(nanstd(mua_msac_wi_norm_avgs(:,2,:)))/sqrt(96),{'color','r'});
shadedErrorBar(lags*dt,squeeze(nanmean(mua_msac_wi_norm_avgs(:,3,:))),squeeze(nanstd(mua_msac_wi_norm_avgs(:,3,:)))/sqrt(96),{'color','k'});

figure
shadedErrorBar(lags*dt,squeeze(nanmean(mua_simsac_wi_norm_avgs(:,1,:))),squeeze(nanstd(mua_simsac_wi_norm_avgs(:,1,:)))/sqrt(96),{'color','b'});
hold on
shadedErrorBar(lags*dt,squeeze(nanmean(mua_simsac_wi_norm_avgs(:,2,:))),squeeze(nanstd(mua_simsac_wi_norm_avgs(:,2,:)))/sqrt(96),{'color','r'});
shadedErrorBar(lags*dt,squeeze(nanmean(mua_simsac_wi_norm_avgs(:,3,:))),squeeze(nanstd(mua_simsac_wi_norm_avgs(:,3,:)))/sqrt(96),{'color','k'});
...
for ss = 1:length(su_probes)
    ...
    %general averages
    [sua_msac_avgs(ss,:),lags,sua_msac_sem(ss,:),sua_nused_msac(ss)] = get_event_trig_avg(all_spike_rate_norm(:,su_probes(ss)),interp_sac_start_inds(used_msacs),backlag,forwardlag,nboot,used_trialvec,0,cur_use_inds);
    [sua_gsac_avgs(ss,:),lags,sua_gsac_sem(ss,:),sua_nused_gsac(ss)] = get_event_trig_avg(all_spike_rate_norm(:,su_probes(ss)),interp_sac_start_inds(used_gsacs),backlag,forwardlag,nboot,used_trialvec,0,cur_use_inds);
    [sua_simsac_avgs(ss,:),lags,sua_simsac_sem(ss,:),sua_nused_simsac(ss)] = get_event_trig_avg(all_spike_rate_norm(:,su_probes(ss)),all_sim_sacs,backlag,forwardlag,nboot,used_trialvec,0,cur_use_inds);
    
    %wi specific
    for ww = 1:3
        cur_trialset = find(all_trial_wi == un_wi_vals(ww));
        cur_trial_inds = cur_use_inds(ismember(all_trialvec(cur_use_inds),cur_trialset));
        sua_trialset_avg(ss,ww) = mean(all_binned_spikes(cur_trial_inds,su_probes(ss)));
        sua_trialset_norm_avg(ss,ww) = mean(all_spike_rate_norm(cur_trial_inds,su_probes(ss)));
        cur_msacs = msac_inds(ismember(msac_trial_nums,cur_trialset));
        cur_gsacs = gsac_inds(ismember(gsac_trial_nums,cur_trialset));
        cur_simsacs = all_sim_sacs(ismember(simsac_trial_nums,cur_trialset));
        
        [sua_msac_wi_avgs(ss,ww,:),lags,sua_msac_wi_sem(ss,ww,:)] = ...
            get_event_trig_avg(all_spike_rate_norm(:,su_probes(ss)),cur_msacs,backlag,forwardlag,nboot,used_trialvec,0,cur_use_inds);
        [sua_gsac_wi_avgs(ss,ww,:),lags,sua_gsac_wi_sem(ss,ww,:)] = ...
            get_event_trig_avg(all_spike_rate_norm(:,su_probes(ss)),cur_gsacs,backlag,forwardlag,nboot,used_trialvec,0,cur_use_inds);
        [sua_simsac_wi_avgs(ss,ww,:),lags,sua_simsac_wi_sem(ss,ww,:)] = ...
            get_event_trig_avg(all_spike_rate_norm(:,su_probes(ss)),cur_simsacs,backlag,forwardlag,nboot,used_trialvec,0,cur_use_inds);
    end
    %start and stop trig avgs
    %micro sac burst and non-burst
end
...
save g093_wisacmod_data lags dt sua_* mua_*
%%
close all
for ss = 1:length(su_probes)
    subplot(1,3,1)
    hold on
%     plot(lags*dt,squeeze(sua_gsac_wi_avgs(ss,1,:)),'b')
%     plot(lags*dt,squeeze(sua_gsac_wi_avgs(ss,2,:)),'r')
%     plot(lags*dt,squeeze(sua_gsac_wi_avgs(ss,3,:)),'k')
    plot(lags*dt,squeeze(sua_gsac_wi_avgs(ss,1,:))/sua_trialset_norm_avg(ss,1),'b')
    plot(lags*dt,squeeze(sua_gsac_wi_avgs(ss,2,:))/sua_trialset_norm_avg(ss,2),'r')
    plot(lags*dt,squeeze(sua_gsac_wi_avgs(ss,3,:))/sua_trialset_norm_avg(ss,3),'k')
    xlim([-0.2 0.5])
    
        subplot(1,3,2)
    hold on
%     plot(lags*dt,squeeze(sua_msac_wi_avgs(ss,1,:)),'b')
%     plot(lags*dt,squeeze(sua_msac_wi_avgs(ss,2,:)),'r')
%     plot(lags*dt,squeeze(sua_msac_wi_avgs(ss,3,:)),'k')
    plot(lags*dt,squeeze(sua_msac_wi_avgs(ss,1,:))/sua_trialset_norm_avg(ss,1),'b')
    plot(lags*dt,squeeze(sua_msac_wi_avgs(ss,2,:))/sua_trialset_norm_avg(ss,2),'r')
    plot(lags*dt,squeeze(sua_msac_wi_avgs(ss,3,:))/sua_trialset_norm_avg(ss,3),'k')
    xlim([-0.2 0.5])
    
        subplot(1,3,3)
    hold on
%     plot(lags*dt,squeeze(sua_simsac_wi_avgs(ss,1,:)),'b')
%     plot(lags*dt,squeeze(sua_simsac_wi_avgs(ss,2,:)),'r')
%     plot(lags*dt,squeeze(sua_simsac_wi_avgs(ss,3,:)),'k')
    plot(lags*dt,squeeze(sua_simsac_wi_avgs(ss,1,:))/sua_trialset_norm_avg(ss,1),'b')
    plot(lags*dt,squeeze(sua_simsac_wi_avgs(ss,2,:))/sua_trialset_norm_avg(ss,2),'r')
    plot(lags*dt,squeeze(sua_simsac_wi_avgs(ss,3,:))/sua_trialset_norm_avg(ss,3),'k')
     xlim([-0.2 0.5])
