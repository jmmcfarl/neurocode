...
trial_gsac_mat = zeros(length(all_t_axis),n_sac_bins);
for i = 1:n_sac_bins
    cur_set = find(time_to_gsac == round(sac_bin_cents(i)/dt));
    trial_gsac_mat(cur_set,i) = 1;
end
max_gsac = max(trial_gsac_mat,[],2);
...
for ss = 1:length(su_probes)
   %% 
    ...
    cur_used_inds(max_gsac(used_inds(cur_used_inds)) == 0) = [];
    ...
    if ~isempty(cur_used_inds)
        ...
        sua_data(ss).nblocks = length(cur_used_blocks);
        ...
        spike_cond_stim = all_Xmat_cor(cur_used_inds(spikebins),use_kInds);
        sta = mean(spike_cond_stim) - mean(all_Xmat_cor(cur_used_inds,use_kInds));
        sta = sta/norm(sta);
        %         proj_mat = sta'/(sta*sta')*sta;
        %         stim_proj = all_Xmat_cor(cur_used_inds,:) - all_Xmat_cor(cur_used_inds,:)*proj_mat;
        stim_proj = all_Xmat_cor(cur_used_inds,use_kInds);
        ...
        spike_cond_stim = abs(all_Xmat_cor(cur_used_inds(spikebins),use_kInds));
        abs_sta = mean(spike_cond_stim) - mean(abs(all_Xmat_cor(cur_used_inds,use_kInds)));
        ...
        %%
        cur_sta_a = abs_sta + abs(sta);
        cur_sta_a = reshape(cur_sta_a,flen,use_nPix);
        temp_profile = var(cur_sta_a,[],2);
        [~,su_best_lag(ss)] = max(temp_profile);
        cur_use_kInds = find(ismember(Xinds(:),cur_use_pix) & Tinds(:) == su_best_lag(ss));
        
        clear cur_X
        emb_stim =all_Xmat_cor(cur_used_inds,cur_use_kInds);
        cur_X{1} = emb_stim;
        cur_X{2} = trial_gsac_mat(used_inds(cur_used_inds),:);
        clear emb_stim
        stim_params = NMMcreate_stim_params([use_nPix],dt);
        stim_params(2) = NMMcreate_stim_params([n_sac_bins],dt);
        reg_params = NMMcreate_reg_params('lambda_d2X',[5;5;0],'lambda_d2T',[0;0;100],'boundary_conds',[0;0;Inf]);
        init_fit = NMMinitialize_model(stim_params,[1 1 1],{'lin','quad','lin'},reg_params,[1 1 2]);
        init_fit = NMMfit_filters(init_fit,Robs,cur_X,[],[],0);
        [LL,penLL,init_prate] = NMMmodel_eval(init_fit,Robs,cur_X);
        
        emb_stim = bsxfun(@times,reshape(all_Xmat_cor(cur_used_inds,cur_use_kInds),...
            [length(cur_used_inds) 1 length(cur_use_kInds)]),trial_gsac_mat(used_inds(cur_used_inds),:));
        emb_stim = reshape(emb_stim,size(emb_stim,1),n_sac_bins*use_nPix);
        
        init_lin = repmat(init_fit.mods(1).filtK,1,n_sac_bins)';
        init_quad = repmat(init_fit.mods(2).filtK,1,n_sac_bins)';
%         init_quad2 = repmat(init_fit.mods(3).filtK,1,n_sac_bins)';
        init_sac = init_fit.mods(3).filtK;
        clear cur_X
        cur_X{1} = emb_stim;
        cur_X{2} = trial_gsac_mat(used_inds(cur_used_inds),:);
        clear stim_params
        stim_params = NMMcreate_stim_params([n_sac_bins use_nPix],dt);
        stim_params(2) = NMMcreate_stim_params([n_sac_bins],dt);
%         reg_params = NMMcreate_reg_params('lambda_d2XT',[100;20;0],'mixing_prop',[1 0.02;1 0.02;0 0],...
%             'lambda_d2T',[0;0;100],'boundary_conds',[0 0;0 0;0 0],'lambda_L2',[1;1;50]);
        reg_params = NMMcreate_reg_params('lambda_dT',[100;100;100],'lambda_d2X',[5;1;0],'boundary_conds',[0 0;0 0;0 0],'lambda_L2',[1;1;50]);
        init_filts{1} = init_lin(:);
        init_filts{2} = init_quad(:);
%         init_filts{3} = init_quad2(:);
        init_filts{3} = init_sac;
        su_curfit(ss) = NMMinitialize_model(stim_params,[1 1 1],{'lin','quad','lin'},reg_params,[1 1 2],init_filts);
        su_curfit(ss).spk_NL_params(1) = init_fit.spk_NL_params(1);
       [LL,penLL,new_prate] = NMMmodel_eval(su_curfit(ss),Robs,cur_X);
       
       optim_p.optTol = 1e-4;
        su_curfit(ss) = NMMfit_filters(su_curfit(ss),Robs,cur_X,[],[],0,optim_p);
        
        %         su_curfit(ss) = NMMadjust_regularization(su_curfit(ss),1,'lambda_d2XT',10);
        %         su_curfit(ss) = NMMfit_filters(su_curfit(ss),Robs,cur_X,[],[],0);
        
        su_trig_avg(ss,:) = nan(1,n_sac_bins);
        for gg = 1:n_sac_bins
            curset = find(trial_gsac_mat(used_inds(cur_used_inds),gg) == 1);
            su_trig_avg(ss,gg) = mean(Robs(curset));
        end
    end
end
...
%     cur_used_blocks = cur_used_blocks(ismember(cur_used_blocks,cur_blocks)); %blocks when SU
    ...
    if ~isempty(cur_used_inds)
        ...
        mua_data(ss).nblocks = length(cur_used_blocks);
        cur_n_gsacs = sum(ismember(gsac_start_inds,cur_used_inds));
        cur_n_msacs = sum(ismember(gsac_start_inds,cur_used_inds));
        mua_data(ss).nsacs = [cur_n_gsacs cur_n_msacs];
        ...
        mua_data(ss).evals = evals;
        mua_data(ss).abs_sta = abs_sta;
        cur_eval_diff = diff(flipud(mua_data(ss).evals));
        mua_data(ss).npos_stc = min(10,find(cur_eval_diff < stc_thresh,1,'last'));
        mua_data(ss).nneg_stc = min(10,find(cur_eval_diff(end:-1:1) < stc_thresh,1,'last'));
        
        %%
        cur_sta_a = abs_sta + abs(sta);
        cur_sta_a = reshape(cur_sta_a,flen,use_nPix);
        temp_profile = var(cur_sta_a,[],2);
        [~,mu_best_lag(ss)] = max(temp_profile);
        cur_use_kInds = find(ismember(Xinds(:),cur_use_pix) & Tinds(:) == mu_best_lag(ss));
        emb_stim = bsxfun(@times,reshape(all_Xmat_cor(cur_used_inds,cur_use_kInds),...
            [length(cur_used_inds) 1 length(cur_use_kInds)]),trial_gsac_mat(used_inds(cur_used_inds),:));
        emb_stim = reshape(emb_stim,size(emb_stim,1),n_sac_bins*use_nPix);
        cur_X{1} = emb_stim;
        cur_X{2} = abs(emb_stim);
        cur_X{3} = trial_gsac_mat(used_inds(cur_used_inds),:);
        clear emb_stim
        stim_params = NMMcreate_stim_params([n_sac_bins use_nPix],dt);
        stim_params(2) = NMMcreate_stim_params([n_sac_bins use_nPix],dt);
        stim_params(3) = NMMcreate_stim_params([n_sac_bins],dt);
        reg_params = NMMcreate_reg_params('lambda_d2XT',[100;100;0],'boundary_conds',[Inf 0;Inf 0;0 0],...
            'mixing_prop',[0.1 1;0.1 1;0 0],'lambda_d2T',[0;0;100]);
        mu_curfit(ss) = NMMinitialize_model(stim_params,[1 1 1],{'lin','lin','lin'},reg_params,[1 2 3]);
%         mu_curfit(ss) = NMMadjust_regularization(mu_curfit(ss),3,'lambda_d2T',100);
%         mu_curfit(ss) = NMMinitialize_model(stim_params,[1 1],{'lin','lin'},reg_params,[1 2]);
        mu_curfit(ss) = NMMfit_filters(mu_curfit(ss),Robs,cur_X,[],[],0);
        
        mu_trig_avg(ss,:) = nan(1,n_sac_bins);
        for gg = 1:n_sac_bins
           curset = find(trial_gsac_mat(used_inds(cur_used_inds),gg) == 1);
           mu_trig_avg(ss,gg) = mean(Robs(curset));
        end
    end
end
...
for ss = 1:length(su_probes)
    ...
    cur_sta2 = sua_data(ss).abs_sta; ca = max(abs(cur_sta2));
    subplot(3,3,2)
    imagesc(reshape(cur_sta2,flen,use_nPix)); caxis([-ca ca]);
    cur_sta_a = abs(cur_sta) + cur_sta2;
    ca = max(abs(cur_sta_a));
    subplot(3,3,3)
    imagesc(reshape(cur_sta_a,flen,use_nPix)); caxis([-ca ca]);
    cur_stcs = sua_data(ss).stcs; ca = max(abs(cur_stcs(:)));
    for ii = 1:3
        subplot(3,3,3+ii)
    ...
    fprintf('Lag %d\n',su_best_lag(ss));
    figure(f2); clf
    temp = reshape(su_curfit(ss).mods(1).filtK,n_sac_bins,use_nPix)';
    temp2 = reshape(su_curfit(ss).mods(2).filtK,n_sac_bins,use_nPix)';
    temp3 = log(1+exp(temp2 + su_curfit(ss).spk_NL_params(1)));
    temp3 = bsxfun(@rdivide,temp3,sum(temp3));
    tuning_loc_fun = sum(bsxfun(@times,temp3,xlocs'));
    tuning_width_fun = sqrt(sum(bsxfun(@times,temp3,(repmat(tuning_loc_fun,use_nPix,1)-repmat(xlocs',1,n_sac_bins)).^2)));
    ...
    plot(su_trig_avg(ss,:));
    subplot(2,2,4)
    plot(su_curfit(ss).mods(3).filtK);
...
for ss = 1:96
    if ~isempty(mua_data(ss).avg_rate)
        ...
        cur_sta2 = mua_data(ss).abs_sta; ca = max(abs(cur_sta2));
        subplot(3,3,2)
        imagesc(reshape(cur_sta2,flen,use_nPix)); caxis([-ca ca]);
        cur_sta_a = abs(cur_sta) + cur_sta2;
        ca = max(abs(cur_sta_a));
        ...
        fprintf('Lag %d\n',mu_best_lag(ss));
        figure(f2); clf
        temp = reshape(mu_curfit(ss).mods(1).filtK,n_sac_bins,use_nPix)';
        temp2 = reshape(mu_curfit(ss).mods(2).filtK,n_sac_bins,use_nPix)';
        
        temp3 = log(1+exp(temp2 + mu_curfit(ss).spk_NL_params(1)));
        temp3 = bsxfun(@rdivide,temp3,sum(temp3));
        tuning_width_fun = sum(bsxfun(@times,temp3,xlocs'));
