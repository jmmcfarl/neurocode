...
all_Xmat_cor = create_time_embedding(all_stim_mat_cor,init_stim_params);
...
for ss = 1:length(su_probes)
    ...
    if ~isempty(cur_used_inds)
        ...
        %fit GLM
        g_sta = all_Xmat_cor(cur_used_inds,:)*sua_data(ss).sta';
        ...
        for gg = 1:sua_data(ss).npos_stc
            g_posstc(:,gg) = all_Xmat_cor(cur_used_inds,:)*sua_data(ss).stcs(:,gg);
        end
        for gg = 1:sua_data(ss).nneg_stc
            g_negstc(:,gg) =  all_Xmat_cor(cur_used_inds,:)*sua_data(ss).stcs(:,end-nneg+gg);
        end
