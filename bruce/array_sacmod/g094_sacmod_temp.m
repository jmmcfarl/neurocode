...
Expt_name = sprintf('G0%d',Expt_nums);
...
for ee = 1:length(cur_block_set);
    if ismember(ee,grayback_gs_expts)
        fprintf('Expt %d Block %d of %d; grayback GS, ori:%d\n',Expt_nums,ee,length(cur_block_set),expt_bar_ori(cur_block_set(ee)));
    elseif ismember(ee,imback_gs_expts)
        fprintf('Expt %d Block %d of %d; imback GS, ori:%d\n',Expt_nums,ee,length(cur_block_set),expt_bar_ori(cur_block_set(ee)));
    elseif ismember(ee,sim_sac_expts)
        fprintf('Expt %d Block %d of %d; SimSac, ori:%d\n',Expt_nums,ee,length(cur_block_set),expt_bar_ori(cur_block_set(ee)));
    else
        fprintf('Expt %d Block %d of %d;  UNMATCHED EXPT TYPE\n',Expt_nums,ee,length(cur_block_set));
    end
...
%         all_spk_times{cc} = cat(1,all_spk_times{cc},ClusterDetails{cc}.t(ClusterDetails{cc}.clst==2)');
...
%     [mua_data(cc).simsac_avg,lags,mua_data(cc).simsac_sem] = get_event_trig_avg(all_spike_rate_norm(:,cc),all_sim_sacs,backlag,forwardlag,nboot,used_trialvec,0,cur_used_inds);
%     [mua_data(cc).sim_msac_avg,lags,mua_data(cc).sim_msac_sem] = get_event_trig_avg(all_spike_rate_norm(:,cc),all_sim_msacs,backlag,forwardlag,nboot,used_trialvec,0,cur_used_inds);
%     [mua_data(cc).sim_gsac_avg,lags,mua_data(cc).sim_gsac_sem] = get_event_trig_avg(all_spike_rate_norm(:,cc),all_sim_gsacs,backlag,forwardlag,nboot,used_trialvec,0,cur_used_inds);
    %general averages
...
%     [mua_data(cc).sim_msac_avg,lags] = get_event_trig_avg(all_spike_rate_norm(:,cc),all_sim_msacs,backlag,forwardlag,nboot,used_trialvec,0,cur_used_inds);
%     [mua_data(cc).sim_gsac_avg,lags] = get_event_trig_avg(all_spike_rate_norm(:,cc),all_sim_gsacs,backlag,forwardlag,nboot,used_trialvec,0,cur_used_inds);
...
for cc = 79
    cc
    shadedErrorBar(lags*dt,mua_data(cc).sim_msac_avg,mua_data(cc).sim_msac_sem);
    hold on
    shadedErrorBar(lags*dt,mua_data(cc).sim_gsac_avg,mua_data(cc).sim_gsac_sem,{'color','r'});
    xlim([-0.2 0.4]);
    xl = xlim();
...
%%
uset = [2 7 8 10 14 18 26 34 36 65 67 74 76 79 80 84 96];
% uset = [1:96];
figure
% shadedErrorBar(lags*dt,mean(all_sim_sac(:,uset),2),std(all_sim_sac(:,uset),[],2)/sqrt(length(uset)),{'color','k'});
hold on
h1 = shadedErrorBar(lags*dt,mean(all_gsac(:,uset),2),std(all_gsac(:,uset),[],2)/sqrt(length(uset)),{'color','k'});
h2 = shadedErrorBar(lags*dt,mean(all_msac(:,uset),2),std(all_msac(:,uset),[],2)/sqrt(length(uset)),{'color','g'});
h3 = shadedErrorBar(lags*dt,mean(all_sim_gsac(:,uset),2),std(all_sim_gsac(:,uset),[],2)/sqrt(length(uset)),{'color','b'});
h4 = shadedErrorBar(lags*dt,mean(all_sim_msac(:,uset),2),std(all_sim_msac(:,uset),[],2)/sqrt(length(uset)),{'color','r'});
legend([h1.mainLine h2.mainLine h3.mainLine h4.mainLine],{'Guided sacs','Micro sacs','Sim sacs (large)','Sim sacs (micro)'});
% legend([h2.mainLine h3.mainLine],{'Sim sacs (large)','Sim sacs (micro)'});
xlim([-0.15 0.45])
xl = xlim();
line(xl,[1 1],'color','k','linestyle','--');
xlabel('Time since sim sac (s)','fontsize',16);
ylabel('Relative firing rate','fontsize',16);

%%
all_gsac = reshape([mua_data(:).gsac_avg],length(lags),96);
all_msac = reshape([mua_data(:).msac_avg],length(lags),96);
% all_sim_sac = reshape([mua_data(:).simsac_avg],length(lags),96);
all_sim_msac = reshape([mua_data(:).sim_msac_avg],length(lags),96);
all_sim_gsac = reshape([mua_data(:).sim_gsac_avg],length(lags),96);

figure
% shadedErrorBar(lags*dt,mean(all_sim_sac,2),std(all_sim_sac,[],2)/sqrt(96),{'color','k'});
hold on
shadedErrorBar(lags*dt,mean(all_gsac,2),std(all_gsac,[],2)/sqrt(96),{'color','g'});
shadedErrorBar(lags*dt,mean(all_sim_gsac,2),std(all_sim_gsac,[],2)/sqrt(96),{'color','b'});
shadedErrorBar(lags*dt,mean(all_sim_msac,2),std(all_sim_msac,[],2)/sqrt(96),{'color','r'});
xl = xlim();
line(xl,[1 1],'color','k','linestyle','--');
xlabel('Time since sim sac (s)','fontsize',16);
