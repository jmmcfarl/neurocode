...
for ss = 1:length(su_probes)
    ...
    if ~isempty(cur_used_inds)
        ...
        sta = mean(spike_cond_stim) - mean(Xmat_sh(cur_used_inds,:));
        sta = sta/norm(sta);
        ...
        %fit GLM
        g_sta = Xmat_sh(cur_used_inds,:)*sua_data(ss).sta';
        ...
        for gg = 1:sua_data(ss).npos_stc
            g_posstc(:,gg) = Xmat_sh(cur_used_inds,:)*sua_data(ss).stcs(:,gg);
        end
        for gg = 1:sua_data(ss).nneg_stc
            g_negstc(:,gg) =  Xmat_sh(cur_used_inds,:)*sua_data(ss).stcs(:,end-nneg+gg);
        end
...
%         %FIT NMM
%         cur_nsq_filts = min(max_squared_filts,sua_data(ss).npos_stc);
...
for cc = 1:96
    ...
    if ~isempty(cur_used_inds)
        ...
        %fit GLM
        g_sta = Xmat_sh(cur_used_inds,:)*mua_data(cc).sta';
        ...
        for gg = 1:mua_data(cc).npos_stc
            g_posstc(:,gg) = Xmat_sh(cur_used_inds,:)*mua_data(cc).stcs(:,gg);
        end
        for gg = 1:mua_data(cc).nneg_stc
            g_negstc(:,gg) =  Xmat_sh(cur_used_inds,:)*mua_data(cc).stcs(:,end-nneg+gg);
        end
        cur_Xmat = [g_sta g_posstc.^2 g_negstc.^2 Xblock(cur_used_inds,:)];
        [fitmod] = regGLM_fit(cur_Xmat,Robs,[],[],[],[],1);
        [~, ~, ~, G] = regGLM_eval(fitmod,Robs,cur_Xmat);
