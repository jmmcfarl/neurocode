...
% for ex = 3
    ...
    wi_trials = cell(length(un_wi_vals),1);
    for i = 1:length(un_wi_vals)
        wi_trials{i} = find(all_trial_wi == un_wi_vals(i));
        wi_inds{i} = find(ismember(all_trialvec,wi_trials{i}));
    end
    ...
    for cc = 1:96
        ...
        for ww = 1:length(un_wi_vals)
            wi_used_inds = cur_poss_inds(ismember(cur_poss_inds,wi_inds{ww}));
            
            %general averages
            [mua_data(cc).wi_msac_avg(ww,:),lags] = get_event_trig_avg(all_spike_rate_norm(:,cc),interp_sac_start_inds(used_msacs),backlag,forwardlag,nboot,used_trialvec,0,wi_used_inds);
            [mua_data(cc).wi_gsac_avg(ww,:),lags] = get_event_trig_avg(all_spike_rate_norm(:,cc),interp_sac_start_inds(used_gsacs),backlag,forwardlag,nboot,used_trialvec,0,wi_used_inds);
            [mua_data(cc).wi_simsac_avg(ww,:),lags] = get_event_trig_avg(all_spike_rate_norm(:,cc),all_sim_sacs,backlag,forwardlag,nboot,used_trialvec,0,wi_used_inds);
            
            %background dependent
            [mua_data(cc).wi_msac_gray_avg(ww,:),lags] = get_event_trig_avg(all_spike_rate_norm(:,cc),interp_sac_start_inds(used_msacs_grayback),backlag,forwardlag,nboot,used_trialvec,0,wi_used_inds);
            [mua_data(cc).wi_gsac_gray_avg(ww,:),lags] = get_event_trig_avg(all_spike_rate_norm(:,cc),interp_sac_start_inds(used_gsacs_grayback),backlag,forwardlag,nboot,used_trialvec,0,wi_used_inds);
            [mua_data(cc).wi_msac_im_avg(ww,:),lags] = get_event_trig_avg(all_spike_rate_norm(:,cc),interp_sac_start_inds(used_msacs_imback),backlag,forwardlag,nboot,used_trialvec,0,wi_used_inds);
            [mua_data(cc).wi_gsac_im_avg(ww,:),lags] = get_event_trig_avg(all_spike_rate_norm(:,cc),interp_sac_start_inds(used_gsacs_imback),backlag,forwardlag,nboot,used_trialvec,0,wi_used_inds);
            
            mua_data(cc).cond_avg_rate(ww) = nanmean(all_spike_rate_norm(wi_used_inds,cc));
        end
    ...
    for ss = 1:length(su_probes)
        ...
        for ww = 1:length(un_wi_vals)
            wi_used_inds = cur_use_inds(ismember(cur_use_inds,wi_inds{ww}));
            
            %general averages
            [sua_data(ss).wi_msac_avg(ww,:),lags] = get_event_trig_avg(all_spike_rate_norm(:,su_probes(ss)),interp_sac_start_inds(used_msacs),backlag,forwardlag,nboot,used_trialvec,0,wi_used_inds);
            [sua_data(ss).wi_gsac_avg(ww,:),lags] = get_event_trig_avg(all_spike_rate_norm(:,su_probes(ss)),interp_sac_start_inds(used_gsacs),backlag,forwardlag,nboot,used_trialvec,0,wi_used_inds);
            [sua_data(ss).wi_simsac_avg(ww,:),lags] = get_event_trig_avg(all_spike_rate_norm(:,su_probes(ss)),all_sim_sacs,backlag,forwardlag,nboot,used_trialvec,0,wi_used_inds);
            
            %background dependent
            [sua_data(ss).wi_msac_gray_avg(ww,:),lags] = get_event_trig_avg(all_spike_rate_norm(:,su_probes(ss)),interp_sac_start_inds(used_msacs_grayback),backlag,forwardlag,nboot,used_trialvec,0,wi_used_inds);
            [sua_data(ss).wi_gsac_gray_avg(ww,:),lags] = get_event_trig_avg(all_spike_rate_norm(:,su_probes(ss)),interp_sac_start_inds(used_gsacs_grayback),backlag,forwardlag,nboot,used_trialvec,0,wi_used_inds);
            [sua_data(ss).wi_msac_im_avg(ww,:),lags] = get_event_trig_avg(all_spike_rate_norm(:,su_probes(ss)),interp_sac_start_inds(used_msacs_imback),backlag,forwardlag,nboot,used_trialvec,0,wi_used_inds);
            [sua_data(ss).wi_gsac_im_avg(ww,:),lags] = get_event_trig_avg(all_spike_rate_norm(:,su_probes(ss)),interp_sac_start_inds(used_gsacs_imback),backlag,forwardlag,nboot,used_trialvec,0,wi_used_inds);
            
            sua_data(ss).cond_avg_rate(ww) = nanmean(all_spike_rate_norm(wi_used_inds,su_probes(ss)));
        end
