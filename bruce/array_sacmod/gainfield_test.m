...
gs_expts(ismember(cur_block_set(gs_expts),ignore_blocks)) = [];
tr_inds = used_inds(ismember(all_blockvec(used_inds),gs_expts));
...
right_data = find(par_pos > 2);
left_data = find(par_pos < -2);
center_data = find(par_pos > -2 & par_pos < 2);

right_tr_data = find(ismember(used_inds,right_data));
left_tr_data = find(ismember(used_inds,left_data));
center_tr_data = find(ismember(used_inds,center_data));

clear avg_*rate
for ii = 1:length(gs_expts)
   cur_set = left_tr_data(all_blockvec(used_inds(left_tr_data)) == gs_expts(ii));
   avg_left_rate(ii,:) = nanmean(all_binned_spikes(used_inds(cur_set),:));
   cur_set = right_tr_data(all_blockvec(used_inds(right_tr_data)) == gs_expts(ii));
   avg_right_rate(ii,:) = nanmean(all_binned_spikes(used_inds(cur_set),:));
   cur_set = center_tr_data(all_blockvec(used_inds(center_tr_data)) == gs_expts(ii));
   avg_center_rate(ii,:) = nanmean(all_binned_spikes(used_inds(cur_set),:));
   cur_set = find(all_blockvec(used_inds) == gs_expts(ii));
   avg_ov_rate(ii,:) = nanmean(all_binned_spikes(used_inds(cur_set),:));
end

gf_mod = (avg_left_rate - avg_right_rate)./(avg_left_rate + avg_right_rate);

left_rel_rate = avg_left_rate./avg_ov_rate;
right_rel_rate = avg_right_rate./avg_ov_rate;
center_rel_rate = avg_center_rate./avg_ov_rate;
%%
