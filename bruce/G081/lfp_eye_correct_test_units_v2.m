...
%     defmod.lambda_d2T = 300;
    ...
    for i = 2:(1+npq+nnq)
        quad.mods(i).lambda_L1x = 0.25;
        quad.mods(i).lambda_d2XT = 15;
        quad.mods(i).lambda_d2X = 30;
...
%         quad.mods(i).lambda_d2T = 30;
    ...
    quad_fit{1}(cc).mods(bad_filts(ii)).k = zeros(klen,1);
    end
    quad_fit{1}(cc).LL = getLL_GNM(quad_fit{1}(cc),X_z,tr_spkbns,'none');
...
% load ./perfect_fixation_mods_v2
%% VIEW MODEL FITS
% % load ./bar_ori_avg_rates.mat
for cc = 1:96
    fprintf('Cell %d of %d\n',cc,96);
...
tr_set = find(ll_imp > 0.025);
xv_set = find(ll_imp <= 0.025);
