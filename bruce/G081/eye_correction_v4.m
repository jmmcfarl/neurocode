...
defmod.lambda_L1x = lambdal_L1x;
defmod.lambda_d2XT = lambdal_d2XT;
defmod.lambda_d2X = lambdal_d2X;
defmod.lambda_d2T = lambdal_d2T;
npq = 1;
nnq = 0;
kern_types{1} = 'lin';
for i = 2:(1+npq+nnq)
...
for cc = 1:96
    init_kerns = 0.1*randn(upklen,1+npq+nnq);
    ...
    for i = 2:(1+npq+nnq)
        quad.mods(i).lambda_L1x = lambdaq_L1x;
        quad.mods(i).lambda_d2XT = lambdaq_d2XT;
        quad.mods(i).lambda_d2X = lambdaq_d2X;
        quad.mods(i).lambda_d2T = lambdaq_d2T;
    end
...
%     interp_quad_fit(cc) = fitGNM_filters(interp_quad_fit(cc),X_z,tr_spkbns,'none',[],1e-4,1e-6,1);
    interp_quad_fit(cc) = fitGNM_filters(quad,X_z,tr_spkbns,'none',[],1e-4,1e-6,1);
...
cd ~/Data/bruce/G081
save interp_quad_fits_0deg_interp3 interp_quad_fit interp_mod_theta old_LL null_LL
%%
% SET UP XV UNITS
NUNITS = 96;
