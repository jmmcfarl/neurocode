...
dsf = 60;Fsd = Fs/dsf;
niqf = Fsd/2;
[filt_b,filt_a] = butter(2,[2 45]/niqf);
use_lfps = [1:96];
...
dresh_X = [];
for i = 1:SDIM-1
    dressh_X = cat(2,dresh_X,resh_X(:,i,:));
...
%%
measured_eye_pos_orth = interp_eye_vals(tr_inds,2);
measured_eye_pos_orth = measured_eye_pos_orth - mean(measured_eye_pos_orth);
measured_eye_pos_par = interp_eye_vals(tr_inds,1);
measured_eye_pos_par = measured_eye_pos_par - mean(measured_eye_pos_par);

measured_par_speed = abs(interp_eye_vel(:,1));
big_sacs = 1 + find(measured_par_speed(1:end-1) < 20 & measured_par_speed(2:end) > 20);
big_sacs(~ismember(big_sacs,tr_inds)) = [];

eye_shifts = -round(measured_eye_pos_orth/bar_dxd);
max_shift = 16;
eye_shifts(eye_shifts > max_shift) = max_shift;
eye_shifts(eye_shifts < -max_shift) = -max_shift;

resh_X_sh = zeros(size(dresh_X));
for ii = 1:NT
    resh_X_sh(:,:,ii) = dist_shift2d(dresh_X(:,:,ii), eye_shifts(ii),1,0);
end
...
for cc = 1:length(use_lfps)
    ...
    glm_fit_sh(cc) = fitGNM_filters(glm,X_sh,Yobs(tr_inds,cc),'none',[],1e-4,1e-6,0);

    [glm_sse, ~, ~, prate] = getLL_GNM(glm_fit(cc),X_z(tr_inds,:),Yobs(tr_inds,cc),'none');
    ...
    if xv_frac > 0
        [glm_sse, ~, ~, prate] = getLL_GNM(glm_fit(cc),X_z(xv_inds,:),Yobs(xv_inds,cc),'none');
...
for xx = 1:length(shifts)
    ...
    d2 = dist_shift2d(filt_bank,shifts(xx),1,0);
    shifted_filt_bank = reshape(d2,klen,n_tr_chs);
    
    y_pred = X_z(tr_inds,:)*shifted_filt_bank;
...
resh_X = reshape(X_z(tr_inds,:)',[flen up_SDIM NT]);
...
for ii = 1:NT
    d2 = dist_shift2d(resh_X(:,:,ii), -shift_cor(ii),1,0);
    resh_X_sh(:,:,ii) = d2;
end
X_sh = reshape(resh_X_sh,flen*pad_SDIM,NT)';
X_z = reshape(resh_X,flen*pad_SDIM,NT)';
...
for nn = 2:n_ITER+1
    ...
    resh_X_sh = zeros(size(resh_X));
    for ii = 1:NT
        d2 = dist_shift2d(resh_X(:,:,ii), -up_fac*it_shift_cor{nn}(ii),1,0);
        resh_X_sh(:,:,ii) = d2;
    end
    X_sh = reshape(resh_X_sh,flen*up_SDIM,NT)';
