...
cd ~/Data/bruce/G081
% save interp_quad_fits_0deg_v4 interp_quad_fit interp_mod_theta old_LL null_LL
...
sh_flen = 10;
spike_history_mat = makeStimRows(all_binned_spks,sh_flen);
spike_history_mat = spike_history_mat(tr_inds,:);
spike_history_mat = reshape(spike_history_mat',sh_flen,96,NT);
spike_history_mat(end,:,:) = []; sh_flen = sh_flen-1;
spike_history_mat = reshape(spike_history_mat,sh_flen*96,NT)';

full_predmat = [rsac_Tmat spike_history_mat];

lamrange2 = [500 1 ntents 0];
cur_pt = ntents+1;
for i = 1:96
    lamrange2 = [lamrange2; 5 cur_pt+1 cur_pt + sh_flen 0];
    cur_pt = cur_pt + sh_flen;
end
for cc = 1:96
    fprintf('Fitting event kernels for cell %d of %d\n',cc,96);
...
%     ssac_out = ssac_Tmat*it_ssac_kern{nn-1}(tr_set,:)';
    ...
    sh_out = spike_history_mat*it_sh_kern{nn-1}(tr_set,:)';
...
%%
figure
shadedErrorBar(all_stim_times(tr_inds),post_mean*.04,post_std*.04,{'r'})
...
%%
save full_eye_correct_0deg_new3 *_LL it_quad_fit tr_set xv_set it_shift_cor tr_inds eye_times interp_quad_fit it_*
...
post_improve = (null_LL - ref_LL(2,:))/log(2);
...
close all
figure; hold on
% shadedErrorBar(all_stim_times(tr_inds),it_post_mean{1}*.04,it_inferred_std{1}*.04,{'r'})
