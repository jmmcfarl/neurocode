...
%{
This script is set up to handle standard preprocessing of laminar probe recordings, getting spiking
data, stimulus data, and eye-position data needed for most analysis. LFP is not included here, but
could be added easily, or combined through a separate script. This preprocessing should work with
Utah array recordings as well, but it would need to be checked.
%}
...
%if recording was split into two segments with the same stim-ori, need to specify a rec_number (1 is
%default), and also specify the range of recording blocks were processing here
rec_number = 1;
%these specify the block-range for the FIRST rec (so if rec_number is 2, it will use the other
%blocks)
...
%NOTE: you typically want this to be somewhat larger than the final number of pixels you use for
%model-fitting to account for FEM
...
if strcmp(monk_name,'jbe') %for JBE, only the left coil is any good, and even then I was typically getting better results by not using either coild in the EP inference
    params.good_coils = [1 0]; %which coils are any good
    params.use_coils = [0 0]; %which coils to use for EP inference
elseif strcmp(monk_name,'lem'); %both coils in LEM are mostly good, and it seems to be better to use them in the EP inference
...
cluster_dir = ['~/Analysis/bruce/' Expt_name '/clustering']; %directory with clustering data
if rec_number > 1
...
%this is the list of experiment names that covers the different versions of 1d random bar noise
%we've used
...
for ii = 1:length(Expts) %no need to gather these values in a loop, but i never got around to changing this...
    if ~isempty(Expts{ii})
        expt_names{ii} = Expts{ii}.Header.expname;
        expt_dds(ii) = Expts{ii}.Stimvals.dd; %bar density
        expt_bar_ori(ii) = Expts{ii}.Stimvals.or; %bar ori
        expt_sac_dir(ii) = mod(Expts{ii}.Stimvals.Fa,180); %guided saccade direction
        expt_Fr(ii) = Expts{ii}.Stimvals.Fr; %bar frame rate
        expt_sac_amp(ii) = Expts{ii}.Stimvals.Fs; %saccade amplitude (in deg)
        expt_imback(ii) = isfield(Expts{ii}.Trials,'imi'); %background type (bool -> has image back or not)
        included_type(ii) = any(strcmp(expt_names{ii},include_expts)); %is the experiment in the list of included types
    end
end
...
%for this, were only using expts with monocular stimuli, 100Hz frame rate, and the specified
%stim-orientation (so this excludes some datasets with 30Hz stimuli, binocular stim, etc)
cur_block_set = find(included_type & ~expt_binoc' & expt_Fr == 1 & expt_bar_ori == bar_ori); 
cur_block_set(ismember(cur_block_set,ignore_blocks)) = []; %exclude any blocks that are set to bad
if length(unique(expt_dds(cur_block_set))) > 1 %check if there are multiple different dds
...
%for older experiments, where we were varying the experiment-protocol from block-to-block, this will
%keep track of which blocks are of which types, in newer experiments, these were varied from
%trial-to-trial within a block (if at all)
sim_sac_expts = find(~expt_has_ds(cur_block_set)); %blocks with simulated saccades (periodic translations of image background)
imback_gs_expts = find(expt_has_ds(cur_block_set) & expt_imback(cur_block_set)'); %blocks that have an image background
grayback_gs_expts = find(expt_has_ds(cur_block_set) & ~expt_imback(cur_block_set)'); %blocks with graybackground

n_blocks = length(cur_block_set);

all_nfs = cellfun(@(x) x.Stimvals.nf,Expts(cur_block_set)); %number of stim-frames per trial
if length(unique(all_nfs)) > 1 %check if trial duration was changed across blocks
...
cur_expt_npix = unique(expt_npix(cur_block_set)); %number of stimulus-pixels
if length(cur_expt_npix) > 1 %check if this was varied
    warning('multiple Npix detected');
    cur_expt_npix = mode(cur_expt_npix);
end
if params.full_nPix > cur_expt_npix %cant try to grab more pixels than there are
...
%this is a rediculous way of grabbing all this info... 'historical reasons' :)
[all_stim_times, all_stim_mat, all_t_axis, all_t_bin_edges, all_tsince_start, all_blockvec,all_trialvec,all_trial_Se,...
    all_trial_wi,all_trial_back,all_trial_Ff,all_trial_exvals,all_trial_blk,all_trial_blocknums,all_trial_start_times,...
    all_trial_end_times,all_bin_edge_pts,all_trial_rptframes,all_trial_nrptframes] = deal([]);
[all_spk_times,all_clust_ids,all_spk_inds] = deal(cell(params.n_probes,1));
...
for ee = 1:n_blocks;    
    ...
    rpt_trials = false;
    if length(un_ids) < length(trial_ids) %this was detecting an issue i was having a while back. Shouldnt be appearing now
        rpt_trials = true;
    ...
    %add in time offset (for LP recordings, timestamps are relative within each block)
    ...
    %get width of stimulus region for each trial (this was only varied trial-to-trial in one expt
    %(G093)
    ...
    if params.is_TBT_expt
        if isfield(Expts{cur_block}.Trials,'Bs') %background stim-condition
            trial_back = strcmp('image',{Expts{cur_block}.Trials(:).Bs}); %make this a boolean (is image-back or not)
        else
        ...
        %frame number where the guided saccade occured (Ff=70 means guided sacs happen every 70
        %frames or 700 ms)
    ...
    load(fname);
    buffer_pix = floor((expt_npix(cur_block) - params.full_nPix)/2); %don't save this many pixels on either side of the full stim-display
    if buffer_pix == -1 %in case there is slightly less pixels than desired by full_nPix, just buffer by a couple zeros
        for ii = 1:length(left_stim_mats) %only need to store the left-eye pixels cuz this is monocular stim
            left_stim_mats{ii} = [zeros(size(left_stim_mats{ii},1),1) left_stim_mats{ii} zeros(size(left_stim_mats{ii},1),1)];
        end
        buffer_pix = 0;
    end
    cur_use_pix = (1:params.full_nPix) + buffer_pix; %use central range of pixels
    ...
    for tt = 1:n_trials
        cur_stim_times = Expts{cur_block}.Trials(use_trials(tt)).Start'/1e4; %stimulus frame onset times (in sec)
        ...
        if n_frames > 0 
            if length(cur_stim_times) == 1 %in some cases all the stim_times are stored. If only the first stim time is stored, create uniform grid of stim times
                ...
                cur_t_edges = [cur_stim_times; cur_stim_times(end) + params.dt*params.Fr]; %edges of time bins (frame refresh times)
            end
        end
        cur_t_axis = 0.5*cur_t_edges(1:end-1) + 0.5*cur_t_edges(2:end); %time bin centers
        ...
        if ~any(isnan(left_stim_mats{use_trials(tt)}(:))) && n_frames > params.min_trial_dur/params.dt %if using this trial
            ...
            %cat values
            all_stim_times = [all_stim_times; cur_stim_times + cur_toffset]; %times of stim frame onsets
            all_t_axis = [all_t_axis; cur_t_axis + cur_toffset]; %time bin centers 
            all_t_bin_edges = [all_t_bin_edges; cur_t_edges + cur_toffset]; %edges of time bins
            all_stim_mat = [all_stim_mat; cur_stim_mat]; 
            all_tsince_start = [all_tsince_start; cur_tsince_start];
            all_blockvec = [all_blockvec; ones(size(cur_t_axis))*ee];
            all_trialvec = [all_trialvec; ones(size(cur_t_axis))*(tt + trial_cnt)];
            all_bin_edge_pts = [all_bin_edge_pts; length(all_t_bin_edges)]; %set of trial edge points (useful in later binning of spiking data)
        end
    end
    trial_cnt = trial_cnt + n_trials;
    
    %keep track of repeat frames
    ...
    if strcmp(rec_type,'LP')
        ...
        cur_spkind_offset = cur_spkind_offset + round(40e3*(max(trial_end_times)-min(trial_start_times) + 50)); %this just needs to be some number large enough so that we get a unique spike index for each spike. I dont think this really has any use though?
    end
end
...
if params.is_TBT_expt
    if any(~isnan(all_trial_exvals(:,3)))
        ...
        all_trial_Ff(all_trial_exvals(:,3) == 1) = 0; %these are sim sac trials (background is image, but there's no guided sac)
        all_trial_Ff(all_trial_exvals(:,3) > 1) = 70; 
        all_trial_back(all_trial_exvals(:,3) == 2) = 0; %these are gray-back trials
        all_trial_back(ismember(all_trial_exvals(:,3),[1 3])) = 1; %image-back trials
    end
end
...
%I exclude data from the beginning and end of each trial (beg_buffer and end_buffer). Excluding data
%at the beginning is more important, because there are stimulus-onset non-stationarities. The
%exclusion at the end probably doesnt really matter, but it doesnt get rid of much data either
used_inds = find(all_tsince_start >= params.beg_buffer & (params.trial_dur-all_tsince_start) >= params.end_buffer);

%for G093 use only data where stripe width is AT LEAST 2 deg (the stimulus 'stripe' width was varied
%from trial to trial in this expt)
...
%% handle repeat frame issues for stim-repeat trials
