function new_autoclust = refine_gmms(probe_num,stored_spkxy,stored_times,autoclust,...
    poss_n_comps,n_reps,Expt_name,target_probes,target_expts,dsize,reload)
...
for ee = target_expts
    ...
    for nc = 1:length(poss_n_comps)
        ...
        D = mahal(clust_gmm,stored_spkxy{ee,probe_num});
        D = D(:,clust_ord(1));
        sug = find(clust_idx == 1);
        too_far = sug(D(sug) > max_mahal);
        clust_idx(too_far) = 2;
...
fprintf('Saving tempclust data\n');
save tempclust tempclust
else
    load tempclust
end
...
for ee = target_expts
    ...
    for nc = 1:length(poss_n_comps)
        ...
        plot(stored_spkxy{ee,probe_num}(tempclust(ee,nc).idx == 1,1),stored_spkxy{ee,probe_num}(tempclust(ee,nc).idx == 1,2),'r.','markersize',dsize);
        hold on
        plot(stored_spkxy{ee,probe_num}(tempclust(ee,nc).idx == 2,1),stored_spkxy{ee,probe_num}(tempclust(ee,nc).idx == 2,2),'k.','markersize',dsize);
    ...
    use_linesep = 0; use_robust = 0; refit = 0; use_mansep = 0;
    have_char = 0;
    while have_char == 0
        ui = input(sprintf('Expt %d use which model? (q to exit, 0 use old, l# line-sep, r# repeat robust, m# man sep)\n',ee),'s');
        if ~isempty(ui)
            have_char = 1;
        end
    ...
    elseif refit == 0
        ...
        while refit == 1
            ...
            elseif use_linesep == 1
                newtempclust = linesep_clust(Expt_name,target_probes(probe_num),ee,stored_times{ee,probe_num},stored_spkxy{ee,probe_num},cur_n_comps,dsize);
            elseif use_mansep == 1
                newtempclust = mansep_clust(Expt_name,target_probes(probe_num),ee,stored_times{ee,probe_num},stored_spkxy{ee,probe_num},cur_n_comps,dsize);
            end
            figure(f1);
            subplot(n_rows,n_cols,find(poss_n_comps == cur_n_comps))
            plot(stored_spkxy{ee,probe_num}(newtempclust.idx == 1,1),stored_spkxy{ee,probe_num}(newtempclust.idx == 1,2),'r.','markersize',dsize);
            hold on
            plot(stored_spkxy{ee,probe_num}(newtempclust.idx == 2,1),stored_spkxy{ee,probe_num}(newtempclust.idx == 2,2),'k.','markersize',dsize);
            set(gca,'xticklabel',[],'yticklabel',[]);
            title(sprintf('Expt %d, %d comps',ee,poss_n_comps(nc)));
            axis tight
            ...
            ni = input('Accept? (y/# for yes, r to retry robust, l to retry linesep, m to retry mansep, a to abort)\n','s');
            if strcmpi(ni,'a')
                refit = 0;
                new_autoclust(ee,probe_num).man_code = 0;
            elseif strcmpi(ni,'y') || any(strcmp({'1' '2' '3' '4' '5' '6' '7' '8' '9'},ni)) || isempty(ni)
                ...
                new_autoclust(ee,probe_num).man_code = cur_clust.man_code;
                new_autoclust(ee,probe_num).gmm = cur_clust.gmm;
                new_autoclust(ee,probe_num).idx = cur_clust.idx;
                new_autoclust(ee,probe_num).avg_wvfrm = cur_clust.avg_wvfrm;
                new_autoclust(ee,probe_num).std_wvfrm = cur_clust.std_wvfrm;
...
clust_gmm = gmdistribution.fit(stored_xy,n_comps,'Replicates',n_reps1,'Regularize',1e-12,'Options',statset('MaxIter',1000));
...
clust_gmm = gmdistribution.fit(stored_xy(not_outliers,:),n_comps,'Replicates',n_reps2,'Regularize',1e-12,'Options',statset('MaxIter',1000));
[clust_idx,nlogl] = cluster(clust_gmm,stored_xy);
...
tempclust.man_code = 2;
end

%%
function tempclust = linesep_clust(ename,probe,expt,stored_times,stored_xy,n_comps,dsize)
...
ftemp = figure;
while acc == 0
    plot(stored_xy(:,1),stored_xy(:,2),'k.','markersize',dsize);
    ...
    hold on
    plot(stored_xy(above_thresh,1),stored_xy(above_thresh,2),'r.','markersize',dsize);
    %     ui = input('Accept (y)\n','s');
    %     if strcmpi(ui,'y')
    %         acc = 1;
    %     end
...
if n_comps > 2
    cur_avg_wvfrm = zeros(40,2);
    for nn = 1:2
        cur_avg_wvfrm(:,nn) = mean(spk_values(init_idx(use_set2)==nn,:));
    end
    smallest_vals = min(cur_avg_wvfrm);
    [~,best_clust] = min(smallest_vals);
    if best_clust ~= 1
        flip = [2; 1];
        init_idx = flip(init_idx);
    end
    rand_idxs = ceil(rand(size(init_idx))*(n_comps-1)) + 1;
    init_idx(init_idx == 2) = rand_idxs(init_idx == 2);
end
...
tempclust.mahal_d = mahal_d;
tempclust.tmahal_d = tmahal_d;
tempclust.man_code = 3;
end

%%
function tempclust = mansep_clust(ename,probe,expt,stored_times,stored_xy,n_comps,dsize)
...
tempclust.avg_wvfrm = avg_wvfrm;
tempclust.std_wvfrm = std_wvfrm;
tempclust.mahal_d = nan;
tempclust.tmahal_d = tmahal_d;
tempclust.nlogl = nan;
tempclust.gmm = nan;
tempclust.man_code = 4;
end
