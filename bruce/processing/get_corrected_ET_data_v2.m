function [corrected_eye_vals,corrected_eye_vals_interp]  = get_corrected_ET_data_v2(Expts,all_eye_vals,all_eye_ts,all_t_axis,all_blockvec,block_bar_oris,used_inds,lin_correction)
...
%flip horizontal component of coil signal so we're in screen coordinates
all_eye_vals(:,[1 3]) = -all_eye_vals(:,[1 3]);
...
for bb = 1:length(unique_bar_oris)
   ...
   %subtract off median in orth dimensions
   corrected_eye_vals_interp(cur_inds,[2 4]) = bsxfun(@minus,corrected_eye_vals_interp(cur_inds,[2 4]),nanmedian(corrected_eye_vals_interp(cur_inds,[2 4])));
   corrected_eye_vals(cur_eye_inds,[2 4]) = bsxfun(@minus,corrected_eye_vals(cur_eye_inds,[2 4]),nanmedian(corrected_eye_vals_interp(cur_inds,[2 4])));
   ...
   %subtract off median again in orth dims
   corrected_eye_vals_interp(cur_inds,[2 4]) = bsxfun(@minus,corrected_eye_vals_interp(cur_inds,[2 4]),nanmedian(corrected_eye_vals_interp(cur_inds,[2 4])));
   corrected_eye_vals(cur_eye_inds,[2 4]) = bsxfun(@minus,corrected_eye_vals(cur_eye_inds,[2 4]),nanmedian(corrected_eye_vals_interp(cur_inds,[2 4])));
   end
end
