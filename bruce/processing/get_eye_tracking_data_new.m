...
% [eye_vals_interp,eye_ts_interp,eye_insample] = get_eye_tracking_data_new(eye_structure,ts_range)
% gets raw eye position data over a given time sample range
% INPUTS: 
%     eye_structure: data struct from raw ET file
%     ts_range: range of time samples desired
% OUTPUTS:
%     eye_vals_interp: continuous matrix of ET time series at tracker time res
%     eye_ts_interp: corresponding time samples
%     eye_insample: bool vector telling whether the eye data was recorded at each sample

eye_dt = eye_structure.Header.CRrates(1);
if isfield(eye_structure.Trials,'good')
    is_good = [eye_structure.Trials(:).good];
else
    is_good = ones(length(eye_structure.Trials),1);
end
eye_structure.Trials = eye_structure.Trials(is_good > 0);

%get start time of eye data on each trial
if isfield(eye_structure.Trials,'ftime')
    eye_track_ftime = [eye_structure.Trials(:).ftime]/1e4;
else
    eye_track_ftime = [eye_structure.Trials(:).Start]/1e4;
end
eye_track_ends = [eye_structure.Trials(:).End]/1e4; %time stamps of trial end points 

%get set of trials with relevant data
...
segs_to_get = first_needed_trial:last_needed_trial; %range of data segments to grab
eye_vals = [];
eye_ts = [];
% eye_data_lens = zeros(length(segs_to_get),4);
for i = segs_to_get
    cur_rh = eye_structure.Trials(i).Eyevals.rh; %right horizontal
    cur_lh = eye_structure.Trials(i).Eyevals.lh; %left horizontal
    cur_rv = eye_structure.Trials(i).Eyevals.rv; %right vertical
    cur_lv = eye_structure.Trials(i).Eyevals.lv; %left vertical
...
%     eye_data_lens(i,4) = length(cur_rv);
    ...
    %time axis of current ET segment
...
cur_back_pts = find(dts <= 0); %find any points where the time axis jumps backwards (because of overlapping trial segments)
...
%interpolate data onto monotonic increasing time vector, getting rid of
%overlap regions
