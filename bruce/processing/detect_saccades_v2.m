...
sac_peakvel = all_eye_speed(saccade_inds);

sac_dbuff = round(0.0*et_params.eye_fs); %for computing saccade amplitudes
pre_inds = sac_start_inds - sac_dbuff;
pre_inds(pre_inds < 1) = 1;
sac_pre_pos = all_eye_vals(pre_inds,:);
sac_pre_pos_raw = orig_eye_vals(pre_inds,:);
...
sac_post_pos_raw = orig_eye_vals(post_inds,:);

if et_params.use_coils(1) && ~et_params.use_coils(2)
...
elseif ~et_params.use_coils(1) && et_params.use_coils(2)
...
sac_delta_pos = sac_post_pos - sac_pre_pos;
sac_delta_pos_raw = sac_post_pos_raw - sac_pre_pos_raw;
