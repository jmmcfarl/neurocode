...
trial_inds = zeros(size(full_taxis));
trial_inds(all_small_inds) = 1;
Tmat3 = zeros(length(full_taxis),ntents);
for i = 1:ntents
    Tmat3(:,i) = conv(trial_inds,tbmat(i,:),'same');
end
...
% [fitp,grad,se] = GLMsolve_jmm(cur_Xmat(tr_inds,:), tr_spkbns, K0, 1, [], lamrange2,[], [], llist, [], NLtype);
...
l1_pen = 0;
NLtype = 0;
cur_Xmat = [trial_Tmat Tmat3 stim_out];
...
sac_kern_ind(cc,:) = fitp.k(1:ntents);
sac_kern_se(cc,:) = se(1:ntents);
msac_kern_ind(cc,:) = fitp.k(ntents+1:2*ntents);
msac_kern_se(cc,:) = se(ntents+1:2*ntents);
...
% 
%     xv_LL(cc) = -sum(xv_Robs.*log(xv_pred_rate) - xv_pred_rate)/sum(xv_Robs);
...
%%
save random_bar_sackern_mods *kern* tent* dt
%%
