...
addpath('/mnt/PFC/James_scripts/code_iSTAC/')
...
%         cur_res_t = expt_lfp_t_axis(seg_start_inds(jj)):1/Fs:expt_lfp_t_axis(seg_stop_inds(jj));
        cur_res_t = expt_lfp_t_axis(cur_res_inds);
        cur_res_t = downsample(cur_res_t,dsf_init);
...
%         interp_lfps = interp1(expt_lfp_t_axis(cur_res_inds),CSD(cur_res_inds,:),cur_res_t);
        ...
        interp_lfps = downsample(expt_lfps(cur_res_inds,:),dsf_init);
...
% defmod.lambda_d2T = 50;
% defmod.lambda_L2x = 25;
% 
% for cc = 1:24
%     
...
%     %     glm.spk_nl = 'exp';
%     glm_fit(cc) = fitGNM_filters_v2(glm,full_Xmat(tr_inds,:),lin_X(tr_inds,:),tr_spkbns,'none',[],1e-4,1e-6);
%     
...
% %     gnm = add_gnm_filter(glm_fit(cc),0.01*randn(klen,1),-1,'threshlin');
% %     gnm_fit(cc) = fitGNM_filters_v2(gnm,full_Xmat(tr_inds,:),lin_X(tr_inds,:),tr_spkbns,'none',[],1e-4,1e-6);
% %     gnm_fit_nl(cc) = adjust_all_reg(gnm_fit(cc),'lnl2',200);
% %     gnm_fit_nl(cc)  = setGNM_NLBFs(gnm_fit_nl(cc) ,full_Xmat(tr_inds,:));
% %     gnm_fit_nl(cc) = adjust_all_reg(gnm_fit_nl(cc),'nltype','uncon');
% %     gnm_fit_nl(cc) = adjust_all_reg(gnm_fit_nl(cc),'nlmon',1);
% %     gnm_fit_nl(cc) = fitGNM_internal_NLs_v2(gnm_fit_nl(cc),full_Xmat(tr_inds,:),lin_X(tr_inds,:),tr_spkbns,1,0);
%     
%     
%     K0 = zeros(size(lin_X,2)+1,1);
%     [fitp_n,grad] = GLMsolve_jmm(lin_X(tr_inds,:), tr_spkbns, K0, 1, [], [],[], [], [], [], 0);
%     
%     xv_Robs = full_spkbinned(xv_inds,cc);
%     xv_spkbns = convert_to_spikebins(full_spkbinned(xv_inds,cc));
%     
%     xv_pred_rate = full_Xmat(xv_inds,:)*glm_fit(cc).mods(1).k + lin_X(xv_inds,:)*glm_fit(cc).linK + glm_fit(cc).spk_theta;
%     xv_pred_rate = log(1+exp(xv_pred_rate));
%     %     xv_pred_rate = exp(xv_pred_rate);
%     
% %     [nll, pnll, lpen, xv_gpred_rate] = getLL_GNM_v2(gnm_fit(cc),full_Xmat(xv_inds,:),lin_X(xv_inds,:),xv_spkbns,'none');
% %     [nll, pnll, lpen, xv_gnlpred_rate] = getLL_GNM_v2(gnm_fit_nl(cc),full_Xmat(xv_inds,:),lin_X(xv_inds,:),xv_spkbns,'none');
%     
%     xv_null_rate = lin_X(xv_inds,:)*fitp_n.k(1:end-1) + fitp_n.k(end);
%     xv_null_rate = log(1+exp(xv_null_rate));
%     
%     xv_glmLL(cc) = -sum(xv_Robs.*log(xv_pred_rate) - xv_pred_rate)/sum(xv_Robs);
% %     xv_gnmLL(cc) = -sum(xv_Robs.*log(xv_gpred_rate) - xv_gpred_rate)/sum(xv_Robs);
% %     xv_gnm_nl_LL(cc) = -sum(xv_Robs.*log(xv_gnlpred_rate) - xv_gnlpred_rate)/sum(xv_Robs);
% %     xv_nullLL(cc) = -sum(xv_Robs.*log(xv_null_rate) - xv_null_rate)/sum(xv_Robs);
%     
% %     fprintf('GLM imp: %.4f  GNM imp: %.4f GNM NL imp: %.4f\n',xv_nullLL(cc) - xv_glmLL(cc),xv_nullLL(cc) - xv_gnmLL(cc),xv_nullLL(cc) - xv_gnm_nl_LL(cc));
% end

cd /home/james/Data/bruce/2_27_12/M232
% save random_bar_phasemods_stimmods_v2 glm_fit
...
%     ca = max(abs(k));
%     kmat = reshape(k,flen,15);
%     imagesc(un_bar_pos,(0:-1:(-flen+1))*new_dt,flipud(kmat))
