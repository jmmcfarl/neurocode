...
use_trials(all_trial_durs(use_trials) < trial_dur) = []; %don't use incomplete trials
...
%%
trial_uinds = find(ismember(all_trialvec(used_inds),use_trials));
...
all_Xmat_shift = all_Xmat_shift(used_inds(trial_uinds),use_kInds_up);

%%
silent = 1;

mod_prates = nan(length(trial_uinds),length(targs));
for ss = 1:length(targs)
    cc = targs(ss);
    cur_Robs = Robs_mat(trial_uinds,cc);
    cc_uinds = find(~isnan(cur_Robs));
    
    if ~isempty(cc_uinds)
%         cur_mod = ModData(cc).rectGQM;
        cur_mod = dit_mods(cc);
        cur_mod.mods([cur_mod.mods(:).Xtarget]~=1) = [];
...
cur_mod = NMMfit_logexp_spkNL(cur_mod,cur_Robs(cc_uinds),all_Xmat_shift(cc_uinds,:));
       cur_mod = NMMfit_scale(cur_mod,cur_Robs(cc_uinds),all_Xmat_shift(cc_uinds,:));
        ...
        [~,~,mod_prates(:,ss)] = NMMmodel_eval(cur_mod,[],all_Xmat_shift);
    end
end

%%
n_simtrials = 10;
shuff_EP = fin_tot_corr;
% all_mod_prates = nan(n_simtrials,length(trial_uinds),length(targs));
all_sim_psths = zeros(length(trial_uinds),length(targs));
for ii = 1:n_simtrials
    fprintf('sim trial %d/%d\n',ii,n_simtrials);
    tperm = use_trials(randperm(length(use_trials)));
    for tt = 1:length(use_trials)
        cur_set = find(all_trialvec(used_inds) == tperm(tt));
        orig_set = find(all_trialvec(used_inds) == use_trials(tt));
        shuff_EP(cur_set) = fin_tot_corr(orig_set);
    end
   
    fin_shift_cor = round(shuff_EP);
    fin_shift_cor(isnan(fin_shift_cor)) = 0;
    
    %RECOMPUTE XMAT
    cur_shift_stimmat_up = all_stimmat_up;
    for jj=1:length(trial_uinds)
        cur_shift_stimmat_up(used_inds(trial_uinds(jj)),:) = shift_matrix_Nd(all_stimmat_up(used_inds(trial_uinds(jj)),:),-fin_shift_cor(trial_uinds(jj)),2);
    end
    
    cur_Xmat_shift = create_time_embedding(cur_shift_stimmat_up,stim_params_us);
    cur_Xmat_shift = cur_Xmat_shift(used_inds(trial_uinds),use_kInds_up);
        
    cur_mod_prates = nan(length(trial_uinds),length(targs));
    for ss = 1:length(targs)
        cc = targs(ss);
        [~,~,cur_mod_prates(:,ss)] = NMMmodel_eval(mod_fits(ss),[],cur_Xmat_shift);
    end
%     all_mod_prates(ii,:,:) = cur_mod_prates;
    all_sim_psths = all_sim_psths + cur_mod_prates;
end
all_sim_psths = all_sim_psths/n_simtrials;
%%
% all_sim_psths = squeeze(nanmean(all_mod_prates));
% all_mod_prates_ms = bsxfun(@minus,all_mod_prates,mean(all_mod_prates));
mod_prates_ms = bsxfun(@minus,mod_prates,mean(mod_prates));
psths_ms = bsxfun(@minus,all_sim_psths,mean(all_sim_psths));

ov_cov = cov(mod_prates);
psth_cov = cov(all_sim_psths);

% em_cov = nan(length(targs));
% for ii = 1:length(targs)
%     cur1 = squeeze(all_mod_prates_ms(:,:,ii));
%     for jj = ii:length(targs)
%         cur2 = squeeze(all_mod_prates_ms(:,:,jj));
%         em_cov(ii,jj) = nanmean(nanmean(cur1.*cur2));
%         em_cov(jj,ii) = em_cov(ii,jj);
%     end
% end
em_cov = ov_cov - psth_cov;

all_vars = diag(ov_cov);
norm_matrix = sqrt(all_vars*all_vars');
ov_corr = ov_cov./norm_matrix;
psth_corr = psth_cov./norm_matrix;
em_corr = em_cov./norm_matrix;
to_nan = logical(eye(length(targs)));
ov_corr(to_nan) = nan;
psth_corr(to_nan) = nan;
em_corr(to_nan) = nan;
...
% ov_xcov = nan(nlags,length(targs),length(targs));
% psth_xcov = nan(nlags,length(targs),length(targs));
% for tt = 1:nlags
%     tt
%     shift_mod_prates = reshape(shift_matrix_Nd(mod_prates_ms,lags(tt),1),[],1,length(targs));
%     ov_xcov(tt,:,:) = squeeze(nanmean(bsxfun(@times,shift_mod_prates,mod_prates_ms)));
%     shift_psths = reshape(shift_matrix_Nd(psths_ms,lags(tt),1),[],1,length(targs));
%     psth_xcov(tt,:,:) = squeeze(nanmean(bsxfun(@times,shift_psths,psths_ms)));
% end
% em_xcov = ov_xcov-psth_xcov;

n_rpts = 5;
ov_xcov = nan(n_rpts,nlags,length(targs),length(targs));
psth_xcov = nan(n_rpts,nlags,length(targs),length(targs));
for rr = 1:n_rpts
    randshift = randi(length(trial_uinds),1); 
    cur_mod_prates = circshift(mod_prates_ms,randshift);
    cur_mod_prates = cur_mod_prates(1:1e4,:);
    cur_psth = circshift(psths_ms,randshift);
    cur_psth = cur_psth(1:1e4,:);
    for tt = 1:nlags
        tt
        shift_mod_prates = reshape(shift_matrix_Nd(cur_mod_prates,lags(tt),1),[],1,length(targs));
        ov_xcov(rr,tt,:,:) = squeeze(nanmean(bsxfun(@times,shift_mod_prates,cur_mod_prates)));
        shift_psths = reshape(shift_matrix_Nd(cur_psth,lags(tt),1),[],1,length(targs));
        psth_xcov(rr,tt,:,:) = squeeze(nanmean(bsxfun(@times,shift_psths,cur_psth)));
    end
end
ov_xcov = squeeze(nanmean(ov_xcov));
psth_xcov = squeeze(nanmean(psth_xcov));

em_xcov = ov_xcov-psth_xcov;

%%
c1 = 99;
cur_Xcovs = nan(nlags,length(targs));
for cc = 1:length(targs)
    cur_uinds = find(~isnan(Robs_mat(:,c1)) & ~isnan(Robs_mat(:,cc)));
     cur_Xcovs(:,cc) = xcov(Robs_mat(cur_uinds,c1),Robs_mat(cur_uinds,cc),max_lags,'biased');
end
...
others = setdiff(targs,cc);
f1 = figure;
subplot(1,3,1)
imagesc(lags*dt,1:102,squeeze(ov_xcov(:,cc,others))');
xlim(xl);
caxis(ca);
xlabel('Time lag (s)');
subplot(1,3,2)
imagesc(lags*dt,1:102,squeeze(psth_xcov(:,cc,others))');
xlim(xl);
caxis(ca);
xlabel('Time lag (s)');
subplot(1,3,3)
imagesc(lags*dt,1:102,squeeze(em_xcov(:,cc,others))');
xlim(xl);
caxis(ca);
xlabel('Time lag (s)');
colormap(jet);
...
cc2 = 91;
f2 = figure
plot(lags*dt,squeeze(ov_xcov(:,cc,cc2))/norm_matrix(cc,cc2),'k','linewidth',1)
hold on
plot(lags*dt,squeeze(psth_xcov(:,cc,cc2))/norm_matrix(cc,cc2),'r','linewidth',1)
plot(lags*dt,squeeze(em_xcov(:,cc,cc2))/norm_matrix(cc,cc2),'b','linewidth',1);
xlabel('Time lag (s)');
ylabel('Correlation');

f3 = NMMdisplay_model(mod_fits(cc));
f4 = NMMdisplay_model(mod_fits(cc2));
f3 = f3.stim_filts;
f4 = f4.stim_filts;

fig_width = 9; rel_height = 0.6;
figufy(f1);
fname = ['/Users/james/Desktop/' 'all_xcorrs.pdf'];
exportfig(f1,fname,'width',fig_width,'height',rel_height*fig_width,'fontmode','scaled','fontsize',1);
close(f1);

fig_width = 3.5; rel_height = 0.8;
figufy(f2);
fname = ['/Users/james/Desktop/' 'examp_xcorrs.pdf'];
exportfig(f2,fname,'width',fig_width,'height',rel_height*fig_width,'fontmode','scaled','fontsize',1);
close(f2);

fig_width = 6; rel_height = 1.5;
figufy(f3);
fname = ['/Users/james/Desktop/' 'examp1_mod.pdf'];
exportfig(f3,fname,'width',fig_width,'height',rel_height*fig_width,'fontmode','scaled','fontsize',1);
close(f3);

fig_width = 6; rel_height = 1.5;
figufy(f4);
fname = ['/Users/james/Desktop/' 'examp2_mod.pdf'];
