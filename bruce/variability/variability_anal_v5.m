...
for ii = 1:sac_buff+1
    cur_inds = saccade_start_inds + (ii-1);
...
%%
rpt_EP = fin_shift_cor(all_rpt_inds);
rpt_EP(in_sac_inds(all_rpt_inds)) = nan;
rpt_EP(in_blink_inds(all_rpt_inds)) = nan;
...
for cc = 1:length(targs)
    ...
    all_XC(:,:,cc,:) = squeeze(nanmean(cur_XC));
end
