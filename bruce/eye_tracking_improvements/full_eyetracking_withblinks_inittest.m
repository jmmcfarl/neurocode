...
use_measured_pos = 2; %1 for init with coils, 2 for init with trial-sub coils, 3 for random init, 0 for init to perfect fixation
...
mod_data_name = 'full_eyetrack_initmods_inittest';
anal_name = 'full_eyetrack_inittest';
...
%     cur_shift_stimmat = all_stim_mat;
    for ii = 1:NT
        cur_shift_stimmat_up(used_inds(ii),:) = shift_matrix_Nd(cur_shift_stimmat_up(used_inds(ii),:),-init_eyepos_rnd(used_inds(ii)),2);
%         cur_shift_stimmat(used_inds(ii),:) = shift_matrix_Nd(cur_shift_stimmat(used_inds(ii),:),-init_eyepos_ds(used_inds(ii)),2);
    end
    all_Xmat_us = create_time_embedding(cur_shift_stimmat_up,stim_params_us);
%     all_Xmat = create_time_embedding(cur_shift_stimmat,stim_params);
end
...
% block_trial_LL = nan(n_blocks,n_tr_chs);
% for tt = 1:n_blocks
%    cur_trials = find(all_trial_blk == tt);
%    block_trial_LL(tt,:) = nanmean(trial_LLimp_zscore(cur_trials,:));
% end
...
for nn = 1:n_fix_inf_it
    ...
    if all(use_coils==0) %if not using coil info
