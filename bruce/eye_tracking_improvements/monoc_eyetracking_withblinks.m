...
base_fA = repmat(fix_prior,n_shifts,1);
...
for nn = 1:n_fix_inf_it
    ...
    if all(use_coils==0) %if not using coil info
        ...
        for t=2:n_fixs
            if ~fix_post_blink(t) %if this fixation isn't following a blink use coil info
...
cd(anal_dir);
save(anal_name,'it_*','drift_post_*','fix_ids','dit_*','et_used_inds','et_tr_set','et_saccades','et_is_blink','et_params','et_tr_trials','et_xv_trials');
