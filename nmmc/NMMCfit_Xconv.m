function nim_out = NMMCfit_Xconv( nim, Robs, Xstims, Gmults, desired_targets, silent, desired_optim_params, regmat_custom )
%
% Usage: nim_out = NMMfit_upstreamNLs( nim, Robs, Xstims, <Gmults>, <desired_targets>, <silent>, <desired_optim_params>, <regmat_custom> )
%
...
if (nargin < 7) || isempty(desired_optim_params)
...
mod_NXc = zeros(Nmods,1);
for imod = 1:Nmods
	mod_NXc(imod) = length(nim.mods(imod).Xshifts);
	if mod_NXc(imod) > 1
		lambda_smooth(imod) = nim.mods(imod).reg_params.lambda_conv_d2X;
		lambda_L1(imod) = nim.mods(imod).reg_params.lambda_conv_L1;
		poss_targets = [poss_targets imod];
	end
end
NXconv = unique(mod_NXc(mod_NXc > 1));
if length(NXconv) ~= 1
	error('Have to have same number of Xconv for each subunit');
end

if isempty(poss_targets)
	disp('No subunits have X-convolutions defined. Exiting function...');
	return
end
if spkhstlen > 0
	poss_targets = [poss_targets -1]; %add spk hist to possible targets
end
...
for imod = non_targets(non_targets > 0)
	...
	if ~isempty(Gmults{imod})
		nt_gout = nt_gout + (fgint{imod}.*Gmults{imod}) * nim.mods(imod).sign;
	else
		nt_gout = nt_gout + fgint{imod}*nim.mods(imod).sign;
	end
end
...
%% COMPUTE NEW X-MATRIX OUT OF SHIFTED PROCESSED UNITS
XNL = zeros(NT,Ntargets*NXconv); %initialize X matrix which is for the NL BFs of each module
for ii = 1:Ntargets %for each module
		tar = targets(ii);
		if isempty(Gmults{tar})
			Xconv_out = nim.mods(tar).sign * fgint{tar};
		else
			Xconv_out = nim.mods(tar).sign * bsxfun(@times, fgint{tar}, Gmults{tar} );
		end
    XNL(:,((ii-1)*NXconv + 1):(ii*NXconv)) = Xconv_out; % assemble filtered NLBF outputs into X matrix
end
...
for imod = targets(targets > 0)
	initial_params = [initial_params; nim.mods(imod).Xconv]; %not incorporating the multiplier here because doing so messes with regularization
end
Xconv_indx = 1:length(initial_params);  % Positive constraint on everything up to here
...
A = []; Aeq = []; % initialize constraint parameters
use_con = 1; % X convolutions are automatically positively constrained
LB(Xconv_indx) = 0;
...
if spkhstlen > 0 && ismember(-1,targets)
	...
	if nim.spk_hist.negCon == 1
		spkhist_inds = (Ntargets*NXconv + 1):(Ntargets*NXconv + spkhstlen);
		UB(spkhist_inds) = 0;
		use_con = 1;
	end
end
...
if max(lambda_smooth) > 0
	ex = ones(NXconv,1);
	L2_mat = spdiags([ex -2*ex ex], [-1 0 1], NXconv,NXconv)';
else
	L2_mat = [];
end
...
if use_con
	...
	[params] = fmincon( @(K) NIMfit_upstreamNLs_internal(nim, K, Robs, XNL, Xspkhst, L2_mat, targets,nt_gout),...
		initial_params, A,b,Aeq,beq,LB,UB,[],optim_params);
else
	[params] = minFunc( @(K) NIMfit_upstreamNLs_internal( nim, K, Robs, XNL, Xspkhst, L2_mat, targets, nt_gout), initial_params, optim_params);
end

%%
nim_out = nim;
nim_out.spk_NL_params(1) = params(end);
convX_mat = reshape(params(1:Ntargets*NXconv),NXconv,Ntargets); %take output K vector and restructure into a matrix of NLBF coefs, one for each module
for ii = 1:Ntargets;
	nim_out.mods(targets(ii)).Xconv = convX_mat(:,ii); %convX coefs for current module
end

if ismember(-1,targets) && spkhstlen > 0
	nim_out.spk_hist.coefs = params((Ntargets*NXconv+1):(Ntargets*NXconv+spkhstlen));
end
...
function [LL, LLgrad] = NIMfit_upstreamNLs_internal(nim, params, Robs, Xnl, Xspkhst, L2_mat, targets, nt_gout)
%
...
G = theta + nt_gout;
G = G + Xnl*params(1:Ntargets*mod_NXcs);
...
LLgrad(1:Ntargets*mod_NXcs) = residual'*Xnl;
...
for n = 1:Ntargets
    cur_convX = params((n-1)*mod_NXcs + (1:mod_NXcs));
    if nim.mods(targets(n)).reg_params.lambda_conv_d2X > 0
        smooth_penalty(n) = nim.mods(targets(n)).reg_params.lambda_conv_d2X*sum((L2_mat * cur_convX).^2);
        
        cur_grad_pen = 2*nim.mods(targets(n)).reg_params.lambda_conv_d2X*(L2_mat' * L2_mat * cur_convX);
        LLgrad_pen((n-1)*mod_NXcs + (1:mod_NXcs)) = cur_grad_pen;
    end
end
