function nim_out = NMMCfit_filters( nim, Robs, Xstims, Gmults, targets, silent, desired_optim_params, regmat_custom )
%
% Usage: nim_out = NMMCfit_filters( nim, Robs, Xstims, <Gmults>, <targets>, <silent>, <desired_optim_params>, <regmat_custom> )
%
...
use_conv = find(mod_NXc > 1);
if ~isempty(use_conv) 
    NXconv = unique(mod_NXc(use_conv));
    if length(NXconv) > 1
        error('Need convolution kernels to be the same size');
    end
    Xshifts = nim.mods(use_conv(1)).Xshifts;
    cur_Xtarg = nim.mods(use_conv(1)).Xtarget;
    sdims = nim.stim_params(cur_Xtarg).stim_dims;
    L = prod(sdims);
    for n = 1:length(nim.mods(use_conv(1)).Xconv)
        shift_mat{n} = spdiags( ones(L,1), Xshifts(n)*sdims(1), L, L );
    end
else
    error('No convolution kernels used, fit using NMMfit_filters instead');
end
...
for imod = non_targets(non_targets > 0) %for all subunits that aren't targeted
    ...
    %spatial convolution
    fgint{imod} = fgint{imod} * nim.mods(imod).Xconv;
...
if use_con == 0 %if no constraints
    ...
    if max(lambda_L1) > 0
        if exist('L1General2_PSSas','file') == 2
            ...
            [params] = L1General2_PSSas(@(K) LLfit_filters_internal(nim, K, Robs, Xstims,Xspkhst,Gmults,L2_mats,targets,nt_gout,fprimes,shift_mat),...
        ...
        if exist('minFunc','file') == 2 %try to use Mark Schmidt's optimizer
            ...
            [params] = minFunc( @(K) LLfit_filters_internal( nim, K, Robs, Xstims, Xspkhst, Gmults, L2_mats, targets, nt_gout, fprimes,shift_mat), initial_params, optim_params);
            ...
            [params] = fminunc( @(K) LLfit_filters_internal( nim, K, Robs, Xstims, Xspkhst, Gmults, L2_mats, targets, nt_gout, fprimes,shift_mat), initial_params, optim_params);
    ...
    if exist('minConf_TMP','file') == 2 && isempty(A) && isempty(Aeq)
        ...
        [params] = minConf_TMP( @(K) LLfit_filters_internal( nim, K, Robs, Xstims, Xspkhst, Gmults, L2_mats, targets, nt_gout, fprimes,shift_mat),...
        ...
        optim_params.optTol = 1e-4;
        optim_params.progTol = 1e-6;
        [params] = fmincon( @(K) LLfit_filters_internal( nim, K, Robs, Xstims, Xspkhst, Gmults, L2_mats, targets, nt_gout, fprimes,shift_mat),...
...
function [LL, LLgrad] = LLfit_filters_internal(nim, params, Robs, Xstims, Xspkhst, Gmults, L2_mats, targets, nt_gout, fprimes,shift_mat)
%
...
%%
mod_NXc = ones(Ntargets,1);
for n = 1:Ntargets
    mod_NXc(n) = length(nim.mods(targets(n)).Xshifts);
end
use_conv = mod_NXc > 1;
NXconv = unique(mod_NXc(use_conv));
...
% gint = nan(length(Robs),Ntargets,mod_NXc);
...
for ii = 1:Ntargets
    ...
    if use_conv(tar)
        ks{ii} = zeros(length(base_ks{ii}),NXconv);
        for n = 1:NXconv
            ks{ii}(:,n) = shift_mat{n} * base_ks{ii};
        end
    else
        ks{ii} = base_ks{ii};
    end
    ...
    %spatial convolution
    fgint = fgint * nim.mods(tar).Xconv;
    ...
    if use_conv(ii)
        for jj = 1:mod_NXc(ii)
            ...
            if strcmp(nim.mods(tar).NLtype,'lin')
                ...
                if isempty(Gmults{tar})
                    LLgrad(placeholder+(1:filtLen(ii))) = LLgrad(placeholder+(1:filtLen(ii))) + ...
                        nim.mods(tar).Xconv(jj)*residual'*(Xstims{nim.mods(tar).Xtarget} * shift_mat{jj}) * nim.mods(tar).sign;
                else
                    LLgrad(placeholder+(1:filtLen(ii))) = LLgrad(placeholder+(1:filtLen(ii))) + ...
                        nim.mods(tar).Xconv(jj)*(Gmults{tar}.*residual')* (Xstims{nim.mods(tar).Xtarget} * shift_mat{jj})* nim.mods(tar).sign;
                end
                
            else
                if strcmp(nim.mods(tar).NLtype,'nonpar')
                    fpg = piececonst_process(gint{ii}(:,jj),fprimes{ii}, nim.mods(tar).NLx);
                elseif strcmp(nim.mods(tar).NLtype,'quad')
                    fpg = 2*gint{ii}(:,jj);
                elseif strcmp(nim.mods(tar).NLtype,'threshlin')
                    fpg = gint{ii}(:,jj) >= 0;
                ...
                LLgrad(placeholder+(1:filtLen(ii))) = LLgrad(placeholder+(1:filtLen(ii))) + ...
                    (nim.mods(tar).Xconv(jj)*(fpg.*residual)' * (Xstims{nim.mods(tar).Xtarget} * shift_mat{jj}) * nim.mods(tar).sign)';
            end
        end
        
    else %if NO CONV KERNEL
        if strcmp(nim.mods(tar).NLtype,'lin')
            % Check for multiplicative interactions
            if isempty(Gmults{tar})
...
for ii = 1:Ntargets
    ...
    if nim.mods(tar).reg_params.lambda_L2 > 0
        ...
        LLgrad_pen(placeholder+(1:filtLen(ii))) = LLgrad_pen(placeholder+(1:filtLen(ii))) + nim.mods(tar).reg_params.lambda_L2*base_ks{ii};
    end
