function [ev_avg,lags,ev_std,n_events] = get_event_trig_avg(sig,event_inds,backlag,forwardlag,nboot,trial_ids,inc_prev_trial,use_inds)
%
...
if isempty(nboot) || nargout < 3 %if no error bars are requested
    ...
    nan_ind = any(isnan(sig),2);
    cnt = zeros(length(lags),1); %need a counter to store how many useable data points for each lag and dimension
    for i = 1:n_events
        ...
        ev_avg(uset,:) = ev_avg(uset,:) + sig(cur_ids(uset),:);
        cnt(uset) = cnt(uset) + 1;
    end
    ev_avg = bsxfun(@rdivide,ev_avg,cnt);
