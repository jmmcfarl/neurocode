...
for ii = 1:length(L1vals)
    ...
    glm_quad{ii} = fitGLM_lexp(glm_stcb,stim_emb,spikebins,'tots',[],1e-4,1e-6);
    
    xvLL_quad(ii) = getLLGLM_lexp(glm_quad{ii},xvstim_emb,xvspikebins,'tots')
end

%%
fo = glm_quad{1};
fo = adjust_all_reg(fo,'lambda_d2XT',20);
...
xvLL_lexpfN = getLLGLM_lexp(glm_lexpfN,xvstim_emb,xvspikebins,'tots')
