...
for i = 1:nfilts
    ...
    cur_filt = squeeze(filt(i,:,:))*cvals(i);
    imagesc(cur_filt);colormap(gray(256))
    set(gca,'xtick',[],'ytick',[])
%     set(gca,'ydir','normal')
    mval = max(abs(cur_filt(:)));
    caxis([-mval mval])
...
defmod.lambda_d2XT = 0;
    ...
    [~, ~, ~, prate] = getLL_GNM(quad_mod,xvstim_emb,xvspikebins,'none');
    quad_dev = compute_deviance(xvspikes,prate);
% 
%%
sdim = SDIM; flen = 14;
stim_params.spatial_dims = 1;
...
gnm_dev = compute_deviance(xvspikes,prate);
[~,coms,peak_locs] = get_filter_coms_1d(gnm);
[~,ord] = sort(coms);
gnm.mods = gnm.mods(ord);

%for fitting internal NLs
gnmr = adjust_all_reg(gnm,'lnl2',20);
gnmr = adjust_all_reg(gnmr,'nltype','uncon');
gnmr = adjust_all_reg(gnmr,'nlmon',0);
gnmr = setGNM_NLBFs(gnmr,stim_emb);
cur_xvLL = getLL_GNM(gnmr,xvstim_emb,xvspikebins,'none')
% [mod_seq,LL_seq,kscale_seq] = iterateGNM_filts_NLs(gnmr,stim_emb,spikebins);
dxvLL = Inf;
cnt = 1;
while dxvLL > 1e-3
    gnmr = fitGNM_internal_NLs(gnmr,stim_emb,spikebins,0,2);
    gnmr = setGNM_NLBFs(gnmr,stim_emb);
    ...
    new_xvLL(cnt) = getLL_GNM(gnmr,xvstim_emb,xvspikebins,'none');
    dxvLL = cur_xvLL - new_xvLL(cnt);
    cur_xvLL = new_xvLL(cnt);
    cnt = cnt + 1;
end
gnmr_xvLL = getLL_GNM(gnmr,xvstim_emb,xvspikebins,'none')
[~, ~, ~, prate] = getLL_GNM(gnmr,xvstim_emb,xvspikebins,'none');
gnmr_dev = compute_deviance(xvspikes,prate);
[~,coms,peak_locs] = get_filter_coms_1d(gnmr);
[~,ord] = sort(coms);
gnmr.mods = gnmr.mods(ord);

%%
save quadsim_4ilt_data gnm* quad* filt*