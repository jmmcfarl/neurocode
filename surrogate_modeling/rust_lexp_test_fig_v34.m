...
for n = 1:n_iter
    fo = normalizeRFs_full(fo,stim_emb,1); %This normalizes filter coefs so that the filtered stimulus distribution is standard normal (not really necessary)
    k_mat = get_k_mat(fo);
    input = stim_emb*k_mat;
    fo = fitWeights_full(fo,input,spikebins,1);
    fo = fitNL_full(fo,input,spikebins,0);
    fo = fitGLM_lexp(fo,stim_emb,spikebins,'tots',[],1e-4,1e-6,[]); %fit the model
    ref_LL(n) = fo.LL;
end
