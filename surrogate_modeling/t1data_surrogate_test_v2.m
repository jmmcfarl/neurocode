...
y0 = permute(repmat(4,[SDIM SDIM flen]),[3 1 2]);
...
theta = permute(repmat(3*pi/4,[SDIM SDIM flen]),[3 1 2]);
b = repmat(linspace(2,0,flen)',[1 SDIM SDIM]);
psi = repmat(linspace(0,pi/2,flen)',[1 SDIM SDIM]);

psi2 = psi+pi/2;
x02 = permute(repmat(4,[SDIM SDIM flen]),[3 1 2]);
y02 = permute(repmat(4,[SDIM SDIM flen]),[3 1 2]);
...
% for i = 1:6
%     subplot(2,1,1)
% imagesc(squeeze(filt3(i,:,:)))
% subplot(2,1,2)
% imagesc(squeeze(filt4(i,:,:)))
% pause
% clf
% end
