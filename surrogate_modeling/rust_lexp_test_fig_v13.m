...
for i = 1:length(poss_nmods)
    ...
    [~, ~, ~, prate] = getLL_GNM(quad_mod(i),xvstim_emb,xvspikebins,'none');
    quad_dev(i) = compute_deviance(xvspikes,prate)
end
...
for i = 1:length(poss_nmods)
    ...
    gnmu(i) = fitGNM_spkNL(gnm,g,spikebins,0);
    gnmu_xvLL(i) = getLL_GNM(gnmu(i),xvstim_emb,xvspikebins,'none')
    [~, ~, ~, prate] = getLL_GNM(gnmu(i),xvstim_emb,xvspikebins,'none');
    gnmu_dev(i) = compute_deviance(xvspikes,prate)
    [~,coms,peak_locs] = get_filter_coms_1d(gnmu(i));
    [~,ord] = sort(coms);
    gnmu(i).mods = gnmu(i).mods(ord);
    
    n_iter = 3;
    %for fitting internal NLs
    gnmr(i) = adjust_all_reg(gnmu(i),'lnl2',500); %1000
    gnmr(i) = setGNM_NLBFs(gnmr(i),stim_emb);
    gnmr(i) = adjust_all_reg(gnmr(i),'nltype','uncon');
    gnmr(i) = adjust_all_reg(gnmr(i),'nlmon',1);
    for j = 1:n_iter
        gnmr(i) = fitGNM_internal_NLs(gnmr(i),stim_emb,spikebins,0,2);
        gnmr(i) = fitGNM_filters(gnmr(i),stim_emb,spikebins,'none',[],1e-4,1e-6);
    ...
    [~, ~, ~, prate] = getLL_GNM(gnmr(i),xvstim_emb,xvspikebins,'none');
    gnmr_dev(i) = compute_deviance(xvspikes,prate)
end

%%
save lexp_test_nfilt_data gnm* quad* filt_mat
%%
...
for nn = 1:8
cur_gnm = gnmr(nn);
cur_quad = quad_mod(nn);
[~,coms,peak_locs] = get_filter_coms_1d(cur_gnm);
[~,ord] = sort(coms);
cur_gnm.mods = cur_gnm.mods(ord);

[~,coms,peak_locs] = get_filter_coms_1d(cur_quad);
[~,ord] = sort(coms);
cur_quad.mods = cur_quad.mods(ord);

gnm_k_mat = get_k_mat(cur_gnm);
quad_k_mat = get_k_mat(cur_quad);

gnm_k_norm = bsxfun(@rdivide,gnm_k_mat,sqrt(sum(gnm_k_mat.^2)));
quad_k_norm = bsxfun(@rdivide,quad_k_mat,sqrt(sum(quad_k_mat.^2)));

clear beta_gnm max_gcorr
for i = 1:size(gnm_k_norm,2)
    beta_gnm(i,:)= regress(gnm_k_norm(:,i),filt_mat);
   cur_corr = corr(gnm_k_norm(:,i),filt_mat);
   max_gcorr(i) = max(cur_corr);
end
clear beta_quad max_qcorr
for i = 1:size(quad_k_norm,2)
    beta_quad(i,:)= regress(quad_k_norm(:,i),filt_mat);
   cur_corr = corr(quad_k_norm(:,i),filt_mat);
   max_qcorr(i) = max(cur_corr);
end
all_first(nn,:) = beta_gnm(ceil(nn/2),:);
all_first_k(nn,:) = gnm_k_norm(:,ceil(nn/2));
...
% figure
% % imagesc(beta_gnm)
% plot(beta_gnm'); ylim([-0.2 1])
% figure
% imagesc(beta_quad)
% plot(beta_quad'); ylim([-0.2 1])
pause
close all
end
%%
...
%%
quad_k_mat = get_k_mat(quad_mod(5));
