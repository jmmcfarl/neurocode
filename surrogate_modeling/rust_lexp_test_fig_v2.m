...
defmod.lambda_dX = 150;
defmod.lambda_L1x = 2; %was 20
defmod.lambda_dT = 150;
% defmod.lambda_dX = 10;
...
for ii = 2:n_iter+1
    ...
    for i = 1:length(glm_quad_ref{ii}.mods)
        glm_quad_ref{ii}.mods(i).lambda_dX = glm_quad_ref{ii}.mods(i).lambda_dX*norm_vals(i)^2;
        glm_quad_ref{ii}.mods(i).lambda_dT = glm_quad_ref{ii}.mods(i).lambda_dT*norm_vals(i)^2;
        glm_quad_ref{ii}.mods(i).lambda_L1x = glm_quad_ref{ii}.mods(i).lambda_L1x*norm_vals(i);
    end
...
glm_quad = fitWeights_lexp(glm_quad,g_mat,spikebins,0);
glm_quad = fitGLM_lexp(glm_quad,stim_emb,spikebins,'tots',[],1e-4,1e-6);
...
glm_stcb.image_type = '1d';
glm_stcb.spk_nl = 'logexp';
glm_lexp = fitGLM_lexp(glm_stcb,stim_emb,spikebins,'tots',[],1e-3,1e-5);
orig_mod = glm_lexp;
...
for ii = 2:n_iter+1
    ...
    for i = 1:length(glm_lexp_ref{ii}.mods)
        glm_lexp_ref{ii}.mods(i).lambda_dX = glm_lexp_ref{ii}.mods(i).lambda_dX*norm_vals(i)^2;
        glm_lexp_ref{ii}.mods(i).lambda_dT = glm_lexp_ref{ii}.mods(i).lambda_dT*norm_vals(i)^2;
        glm_lexp_ref{ii}.mods(i).lambda_L1x = glm_lexp_ref{ii}.mods(i).lambda_L1x*norm_vals(i);
    end
    g_mat = stim_emb*get_k_mat(glm_lexp_ref{ii});
    glm_lexp_ref{ii} = fitWeights_lexp(glm_lexp_ref{ii},g_mat,spikebins,0);
    glm_lexp_ref{ii} = fitGLM_lexp(glm_lexp_ref{ii},stim_emb,spikebins,'tots',[],1e-3,1e-5,[]);
end
glm_lexp = glm_lexp_ref{end};
[glm_lexp,norm_vals] = normalizeRFs_full(glm_lexp,stim_emb);
for i = 1:length(glm_lexp.mods)
    glm_lexp.mods(i).lambda_dX = glm_lexp.mods(i).lambda_dX*norm_vals(i)^2;
    glm_lexp.mods(i).lambda_dT = glm_lexp.mods(i).lambda_dT*norm_vals(i)^2;
    glm_lexp.mods(i).lambda_L1x = glm_lexp.mods(i).lambda_L1x*norm_vals(i);
end
g_mat = stim_emb*get_k_mat(glm_lexp);
glm_lexp = fitWeights_lexp(glm_lexp,g_mat,spikebins,0);
