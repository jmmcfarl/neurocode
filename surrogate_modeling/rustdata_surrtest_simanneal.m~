...
STCcf_0 = randn(n_bvs,nmods);
for i = 1:nmods; STCcf_0(:,i) = STCcf_0(:,i)/norm(STCcf_0(:,i)); end;
glm_stcb = createGLM0_stcb(basis_vecs,STCcf_0,defmod,ones(nmods,1),ones(n_bvs,1),'test'); %initialize
glm_stcb.basis = basis;
...
for i = 1:n_pts
    ...
    new_y = STCBF_LLinternal_nonlpsc_constmin(new_x(:),Robs,kern_output,glm_stcb);
    delta_y = new_y - old_y;
...
for i = 1:n_pts
    i
    new_x = propose_new_x_nl(old_x,radius,n_bvs,n_filts);
    new_y = STCBF_LLinternal_nopsc_constmin(new_x,Robs,kern_output,glm_stcb);
    delta_y = new_y - old_y;
    acc_prob(i) = min(1,exp(-delta_y/T(i)));
    if rand < acc_prob(i)
        y_vec(i) = new_y;
...
STCcf_0 = randn(n_bvs,nmods);
for i = 1:nmods; STCcf_0(:,i) = STCcf_0(:,i)/norm(STCcf_0(:,i)); end;
fit_mod = createGLM0_stcb(basis_vecs,reshape(new_x{1},n_bvs,nmods),defmod,ones(nmods,1),ones(n_bvs,1),'test'); %initialize
for n = 1:nmods
    cur_xmod_inds = (n-1)*n_bvs + (1:n_bvs);
    cur_nlmod_inds = (n-1)*11 + (1:11);
    fit_mod.mods(n).nly = new_x{2}(cur_nlmod_inds);
end