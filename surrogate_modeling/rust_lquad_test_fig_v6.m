...
for i = 1:length(glm_quadfN.mods)
...
defmod.maxLocPen = 0;
defmod.lambda_L2x = 0;
defmod.lambda_dX = 0; %50
defmod.lambda_L1x = 200; %200
defmod.lambda_dT = 0;
defmod.lambda_d2X = 0;
defmod.lambda_d2XT = 10; %10
defmod.SDIM = sdim;
defmod.fsdim = sdim;
defmod.pids = 1:sdim;
...
glm_stcb.image_type = '1d';
glm_stcb.spk_nl = 'logexp';
glm_stcb = adjust_all_reg(glm_stcb,'w',1);
...
%%
fo = glm_lexpfN;
fo = adjust_all_reg(fo,'lambda_L1x',50);
fo = adjust_all_reg(fo,'lambda_d2XT',0);
fo = fitGLM_lexp(fo,stim_emb,spikebins,'tots',[],1e-4,1e-6);
xvLL_lexp_fo = getLLGLM_lexp(fo,xvstim_emb,xvspikebins,'tots')
...
xvLL_lexpf = getLLGLM_lexp(glm_lexpf,xvstim_emb,xvspikebins,'tots')
