...
desired_spacing = LAMBDA/6.5; %6.5
...
%     psi1(1:5,:) = repmat(psi1(6,:),5,1);
    psi1 = repmat(((1:flen)'/6).^3+4,1,SDIM); 
...
for i = 1:nfilts
    ...
    hold on
    plot(fox,foy,'k--')
    plot(fox,log(1+exp(beta*(fox-theta)))/10)
    axis tight
    xlim([-3 3])
    ylim([0 0.6])
end
...
lam_vals = [0 10 25 50 100 250 500];
...
quad_mod = fitGNM_filters(quad_mod(i),stim_emb,spikebins,'none',[],1e-4,1e-6);
...
gnmr(i) = adjust_all_reg(gnm,'lnl2',1000);
...
save lexp_mod_gnm_fig_l gnmr* quad* stc* sta*
