...
% down_3 = downsample(lf3,dsf):
    ...
    [Sw_ldown{d},f]= mtspectrumc_unequal_length_trials(down_w, movingwin, params, down_markers8);
        ...
        [S3_up{d},f]= mtspectrumc_unequal_length_trials(down_3, movingwin, params, up_markersw);
    [S3_down{d},f]= mtspectrumc_unequal_length_trials(down_3, movingwin, params, down_markersw);
        [S3_lup{d},f]= mtspectrumc_unequal_length_trials(down_3, movingwin, params, up_markers8);
    [S3_ldown{d},f]= mtspectrumc_unequal_length_trials(down_3, movingwin, params, down_markers8);
...
%     [Cmn_ldown{d},Phimn_ldown{d},Smn,Smm,f,ConfC_ldown{d},PhiStd_ldown{d},Cerr_ldown{d}] = coherencyc_unequal_length_trials([down_8 down_w], movingwin, params,down_markers8 );
    ...
    S3_up{d}(exclude_f) = nan;
    S3_down{d}(exclude_f) = nan;
        S3_lup{d}(exclude_f) = nan;
    S3_ldown{d}(exclude_f) = nan;
...
%     S8_diff{d} = 10*log10(S8_lup{d})-10*log10(S8_ldown{d});
...
mean_diff8_pow(d) = nanmean(10*log10(S8_up{d})-10*log10(S8_down{d}));
    ...
    mean_ldiff_pow(d) = nanmean(10*log10(Sw_lup{d})-10*log10(Sw_ldown{d}));
        mean_up3_pow(d) = nanmean(10*log10(S3_up{d}));
    mean_down3_pow(d) = nanmean(10*log10(S3_down{d}));
    mean_diff3_pow(d) = nanmean(10*log10(S3_up{d})-10*log10(S3_down{d}));
    mean_lup3_pow(d) = nanmean(10*log10(S3_lup{d}));
    mean_ldown3_pow(d) = nanmean(10*log10(S3_ldown{d}));
    mean_ldiff3_pow(d) = nanmean(10*log10(S3_lup{d})-10*log10(S3_ldown{d}));
...
save up_down_spectra_data_no_spike_sub f S* mean*