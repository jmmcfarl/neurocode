...
load('C:\WC_Germany\Cortical_analysis\cortical_dir.mat')
...
%      [Pw{d},t{d},f{d}]=mtspecgramc(down_w,movingwin,params);
    ...
    subplot(2,1,1)
    imagesc(t{d},f{d},10*log10(P8{d}'));shading flat;
    caxis([-25 2]);
    ylim([0 7])
    subplot(2,1,2)
%     imagesc(At{d},lags/Fsd,A8{d}');shading flat; colorbar
%     ylim([0 maxLag/Fsd])
%     caxis([-0.4 0.4])
...
xlim([t{d}(1) t{d}(end)])
    ...
    tname = ['C:\WC_Germany\Cortical_analysis\desynch_detect\' cell_name];
    print('-dpng',tname);
    close
...
cd C:\WC_Germany\Cortical_analysis\desynch_detect
