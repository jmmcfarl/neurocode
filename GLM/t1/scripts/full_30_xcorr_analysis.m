clear all;
cd /Users/James/Data/blanche/matlabdata
stf30=load('stimfiles30.mat');
load spksegsRec30.mat;
load stdparsRec30;

spksegs30 = spksegs30';
nspks = cellfun(@length,spksegs30);

[nconds,ncells]   = size(nspks);

cnames   = stf30.stimfnames30;
cellfun(@disp,cnames)
...
uset = cellfun(@(x) ~isempty(x),spksegs30);
tmax    = max(max(cellfun(@(x)max(x),spksegs30(uset))));
...
for icell1 = 1:ncells-1
    for icell2 = (icell1+1):ncells
        ...
        for n = 1:length(all_conds)
            fprintf('Cond %d of %d\n',n,length(all_conds));
            cur_spks1 = spksegs30{all_conds(n),icell1};
            cur_spks2 = spksegs30{all_conds(n),icell2};
            
            if length(cur_spks1 > 10) & length(cur_spks2) > 10
                ...
                rand_ccs(n,:,:) = nan;
                lfp_stim_dep_ccs(n,:) = nan;
            end
        end
        cond_avg_rand_ccs = squeeze(nanmean(rand_ccs));
        ltp_cond_avg_ccs{icell1,icell2} = ltp_cond_avg_ccs{icell1,icell2}/used_recs;
       ...
       for n = 1:length(all_conds)
            ...
            if length(cur_spks1 > 10) & length(cur_spks2) > 10
                ...
                rand_ccs(n,:,:) = nan;
                mtp_stim_dep_ccs(n,:) = nan;
            end
        end
        cond_avg_rand_ccs = squeeze(nanmean(rand_ccs));
        mtp_cond_avg_ccs{icell1,icell2} = mtp_cond_avg_ccs{icell1,icell2}/used_recs;
        ...
        htp_cond_avg_ccs{icell1,icell2} = htp_cond_avg_ccs{icell1,icell2}/used_recs;
        
        
        cd ~/James_scripts/GLM/t1/rec30_xcorrs/
        ...
        cd ~/Data/blanche/
        save xcorrs30_fullmultires.mat htp* mtp* ltp*
