...
ncomps = 400;
offset = 5;
compids   = offset:ncomps+offset-1;
...
rawX = scores(tr_inds{xv},compids);
...
%     [glm_stcb,sub_means] = mean_sub_filters(glm_stcb);
    ...
    stc_glmW{cur_ndims} = fitNLHI2d_fullbf(glm_stcb,WX,spikebins,'tots',4);
    stc_glmW{cur_ndims}.xvLL = getLLGLM_FULL2d(stc_glmW{cur_ndims},raw_X_xv,xv_spikebins,'none');
       
    w = arrayfun(@(x) x.w,stc_glmW{cur_ndims}.mods);
    stc_glmW{cur_ndims}.mods(w==0) = [];
...
if nmods > 1
    ...
    save cur_model_dump rotated_mod *_LL *_LP *_xvLL* n_*_dims *_inds
