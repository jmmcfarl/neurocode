function [ll,grad] = LLexp_jmm(K,X,S,lamrange,lamrange2,llist, hold_const, nlxrange)
%
...
ll = sum(ks) - rtot - hcpun;
grad = sum(S) - r' * X; %'    % grad w.r.t. k
grad(end+1) = length(ks) - rtot;    % grad w.r.t. b
...
%              grad(range(2:end)) = grad(range(2:end)) - 2*els(i)*(chunk(3:end-1) - chunk(2:end-2));
%             grad(range(1:end-1)) = grad(range(1:end-1)) - 2*els(i)*(chunk(2:end-2) - chunk(3:end-1));

%        ll = ll - els(i) * (chunk(end)-chunk(1))^2;
% 	    grad(range(1)) = grad(range(1)) - 2*els(i)*(chunk(1) - chunk(end));
%       grad(range(end)) = grad(range(end)) - 2*els(i)*(chunk(end) - chunk(1));
...
for i = 1:length(els2)
    ...
    K_lapl = nan(size(range));
    K_lapl(2:end-1) = 2*chunk(2:end-1) - chunk(1:end-2) - chunk(3:end);
    if circ2(i)==1
        K_lapl(1) = 2*chunk(1)-chunk(end)-chunk(2);
        K_lapl(end) = 2*chunk(end)-chunk(1)-chunk(end-1);
    else
        K_lapl(1) = 0;
        K_lapl(end) = 0;
    end
    cur_pen = els2(i)*sum(K_lapl(:).^2);
    ll = ll - cur_pen - hcpun;
    
    K_d2grad = zeros(size(K_lapl));
    K_d2grad(2:end-1) = 2*K_lapl(2:end-1) - K_lapl(1:end-2) - K_lapl(3:end);
    if circ2(i)==1
        K_d2grad(1) = 2*K_lapl(1)-K_lapl(2)-K_lapl(end);
        K_d2grad(end) = 2*K_lapl(end)-K_lapl(1)-K_lapl(end-1);
    else
%         K_d2grad(1) = 2*K_lapl(1) - 2*K_lapl(2);
%         K_d2grad(end) = 2*K_lapl(end) - 2*K_lapl(end-1);
        K_d2grad(1) = - K_lapl(2);
        K_d2grad(end) = - K_lapl(end-1);
    end
    grad(range) = grad(range) - 2*els2(i)*K_d2grad;
...
%     
    %JAMES NEWER METHOD THAT INCORPORATES NONUNIFORM GRID SPACING
