function [myglm] = fitNLHI_stcb_connl(myglm,ustim,spkbs,disp_type,max_iter,min_iter)
...
[ll0, ll0p] = getLLGLM_STCBF_connl(myglm,kern_output,spkbs,disp_type);
...
while ((((LLlast-myglm.LL) >= thresh) && (iter < max_iter))) || (iter < min_iter)
    ...
    myglm = fitWeights_stcb_connl(myglm,g_mat,spkbs,0,w_TolFun);    
    [ll0 ll0p] = getLLGLM_STCBF_connl(myglm,kern_output,spkbs,disp_type);
    myglm.LP_seq = [myglm.LP_seq ll0p];
    myglm.LL_seq = [myglm.LL_seq ll0];
    ...
    myglm = fitSTCBF_connl(myglm,kern_output,Robs,0);
    [ll0, ll0p] = getLLGLM_STCBF_connl(myglm,kern_output,spkbs,disp_type);
...
getLLGLM_STCBF_connl(myglm,kern_output,spkbs,disp_type);
