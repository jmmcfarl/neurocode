function [x,f,exitflag,output] = minFunc_simanneal(funObj,x0,kernel_group,options,varargin)
...
T0 = 2;
alpha = 0.95;
cooling_schedule = T0*alpha.^((1:maxIter)-1);
...
for i = 1:maxIter
    ...
    %every n_sa_iter iterations, try a reflection using simulated
    %annealing
    if mod(i,n_sa_iter) == 0
        n_groups = length(unique(kernel_group))-1;
        group_flip = ceil(rand*n_groups);
        flip_fac = ones(size(x));
        flip_fac(kernel_group==group_flip) = -1;
        new_x = x.*flip_fac;
        
        d = zeros(size(x));
        d(kernel_group == group_flip) = 1;
        d = d/norm(d);
        t = 1;
        [f_new,g_new] = funObj(new_x,varargin{:});

        [t_test,x_test,f_test,g_test,LSfunEvals] = ArmijoBacktrack(new_x,t,d,f_new,f_new,g_new,gtd,c1,LS_interp,LS_multi,progTol,debug,doPlot,1,funObj,varargin{:});
        [t_test,f_test,g_test,LSfunEvals] = WolfeLineSearch(new_x,t,d,f_new,g_new,gtd,c1,c2,LS_interp,LS_multi,25,progTol,debug,doPlot,1,funObj,varargin{:});
       
        
        acc_prob = min(1,exp((f-f_new)/cooling_schedule(i)));
        is_acc = rand < acc_prob;
        if is_acc
            x = new_x;
            f = f_new;
            g = g_new;
            fprintf('Proposed flip accepted\n');
        else
            fprintf('Proposed flip rejected\n');
        end
    end
    ...
    % Output Function
    if ~isempty(outputFcn)
        callOutput(outputFcn,x,'iter',i,funEvals,f,t,gtd,g,d,optCond,varargin{:});
	end
