function [nll, pnll,lpen,prate] = getLLGLM_lexp(glmod,X,spkbs,disp_type)
...
[nll,pnll,lpen,prate] = getLL_lexp(kx,glmod,spkbs);
...
if disp_level > 0
    ...
    if disp_level > 1
        fprintf('Totals   ::: W-sparse: %.3g | Loc:%.3g | Ksmooth:%.3g | KLapl:%.3g | KLaplXT:%.3g | KL1:%.3g | KL2:%.3g \n',...
            sum(lpen.w),sum(lpen.loc),sum(lpen.ksmooth),sum(lpen.lapl),sum(lpen.laplXT),sum(lpen.l1x),sum(lpen.l2x));
        if disp_level > 2
            fprintf('***\n');
            for imod = 1:nmods
                fprintf('Module %d ::: W-sparse: %.3g | Loc:%.3g | Ksmooth:%.3g | KLapl:%.3g | KLaplXT:%.3g | KL1:%.3g | KL2:%.3g\n',...
                    imod,lpen.w(imod),lpen.loc(imod),lpen.ksmooth(imod),lpen.lapl(imod),lpen.laplXT(imod),lpen.l1x(imod),lpen.l2x(imod));
            end
            fprintf('***\n');
        end
        fprintf('.........\n');
