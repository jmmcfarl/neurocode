...
for xv = 1:nfold
    ...
    gnm_mod(xv) = createGNM(init_kerns,[ones(1,npos) -1*ones(1,nneg)],kern_types,defmod,stim_params);
    for i = (npos+1):(npos+nneg)
        gnm_mod(xv).mods(i).lambda_d2XT = 400; %200
        gnm_mod(xv).mods(i).lambda_L1x = 40; %60
    end
    gnm_mod(xv) = fitGNM_filters(gnm_mod(xv),cur_tr_stimemb,cur_tr_spkbns,'none',[],1e-4,1e-6);
    gnm_xvLL(xv) = getLL_GNM(gnm_mod(xv),cur_xv_stimemb,cur_xv_spkbns,'none');
    [~, ~, ~, ~, g] = getLL_GNM(gnm_mod(xv),cur_tr_stimemb,cur_tr_spkbns,'none');
    gnm_mod(xv) = fitGNM_spkNL(gnm_mod(xv),g,cur_tr_spkbns,0);
    gnm_xvLL(xv) = getLL_GNM(gnm_mod(xv),cur_xv_stimemb,cur_xv_spkbns,'none');

    gnmr_mod(xv) = setGNM_NLBFs(gnm_mod(xv),cur_tr_stimemb);
    gnmr_mod(xv) = adjust_all_reg(gnmr_mod(xv),'nltype','uncon');
    gnmr_mod(xv) = fitGNM_internal_NLs(gnmr_mod(xv),cur_tr_stimemb,cur_tr_spkbns,0,2);
    gnmr_xvLL(xv) = getLL_GNM(gnmr_mod(xv),cur_xv_stimemb,cur_xv_spkbns,'none');
    
    gnmr2_mod(xv) = fitGNM_filters(gnmr_mod(xv),cur_tr_stimemb,cur_tr_spkbns,'none',[],1e-4,1e-6);
    gnmr2_mod(xv) = fitGNM_internal_NLs(gnmr2_mod(xv),cur_tr_stimemb,cur_tr_spkbns,0,0);
    gnmr2_xvLL(xv) = getLL_GNM(gnmr2_mod(xv),cur_xv_stimemb,cur_xv_spkbns,'none');
    %
...
save rust_gnm_examp_new *xvLL gnm* quad* stc* glm*
...
xv
    ...
    quad_filts = get_k_mat(quad_mod(xv));
    del_dims = find(max(abs(quad_filts)) == 0);
    quad_filts(:,del_dims) = [];
    gnm_filts = get_k_mat(gnmr2_mod(xv));
    
    gnm_quad_ov(xv) = subspace_overlap(gnm_filts,quad_filts);
    stc_quad_ov(xv) = subspace_overlap(stc_filts,quad_filts);
    gnm_stc_ov(xv) = subspace_overlap(gnm_filts,stc_filts);
