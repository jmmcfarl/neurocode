...
for xv = 1:nfold
    ...
    for ii = 1:length(n_stc_dims)
        
        nposdims = n_stc_dims(ii)/2; nnegdims = n_stc_dims(ii)/2;
        posdims = 1:nposdims; negdims = 1:nnegdims;
        STCbvs = [stcs(:,posdims) rstcs(:,negdims)]; %use only expansive subspace
        negdims = negdims + length(posdims);
        ...
        [klen,n_stc_dims(ii)] = size(STCbvs);
        flen = klen/sdim;
        ...
        if nmods <= n_stc_dims(ii)
            negmod_inds = (length(posdims)+1):(length(posdims)+length(negmods));
            unused_stcs = setdiff(1:n_stc_dims(ii),[posmod_inds negmod_inds]);
            mod_signs(posmods) = 1;
            mod_signs(negmods) = -1;
        else
            extra_mods = (n_stc_dims(ii)+1):nmods;
            mod_signs(posdims) = 1;
            mod_signs(negdims) = -1;
        ...
        if jj == 1 %for first iteration initialize on STC dims
            ...
            STCcf_0 = eye(n_stc_dims(ii));
            STCcf_0(:,unused_stcs) = [];
            if nmods > n_stc_dims(ii)
                n_extra = cur_nmods-n_stc_dims(ii);
                STCcf_0 = [STCcf_0 randn(n_stc_dims(ii),n_extra)];
            end
        else %for rest use random initialization
            n_extra = nmods;
            STCcf_0 = randn(n_stc_dims(ii),n_extra);
        end
        ...
        pname = ['~\James_scripts\stc_sparse_test_figs\nstcd_test\' tuid sprintf('_%dSTCd_%d_IT_%d',n_stc_dims(ii),xv)];
...
%% Save output
cd ~/James_scripts/stc_sparse_test_figs/nstcd_test
save stcbf_nstcd_mods n_stc_dims nmods stc_posneg_mod stc_posneg_xvLL nfold
