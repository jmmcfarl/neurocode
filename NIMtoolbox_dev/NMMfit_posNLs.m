...
for ii = 1:Ntargets
	...
	% Add ranges
	LB(cur_range) = 0;
	if nim.mods(targets(ii)).con_type > 0
		UB(cur_range) = 1;
	end

	% For monotonicity constraint
	if nim.mods(targets(ii)).con_type < 2
		nim.mods(targets(ii)).NLmon = 1;
	elseif nim.mods(targets(ii)).con_type == 2
		nim.mods(targets(ii)).NLmon = -1;
	else
		nim.mods(targets(ii)).NLmon = 0;
	end
	...
	if nim.mods(targets(ii)).con_type == 0
		cur_vec(cur_range(1)) = 1;
		beq(end+1) = 0;
	elseif nim.mods(targets(ii)).con_type == 1
		cur_vec(cur_range(1)) = 1;
		beq(end+1) = 1;
	else
		cur_vec(cur_range(nim.mods(targets(ii)).con_type)) = 1;
		beq(end+1) = 1;
	end
...
[params] = fmincon( @(K) NMMfit_upstreamNLs_internal(nim, K, Robs, XNL, Xspkhst, L2_NLd2, targets,nt_gout),...
...
function [LL, LLgrad] = NMMfit_upstreamNLs_internal(nim, params, Robs, Xnl, Xspkhst, L2_NLd2, targets, nt_gout)
%
