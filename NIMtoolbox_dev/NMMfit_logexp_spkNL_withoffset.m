function [nim_out,bin_centers,nonpar] = NMMfit_logexp_spkNL_withoffset( nim, Robs, Xstim, Gmults, display, hold_const )
%
...
[fit_params,newLL] = fmincon(@(K) spkNL_internal_LL(K,G,Robs), initial_params,[],[],Aeq,Beq,LB,UB,[],opts);

nim_out = nim;
nim_out.spk_NL_params = fit_params;

% [LL, penLL] = NMMmodel_eval(nim_out,Robs,Xstim);
LL = -newLL;
nim_out.LL_seq = cat(1,nim_out.LL_seq,LL);
nim_out.penLL_seq = cat(1,nim_out.penLL_seq,nan);
nim_out.opt_history = cat(1,nim_out.opt_history,{'spkNL_wo'});
