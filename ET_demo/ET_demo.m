clear all
ET_demo_dir = '~/James_scripts/bruce/ET_demo';
cd(ET_demo_dir);
addpath(genpath(ET_demo_dir));
load simDATA.mat

init_mods_name = 'init_mod_data.mat';
%% SET MODEL PARAMETERS
useLen = Inf; %use this to limit the amount of data used. Set to Inf to use all data (requires about twice as much RAM: ~16GB).
use_coil = true; %use or don't use information from the (simulated) eye coil signal
...
flen = 12; %number of time lags to include in the time-embedded stimulus matrix
use_nPix = 16; %number of pixels to include in the stimulus models
spatial_usfac = 2; % (2) up-sample factor for initial model-fitting, and estimation of fixation-corrections. Only set up for multiples of 2.
hr_usfac = 4; % (4) up-sample factor for estimating drift (helpful to use a higher resolution here).

n_fix_inf_it = 1; % (3) number of iterations of estimating fixation corrections
n_drift_inf_it = 1; % (1) number of iterations of estimating drift corrections

fix_prior_sigma = 0.15; %SD of Gaussian prior on the fixation corrections (in deg)
drift_prior_sigma = 0.004; %SD of Gaussian prior on the change in eye position between adjacent time points
drift_jump_sigma = 0.075;  %SD of Gaussian prior on the eye position deviation from the fixation avg
drift_dsf = 2; %down-sample factor for inferring drift corrections (relative to base time resolution).

fix_noise_sigma = 0.1; %assumed noise SD of coils, in deg (for measuring change in fixation-avg position)
drift_noise_sigma = 0.004; %assumed noise SD of coils, in deg/sec (for measuring drift vel).
...
recompute_initial_models = true; %load in precomputed initial models if they exist in the current dir
%% PRE-COMPUTATIONS
...
%create parameter structures for the stimulus
stim_params = NMMcreate_stim_params([flen full_nPix],dt);
stim_params_us = NMMcreate_stim_params([flen full_nPix_us],dt);
stim_params_hr = NMMcreate_stim_params([flen full_nPix_hr],dt);
...
dshift = 1; %bin size in pixels for discretization of eye positions
...
%generate shift matrices for fix-corrections. Must be applied to the stimulus (not the filters)
It = speye(flen);
...
fix_Lprior = fix_Lprior - logsumexp(fix_Lprior);
...
base_lambda_d2XT = 10; %spatiotemporal smoothness regularization strength
base_lambda_L1 = 2; %sparseness regularization strength
...
n_squared_filts = 2; %number of squared filters to use
mod_signs = ones(1,n_squared_filts+1); %assume all filters are 'excitatory' (sign doesn't matter for linear filter)
NL_types = [{'lin'} repmat({'quad'},1,n_squared_filts)];
init_d2XT = ones(n_squared_filts+1,1); %weak initial smoothness regularization
init_reg_params = NMMcreate_reg_params('lambda_d2XT',init_d2XT);

if ~exist(init_mods_name,'file') || recompute_initial_models
...
for ss = 1:n_units
    ...
    gqm1 = NMMfit_filters(gqm1,cur_Robs,Xmat,[],[],silent);
    ...
    %initialize up-sampled stim model
    for ii = 1:n_squared_filts+1
        init_filts{ii} = base_filts_up(:,ii);
    end
    gqm2 = NMMinitialize_model(fin_stim_params,mod_signs,NL_types,init_reg_params,[],init_filts);
    gqm2.spk_NL_params(1) = gqm1.spk_NL_params(1);
    
    %adjust regularization strength to match scale of filters
    [LL, penLL, pred_rate, G, gint] = NMMmodel_eval(gqm2,cur_Robs,Xmat_us);
    gqm2 = NMMadjust_regularization(gqm2,[],'lambda_d2XT',base_lambda_d2XT./var(gint)');
    gqm2 = NMMadjust_regularization(gqm2,[],'lambda_L1',base_lambda_L1./std(gint)');
    
    %estimate model
    gqm2 = NMMfit_filters(gqm2,cur_Robs,Xmat_us,[],[],silent);
    all_mod_fits(ss) = gqm2; %store model
    all_mod_fits_spkNL(ss) = NMMfit_logexp_spkNL(gqm2,cur_Robs,Xmat_us); %estimate spiking NL parameters
    
    %compute model performance measures
    [LL,~,pred_rate,~,~,~,null_LL(ss)] = NMMmodel_eval(all_mod_fits_spkNL(ss),cur_Robs,Xmat_us);
...
for nn = 1:n_fix_inf_it
    ...
    for ss = 1:n_units
        ...
        it_mods{nn+1}(ss) = NMMfit_filters(it_mods{nn+1}(ss),cur_Robs,Xmat_up_fixcor,[],[],silent); %fit stimulus filters
        
        %refit spk NL
        it_mods_spkNL{nn+1}(ss) = NMMfit_logexp_spkNL(it_mods{nn+1}(ss),cur_Robs,Xmat_up_fixcor);
        
        %compute performance
        [newLL,~,new_prate] = NMMmodel_eval(it_mods_spkNL{nn+1}(ss),cur_Robs,Xmat_up_fixcor);
...
fin_stim_params = NMMcreate_stim_params([flen use_nPix_hr],dt);
if hr_usfac > spatial_usfac %if using further spatial up-sampling
    for ss = 1:n_units
        ...
        gqm2 = NMMadjust_regularization(gqm2,[],'lambda_d2XT',base_lambda_d2XT./vgint);
        gqm2 = NMMadjust_regularization(gqm2,[],'lambda_L1',base_lambda_L1./sgint);
        all_mod_fits_hr(ss) = gqm2;
        all_mod_fits_hr(ss).spk_NL_params(1) = it_mods{end}(ss).spk_NL_params(1);
        
        %estimate model parameters
        all_mod_fits_hr(ss) = NMMfit_filters(all_mod_fits_hr(ss),cur_Robs,Xmat_us);
        
        %estimate spk NL parameters
        all_mod_fits_hr_spkNL(ss) = NMMfit_logexp_spkNL(all_mod_fits_hr(ss),cur_Robs,Xmat_us);
        
        %model improvements
        [LL,~,pred_rate] = NMMmodel_eval(all_mod_fits_hr_spkNL(ss),cur_Robs,Xmat_us);
...
if use_coil
    ...
    post_mean_drift = post_drift_var*coil_drift_vel/drift_noise_sigma^2;
...
for nn = 1:n_drift_inf_it
    ...
    for ss = 1:n_units
        ...
        dit_mods{nn+1}(ss) = NMMfit_filters(dit_mods{nn+1}(ss),cur_Robs,cur_X,[],[],silent); %fit stimulus filters
        dit_mods_spkNL{nn+1}(ss) = NMMfit_logexp_spkNL(dit_mods{nn+1}(ss),cur_Robs,cur_X); %estimate spk NL params
        
        [newLL,~,new_prate] = NMMmodel_eval(dit_mods_spkNL{nn+1}(ss),cur_Robs,cur_X);
        null_prate = ones(NT,1)*mean(cur_Robs);
...
%%
% save demo_output dit_* it_* drift_* spatial_usfac hr_usfac sp_dx sp_dx_hr fix_ids trial_*inds NT sac_shift use_coil
...
fin_drift_mean = drift_post_mean(end,:)*sp_dx_hr;
fin_drift_std = drift_post_std(end,:)*sp_dx_hr;
