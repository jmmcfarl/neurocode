...
%structure
[NT,Npix] = size(stim);
if prod(params.stim_dims(2:end)) ~= Npix
...
if Npix == 1 && isempty(params.tent_spacing)
    Xmat = toeplitz(stim,[stim(1) zeros(1,params.stim_dims(1) - 1)]);
else
    %otherwise loop over lags and manually shift the stim matrix
    Xmat = zeros( NT, prod(params.stim_dims));
    for n = 1:params.stim_dims(1)
        Xmat(:,n-1+(1:params.stim_dims(1):(Npix*params.stim_dims(1)))) = shift_mat_zpad( stim, lag_spacing*(n-1), 1);
    end    
end
